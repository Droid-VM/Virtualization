...
fn openpty() -> Result<Pty> {
    ...
    // For arm architectures this is a cast from u8 to u8, but for x86 it converts from i8 to u8.
    #[cfg(target_arch = "x86_64")]
    let follower_name: Vec<u8> = follower_name.iter_mut().map(|x| *x as u8).collect();
