...
pub struct MicrofuchsiaInstance {
    ...
    #[allow(dead_code)] // Keeps microfuchsiad process alive
...
impl InstanceStarter {
    ...
    pub fn start_new_instance(
        ...
        // Open the kernel and initrd files from the microfuchsia apex.
        let kernel_fd =
...
fn openpty() -> Result<Pty> {
    ...
    let (leader, follower) = unsafe { (File::from_raw_fd(leader), File::from_raw_fd(follower)) };
    let follower_name =
        String::from_utf8(follower_name).context("pty filename is not valid utf8")?;
    let follower_name = follower_name.trim_matches('\0').to_string();
    Ok(Pty { leader, follower, follower_name })
}
