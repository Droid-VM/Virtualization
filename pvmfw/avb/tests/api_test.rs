...
fn payload_with_multiple_capabilities() -> Result<()> {
    ...
    assert!(verified_boot_data.capabilities.contains(&Capability::RemoteAttest));
    assert!(verified_boot_data.capabilities.contains(&Capability::SecretkeeperProtection));
    Ok(())
}
