...
fn vbmeta_with_verification_flag_disabled_fails_verification() -> Result<()> {
    ...
    // Big-endian representation of the u32 flag.
    const VBMETA_VERIFICATION_DISABLED_FLAG_BE: [u8; 4] = [0, 0, 0, 2u8];
    ...
    let flags_offset = footer.vbmeta_offset as usize + usize::try_from(flags_offset)?;

    // Act
    kernel[flags_offset..(flags_offset + size_of::<u32>())]
        .copy_from_slice(&VBMETA_VERIFICATION_DISABLED_FLAG_BE);
