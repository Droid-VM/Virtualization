...
extern "C" fn read_rollback_index(
    ...
    // `avb_slot_verify()`.
    // We write -1 to the rollback_index to ensure that the default rollback index (0)
    // is always superieur than it, thus the rollback index check will pass.
    to_avb_io_result(write(out_rollback_index, -1))
}
