...
impl<'a> Payload<'a> {
    ...
    fn get_partition(&self, partition_name: &CStr) -> utils::Result<&[u8]> {
...
pub(crate) struct Ops<'a> {
    payload: &'a mut Payload<'a>,
}

impl<'a> From<&'a mut Payload<'a>> for Ops<'a> {
    fn from(payload: &'a mut Payload<'a>) -> Self {
        Self { payload }
    }
}
...
impl<'a> Ops<'a> {
    pub(crate) fn verify_partition(
        &mut self,
        partition_name: &CStr,
    ) -> Result<avb::SlotVerifyData, avb::SlotVerifyError> {
        avb::slot_verify(
            self,
            &[partition_name],
            None,
            avb::SlotVerifyFlags::AVB_SLOT_VERIFY_FLAGS_NONE,
            avb::HashtreeErrorMode::AVB_HASHTREE_ERROR_MODE_RESTART_AND_INVALIDATE,
        )
    }
}
...
impl<'a> avb::Ops for Ops<'a> {
    ...
    fn get_preloaded_partition(&mut self, partition: &CStr) -> utils::Result<&[u8]> {
    ...
    fn read_rollback_index(&mut self, _rollback_index_location: usize) -> utils::Result<u64> {
    ...
    fn write_rollback_index(
        ...
        Err(avb::IoError::NotImplemented)
    }

    fn read_is_device_unlocked(&mut self) -> utils::Result<bool> {
        Ok(false)
    }

    fn get_size_of_partition(&mut self, partition: &CStr) -> utils::Result<u64> {
        let partition = self.payload.get_partition(partition)?;
        u64::try_from(partition.len()).map_err(|_| avb::IoError::InvalidValueSize)
    }

    fn read_persistent_value(&mut self, _name: &CStr, _value: &mut [u8]) -> utils::Result<usize> {
        Err(avb::IoError::NotImplemented)
    }

    fn write_persistent_value(&mut self, _name: &CStr, _value: &[u8]) -> utils::Result<()> {
        Err(avb::IoError::NotImplemented)
    }

    fn erase_persistent_value(&mut self, _name: &CStr) -> utils::Result<()> {
        Err(avb::IoError::NotImplemented)
    }
    ...
    fn validate_public_key_for_partition(
        ...
        _public_key: &[u8],
        _public_key_metadata: Option<&[u8]>,
    ) -> utils::Result<avb::PublicKeyForPartitionInfo> {
        Err(avb::IoError::NotImplemented)
    }
}
