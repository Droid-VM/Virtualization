...
use crate::Digest;
use avb_bindgen::{
    avb_descriptor_validate_and_byteswap, avb_hash_descriptor_validate_and_byteswap,
    avb_property_descriptor_validate_and_byteswap, AvbDescriptor, AvbDescriptorTag,
    AvbHashDescriptor, AvbPropertyDescriptor,
};
use core::{
    ...
    pub(super) unsafe fn from_descriptor_ptr(
        ...
        match avb_descriptor.tag.try_into() {
            ...
            Ok(AvbDescriptorTag::AVB_DESCRIPTOR_TAG_PROPERTY) => {
                ...
                Ok(Self::Property(PropertyDescriptor::new(&h, data)?))
            }
...
impl<'a> PropertyDescriptor<'a> {
    fn new(header: &PropertyDescriptorHeader, data: &'a [u8]) -> utils::Result<Self> {
