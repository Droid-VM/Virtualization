...
impl fmt::Display for AvbImageVerifyError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::InvalidCStr => write!(f, "Invalid C String"),
        }
    }
}
...
extern "C" fn read_is_device_unlocked(
    ...
    if let Err(e) = is_null(out_is_unlocked) {
        return e.into();
    }
...
fn try_read_from_partition(
    ...
    is_null(ops)?;
    // SAFETY: It is safe as the raw pointer `ops` is a valid pointer.
    let ops = unsafe { &*ops };
    let kernel = ops.as_ref().kernel;
    is_null(buffer)?;
    ...
    copy_data_to_dst(kernel, offset, buffer_slice)?;
    is_null(out_num_read)?;
    // SAFETY: The raw pointer `out_num_read` was created to point to a valid a `usize`
    // and we have checked it is non-null.
    unsafe {
        *out_num_read = buffer_slice.len();
    }
    Ok(())
}
...
fn try_get_size_of_partition(
    ...
    let ops = unsafe { &*ops };
    let kernel_size =
        u64::try_from(ops.as_ref().kernel.len()).map_err(|_| AvbIOError::InvalidValueSize)?;
    is_null(out_size_num_bytes)?;
    // SAFETY: The raw pointer `out_size_num_bytes` was created to point to a valid a `u64`
    // and we have checked it is non-null.
    unsafe {
        *out_size_num_bytes = kernel_size;
    }
    Ok(())
}
...
fn try_validate_public_key_for_partition(
    ...
    is_null(out_is_trusted)?;
    // SAFETY: It is safe as the raw pointer `out_is_trusted` is a valid pointer.
    unsafe {
        *out_is_trusted = public_key == trusted_public_key;
    }
    Ok(())
}

fn is_null<T>(ptr: *const T) -> Result<(), AvbIOError> {
    if ptr.is_null() {
...
pub fn verify_payload(kernel: &[u8], trusted_public_key: &[u8]) -> Result<(), AvbImageVerifyError> {
    ...
    let kernel =
        CStr::from_bytes_with_nul(b"bootloader\0").map_err(|_| AvbImageVerifyError::InvalidCStr)?;
    // NULL is needed to mark the end of the array.
    let requested_partitions: [*const c_char; 2] = [kernel.as_ptr(), ptr::null()];
    let ab_suffix =
        CStr::from_bytes_with_nul(b"\0").map_err(|_| AvbImageVerifyError::InvalidCStr)?;
