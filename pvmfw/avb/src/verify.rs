...
extern "C" fn read_from_partition(
    ...
    if read_data_to_dst(kernel, offset, buffer_slice).is_err() {
...
fn read_data_to_dst(src: &[u8], offset: i64, dst: &mut [u8]) -> Result<(), ()> {
    let start = to_read_start(offset, src.len()).ok_or(())?;
