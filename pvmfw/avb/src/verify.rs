...
impl PartitionName {
    ...
    fn partition_name_matches_initrd(name_not_null_terminated: &[u8]) -> bool {
        return name_not_null_terminated
            == as_byte_str_without_null(Self::INITRD_NORMAL_PARTITION_NAME)
            || name_not_null_terminated
                == as_byte_str_without_null(Self::INITRD_DEBUG_PARTITION_NAME);
    }
}

fn as_byte_str_without_null(s: &[u8]) -> &[u8] {
    &s[0..(s.len() - 1)]
}
