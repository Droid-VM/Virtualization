...
fn try_search_initrd_hash_descriptor(
    ...
    let hash_desc = AvbHashDescriptorWrap::try_from(descriptor)?;
    if PartitionName::partition_name_matches_initrd(hash_desc.partition_name_raw()?) {
...
impl AvbHashDescriptorWrap {
    fn check_is_in_range(&self, index: usize) -> Result<(), AvbIOError> {
        ...
        let total_len = usize_checked_add(
            ...
            u64_to_usize(parent_desc.num_bytes_following)?,
        )?;
        if index <= total_len {
            Ok(())
        } else {
    ...
    fn partition_name_raw(&self) -> Result<&[u8], AvbIOError> {
        ...
        self.check_is_in_range(usize_checked_add(
...
fn u64_to_usize(num: u64) -> Result<usize, AvbIOError> {
...
impl PartitionName {
    ...
    fn partition_name_matches_initrd(name_non_null_terminated: &[u8]) -> bool {
        return name_non_null_terminated
            == as_byte_str_non_null_terminated(Self::INITRD_NORMAL_PARTITION_NAME)
            || name_non_null_terminated
                == as_byte_str_non_null_terminated(Self::INITRD_DEBUG_PARTITION_NAME);
    }
}

fn as_byte_str_non_null_terminated(s: &[u8]) -> &[u8] {
    &s[0..(s.len() - 1)]
}
