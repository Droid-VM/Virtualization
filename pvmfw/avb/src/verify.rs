...
use avb_bindgen::{
    ...
    AvbSlotVerifyData, AvbSlotVerifyFlags,
};
use core::{
...
impl<'a> Payload<'a> {
    ...
    fn verify_partitions(
    ...
    ) -> Result<&'a AvbSlotVerifyData, AvbSlotVerifyError> {
        ...
        as_ref(out_data).map_err(|_| AvbSlotVerifyError::Io)
    }

    fn verify_vbmeta(&self, verify_result: &AvbSlotVerifyData) -> Result<(), AvbSlotVerifyError> {
        ...
        is_not_null(verify_result.vbmeta_images).map_err(|_| AvbSlotVerifyError::Io)?;
