...
mod tests {
    ...
    /// This test uses the Microdroid payload (kernel+initrd) compiled on the
    /// fly to check that the latest payload can be verified successfully.
    #[test]
    fn latest_valid_payload_is_verified_successfully() -> Result<()> {
        let kernel = load_valid_signed_kernel()?;
    ...
    fn unsigned_kernel_fails_verification() -> Result<()> {
        let kernel = read_file_to_bytes("unsigned_test_image")?;
        let public_key = read_file_to_bytes("data/testkey_rsa4096_pub.bin")?;
    ...
    fn load_valid_signed_kernel() -> Result<Vec<u8>> {
