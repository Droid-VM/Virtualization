...
extern "C" fn read_from_partition(
    ...
    let start = if let Some(v) = to_read_start(offset, kernel.len()) {
        v
    } else {
        return AvbIOResult::AVB_IO_RESULT_ERROR_IO;
    };
    let end = if let Some(v) = start.checked_add(num_bytes) {
        v
    } else {
        return AvbIOResult::AVB_IO_RESULT_ERROR_IO;
    };
    ...
    if let Some(data) = kernel.get(start..end) {
...
fn to_read_start(offset: i64, len: usize) -> Option<usize> {
    let usize_offset = usize::try_from(offset);
    if usize_offset.is_ok() {
        return usize_offset.ok();
    }
    if let Ok(v) = isize::try_from(offset) {
        len.checked_add_signed(v)
    } else {
        None
    }
}
...
mod tests {
    ...
    const PUBLIC_KEY_RSA2048_PATH: &str = "testkey_rsa2048_pub.bin";
    ...
    fn latest_valid_payload_is_verified_successfully() -> Result<()> {
        ...
        let public_key = read_file_to_bytes(PUBLIC_KEY_RSA4096_PATH)?;
    ...
    fn payload_with_a_different_valid_public_key_fails_verification() -> Result<()> {
        let kernel = load_latest_signed_kernel()?;
        let public_key = read_file_to_bytes(PUBLIC_KEY_RSA2048_PATH)?;
