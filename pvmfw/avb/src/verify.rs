...
extern "C" fn get_size_of_partition(
    ...
    if let Err(e) = try_get_size_of_partition(ops, partition, out_size_num_bytes) {
        return e.into();
    }
    AvbIOResult::AVB_IO_RESULT_OK
}
...
fn try_get_size_of_partition(
    ...
    // and we have checked it is non-null.
    unsafe {
        *out_size_num_bytes = payload.kernel.len() as u64;
    }
    Ok(())
}
...
extern "C" fn validate_public_key_for_partition(
    ...
    let ops = unsafe { &*ops };
    let payload = ops.as_ref();
    let trusted_public_key = payload.trusted_public_key;
    if out_is_trusted.is_null() {
        return AvbIOResult::AVB_IO_RESULT_ERROR_NO_SUCH_VALUE;
    }
