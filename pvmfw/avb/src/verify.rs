...
impl<'a> Payload<'a> {
    ...
    const MAX_NUM_OF_VBMETA_IMAGES: usize = 3;
    ...
    fn get_partition(&self, partition_name: *const c_char) -> Result<&[u8], AvbIOError> {
        ...
        let partition_name = partition_name.to_bytes_with_nul();
        if Self::KERNEL_PARTITION_NAME == partition_name {
            return Ok(self.kernel);
        }
        if Self::INITRD_NORMAL_PARTITION_NAME == partition_name
            || Self::INITRD_DEBUG_PARTITION_NAME == partition_name
        {
            if let Some(initrd) = self.initrd {
                return Ok(initrd);
            }
        }
        Err(AvbIOError::NoSuchPartition)
    }

    fn verify_partitions(&mut self, partition_names: &[&CStr]) -> Result<(), AvbImageVerifyError> {
        if partition_names.len() > Self::MAX_NUM_OF_VBMETA_IMAGES {
            return Err(AvbImageVerifyError::InvalidArgument);
        }
        let mut requested_partitions = [ptr::null(); Self::MAX_NUM_OF_VBMETA_IMAGES + 1];
        partition_names
            .iter()
            .enumerate()
...
mod tests {
    ...
    const TEST_IMG_WITH_ONE_VBMETA_PATH: &str = "test_image_with_one_vbmeta.img";
    ...
    #[test]
    fn payload_with_one_vbmeta_image_and_no_initrd_passes_verification() -> Result<()> {
        let kernel = fs::read(TEST_IMG_WITH_ONE_VBMETA_PATH)?;
        let public_key = fs::read(PUBLIC_KEY_RSA4096_PATH)?;
    ...
    // e.g. payload_with_two_vbmeta_images_and_no_initrd_fails_verification
