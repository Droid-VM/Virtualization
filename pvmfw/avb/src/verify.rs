...
impl<'a> Payload<'a> {
    ...
    const MAX_NUM_OF_PARTITIONS: usize = 3;
    ...
    fn verify_partitions(&mut self, partition_names: &[&CStr]) -> Result<(), AvbImageVerifyError> {
        if partition_names.len() > Self::MAX_NUM_OF_PARTITIONS {
            return Err(AvbImageVerifyError::InvalidArgument);
        }
        let mut requested_partitions = [ptr::null(); Self::MAX_NUM_OF_PARTITIONS + 1];
        partition_names
            .iter()
            .enumerate()
