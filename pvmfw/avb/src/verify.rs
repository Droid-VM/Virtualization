...
impl<'a> Payload<'a> {
    ...
    fn get_partition(&self, partition_name: *const c_char) -> Result<&[u8], AvbIOError> {
        ...
        match partition_name.to_bytes_with_nul() {
            ...
            Self::INITRD_NORMAL_PARTITION_NAME => Ok(self.initrd),
            Self::INITRD_DEBUG_PARTITION_NAME => Ok(self.initrd),
            _ => Err(AvbIOError::NoSuchPartition),
        }
    }

    fn verify_partitions(&mut self, partition_names: &[&[u8]]) -> Result<(), AvbImageVerifyError> {
        let partition_names = partition_names
            .iter()
            .map(|p| CStr::from_bytes_with_nul(p).unwrap())
            .collect::<Vec<_>>();
        let requested_partitions = partition_names
            .iter()
            .map(|p| if p.to_bytes().is_empty() { ptr::null() } else { p.as_ptr() })
            .collect::<Vec<_>>();
        if requested_partitions.last().map(|p| !p.is_null()).unwrap_or(true) {
...
pub fn verify_payload(
    ...
    let mut payload = Payload { kernel, initrd: initrd.unwrap_or(&[]), trusted_public_key };
    // Mark the end of the array with NULL byte.
    let requested_partitions = vec![Payload::KERNEL_PARTITION_NAME, NULL_BYTE];
...
mod tests {
    ...
    #[test]
    fn kernel_with_one_hashdesc_in_footer_and_no_initrd_passes_verification() -> Result<()> {
    ...
    // e.g. kernel_with_two_hashdescs_in_footer_and_no_initrd_fails_verification
    ...
    fn assert_payload_verification_fails(
        ...
        assert_eq!(
            Err(expected_error),
            verify_payload(kernel, Some(initrd_normal), trusted_public_key)
        );
        Ok(())
    }
