...
fn to_avb_verify_result_allow_verification_error(
    ...
    match to_avb_verify_result(result) {
        ...
        | Err(AvbImageVerifyError::PublicKeyRejected)
        | Err(AvbImageVerifyError::RollbackIndex)
        | Err(AvbImageVerifyError::Verification) => Ok(()),
        Err(e) => Err(e),
    }
}
...
pub fn verify_payload(
    ...
    verify_partitions(verify_result)
}
...
fn verify_partitions(verify_result: &AvbSlotVerifyData) -> Result<Mode, AvbImageVerifyError> {
    ...
    for loaded_partition in loaded_partitions.iter() {
        is_not_null(loaded_partition.partition_name)?;
        ...
        match partition_name.to_bytes_with_nul() {
            Payload::KERNEL_PARTITION_NAME => {
                if partition_verify_result.is_err() {
                    return partition_verify_result;
                }
            }
            Payload::INITRD_NORMAL_PARTITION_NAME => {
            ...
            Payload::INITRD_DEBUG_PARTITION_NAME => {
                if let Err(e) = partition_verify_result {
                    partition_verify_errors.push(e);
                } else {
    ...
    if modes.len() == 1 {
        return Ok(modes.pop().unwrap());
    } else {
        return Err(partition_verify_errors.pop().unwrap_or(AvbImageVerifyError::Io));
    }
    Ok(())
}
