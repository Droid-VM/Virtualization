...
impl AvbHashDescriptorWrap {
    ...
    fn partition_name_raw(&self) -> Result<&[u8], AvbIOError> {
        ...
        let partition_name_len = self.hash_desc.partition_name_len as usize;
...
impl PartitionName {
    ...
    fn partition_name_matches_initrd(non_null_terminated_name: &[u8]) -> bool {
        return non_null_terminated_name
            == as_non_null_terminated_byte_str(Self::INITRD_NORMAL_PARTITION_NAME)
            || non_null_terminated_name
                == as_non_null_terminated_byte_str(Self::INITRD_DEBUG_PARTITION_NAME);
    }
}

fn as_non_null_terminated_byte_str(s: &[u8]) -> &[u8] {
    &s[0..(s.len() - 1)]
}
