...
extern "C" fn read_from_partition(
    ...
    if (offset >= kernel.len()) || (offset + num_bytes) > kernel.len() {
        return AvbIOResult::AVB_IO_RESULT_ERROR_IO;
    }
...
impl Payload {
    ...
    fn kernel(&self) -> &[u8] {
        // SAFETY: It is safe to restore the kernel with its saved pointer and length.
        unsafe { slice::from_raw_parts(self.kernel_data, self.kernel_length) }
    }

    fn trusted_public_key(&self) -> &[u8] {
        // SAFETY: It is safe to restore the public key with its saved pointer and length.
        unsafe {
...
mod tests {
    ...
    #[test]
    fn valid_payload_is_verified_successfully() -> Result<()> {
        let kernel = load_signed_microdroid_kernel()?;
    ...
    #[test]
    fn kernel_signed_with_different_key_fails_verification() -> Result<()> {
        let kernel = load_signed_microdroid_kernel()?;
    ...
    fn load_signed_microdroid_kernel() -> Result<Vec<u8>> {
