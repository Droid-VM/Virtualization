...
fn try_search_initrd_hash_descriptor(
    ...
    if !PartitionName::matches_initrd_partition_name_len(hash_desc.partition_name_len()) {
        // As we only look for initrd_* partitions, we don't need to continue if
...
impl<'a> Payload<'a> {
    ...
    fn verify_vbmeta(&self, verify_data: AvbSlotVerifyDataWrap) -> Result<(), AvbSlotVerifyError> {
        let vbmeta_images = verify_data.vbmeta_images()?;
        if vbmeta_images.len() != Self::EXPECTED_NUM_OF_VBMETA {
        ...
        if initrd_descriptor_exists {
            // There shouldn't any initrd descriptor when initrd is None.
            return Err(AvbSlotVerifyError::InvalidMetadata);
        }
        Ok(())
    }
