...
use alloc::vec::Vec;
use avb::{
    Descriptor, HashDescriptor, IoError, PartitionData, PropertyDescriptor, SlotVerifyError,
...
fn verify_property_and_get_capabilities(
    ...
    if iter.next().is_some() {
        return Err(PvmfwVerifyError::InvalidDescriptors(IoError::Io));
    }
...
impl<'a> HashDescriptors<'a> {
    ...
    /// is an error.
    fn get(descriptors: &'a [Descriptor<'a>]) -> Result<Self, PvmfwVerifyError> {
        let mut kernel = None;
        ...
        for descriptor in descriptors.iter().filter_map(|d| match d {
            ...
            let target = match descriptor
                ...
                .as_bytes()
                .try_into()
                .map_err(PvmfwVerifyError::InvalidDescriptors)?
            {
        ...
        Ok(Self {
            kernel: kernel.ok_or(SlotVerifyError::InvalidMetadata)?,
            initrd_normal,
            initrd_debug,
        })
    }
...
pub fn verify_payload<'a>(
    ...
    let initrd_descriptor =
        initrd_descriptor.ok_or(PvmfwVerifyError::InvalidDescriptors(IoError::Io))?;
    Ok(VerifiedBootData {
        debug_level,
        kernel_digest: copy_digest(hash_descriptors.kernel)?,
