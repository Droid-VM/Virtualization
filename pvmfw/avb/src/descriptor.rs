...
impl HashDescriptors {
    ...
    fn push(&mut self, descriptor: HashDescriptor) -> utils::Result<()> {
        if self.len() >= self.0.capacity() || self.iter().any(|d| d.partition_name_eq(&descriptor))
        {
...
unsafe fn try_check_and_save_descriptor(
    ...
    let desc = unsafe { AvbHashDescriptorWrap::from_decriptor_ptr(descriptor)? };
    // SAFETY: It is safe because the caller ensures that `descriptor` is a nonnull pointer
    // pointing to a valid struct.
...
impl AvbHashDescriptorWrap {
    ...
    unsafe fn from_decriptor_ptr(descriptor: *const AvbDescriptor) -> utils::Result<Self> {
        is_not_null(descriptor)?;
