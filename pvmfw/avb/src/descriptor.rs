...
use crate::utils::{is_not_null, to_nonnull, to_usize, usize_checked_add, DIGEST_SIZE};
use avb_bindgen::{
...
impl HashDescriptors {
    /// The maximum number of descriptor is also the maximum number of known partitions.
    ...
    pub(crate) fn find(&self, partition_name: PartitionName) -> &Option<HashDescriptor> {
        self.iter()
...
fn try_check_and_save_descriptor(
    ...
    let partition_name = data
        .get(desc.partition_name_start()..desc.partition_name_end()?)
        .ok_or(AvbIOError::RangeOutsidePartition)?;
    let digest = data
        .get(desc.digest_start()?..desc.digest_end()?)
        .ok_or(AvbIOError::RangeOutsidePartition)?;
...
impl HashDescriptor {
    fn new(
        partition_name: PartitionName,
        partition_digest: &[u8; DIGEST_SIZE],
    ) -> Result<Self, AvbIOError> {
...
impl AvbHashDescriptorWrap {
    ...
    fn partition_name_end(&self) -> Result<usize, AvbIOError> {
        usize_checked_add(self.partition_name_start(), to_usize(self.0.partition_name_len)?)
    }

    fn digest_start(&self) -> Result<usize, AvbIOError> {
        usize_checked_add(self.partition_name_end()?, to_usize(self.0.salt_len)?)
    }

    fn digest_end(&self) -> Result<usize, AvbIOError> {
        usize_checked_add(self.digest_start()?, to_usize(self.0.digest_len)?)
    }
}
