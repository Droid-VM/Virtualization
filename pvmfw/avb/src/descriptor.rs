...
#[derive(Default)]
pub(crate) struct HashDescriptors(ArrayVec<[Option<HashDescriptor>; Self::MAX_NUM_OF_DESCRIPTORS]>);
...
impl HashDescriptors {
    ...
    pub(crate) unsafe fn new_from(vbmeta: AvbVBMetaData) -> Result<Self, AvbSlotVerifyError> {
        is_not_null(vbmeta.vbmeta_data).map_err(|_| AvbSlotVerifyError::Io)?;
        let mut descriptors = Self::default();
        // SAFETY: It is safe as the raw pointer `vbmeta.vbmeta_data` is a nonnull pointer and
    ...
    fn iter(&self) -> HashDescriptorRefIter {
        self.0.iter().map(|d| d.as_ref().unwrap())
    }
}
...
unsafe fn try_check_and_save_descriptor(
    ...
    // SAFETY: It is safe because the caller ensures that `descriptor` is a nonnull pointer
    // pointing to a valid struct.
    let desc = unsafe { AvbHashDescriptorWrap::new_from(descriptor)? };
    // SAFETY: It is safe because the caller ensures that `descriptor` is a nonnull pointer
    // pointing to a valid struct.
    ...
    // SAFETY: It is safe because the caller ensures that `user_data` is a nonnull pointer
    // pointing to a valid struct.
...
impl AvbHashDescriptorWrap {
    ...
    unsafe fn new_from(descriptor: *const AvbDescriptor) -> utils::Result<Self> {
        is_not_null(descriptor)?;
