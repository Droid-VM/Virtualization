...
unsafe extern "C" fn check_and_save_descriptor(
    ...
    let desc = match AvbHashDescriptorWrap::try_from(descriptor) {
        Ok(desc) => desc,
        Err(_) => return false,
    };
    let data_len = match desc.data_len() {
        Ok(v) => v,
        Err(_) => return false,
    };
    // SAFETY: It is safe because `descriptor` is a nonnull pointer pointing to a valid
    // struct.
    let data = unsafe { slice::from_raw_parts(descriptor as *const u8, data_len) };
    let mut descriptors = match to_nonnull(user_data as *mut HashDescriptors) {
        Ok(v) => v,
        Err(_) => return false,
    };
    ...
    if let Ok(hash_desc) = HashDescriptor::new(&desc, data) {
        descriptors.push(hash_desc).is_ok()
    } else {
        false
    }
}
