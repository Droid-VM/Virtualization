...
pub fn get_or_generate_instance_salt(
    ...
    match entry {
        ...
        PvmfwEntry::New { header_index } => {
            ...
            // We currently only support single-blk entries.
            assert!(size_of::<EntryBody>() + aead.aead().unwrap().max_overhead() < blk.len());
            let encrypted = aead.seal(&mut blk, body.as_bytes()).map_err(Error::FailedSeal)?;
            ...
            header.write_to(blk_header).unwrap();
            blk_rest.fill(0);
