...
pub fn get_instance_salt(
    ...
    let entry = locate_entry(&mut partitions, &instance_img)?;
    trace!("Found pvmfw instance.img entry: {entry:?}");
    match entry {
        Entry::Existing { header_index, payload_size } => {
        ...
        Entry::New { header_index } => {
            ...
            let header = EntryHeader::new(Entry::UUID, payload_size);
            let (blk_header, blk_rest) = blk.split_at_mut(size_of::<EntryHeader>());
            blk_header.copy_from_slice(header.as_ref());
            blk_rest.fill(0);
            partitions
                .write_partition_block(&instance_img, header_index, &blk)
                .map_err(Error::FailedIo)?;
...
fn locate_entry(parts: &mut Partitions, part: &Partition) -> Result<Entry> {
    let mut blk = [0; BLK_SIZE];
    ...
    while let Some(header_index) = indices.next() {
        ...
        match (header.uuid(), header.payload_size()) {
            (uuid, _) if uuid.is_nil() => return Ok(Entry::New { header_index }),
            (Entry::UUID, 0) => return Err(Error::EmptyPvmfwEntry),
            (Entry::UUID, payload_size) => {
                return Ok(Entry::Existing { header_index, payload_size })
            }
            (uuid, payload_size) => {
                trace!("Skipping instance.img entry {uuid}: {payload_size:?} bytes");
                let n = blk_count(payload_size);
                if n > 0 {
...
impl EntryBody {
    ...
    fn from_bytes_matching<'a>(bytes: &'a [u8], dice_inputs: &PartialInputs) -> Result<&'a Self> {
        let body: &Self = bytes.as_ref();
        ...
        } else if body.mode().ok_or(Error::RecordedDiceModeMismatch)? != dice_inputs.mode {
