...
use crate::helpers::{self, dbm_available, page_4kb_of, RangeExt, SIZE_4KB, SIZE_4MB};
use crate::mmu;
...
fn mmio_guard_unmap_page(
    ...
    {
        if va_range.len() != SIZE_4KB {
            // Sanity check, this should not happen as we break up blocks into pages before MMIO
            // guard mapping.
            return Err(());
        }
        let page_base = page_4kb_of(va_range.start().0);
