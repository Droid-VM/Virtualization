...
pub struct MemoryTracker {
    ...
    mmio_regions: ArrayVec<[MemoryRange; MemoryTracker::CAPACITY]>,
}
...
impl MemoryTracker {
    ...
    pub fn map_mmio_range(&mut self, range: MemoryRange) -> Result<()> {
        ...
        for page_base in PageIterator::new(&range) {
    ...
    /// with any other previously allocated regions.
    ...
    pub fn mmio_unmap_all(&self) -> Result<()> {
        for region in &self.mmio_regions {
            for page_base in PageIterator::new(region) {
...
/// Iterator which yields the base address of each 4 KiB page within the given range.
struct PageIterator {
    next: usize,
    end: usize,
}
...
impl PageIterator {
    pub fn new(range: &MemoryRange) -> Self {
        Self { next: page_4kb_of(range.start), end: range.end }
    }
}
