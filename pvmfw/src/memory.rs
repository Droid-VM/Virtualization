...
use vmbase::{
    dsb, isb, layout,
    memory::{self, page_4kb_of, set_dbm_enabled, MemorySharer, PageTable, MMIO_LAZY_MAP_FLAG},
    tlbi,
};
...
impl MemoryTracker {
    ...
    const PVMFW_RANGE: MemoryRange = (BASE_ADDR - memory::SIZE_4MB)..BASE_ADDR;
...
pub fn appended_payload_range() -> Range<usize> {
    let start = memory::align_up(layout::binary_end(), memory::SIZE_4KB).unwrap();
    // pvmfw is contained in a 2MiB region so the payload can't be larger than the 2MiB alignment.
    let end = memory::align_up(start, memory::SIZE_2MB).unwrap();
    start..end
}
