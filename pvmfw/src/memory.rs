...
use crate::helpers::align_down;
use crate::helpers::align_up;
use crate::helpers::{self, page_4kb_of, SIZE_4KB};
use crate::hypervisor::hyp_meminfo;
use crate::hypervisor::mem_share;
use crate::hypervisor::mem_unshare;
...
/// align.
pub fn share_range(range: &MemoryRange, granule: usize) -> smccc::Result<()> {
...
pub fn unshare_range(range: &MemoryRange, granule: usize) -> smccc::Result<()> {
    for base in (align_down(range.start, granule)
        .expect("Memory protection granule was not a power of two")..range.end)
        .step_by(granule)
    {
...
///
/// Panics if `size` is 0.
pub fn shared_buffer_layout(size: usize) -> smccc::Result<Layout> {
    assert_ne!(size, 0);
