...
use crate::mmu;
use crate::smccc;
use crate::{mmio_guard, read_sysreg, write_sysreg};
...
impl MemoryTracker {
    ...
    pub fn flush_dirty_pages(&mut self) -> Result<()> {
        // Safe because this is just a memory barrier to ensure all hardware updates to the
        // page table have been observed before reading dirty state.
        unsafe {
            asm!("dsb ish");
        }
        ...
        let writable_static_regions = static_layout
            .iter()
            .filter(|(_, a)| !a.contains(Attributes::READ_ONLY) || a.contains(Attributes::DBM))
            .map(|(range, _)| range);
...
fn is_leaf_pte(desc: &Descriptor, level: usize) -> bool {
