...
pub unsafe fn init() {
    ...
    // SAFETY: Nothing else can access this memory, and we hand it over to the heap to manage and
    ...
    // SAFETY: The heap is locked, we are supplying a valid memory range, and we only do this once.
...
unsafe extern "C" fn free(ptr: *mut c_void) {
    ...
    let (ptr, size) = unsafe {
        // SAFETY: p is non-null and was allocated by malloc_, which prepends a correctly aligned
        // usize.
...
fn malloc_layout(size: NonZeroUsize) -> Option<Layout> {
    ...
    const ALIGN: usize = max_size(mem::size_of::<usize>(), mem::size_of::<u64>());
