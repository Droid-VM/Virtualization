...
unsafe extern "C" fn calloc(nmemb: usize, size: usize) -> *mut c_void {
    calloc_(nmemb, size).map_or(ptr::null_mut(), |p| p.cast::<c_void>().as_ptr())
}
...
unsafe fn calloc_(nmemb: usize, size: usize) -> Option<NonNull<usize>> {
    let size = NonZeroUsize::new(nmemb.checked_mul(size)?)?.checked_add(mem::size_of::<usize>())?;
