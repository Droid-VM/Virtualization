...
pub enum DebugPolicyError {
    ...
    /// The overlay result is invalid or malformed, and may be corrupted.
    ResultFdt(&'static str, FdtError),
}
...
impl fmt::Display for DebugPolicyError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::InvalidResultFdt(s, e) => write!(f, "Invalid result FDT. {s}: {e}"),
        }
    }
}
...
unsafe fn apply_debug_policy(
    ...
    fdt.pack().map_err(|e| DebugPolicyError::InvalidResultFdt("Failed to re-pack", e))
}
...
unsafe fn disable_ramdump(fdt: &mut libfdt::Fdt) -> Result<(), DebugPolicyError> {
    ...
    chosen_mut.setprop(bootargs_name, new_bootargs.as_slice()).map_err(|e| {
        DebugPolicyError::InvalidResultFdt(
            "Failed to remove crashkernel. FDT might be corrupted",
            e,
        )
    })
}
