...
unsafe fn disable_ramdump(fdt: &mut libfdt::Fdt) -> Result<(), DebugPolicyError> {
    ...
    let chosen = match fdt.node(chosen_path) {
        Ok(Some(node)) => node,
        _ => {
            warn!("Ramdump is already disabled. Nothing to do.");
            return Ok(());
        }
    };
    ...
    let bootargs = match chosen.getprop_str(bootargs_name) {
        ...
        _ => {
            warn!("crashkernel doesn't exist already because of missing bootargs in /chosen. Nothing to do.");
            return Ok(());
        }
    };
    ...
    match chosen_mut.setprop(
        bootargs_name,
        CStr::from_bytes_with_nul_unchecked(new_bootargs.as_slice()).to_bytes_with_nul(),
    ) {
        Ok(_) => {
...
unsafe fn is_ramdump_enabled(fdt: &libfdt::Fdt) -> bool {
    let common = match fdt.node(CStr::from_bytes_with_nul_unchecked(b"/avf/guest/common\0")) {
        Ok(Some(node)) => node,
        _ => {
            info!("Assumes no ramdump because of missing /avf/guest/common");
            return false;
        }
    };
    ...
    match common.getprop_u32(CStr::from_bytes_with_nul_unchecked(b"ramdump\0")) {
        Ok(Some(1)) => true,
        _ => {
            info!("Assumes no ramdump because ramdump isn't <1>");
            false
        }
    }
}

unsafe fn handle_ramdump(fdt: &mut libfdt::Fdt) -> Result<(), DebugPolicyError> {
...
/// Handles debug policies. This restores fdt if it's corrupted.
pub fn handle_debug_policy(
    fdt: &mut libfdt::Fdt,
    debug_policy: Option<&mut [u8]>,
) -> Result<(), DebugPolicyError> {
    let fdt_slices = fdt.as_slice();
    ...
    if res.is_err() {
        unsafe {
            fdt_backup
                .as_ptr()
                .copy_to_nonoverlapping(mem::transmute::<_, _>(&fdt), fdt_backup.len());
        }
    }
