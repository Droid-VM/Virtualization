...
unsafe fn disable_ramdump(fdt: &mut libfdt::Fdt) -> Result<(), DebugPolicyError> {
    ...
    let chosen = match fdt.node(chosen_path).map_err(|e| {
        ...
        Some(node) => node,
        None => {
            debug!("/chosen node doesn't exist. Assumes that ramdump is disabled already");
            return Ok(());
        }
    };
    ...
    let bootargs = match chosen.getprop_str(bootargs_name).map_err(|e| {
        ...
        Some(_) => {
            debug!("bootargs prop is empty. Assumes that ramdump is disabled already");
            return Ok(());
        }
        None => {
            debug!("bootargs prop doesn't exist. Assumes that ramdump is disabled already");
            return Ok(());
        }
    };
...
fn is_ramdump_enabled(fdt: &libfdt::Fdt) -> Result<bool, DebugPolicyError> {
    let common =
        match fdt.node(CStr::from_bytes_with_nul(b"/avf/guest/common\0").unwrap()).map_err(|e| {
    ...
    match common.getprop_u32(CStr::from_bytes_with_nul(b"ramdump\0").unwrap()).map_err(|e| {
        error!("Failed to find /avf/guest/common node: {e}");
        DebugPolicyError::InvalidDebugPolicyFdt
    })? {
        Some(0) => Ok(false),
        Some(v) => {
            debug!("Assumes ramdump with /avf/guest/common/ramdump: {v}");
            Ok(true)
        }
        None => {
