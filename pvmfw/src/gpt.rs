...
impl Partitions {
    ...
    pub fn new(mut device: VirtIOBlk) -> Result<Self> {
    ...
    pub fn get_partition_by_name(&mut self, name: &str) -> Result<Option<Partition>> {
        assert_eq!(Self::LBA_SIZE.rem_euclid(size_of::<Entry>()), 0);
        let entries_per_blk = Partitions::LBA_SIZE.checked_div(size_of::<Entry>()).unwrap();

        let mut name_buf = [0; Entry::NAME_LEN];
        for (dest, src) in name_buf.iter_mut().zip(name.encode_utf16()) {
            *dest = src;
        }

        let mut blk = [0; Self::LBA_SIZE];
        let mut rem = self.entries_count;
        let num_blocks = align_up(self.entries_count, entries_per_blk).unwrap() / entries_per_blk;
        for i in Header::ENTRIES_LBA..Header::ENTRIES_LBA.checked_add(num_blocks).unwrap() {
            ...
            for entry in entries {
                ...
                if entry_name == name_buf {
                    return Ok(Some(Partition::from_entry(entry)));
                }
                rem -= 1;
            }
        }
    ...
    pub fn read_partition_block(
        &mut self,
        part: &Partition,
        index: usize,
        blk: &mut [u8],
    ) -> Result<()> {
        let index = part.block_index(index).ok_or(Error::BlockOutsidePartition(index))?;
        self.read_block(index, blk)
    }

    pub fn write_partition_block(
        &mut self,
        part: &Partition,
        index: usize,
        blk: &[u8],
...
/// Structure as defined in release 2.10 of the UEFI Specification (5.3.3 GPT Partition Entry Array).
#[repr(C, packed)]
struct Entry {
    type_guid: Uuid,
    guid: Uuid,
    ...
    last_lba: Lba,
    flags: u64,
    name: [u16; Entry::NAME_LEN], // UTF-16
}

impl Entry {
    const NAME_LEN: usize = 36; // number of u16
