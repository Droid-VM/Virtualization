...
fn to_utf16<'a>(s: &str, res: &'a mut [u8]) -> Option<&'a [u8]> {
    let len = (s.len() + 1) * 2;
...
impl Partitions {
    ...
    pub fn get_partition_by_name(&mut self, name: &str) -> Result<Option<Partition>> {
        ...
        let mut name_buf = [0; Entry::NAME_SIZE];
        let name = to_utf16(name, &mut name_buf).unwrap();

        let mut blk = [0; Self::LBA_SIZE];
        let mut i = Header::ENTRIES_LBA;
        ...
        while rem != 0 {
            ...
            let entries = unsafe { slice::from_raw_parts(entries, entries_per_blk) };
...
struct Entry {
    ...
    last_lba: Lba,
    flags: u64,
    name: [u8; Entry::NAME_SIZE], // UTF-16
}
