...
impl From<trng::Error> for Error {
    fn from(e: trng::Error) -> Self {
        Self::Trng(e)
    }
}
...
fn fill_with_entropy(s: &mut [u8]) -> Result<()> {
    log::info!("fill_with_entropy: {:?}, size: {:#x}(={})", s.as_ptr(), s.len(), s.len());
    const MAX_BYTES_PER_CALL: usize = size_of::<u64>() * 3;
    ...
    for chunk in aligned.chunks_exact_mut(MAX_BYTES_PER_CALL) {
        let (r, s, t) = hvc::trng_rnd64((chunk.len() * (u8::BITS as usize)) as _)?;
    ...
    if !remainder.is_empty() {
        ...
        let (r, s, t) = hvc::trng_rnd64((remainder.len() * (u8::BITS as usize)) as _)?;
