...
#[repr(transparent)]
pub struct AeadCtx<'a>(EVP_AEAD_CTX, PhantomData<&'a [u8]>);

impl<'a> AeadCtx<'a> {
    pub fn new_aes_256_gcm_randnonce(key: &'a [u8]) -> Result<Self> {
        let aead = Aead::aes_256_gcm_randnonce().unwrap();

        Self::new(aead, key)
    }

    fn new(aead: &'static Aead, key: &'a [u8]) -> Result<Self> {
        ...
        if result == 0 {
            ...
            // SAFETY - We assume that the non-NULL value points to a valid and static EVP_AEAD.
            Ok(Self(unsafe { ctx.assume_init() }, PhantomData))
        }
    }
...
struct EVP_AEAD_CTX {
    aead: *const EVP_AEAD,
    state: [u8; 580], // This field is a C union; replace it with its largest member her.
    tag_len: u64,     // This seems to be a `uint8_t` with `alignof(uint64_t)`.
}

impl AsRef<EVP_AEAD_CTX> for AeadCtx<'_> {
