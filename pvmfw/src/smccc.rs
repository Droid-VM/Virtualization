...
const fn check_smccc_err(ret: i64) -> Result<(), Error> {
    match check_smccc_value(ret) {
        Err(e) => Err(e),
        Ok(0) => Ok(()),
        Ok(v) => Err(Error::Unexpected(v)),
    }
}

const fn check_smccc_value(ret: i64) -> Result<u64, Error> {
    match ret {
...
pub fn mmio_guard_unmap(ipa: u64) -> Result<(), Error> {
    let mut args = [0u64; 17];
    args[0] = ipa;
