...
fn mmio_guard_info() -> smccc::Result<u64> {
    const FUNC_ID: u32 = 0xc6000005;
    let args = [0u64; 17];

    smccc::checked_hvc64(FUNC_ID, args)
}

fn mmio_guard_enroll() -> smccc::Result<()> {
    const FUNC_ID: u32 = 0xc6000006;
    let args = [0u64; 17];

    smccc::checked_hvc64_expect_success(FUNC_ID, args)
}
...
fn mmio_guard_map(ipa: u64) -> smccc::Result<()> {
    ...
    match smccc::checked_hvc64_expect_success(FUNC_ID, args) {
        Err(smccc::Error::Unexpected(e)) if e <= (u32::MAX as u64) && (e as u32 as i32) < 0 => {
