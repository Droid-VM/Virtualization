...
impl DeviceTreeMask {
    ...
    fn mask(&mut self, path: &DtPathTokens) -> Result<()> {
        ...
        for path_token in path.tokens {
            let DeviceTreeChildrenMask::Partial(children) = mask.children else {
                // Everything is masked below. Skip now.
                return Ok(());
            };
            let child = children.iter_mut().find(|child| child.name_bytes == path_token);
            mask = match child {
                Some(child) => child,
                None => {
                    let child = Self {
    ...
    fn is_child_masked(&self, name: &CStr) -> bool {
        let DeviceTreeChildrenMask::Partial(children) = self.children else {
            return true;
        };
        children.iter().find(|node| node.name.as_bytes() == name).is_some()
    }
}
...
impl VmDtbo {
    ...
    fn parse_physical_devices(&self) -> Result<BTreeMap<Phandle, PhysicalDeviceInfo>> {
        let Some(physical) = self.as_ref().node(cstr!("/host"))? else {
        ...
        // TODO(b/277993056): Validate <reg> whether it overlaps.
        // TODO(b/277993056): Validate <iommus> whether it conflicts.
    ...
    fn parse_overlayable_nodes_with_phandle_reference(
        ...
        for (fixup_node, depth) in local_fixups.descendants() {
            ...
            for fixup_prop in fixup_node.properties()? {
                ...
                let node_cells = target_node
                    .getprop_cells(fixup_prop.name()?)?
                    .ok_or(DeviceAssignmentError::InvalidDtbo)?
                    .collect();

                for fixup_prop_cell in fixup_prop_values.chunks(CELL_SIZE) {
                    let phandle_offset = u32::from_be_bytes(
                        fixup_prop_cells.try_into().ok_or(DeviceAssignmentError::InvalidDtbo)?,
                    );
                    let phandle_offset = usize::try_from(phandle_offset);
...
impl AssignedDeviceInfo {
    fn validate_reg(
        ...
        // pv reg and physcial reg should have 1:1 match in oder.
        for (reg, phys_reg) in device_reg.iter().zip(physical_device_reg.iter()) {
            let addr = hypervisor.get_phys_mmio_token(reg.addr, reg.size).map_err(|e| {
                error!("Failed to validate device <reg>, error={e:?}, reg={reg:?}");
                DeviceAssignmentError::InvalidReg
            })?;
            if phys_reg.addr != addr || phys_reg.size != reg.size {
                error!("Failed to validate device <reg>. No matching phys reg for reg={reg:?}");
                return Err(DeviceAssignmentError::InvalidReg);
            }
        }
        Ok(())
