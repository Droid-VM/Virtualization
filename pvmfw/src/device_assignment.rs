...
pub enum DeviceAssignmentError {
    ...
    /// Internal error
    Internal,
    /// Not an overlay node but a physical node.
    NotAnOverlayNode,
...
impl fmt::Display for DeviceAssignmentError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::NotAnOverlayNode => write!(f, "Not an overlay node"),
...
impl VmDtbo {
    ...
    fn locate_overlay_target_path(&self, dtbo_node_path: &CStr, node: &FdtNode) -> Result<CString> {
        ...
        if overlay_node_name != Some(b"__overlay__") {
            return Err(DeviceAssignmentError::NotAnOverlayNode);
        }
...
/// Iommu for both physical and paravirtualized
...
impl Iommu {
    fn parse_iommu_node(node: &FdtNode) -> Result<Self> {
...
struct AssignedDeviceInfo {
    ...
    interrupts: Vec<u8>,
    // Parsed <iommus> property from the crosvm DT. Tuple of Iommu and vSID.
    iommus: Vec<(Iommu, Sid)>,
}
            ...
            match hypervisor.get_phys_mmio_token(reg.addr, reg.size) {
                Ok(addr) => {
                    let phys_reg = physical_device
                        ...
                        .iter()
                        .find(|phys_reg| {
                            phys_reg.addr == addr && phys_reg.size == reg.size
                });
                    if phys_reg.is_none() {
                        let name = node.name().unwrap_or_default();
                        error!("Failed to validate device <reg>. No matching phys address for name={name:?}, reg={reg:?}");
println!("Failed to validate device <reg>. No matching phys address for name={name:?}, reg={reg:?}");
                        return Err(DeviceAssignmentError::InvalidReg);
                    }
                }
                Err(e) => {
...
println!("Failed to validate device <reg>, error={e:?}, name={name:?}, reg={reg:?}");
                    return Err(DeviceAssignmentError::InvalidReg);
                }
            }
        }
    ...
    fn parse_iommus(
        hypervisor: &dyn DeviceAssigningHypervisor,
        pviommus: &BTreeMap<Phandle, Iommu>,
        physical_device: &PhysicalDevice,
        node: &FdtNode,
    ) -> Result<Vec<(Iommu, Sid)>> {
                ...
                Ok((id, sid)) => {
                    let phys_iommu =
                        physical_device.iommus.iter().find(|(phys_iommu, phys_sid)| {
                            (phys_iommu.id as u64, phys_sid.0 as u64) == (id, sid)
                        });
    ...
    fn parse(
        ...
        let node_path = vm_dtbo.locate_overlay_target_path(dtbo_node_path, &node)?;

        // Note: Currently can only assign devices backed by physical devices.
        let phandle = node.get_phandle()?.ok_or(DeviceAssignmentError::InvalidDtbo)?;
        let physical_device =
            physical_devices.get(&phandle).ok_or(DeviceAssignmentError::InvalidDtbo)?;

        let Some(node) = fdt.node(&node_path)? else { return Ok(None) };

        let reg = Self::parse_reg(hypervisor, physical_device, &node)?;
        let interrupts = Self::parse_interrupts(&node)?;
        let iommus = Self::parse_iommus(hypervisor, pviommus, physical_device, &node)?;
        Ok(Some(Self { node_path, dtbo_node_path: dtbo_node_path.into(), reg, interrupts, iommus }))
    }

    fn patch(&self, fdt: &mut Fdt, pviommu_phandles: &BTreeMap<Iommu, Phandle>) -> Result<()> {
        let mut dst = fdt.node_mut(&self.node_path)?.unwrap();
        dst.setprop(cstr!("reg"), &DeviceReg::to_be_bytes(&self.reg))?;
...
impl DeviceAssignmentInfo {
    ...
    fn parse_pviommus(fdt: &Fdt) -> Result<BTreeMap<Phandle, Iommu>> {
    ...
    fn parse_physical_devices(vm_dtbo: &VmDtbo) -> Result<BTreeMap<Phandle, PhysicalDevice>> {
        ...
        // Step 1: build IOMMUs
        // Note: Currently only supports devices backed by physical device
        let physical =
            vm_dtbo.node(cstr!("/physical"))?.ok_or(DeviceAssignmentError::InvalidDtbo)?;
        let mut phys_iommus = BTreeMap::<Phandle, Iommu>::new();
        for node in physical.descendants() {
            if node.getprop_u32(cstr!("#iommu-cells"))?.is_none() {
                continue;
            }
            let Some(phandle) = node.get_phandle()? else {
                continue; // Skips unreachable IOMMU node
            };
            let iommu = Iommu::parse_iommu_node(&node)?;
        ...
        for node in physical.descendants() {
            ...
            physical_devices.insert(
                ...
                PhysicalDevice {
                    ...
                    iommus: Iommu::parse_iommus_prop(&node, &phys_iommus)?,
                },
            );
        }
    ...
    pub fn parse(
        ...
        for symbol_prop in symbols_node.properties()? {
            ...
            match AssignedDeviceInfo::parse(
                fdt,
                vm_dtbo,
                hypervisor,
                &pviommus,
                ...
                Ok(Some(assigned_device)) => assigned_devices.push(assigned_device),
                Ok(None) => filtered_dtbo_paths.push(dtbo_node_path.into()),
                Err(DeviceAssignmentError::NotAnOverlayNode) => continue,
                Err(e) => return Err(e),
            }
        }
    ...
    pub fn filter(&self, vm_dtbo: &mut VmDtbo) -> Result<()> {
        ...
        // Clean up any references from physical from assignable devices
        // so applying overlay doesn't complain about missing phandle references.
        let physical_node = vm_dtbo
            .node_mut(cstr!("/__local_fixups__/physical"))?
            .ok_or(DeviceAssignmentError::Internal)?;
        physical_node.nop()?;

        // Note: Any node without __overlay__ will be ignored when apply_overlay,
        // so OK not to filter here.

        Ok(())
    }

    fn patch_pviommus(&self, fdt: &mut Fdt) -> Result<BTreeMap<Iommu, Phandle>> {
...
mod tests {
    ...
    struct MockHypervisor {
        mmio_token: BTreeMap<(u64, u64), u64>,
        iommu_token: BTreeMap<(u64, u64), (u64, u64)>,
    }

    impl DeviceAssigningHypervisor for MockHypervisor {
        fn get_phys_mmio_token(&self, base_ipa: u64, size: u64) -> hyp::Result<u64> {
            Ok(*self.mmio_token.get(&(base_ipa, size)).ok_or(hyp::Error::KvmError(
        ...
        fn get_phys_iommu_token(&self, pviommu_id: u64, vsid: u64) -> hyp::Result<(u64, u64)> {
            Ok(*self.iommu_token.get(&(pviommu_id, vsid)).ok_or(hyp::Error::KvmError(
                hyp::KvmError::InvalidParameter,
    ...
    fn device_info_assigned_info_without_iommus() {
        ...
        let hypervisor = MockHypervisor {
            mmio_token: [((0x9, 0xFF), 0x300)].into(),
            iommu_token: BTreeMap::new(),
        };
    ...
    fn device_info_assigned_info() {
        ...
        let hypervisor = MockHypervisor {
            mmio_token: [((0x9, 0xFF), 0x10000000)].into(),
            iommu_token: [((0x4, 0xFF0), (0x10E40000, 0x4))].into(),
        };
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, &hypervisor).unwrap().unwrap();
        ...
        let expected = [AssignedDeviceInfo {
            ...
            reg: vec![[0x9, 0xFF].into()],
            interrupts: into_fdt_prop(vec![0x0, 0xF, 0x4]),
            iommus: vec![(Iommu { id: 0x4 }, Sid(0xFF0))],
        }];
    ...
    fn device_info_filter() {
        ...
        let hypervisor = MockHypervisor {
            mmio_token: [((0x9, 0xFF), 0x10000000)].into(),
            iommu_token: [((0x4_u64, 0xFF0_u64), (0x10E40000_u64, 4_u64))].into(),
        };
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, &hypervisor).unwrap().unwrap();
    ...
    fn device_info_multiple_devices_iommus() {
        ...
        let hypervisor = MockHypervisor {
            mmio_token: [
                ((0x9, 0xFF), 0x12F00000),
                ((0x100, 0x100), 0xF00000),
                ((0x200, 0x100), 0xF10000),
            ]
            .into(),
            iommu_token: [
        ...
        let expected_devices = [
            ...
            AssignedDeviceNode {
                ...
                reg: into_fdt_prop(vec![0x0, 0x100, 0x0, 0x100, 0x0, 0x200, 0x0, 0x100]),
    ...
    fn device_info_iommu_sharing() {
        ...
        let hypervisor = MockHypervisor {
            mmio_token: [((0x9, 0xFF), 0x12F00000), ((0x100, 0x9), 0x12F00000)].into(),
            iommu_token: [((0x4, 0xFF0), (0x40000, 4))].into(),
        };
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, &hypervisor).unwrap().unwrap();
        device_info.filter(vm_dtbo).unwrap();
