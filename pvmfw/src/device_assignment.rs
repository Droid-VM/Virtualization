...
impl VmDtbo {
    ...
    // Estimates and returns device node path as if the given dtbo node path is assigned and VM DTBO
    // is overlaid. For given dtbo node path, this concatenates <target-path> of the enclosing
    // fragment and relateive path from __overlay__ node.
    //
    ...
    //    };
    //
    // Then estimate_asssigned_device_node_path(cstr!("/fragment@eh/__overlay__/eh")) is Ok("/eh")
    //
    ...
    fn estimate_asssigned_device_node_path(&self, dtbo_node_path: &CStr) -> Result<CString> {
...
struct AssignedDeviceInfo {
    ...
    interrupts: Vec<u8>,
    // <iommus> property and its node info from crosvm DT. Single Iommu because of `#iommu-cells = <0>`
    iommus: PvIommu,
}
...
impl AssignedDeviceInfo {
    ...
    fn new(fdt: &Fdt, vm_dtbo: &VmDtbo, dtbo_node_path: &CStr) -> Result<Option<Self>> {
        let node_path = vm_dtbo.estimate_asssigned_device_node_path(dtbo_node_path)?;
...
impl DeviceAssignmentInfo {
    ...
    pub fn patch(&self, fdt: &mut Fdt) -> Result<()> {
        ...
        // Patch iommus
        let mut pviommu_node = fdt.compatible_nodes(compatible)
        for device in &self.assigned_devices {
        }
        Ok(())
    }
