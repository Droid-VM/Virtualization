...
impl fmt::Display for DeviceAssignmentError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::InvalidHypervisor => write!(f, "Invalid hypervisor for device assignment"),
...
fn is_physical_node(dtbo_path: &CStr) -> bool {
    dtbo_path
        .to_bytes()
        .split(|char| *char == b'/')
        .filter(|&component| !component.is_empty())
        .take(1)
        .next()
        .map_or(false, |name| name == b"physical")
}
...
impl AssignedDeviceInfo {
    ...
    fn parse(
        ...
        let hypervisor = hypervisor.ok_or(DeviceAssignmentError::InvalidHypervisor)?;
...
impl DeviceAssignmentInfo {
    ...
    pub fn parse(
        ...
        for symbol_prop in symbols_node.properties()? {
            ...
            if is_physical_node(dtbo_node_path) {
                continue;
            }
            let assigned_device =
                AssignedDeviceInfo::parse(fdt, vm_dtbo, hypervisor, &pviommus, dtbo_node_path)?;
        ...
        filtered_dtbo_paths.push(CString::new("/__local_fixups__/physical").unwrap());
...
mod tests {
    ...
    fn device_info_new_without_symbols() {
        ...
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, None).unwrap();
    ...
    fn device_info_assigned_info_without_iommus() {
        ...
        let device_info =
            DeviceAssignmentInfo::parse(fdt, vm_dtbo, Some(&hypervisor)).unwrap().unwrap();
    ...
    fn device_info_new_with_empty_device_tree() {
        ...
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, Some(&hypervisor)).unwrap();
        assert_eq!(device_info, None);
    }
    ...
    fn device_info_iommu_id_conflict() {
        ...
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, Some(&hypervisor));
    ...
    #[test]
    fn device_info_new_without_hypervisor() {
        let mut fdt_data = fs::read(FDT_FILE_PATH).unwrap();
        let mut vm_dtbo_data = fs::read(VM_DTBO_FILE_PATH).unwrap();
        let fdt = Fdt::from_mut_slice(&mut fdt_data).unwrap();
        let vm_dtbo = VmDtbo::from_mut_slice(&mut vm_dtbo_data).unwrap();

        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, None);

        assert_eq!(device_info, Err(DeviceAssignmentError::InvalidHypervisor));
    }
}
