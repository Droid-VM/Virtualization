...
impl VmDtbo {
    ...
    pub fn from_mut_slice(dtbo: &mut [u8]) -> Result<&mut Self> {
        let mut fdt =
            Fdt::from_mut_slice(dtbo).map_err(|e| DeviceAssignmentError::UnexpectedFdtError(e))?;
        // DO NOT SUBMIT: doesn't compile
        Ok(fdt.as_mut())
    }
    ...
    fn overlay_target_path(&self, overlay_node_path: &CStr) -> Result<CString> {
        ...
        let node = self.node(overlay_node_path)?.ok_or(DeviceAssignmentError::InvalidSymbols)?;
...
struct AssignableDeviceInfo {
    /// Node path of assignable device in VM DTBO.
    /// This would be value of a property in /__symbol__ node.
    node_path: CString,
...
impl AssignedDeviceInfo {
    ...
    fn new(fdt: &Fdt, node_path: CString) -> Result<Option<Self>> {
        ...
        Ok(Some(Self { node_path, reg: reg.to_vec(), interrupts: interrupts.to_vec() }))
    }
}
...
impl DeviceAssignmentInfo {
    ...
    pub fn new(fdt: &Fdt, vm_dtbo: &VmDtbo) -> Result<Option<Self>> {
        let Some(symbols_node) = vm_dtbo.symbols()? else {
        ...
        for symbol_prop in symbols_node.properties()? {
            ...
            let node_path = CString::from_vec_with_nul(symbol_prop_value)
                .or(Err(DeviceAssignmentError::InvalidSymbols))?;
            let overlay_target_path = vm_dtbo.overlay_target_path(&node_path)?;

            assignable_devices.push(AssignableDeviceInfo { node_path, overlay_target_path });
            if let Some(assigned_device) = AssignedDeviceInfo::new(vm_dtbo, overlay_target_path)? {
                assigned_devices.push(assigned_device);
            }
        }

        Ok(assigned_devices.is_empty().then_some(Self { assignable_devices, assigned_devices }))
    }
    ...
    pub fn filter(&self, vm_dtbo: &mut VmDtbo) -> Result<()> {
        ...
        for assignable_device in &self.assignable_devices {
            ...
            if device.is_some() {
                let mut node = vm_dtbo.node_mut(&assignable_device.node_path).unwrap().unwrap();
                for prop in UNUSED_VM_DTBO_PROP {
                    node.nop_property(prop)?;
                }
            } else {
                let node = vm_dtbo.node_mut(&assignable_device.node_path).unwrap().unwrap();
                node.nop()?;
            }
        }
...
mod tests {
    ...
    fn device_info_patch() {
        let mut vm_dtbo_data = fs::read(VM_DTBO_FILE_PATH).unwrap();
        let mut vm_dtbo = VmDtbo::from_mut_slice(&mut vm_dtbo_data).unwrap();
        let mut fdt_data = fs::read(FDT_FILE_PATH).unwrap();
        let fdt = Fdt::from_mut_slice(&mut fdt_data).unwrap();
        let mut data = vec![0_u8; fdt.as_slice().len() + vm_dtbo.as_slice().len()];
        let platform_dt = Fdt::create_empty_tree(data.as_mut_slice()).unwrap();

        let device_info = DeviceAssignmentInfo::new(fdt, &mut vm_dtbo.deref()).unwrap().unwrap();
        device_info.filter(vm_dtbo).unwrap();

        // SAFETY: unsafe call in test code
        unsafe {
            platform_dt.apply_overlay(vm_dtbo).unwrap();
        }
