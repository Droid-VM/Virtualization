    ...
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::DuplicatedPvIommuIds => {
                write!(f, "Duplicated PV IOMMU IDs exist. IDs must unique")
            }
...
pub struct DeviceAssignmentInfo {
    assigned_pviommus: BTreeSet<PvIommu>,
...
impl DeviceAssignmentInfo {
    ...
    fn parse_pviommus(fdt: &Fdt) -> Result<BTreeMap<Phandle, PvIommu>> {
        ...
        let mut pviommu_ids = BTreeSet::new();
        ...
        for compatible in fdt.compatible_nodes(Self::PVIOMMU_COMPATIBLE)? {
            let Some(phandle) = compatible.get_phandle()? else {
                continue; // Skips dummy PV IOMMU node which cannot be assigned.
            };
            let pviommu = PvIommu::parse(&compatible)?;
            if !pviommu_ids.insert(pviommu.id) {
    ...
    pub fn parse(fdt: &Fdt, vm_dtbo: &VmDtbo) -> Result<Option<Self>> {
        ...
        let mut assigned_pviommus = BTreeSet::new();
        for device in &assigned_devices {
            assigned_pviommus.append(&mut device.iommus.iter().cloned().collect());
        }
        filtered_dtbo_paths.push(CString::new("/__symbols__").unwrap());

        // Note: pviommu_ids may contain unassigned PV IOMMUs, but it wouldn't be a huge problem.
    ...
    fn patch_pviommus(&self, fdt: &mut Fdt) -> Result<BTreeMap<PvIommu, Phandle>> {
        ...
        for pviommu in &self.assigned_pviommus {
            let mut assigned_pviommu = compatible.ok_or(DeviceAssignmentError::TooManyPvIommu)?;
            let assigned_pviommu_phandle =
                assigned_pviommu.as_node().get_phandle()?.ok_or(DeviceAssignmentError::Internal)?;
            assigned_pviommu.setprop_inplace(cstr!("id"), &pviommu.id.to_be_bytes())?;
            if pviommu_phandles.insert(*pviommu, assigned_pviommu_phandle).is_some() {
...
mod tests {
    ...
    fn device_info_patch() {
        ...
        // Note: Intentionally not using AssignedDeviceNode for matching all nodes.
