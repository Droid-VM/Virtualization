...
impl AssignedDeviceInfo {
    ...
    fn parse_iommus(node: &FdtNode) -> Result<Option<Vec<PvIommu>>> {
        let Some(pviommu_phandles) = node.getprop_cells(cstr!("iommus"))? else { return Ok(None) };
...
impl DeviceAssignmentInfo {
    ...
    pub fn patch(&self, fdt: &mut Fdt) -> Result<()> {
        ...
        for device in &self.assigned_devices {
            let iommu_cnt = match device.iommus.as_ref() {
                Some(iommus) => iommus.len(),
                None => 0,
            };
            device.patch(fdt, &pviommu_phandles[pviommu_used..pviommu_used + iommu_cnt])?;
            pviommu_used += iommu_cnt;
        }
...
mod tests {
    ...
    fn device_info_patch() {
        ...
        type FdtResult<T> = core::result::Result<T, FdtError>;
