...
impl AssignedDeviceInfo {
    ...
    fn parse_iommus(node: &FdtNode) -> Result<Vec<PvIommu>> {
    ...
    fn patch(&self, fdt: &mut Fdt, pviommus_phandles: &[Phandle]) -> Result<()> {
        ...
        if self.iommus.len() != pviommus_phandles.len() {
            return Err(DeviceAssignmentError::InternalError);
        }
...
impl DeviceAssignmentInfo {
    ...
    pub fn parse(fdt: &Fdt, vm_dtbo: &VmDtbo) -> Result<Option<Self>> {
        ...
        for assigned_device in &assigned_devices {
            for pviommu in &assigned_device.iommus {
    ...
    pub fn patch(&self, fdt: &mut Fdt) -> Result<()> {
        // Patch pre-populated PV IOMMU nodes for assigned devices.
        const PVIOMMU_COMPATIBLE: &CStr = cstr!("pkvm,pviommu");
        let mut pviommu_phandles = vec![];
        let mut compatible = fdt.root_mut()?.next_compatible(PVIOMMU_COMPATIBLE)?;
        for assigned_device in &self.assigned_devices {
            for iommu in &assigned_device.iommus {
                let mut assigned_pviommu =
                    compatible.ok_or(DeviceAssignmentError::TooManyPvIommu)?;
                pviommu_phandles.push(assigned_pviommu.as_node().get_phandle()?.unwrap());
                assigned_pviommu.setprop_inplace(cstr!("id"), &u32::to_be_bytes(iommu.id))?;
                compatible = assigned_pviommu.next_compatible(PVIOMMU_COMPATIBLE)?;
            }
        }
        // Filters unassigned pviommus.
        while let Some(filtered_pviommu) = compatible {
            compatible = filtered_pviommu.delete_and_next_compatible(PVIOMMU_COMPATIBLE)?;
        }

        // Patch assigned devices
        let mut pviommu_used = 0;
        for device in &self.assigned_devices {
            let iommu_phandles =
                &pviommu_phandles[pviommu_used..pviommu_used + device.iommus.len()];
            device.patch(fdt, iommu_phandles)?;
            pviommu_used += iommu_phandles.len();
        }
