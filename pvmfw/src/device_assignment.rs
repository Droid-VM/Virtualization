...
impl AssignedDeviceInfo {
    ...
    fn parse(
        fdt: &Fdt,
        vm_dtbo: &VmDtbo,
        labeled_dtbo_node_path: &DtPathTokens,
        physical_devices: &BTreeMap<Phandle, PhysicalDeviceInfo>,
        pviommus: &BTreeMap<Phandle, PvIommu>,
        hypervisor: &dyn DeviceAssigningHypervisor,
    ) -> Result<Option<Self>> {
        let dtbo_node = vm_dtbo
            .as_ref()
            .node(labeled_dtbo_node_path)?
            .ok_or(DeviceAssignmentError::InvalidSymbols)?;
        let node_path = vm_dtbo.locate_overlay_target_path(labeled_dtbo_node_path, &dtbo_node)?;
...
impl DeviceAssignmentInfo {
    ...
    pub fn filter(&self, vm_dtbo: &mut VmDtbo) -> Result<()> {
        ...
        // /__fixups__ node will be implicitly removed to prevent modifying platform DT
