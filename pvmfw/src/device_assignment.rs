...
impl<'a> DtPathTokens<'a> {
    ...
    fn new_with_tokens(tokens: &[&'a [u8]]) -> Result<Self> {
        for token in tokens {
            if token.contains(&b'\0') {
                return Err(DeviceAssignmentError::Internal);
            }
        }
        Ok(Self { tokens: tokens.to_vec() })
    }
...
impl VmDtbo {
    ...
    fn locate_overlay_target_path(
        ...
        let overlaid_path = DtPathTokens::new_with_tokens(&dtbo_node_path.tokens.as_slice()[2..])?;
    ...
    fn node(&self, path: &DtPathTokens) -> Result<Option<FdtNode>> {
        ...
        for token in &path.tokens {
            node = match node.subnode_with_name_bytes(token)? {
...
impl AssignedDeviceInfo {
    fn validate_reg(
        ...
        if device_reg.len() != physical_device_reg.len() {
            println!("Failed to validate device <reg>");
        ...
        for (reg, phys_reg) in device_reg.iter().zip(physical_device_reg.iter()) {
            let addr = hypervisor.get_phys_mmio_token(reg.addr, reg.size).map_err(|e| {
                ...
                println!("Failed to validate device <reg>, error={e:?}, reg={reg:x?}");
            ...
            if phys_reg.addr != addr {
                ...
                println!("Failed to validate device <reg>. No matching phys reg for reg={reg:x?}");
                return Err(DeviceAssignmentError::InvalidReg);
            }
        }
        Ok(())
    ...
    fn validate_iommus(
        ...
        for (pviommu, vsid) in iommus {
            ...
            .map_err(|e| {
                ...
                println!("Failed to validate device <iommus>, error={e:?}, pviommu={pviommu:?}, vsid={vsid:?}");
                DeviceAssignmentError::InvalidIommus
            })?;

            let phys_iommu = physical_device_iommu
                .iter()
                .find(|(phys_iommu, phys_sid)| (phys_iommu.token, phys_sid.0) == (id, sid));
            if phys_iommu.is_none() {
                error!("Failed to validate device <iommus>. No matching phys iommu for pviommu={pviommu:?}, vsid={vsid:?}");
                println!("Failed to validate device <iommus>. No matching phys iommu for pviommu={pviommu:?}, vsid={vsid:?}");
                return Err(DeviceAssignmentError::InvalidIommus);
            }
        }
        Ok(())
...
impl DeviceAssignmentInfo {
    ...
    // Validate whether there's any overlap among all PV regs and phys regs.
    fn validate_all_regs(
        assigned_devices: &[AssignedDeviceInfo],
        physical_devices: &BTreeMap<Phandle, PhysicalDeviceInfo>,
    ) -> Result<()> {
        let mut all_regs = Vec::new();
        for assigned_device in assigned_devices {
            all_regs.extend_from_slice(&assigned_device.reg);
        }
        for physical_device in physical_devices.values() {
            all_regs.extend_from_slice(&physical_device.reg);
        }
        all_regs.sort_unstable();
        for i in 1..all_regs.len() {
            if all_regs[i - 1].addr + all_regs[i - 1].size > all_regs[i].addr {
                error!(
                    "Failed to validate device <reg>. There should be no overlap among all PV and phys <reg>, but found {:?}, {:?}",
                    all_regs[i - 1],
                    all_regs[i]
                );
                println!(
                    "Failed to validate device <reg>. There should b <reg>, {:?} {:?}",
                    all_regs[i - 1],
                    all_regs[i]
                );
                return Err(DeviceAssignmentError::InvalidReg);
    ...
    fn validate_assigned_devices_iommus(assigned_devices: &[AssignedDeviceInfo]) -> Result<()> {
        let mut all_iommus = BTreeSet::new();
        for assigned_device in assigned_devices {
...
mod tests {
    ...
    const FDT_WITH_REG_OVERLAPPING_FILE_PATH: &str = "test_pvmfw_devices_with_reg_overlapping.dtb";
    ...
    #[test]
    fn device_info_invalid_reg_overlapping() {
        let mut fdt_data = fs::read(FDT_WITH_REG_OVERLAPPING_FILE_PATH).unwrap();
        let mut vm_dtbo_data = fs::read(VM_DTBO_FILE_PATH).unwrap();
        let fdt = Fdt::from_mut_slice(&mut fdt_data).unwrap();
        let vm_dtbo = VmDtbo::from_mut_slice(&mut vm_dtbo_data).unwrap();

        let hypervisor = MockHypervisor {
            mmio_tokens: [((0x12F00000, 0xFF), 0x12F00000)].into(),
            iommu_tokens: [((0x4, 0xFF0), (0x12E40000, 0x3))].into(),
        };
        let device_info = DeviceAssignmentInfo::parse(fdt, vm_dtbo, &hypervisor);
    ...
    fn device_info_invalid_reg_out_of_order() {
        ...
        let hypervisor = MockHypervisor {
            mmio_tokens: [((0xF000, 0x10000), 0xF10000), ((0xF100, 0x10000), 0xF00000)].into(),
