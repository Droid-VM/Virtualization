...
impl fmt::Display for DeviceAssignmentError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::TooManyPvIommu => write!(f, "Too many PV IOMMU node"),
...
impl VmDtbo {
    ...
    //         __overlay__ {
    //           eh { ... };       // Actual device node is here. If overlaid, path would be "/eh"
    //         };
    //       };
    ...
    //         eh = "/fragment@eh/__overlay__/eh";
    //       };
    //    };
    //
    // Then locate_overlay_target_path(cstr!("/fragment@eh/__overlay__/eh")) is Ok("/eh")
    //
    ...
    fn locate_overlay_target_path(&self, dtbo_node_path: &CStr) -> Result<CString> {
        ...
        let mut depth = 0;
        for name in dtbo_node_path_bytes.split(|char| char == &b'/') {
            if name.is_empty() {
                // This is expected for root and consecutive '/'. Skip to ignore.
                continue;
            }
...
impl PvIommu {
    fn new(node: &FdtNode) -> Result<Option<Self>> {
        let Some(iommus_phandle) = node.getprop_u32(IOMMUS_PROP_NAME)? else {
            return Ok(None);
        };
        let iommu_phandle =
            Phandle::new(iommus_phandle).ok_or(DeviceAssignmentError::InvalidIommus)?;
...
struct AssignedDeviceInfo {
    // Node path of assigned device (e.g. "/eh")
    node_path: CString,
    // DTBO node patch of the assigned device (e.g. "/fragment@eh/__overlay__/eh")
...
impl AssignedDeviceInfo {
    ...
    fn new(fdt: &Fdt, vm_dtbo: &VmDtbo, dtbo_node_path: &CStr) -> Result<Option<Self>> {
...
impl DeviceAssignmentInfo {
    ...
    pub fn new(fdt: &Fdt, vm_dtbo: &VmDtbo) -> Result<Option<Self>> {
        ...
        for symbol_prop in symbols_node.properties()? {
            ...
            let assigned_device = AssignedDeviceInfo::new(fdt, vm_dtbo, dtbo_node_path)?;
    ...
    pub fn patch(&self, fdt: &mut Fdt) -> Result<()> {
        ...
        for device in &self.assigned_devices {
            let Some(iommus) = device.iommus else {
                // Ignore any assigned device without <iommus>.
                continue;
            };
            let Some(pviommu) = pviommu_iter else {
                return Err(DeviceAssignmentError::TooManyPvIommu);
            };
            let pviommu_phandle = pviommu.as_node().phandle()?.unwrap();

            let mut dst = fdt.node_mut(&device.node_path)?.unwrap();
            dst.setprop(IOMMUS_PROP_NAME, &u32::to_be_bytes(u32::from(pviommu_phandle)));

            pviommu.setprop(ID_PROP_NAME, &u32::to_be_bytes(iommus.id))?;
            pviommu_iter = pviommu.next_compatible(PVIOMMU_COMPATIBLE)?;
        }
        // Remove unused pviommu nodes
...
mod tests {
    ...
    const FDT_WITH_IOMMU_FILE_PATH: &str = "test_pvmfw_devices_with_eh_iommus.dtb";
    ...
    fn device_info_assigned_info() {
        ...
        let expected = AssignedDeviceInfo {
            node_path: cstr!("/eh").into(),
            dtbo_node_path: cstr!("/fragment@eh/__overlay__/eh").into(),
        ...
        assert_eq!(device_info.assigned_devices[0], expected);
    }
    ...
    #[test]
    fn device_info_patch_iommus() {
        let mut fdt_data = fs::read(FDT_WITH_IOMMU_FILE_PATH).unwrap();
        let mut vm_dtbo_data = fs::read(VM_DTBO_FILE_PATH).unwrap();
        let mut data = vec![0_u8; fdt_data.len() + vm_dtbo_data.len()];
        let fdt = Fdt::from_mut_slice(&mut fdt_data).unwrap();
        let vm_dtbo = VmDtbo::from_mut_slice(&mut vm_dtbo_data).unwrap();
        let platform_dt_data = vec![0; pvmfw_fdt_template::RAW.len() * 2];
        platform_dt_data.extend_from_slice(pvmfw_fdt_template::RAW);
        ...
        let eh_node = platform_dt.node(cstr!("/eh")).unwrap().unwrap();
        let eh_iommu_phandle = eh_node.getprop_u32(IOMMUS_PROP_NAME).unwrap().unwrap();
        let eh_iommu_phandle = Phandle::new(eh_iommu_phandle).unwrap();

        let pviommu_node = platform_dt.node_with_phandle(eh_iommu_phandle).unwrap().unwrap();
        assert_eq!(pviommu_node.name(), Ok(cstr!("pviommu@0")));
        assert_eq!(pviommu_node.getprop_u32(ID_PROP_NAME), Ok(Some(0x4)));
    }
}
