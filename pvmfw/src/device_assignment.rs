...
impl DeviceTreeMask {
    ...
    fn mask(&mut self, path: &DtPathTokens) {
        let mut iter = self;
        'next_token: for path_token in &path.tokens {
            ...
            // Note: Can't use iterator because of the borrow checker issue called polonius.
...
impl AssignedDeviceInfo {
    ...
    fn validate_iommus(
        ...
        'next_iommu: for (pviommu, vsid) in iommus {
            ...
            for i in 0..physical_device_iommu.len() {
                let (phys_iommu, phys_sid) = &physical_device_iommu[i];
                if (phys_iommu.token, phys_sid.0) == (id, sid) {
                    physical_device_iommu.remove(i);
                    continue 'next_iommu;
                }
            }
