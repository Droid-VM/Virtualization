...
pub fn trng_version() -> smccc::trng::Result<(u16, u16)> {
    let args = [0u64; 17];

    let version = smccc::trng::hvc64(ARM_SMCCC_TRNG_VERSION, args)?[0];
    Ok(((version >> 16) as u16, version as u16))
}

pub type TrngRng64Entropy = (u64, u64, u64);

pub fn trng_rnd64(nbits: u64) -> smccc::trng::Result<TrngRng64Entropy> {
    let mut args = [0u64; 17];
    args[0] = nbits;

    let regs = smccc::trng::hvc64_expect_zero(ARM_SMCCC_TRNG_RND64, args)?;
