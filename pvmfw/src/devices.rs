...
impl<'a> DeviceInfo<'a> {
    ...
    pub fn apply_overlay_and_patch(self, fdt: &mut Fdt) -> Result<(), FdtError> {
        ...
        for (assignable_device, assigned_device) in &devices {
            ...
            println!("phase #1.. {symbol_path:?} done");
        }
        ...
        for (assignable_device, assigned_device) in &devices {
            ...
            println!("phase #2.. {symbol_name:?}");
            symbols_node.nop_property(symbol_name)?;
            println!("phase #2.. {symbol_name:?} done");
        }

        println!("phase overlay");
        ...
        for (assignable_device, assigned_device) in &devices {
            ...
            println!("phase #3.. {:?}", &assignable_device.path);
            ...
            println!("phase #3.. {:?} onde", &assignable_device.path);
        }
        Ok(())
    }
}
...
mod tests {
    ...
    const VM_DTBO_EMPTY_FILE_PATH: &str = "test_pvmfw_devices_vm_dtbo_empty.dtbo";
    ...
    fn device_info_new_without_symbols() {
        let mut vm_dtbo_data = fs::read(VM_DTBO_EMPTY_FILE_PATH).unwrap();
        let vm_dtbo = Fdt::from_mut_slice(&mut vm_dtbo_data).unwrap();
        let mut fdt_data = fs::read(FDT_FILE_PATH).unwrap();
        let fdt = Fdt::from_mut_slice(&mut fdt_data).unwrap();
    ...
    fn device_info_assigned_info() {
        ...
        for (assignable_device, assigned_device) in &device_info.devices {
            if assignable_device.path == expected_path {
                let assigned_device = assigned_device.unwrap();
    ...
    fn device_info_apply_overlay_and_patch() {
        ...
        let expected: Vec<(&str, Vec<u8>)> = vec![
            ("compatible", b"google,eh\0".to_vec()),
            ("google,eh,ignore-gctrl-reset;", Vec::<u8>::new()),
