...
impl<'a> DeviceInfo<'a> {
    // Returns overlaid path of a overlay node. overlay_node_path must be a valid assignable devices
    // written in /__symbols__.
    ...
    pub unsafe fn apply_overlay_and_patch(self, fdt: &mut Fdt) -> Result<(), DeviceError> {
        ...
        unsafe {
            fdt.apply_overlay(vm_dtbo).map_err(DeviceError::Overlay)?;
        }
...
mod tests {
    ...
    fn device_info_apply_overlay_and_patch() {
        ...
        let device_info = DeviceInfo::new_validated(vm_dtbo, fdt).unwrap().unwrap();
        // SAFETY: nsafe call in test code
        unsafe {
