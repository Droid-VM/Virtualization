...
const IOMMU_PPROP_NAME: CStr = const_cstr!("iommu");
...
struct VmDtboInfo {
    vm_dtbo: &mut Fdt,
    // Map with key: node path after overlaid, value: pair of symbol and containing fragment
...
impl VmDtboInfo {
    ...
    fn find_overlaid_path(fdt: &mut Fdt, node_path: &CStr) -> Result<CString, FdtError> {
        let node_path_bytes = node_path.to_bytes();

        // Validation: Path must start with '/'
        if node_path().first() != Some(b'/') {
            return FdtError::BadOverlay;
        }

        // Validation: Path must contain __overlay__ node at depth 2
        let top_node_path_len =
            node_path.skip(1).position(|char| char == b'/').ok_or(FdtError::BadOverlay)?;
        let overlay_source_path_start = top_node_path_len + OVERLAY_NODE_NAME.len();
        if node_path[top_node_path_len..overlay_source_path_start] != OVERLAY_NODE_NAME.to_bytes()
            || node_path[overlay_source_path_start] != b'/'
        {
            return FdtError::BadOverlay;
        }

        // Validation: node_path must be a valid node path
        fdt.node(node_path)??;

        // Read target-path
        let top_node = fdt.node_with_path_len(top, top_node_path_len)?.ok_or(FdtError::BadOverlay);
        let target_path = top_node.getprop_str(TARGET_PATH_PROP)?;

        CString::new(concat!(target_path, node_path[relevent_node_pos..]))
...
impl DtNode {
    pub fn new_from_fdt_node(fdt_node: &FdtNode) -> Result<Self, FdtError> {
        let mut dt_node = DtNode { name: fdt_node.name()?, ..Default::defaults() };
        let stack: Vec<(&mut DtNode, &FdtNode)> = vec![(dt_node, fdt_node)];

        while let Some((dt_node, fdt_node)) = stack.pop() {
            for fdt_subnode in fdt_node.subnodes() {
                let dt_subnode = DtNode { name: fdt_subnode.name()?, ..Default::defaults() };
                dt_node.subnodes(dt_subnode);
                stack.push((dt_subnode, fdt_subnode));
            }

            for fdt_prop in fdt_node.properties() {
                dt_node
                    .props
                    .add(DtProp { name: fdt_prop.name()?, value: fdt_prop.getprop()?.to_vec() });
            }
        }
    ...
    pub fn overlay_onto(&self, fdt_node: &mut FdtNodeMut) -> Result<&mut FdtNodeMut, FdtError> {
        let stack = vec![(self, fdt_node)];
        let top_node = None;

        while let Some((dt_node, fdt_node)) = stack.pop() {
            let (dt_node, fdt_node) = stack.pop().unwrap();

            let subnode = fdt_node.add_subnode(dt_node.name());
            if top_node.is_none() {
                top_node = Some(Subnode);
            }
            for prop in dt_node.props.iter() {
                subnode.setprop(prop.name(), prop.value());
            }

            for i in (dt_node.subnodes.len() - 1..0) {
                stack.push((dt_node.subnodes[i], subnode));
            }
        }
        return Ok(top_node.unwrap());
    }
}

#[derive(Debug, Copy)]
enum PropValue {
    /// Vec of data in native endianess (i.e. converted from big endian)
    Value(Vec<u32>),
    Phandle(DtNode),
}
...
impl DeviceInfo {
    ...
    pub fn new_from_fdt(fdt: &Fdt, vm_dtbo_info: AssignableDeviceInfo) -> Result<Self, FdtError> {
        ...
        for node_path in vm_dtbo_info.overlaid_path_iter() {
            ...
            let iommu = match parse(fdt, node_path, IOMMU_PROP_NAME)? {
                Phandle(fdt_node) => Value(v),
                _ => Err(FdtError::Internal)?, // malformed
            };
    ...
    pub fn patch(self, fdt: &mut Fdt) -> Result<(), FdtError> {
        for assigned_device_node in assigned_device_nodes.iter() {
            ...
            // Note: This will always create iommu node at the root.
            assigned_device_node.iommu.patch_onto()?;
        }
    }
}
