...
pub enum DeviceError {
    ...
    UnsupportedOverlayTargetSyntax,
...
impl fmt::Display for DeviceError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::UnsupportedOverlayTargetSyntax => write!(
                f,
                "Unsupported overlay target syntax. Only supports <target-path> with full path"
            ),
...
/// This keeps raw ptr to the symbol path to avoid borrow checker issue for having immutable borrow
/// (symbol_path_ptr) when mutable borrow (vm_dtbo) exists.vs
#[derive(Debug)]
...
impl AssignableDevice {
    ...
    fn to_overlaid_path(fdt: &Fdt, overlay_node_path: &CStr) -> Result<CString, DeviceError> {
        ...
        if overlay_node_path_bytes.first() != Some(&b'/') {
            return Err(DeviceError::UnsupportedOverlayTargetSyntax);
        }
