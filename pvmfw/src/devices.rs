...
const INTERRUPTS_CELL_NUM: usize = 4;
...
impl fmt::Display for DeviceError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::Overlay(e) => write!(f, "Failed to apply VM DTBO: {e}"),
            Self::UnsupportedOverlayTarget => write!(
                f,
                "Unsupported overlay target. Only supports <target-path> with absolute path"
            ),
...
impl AssignedDevice {
    ...
    fn new_validated(fdt: &Fdt, node_path: &CStr) -> Result<Option<Self>, DeviceError> {
        ...
        let reg: Vec<_> = node.reg()?.ok_or(DeviceError::InvalidReg)?.collect();
        ...
        println!("jaewan debug {:?}", interrupts.len());
...
impl<'a> DeviceInfo<'a> {
    ...
    fn to_overlaid_path(fdt: &Fdt, overlay_node_path: &CStr) -> Result<CString, DeviceError> {
        ...
        if overlay_node_path_bytes.first() != Some(&b'/') {
            return Err(FdtError::UnsupportedOverlayTarget);
        }

        let node = fdt.node(overlay_node_path)?.ok_or(FdtError::InvalidSymbols)?;
    ...
    pub fn new_validated(vm_dtbo: &'a mut Fdt, fdt: &Fdt) -> Result<Option<Self>, DeviceError> {
        let mut devices = vec![];
        ...
        for symbol in symbols_node.properties()? {
            ...
            let symbol_path =
                CStr::from_bytes_with_nul(symbol.value()?).or(Err(DeviceError::InvalidSymbols))?;
    ...
    /// SAFETY: This damages containing VM DTBO for trimming unassigned devices and overlaying.
    pub unsafe fn apply_overlay_and_patch(self, fdt: &mut Fdt) -> Result<(), DeviceError> {
        ...
        for (assignable_device, assigned_device) in &devices {
            ...
            if assigned_device.is_some() {
                ...
                // Empty fragment will be ignored, so we don't need to care overlay fragment.
        ...
        unsafe {
            fdt.apply_overlay(vm_dtbo).map_err(|e| DeviceError::Overlay(e))?;
        }
