...
impl AssignableDevice {
    ...
    unsafe fn new_with_symbol_prop(vm_dtbo: &Fdt, symbol_prop: &[u8]) -> Result<Self, DeviceError> {
        ...
        Ok(Self { path, symbol_path_ptr: symbol_path.as_ptr().cast::<_>() })
    }
}
...
impl<'a> DeviceInfo<'a> {
    ...
    pub fn new_filtered(vm_dtbo: &'a mut Fdt, fdt: &Fdt) -> Result<Option<Self>, DeviceError> {
        ...
        for assignable_device in assignable_devices {
            ...
            let symbol_path = unsafe { CStr::from_ptr(symbol_path_ptr.cast::<_>()) };
...
mod tests {
    ...
    fn device_info_apply_overlay_and_patch() {
        ...
        let expected: Vec<(&CStr, Vec<u8>)> = vec![
            (const_cstr!("interrupts"), into_fdt_prop(vec![0x0, 0xF, 0x4])),
            (const_cstr!("reg"), into_fdt_prop(vec![0x0, 0x9, 0x0, 0xFF])),
            (const_cstr!("android,eh,ignore-gctrl-reset"), Vec::<u8>::new()),
            (const_cstr!("compatible", b"android,eh\0").to_vec()),
        ];
