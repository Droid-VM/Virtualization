...
impl BootArg<'_> {
    ...
    /// Optinal value of the boot aragument. This includes the '=' prefix.
...
mod tests {
    ...
    #[test]
    fn empty() {
        check(CStr::from_bytes_with_nul(b"\0").unwrap(), Ok(&[]));
        check(CStr::from_bytes_with_nul(b"    \0").unwrap(), Ok(&[]));
        check(CStr::from_bytes_with_nul(b"  \n  \0").unwrap(), Ok(&[]));
    }

    #[test]
    fn single() {
        check(CStr::from_bytes_with_nul(b"foo\0").unwrap(), Ok(&[("foo", None)]));
        check(CStr::from_bytes_with_nul(b"   foo\0").unwrap(), Ok(&[("foo", None)]));
        check(CStr::from_bytes_with_nul(b"foo   \0").unwrap(), Ok(&[("foo", None)]));
        check(CStr::from_bytes_with_nul(b"   foo   \0").unwrap(), Ok(&[("foo", None)]));
    }
    ...
    fn single_with_value() {
        check(CStr::from_bytes_with_nul(b"foo=bar\0").unwrap(), Ok(&[("foo", Some("=bar"))]));
        check(CStr::from_bytes_with_nul(b"   foo=bar\0").unwrap(), Ok(&[("foo", Some("=bar"))]));
        check(CStr::from_bytes_with_nul(b"foo=bar   \0").unwrap(), Ok(&[("foo", Some("=bar"))]));
        check(CStr::from_bytes_with_nul(b"   foo=bar   \0").unwrap(), Ok(&[("foo", Some("=bar"))]));

        check(CStr::from_bytes_with_nul(b"foo=\0").unwrap(), Ok(&[("foo", Some("="))]));
        check(CStr::from_bytes_with_nul(b"   foo=\0").unwrap(), Ok(&[("foo", Some("="))]));
        check(CStr::from_bytes_with_nul(b"foo=   \0").unwrap(), Ok(&[("foo", Some("="))]));
        check(CStr::from_bytes_with_nul(b"   foo=   \0").unwrap(), Ok(&[("foo", Some("="))]));
    }
    ...
    fn single_with_quote() {
        check(
            CStr::from_bytes_with_nul(b"foo=hello\" \"world\0").unwrap(),
            Ok(&[("foo", Some("=hello\" \"world"))]),
        );
    }
    ...
    fn multiple() {
        check(
            CStr::from_bytes_with_nul(b" a=b   c=d   e=  f g  \0").unwrap(),
            Ok(&[("a", Some("=b")), ("c", Some("=d")), ("e", Some("=")), ("f", None), ("g", None)]),
        );
        check(
            CStr::from_bytes_with_nul(b"   a=b  \n c=d      e=  f g\0").unwrap(),
            Ok(&[("a", Some("=b")), ("c", Some("=d")), ("e", Some("=")), ("f", None), ("g", None)]),
        );
    }
    ...
    fn incomplete_quote() {
        check(
            CStr::from_bytes_with_nul(b"foo=incomplete\" quote bar=y\0").unwrap(),
    ...
    #[test]
    fn complex() {
        check(CStr::from_bytes_with_nul(b"  a  a1=  b=c d=e,f,g x=\"value with quote\" y=val\"ue with \"multiple\" quo\"te  \0").unwrap(), Ok(&[
            ("a", None),
            ("a1", Some("=")),
            ("b", Some("=c")),
            ("d", Some("=e,f,g")),
