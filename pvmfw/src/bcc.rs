...
impl Bcc {
    ...
    pub fn new(received_bcc_handover: &[u8]) -> Result<Self> {
        ...
        // ]
        let mut bcc = match bcc_cbor {
            Value::Array(v) if v.len() >= 2 => v,
            _ => return Err(BccError::MalformedBcc("Invalid top level value")),
        };
        let _root_key = bcc.remove(0);
        let bcc_entries = bcc;
        let is_debug_mode = is_any_entry_debug_mode(&bcc_entries)?;
    ...
    pub fn sanitise(&self) -> Result<Option<Vec<u8>>> {
        ...
        // ...
        // }
        let payload = payload_from_entry(previous_entry)?;
        let public_key = payload_value_from_key(&payload, KEY_SUBJECT_PUBLIC_KEY)
        ...
        let public_key = value_from_bytes(bytes)?;
        let bcc = vec![public_key, last_entry.clone()].into();
...
fn is_any_entry_debug_mode(bcc_entries: &[Value]) -> Result<bool> {
    ...
    // context.)
    for entry in bcc_entries {
        let payload = payload_from_entry(entry)?;
        if let Some(value) = payload_value_from_key(&payload, KEY_MODE) {
            // BccPayload = {                     // CWT
            // ...
            //     ? -4670551 : bstr,             // Mode
            // ...
...
/// Given a Ciborium Value representing a BccEntry, return the contained BccPayload as a Value.
fn payload_from_entry(entry: &Value) -> Result<Value> {
    ...
    // ]
    let payload = payload_bytes_from_entry(entry)
        .ok_or(BccError::MalformedBcc("Invalid payload in BccEntry"))?;
    let payload = value_from_bytes(payload)?;
    trace!("Bcc payload: {payload:?}");
    Ok(payload)
}

fn payload_bytes_from_entry(entry: &Value) -> Option<&Vec<u8>> {
...
fn payload_value_from_key(payload: &Value, key: i32) -> Option<&Value> {
