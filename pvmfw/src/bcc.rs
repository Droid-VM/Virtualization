...
impl Bcc {
    ...
    pub fn new(received_bcc: Option<&[u8]>) -> Result<Bcc> {
        ...
        let entries = bcc.into_iter().skip(1).map(BccEntry::new);
        let is_debug_mode = is_any_entry_debug_mode(entries)?;
...
fn is_any_entry_debug_mode(entries: impl Iterator<Item = BccEntry>) -> Result<bool> {
    ...
    // context.)
    // We skip the first element since it is the root public key, not an entry.
