...
fn main_wrapper(fdt: usize, payload: usize, payload_size: usize) -> Result<(), RebootReason> {
    ...
    // SAFETY - We only get the appended payload from here, once and based on a mutable slice which
    // is immediately discarded. It is mapped and the linker script prevents it from overlapping
    // with other objects.
    let mut appended =
...
impl<'a> AppendedPayload<'a> {
    /// SAFETY - The slice should never be written to if some AppendedPayload is returned.
