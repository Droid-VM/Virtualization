...
use crate::fdt;
use crate::memory;
use aarch64_paging::paging::MemoryRegion as VaRange;
use core::arch::asm;
...
pub fn start(fdt_address: u64, payload_start: u64, payload_size: u64, _arg3: u64) {
    ...
    match main_wrapper(fdt_address as usize, payload_start as usize, payload_size as usize) {
        Ok((entry, bcc)) => jump_to_payload(
            fdt_address,
            entry.try_into().unwrap(),
            VaRange::new(bcc.start, bcc.end),
        ),
...
fn main_wrapper(
    ...
    let payload_range = memory::appended_payload_range();
    // Up to this point, we were using the built-in static (from .rodata) page tables.
    MEMORY.lock().replace(MemoryTracker::new(
        page_table,
        crosvm::MEM_START..layout::MAX_VIRT_ADDR,
        crosvm::MMIO_RANGE,
        Some(payload_range.start().0..payload_range.end().0),
    ));
...
fn jump_to_payload(fdt_address: u64, payload_start: u64, bcc: VaRange) -> ! {
    ...
    assert_eq!(bcc.start().0 % ASM_STP_ALIGN, 0, "Misaligned guest BCC.");
    assert_eq!(bcc.end().0 % ASM_STP_ALIGN, 0, "Misaligned guest BCC.");
    ...
    unsafe {
        asm!(
            ...
            bcc = in(reg) u64::try_from(bcc.start().0).unwrap(),
            bcc_end = in(reg) u64::try_from(bcc.end().0).unwrap(),
...
unsafe fn get_appended_data_slice() -> &'static mut [u8] {
    ...
    // objects.
    unsafe { slice::from_raw_parts_mut(range.start().0 as *mut u8, range.len()) }
}
