...
impl From<DebugPolicyError> for RebootReason {
    fn from(error: DebugPolicyError) -> Self {
        match error {
            DebugPolicyError::InternalError => RebootReason::InternalError,
            DebugPolicyError::InvalidFdt => RebootReason::InvalidFdt,
            DebugPolicyError::InvalidDebugPolicyFdt => RebootReason::InvalidConfig,
        }
    }
}
...
fn main_wrapper(fdt: usize, payload: usize, payload_size: usize) -> Result<(), RebootReason> {
    ...
    unsafe {
        handle_debug_policy(slices.fdt, appended.get_debug_policy()).map_err(RebootReason::from)?;
    }
