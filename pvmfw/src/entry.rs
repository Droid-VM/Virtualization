...
fn jump_to_payload(fdt_address: u64, payload_start: u64, bcc: Range<usize>) -> ! {
    ...
    assert_eq!(scratch.start.checked_rem(ASM_STP_ALIGN), None, "scratch memory is misaligned.");
    assert_eq!(scratch.end.checked_rem(ASM_STP_ALIGN), None, "scratch memory is misaligned.");

    assert!(helpers::range_contains(&scratch, &bcc));
    assert_eq!(bcc.start.checked_rem(ASM_STP_ALIGN), None, "Misaligned guest BCC.");
    assert_eq!(bcc.end.checked_rem(ASM_STP_ALIGN), None, "Misaligned guest BCC.");

    let stack = mmu::stack_range();

    assert_ne!(stack.len(), 0, "stack region is empty.");
    assert_eq!(stack.start.checked_rem(ASM_STP_ALIGN), None, "Misaligned stack region.");
    assert_eq!(stack.end.checked_rem(ASM_STP_ALIGN), None, "Misaligned stack region.");
    ...
    unsafe {
        asm!(
            ...
            rwdata_end = in(reg) u64::try_from(scratch.end).unwrap(),
