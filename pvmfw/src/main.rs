...
fn main(
        ...
        && verified_boot_data.has_capability(Capability::SecretkeeperProtection)
    {
        ...
        // For Secretkeeper based rollback protection, rollback_index of the image > 0
        ...
        // With `llpvm_changes` salt is derived from instance_id.
        let (new_instance, _) =
            get_or_generate_instance_salt(&mut pci_root, &dice_inputs, cdi_seal).map_err(|e| {
                error!("Failed to get new_instance flag: {e}");
                RebootReason::InternalError
            })?;
...
// Get the "salt" which is one if the input for DICE derivation.
