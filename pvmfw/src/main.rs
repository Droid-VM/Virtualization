...
fn main(
    ...
    let hashes = Hashes::new(&verified_boot_data).map_err(|e| {
        error!("Failed to compute DICE hashes: {e:?}");
        RebootReason::InternalError
    })?;
    let salt = [0; 64]; // TODO(b/249723852): Get salt from instance.img (virtio-blk) and/or TRNG.
    let dice_inputs = hashes.into_input_values(&salt).map_err(|e| {
