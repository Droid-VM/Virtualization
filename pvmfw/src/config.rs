...
impl Header {
    ...
    pub fn body_offset(&self) -> Result<usize> {
        let entries_offset = unchecked_align_up(mem::size_of::<Self>(), mem::size_of::<u64>());

        Ok(entries_offset + mem::size_of::<HeaderEntry>() * self.entry_count()?)
    }
...
impl<'a> Config<'a> {
    ...
    pub fn new(bytes: &'a mut [u8]) -> Result<Self> {
        ...
        // Now that we can access Header::total_size, resize the &[u8] to catch OOB accesses.
        let rest = if let Some(rest_size) = total_size.checked_sub(mem::size_of::<Header>()) {
