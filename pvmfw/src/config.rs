...
impl Header {
    ...
    const PADDED_SIZE: usize =
        helpers::unchecked_align(mem::size_of::<Self>(), mem::size_of::<u64>());
...
impl<'a> Config<'a> {
    ...
    pub unsafe fn new(data: &'a mut [u8]) -> Option<Self> {
        let max_size = data.len();
        let (_, headers, _) = data.align_to_mut::<Header>();
        let header = headers.first_mut().filter(|h| h.is_valid(max_size))?;

        let body = slice::from_raw_parts_mut(header.body_ptr().cast_mut(), header.body_size());

        Some(Self { body, header })
    }
