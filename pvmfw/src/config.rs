...
struct Header {
    ...
    total_size: u32,
    flags: u32,
    entries: [HeaderEntry; ENTRY_COUNT],
}
...
impl<'a> Config<'a> {
    ...
    pub unsafe fn new(data: &'a mut [u8]) -> Result<Self> {
        const REQUIRED_ENTRIES: &[Entry] = &[Entry::Bcc, Entry::DebugPolicy];

        let header = data.get_mut(..Header::PADDED_SIZE).ok_or(Error::BufferTooSmall)?;

        let header = &mut *(header.as_mut_ptr() as *mut Header);
        ...
        for entry in REQUIRED_ENTRIES {
            if !header.get(*entry).fits_in(total_size) {
                return Err(Error::InvalidEntry(*entry));
            }
        }
    ...
    pub fn get_debug_policy(&mut self) -> &mut [u8] {
        &mut self.body[self.header.get(Entry::DebugPolicy).as_body_range()]
    }
}
