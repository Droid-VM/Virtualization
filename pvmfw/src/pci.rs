...
fn allocate_bars(
    ...
    let mut bar_index = 0;
    while bar_index < 6 {
        let info = root.bar_info(device_function, bar_index).map_err(|e| {
            error!("Error getting PCI BAR information: {}", e);
            RebootReason::PciError
        })?;
        ...
        if let BarInfo::Memory { address_type, size, .. } = info {
            match address_type {
                ...
                MemoryBarType::Width32 => {
                    let address = allocator.allocate_memory_32(size).ok_or_else(|| {
                        error!(
                ...
                _ => {
                    error!("Memory BAR address type {:?} not supported.", address_type);
                    return Err(RebootReason::PciError);
                }
            }
        }
