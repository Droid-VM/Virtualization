...
use pvmfw_avb::{DebugLevel, Digest, VerifiedData};
...
fn to_dice_hash(verified_data: &VerifiedData) -> dice::Result<dice::Hash> {
    if let Some(initrd_digest) = verified_data.initrd_digest {
        let mut digests = [0u8; size_of::<Digest>() * 2];
        digests[..size_of::<Digest>()].copy_from_slice(&verified_data.kernel_digest);
        digests[size_of::<Digest>()..].copy_from_slice(&initrd_digest);
        hash(&digests)
    } else {
        hash(&verified_data.kernel_digest)
    }
}
...
pub fn derive_next_bcc(
    ...
    let code_hash = to_dice_hash(verified_data)?;
