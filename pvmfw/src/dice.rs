...
pub fn derive_next_bcc(
    ...
    let config = Config::Descriptor(config);

    let input_values = InputValues::new(
        &code_hash, None, // code_descriptor
        config, &auth_hash, None, // auth_descriptor
        mode, None, // TODO(b/249723852): Get salt from instance.img (virtio-blk) and/or TRNG.
    );
