...
/// # Safety
///
/// This implementation is marked as `unsafe` because it requires direct access to hardware
/// resources and low-level operations that may result in undefined behavior if not used correctly.
/// It is the responsibility of the caller to ensure that all safety requirements are met when
...
unsafe impl Hal for HalImpl {
    ...
    /// - `pages` is the number of pages that were previously allocated by `dma_alloc` using the
    ///  same layout.
    ...
    /// - The `buffer` parameter must be a valid pointer to memory which will not be accessed by any
    /// other thread for the duration of this method call.
    /// - The `paddr` parameter must be the value previously returned by the corresponding `share`
    /// call.
