...
// TODO: log inside alloc_shared/dellaoc
...
unsafe impl Hal for HalImpl {
    ...
    unsafe fn share(buffer: NonNull<[u8]>, direction: BufferDirection) -> PhysAddr {
        ...
        if direction == BufferDirection::DriverToDevice {
            ...
            trace!("VirtIO bounce buffer at {vaddr:?} (PA:{paddr:#x}) initialized from {src:?}");
            // SAFETY - Both regions are valid, properly aligned, and don't overlap.
            unsafe { copy_nonoverlapping(src, vaddr.as_ptr(), size) };
        }
    ...
    unsafe fn unshare(paddr: PhysAddr, buffer: NonNull<[u8]>, direction: BufferDirection) {
        ...
        if direction == BufferDirection::DeviceToDriver {
            ...
            trace!("VirtIO bounce buffer at {vaddr:?} (PA:{paddr:#x}) copied back to {dest:?}");
            // SAFETY - Both regions are valid, properly aligned, and don't overlap.
            unsafe { copy_nonoverlapping(vaddr.as_ptr(), dest, size) };
        }

        // SAFETY - Memory was allocated by `share` using `alloc_shared` with the same size.
        unsafe { dealloc_shared(vaddr, bb_layout(size)) }
            .expect("Failed to unshare VirtIO buffer with host");
    }
}
