...
/// An implementation of this interface is required to use the virtio-drivers.
///
/// # Safety
///
...
pub unsafe trait VirtIOHal {
    ...
    /// is deallocated by `dealloc_shared`.
    ...
    /// Returns a static reference of `PciInfo`.
    fn pci_info() -> Option<&'static PciInfo>;
}

pub struct HalImpl<H: VirtIOHal> {
...
unsafe impl<H: VirtIOHal> Hal for HalImpl<H> {
    /// # Implementation Safety
    ///
    ...
    fn dma_alloc(pages: usize, _direction: BufferDirection) -> (PhysAddr, NonNull<u8>) {
        let vaddr = H::alloc_shared(dma_layout(pages))
    ...
    unsafe fn dma_dealloc(_paddr: PhysAddr, vaddr: NonNull<u8>, pages: usize) -> i32 {
        ...
        unsafe { H::dealloc_shared(vaddr, dma_layout(pages)) }
    ...
    unsafe fn mmio_phys_to_virt(paddr: PhysAddr, size: usize) -> NonNull<u8> {
        let pci_info = H::pci_info().expect("VirtIO HAL used before PCI_INFO was initialised");
        let bar_range = {
    ...
    unsafe fn share(buffer: NonNull<[u8]>, direction: BufferDirection) -> PhysAddr {
        ...
        let bounce = H::alloc_shared(bb_layout(size))
    ...
    unsafe fn unshare(paddr: PhysAddr, buffer: NonNull<[u8]>, direction: BufferDirection) {
        ...
        unsafe { H::dealloc_shared(bounce, bb_layout(size)) }
