...
pub enum PciError {
    ...
    PciCamMapFailed(MemoryTrackerError),
    /// Failed to map PCI BAR.
    PciBarMapFailed(MemoryTrackerError),
}
...
impl fmt::Display for PciError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::PciCamMapFailed(e) => write!(f, "Failed to map PCI CAM: {e}"),
            Self::PciBarMapFailed(e) => write!(f, "Failed to map PCI BAR: {e}"),
        }
    }
}
...
pub fn initialise(pci_info: PciInfo, memory: &mut MemoryTracker) -> Result<PciRoot, PciError> {
    memory.map_mmio_range(pci_info.cam_range.clone()).map_err(PciError::PciCamMapFailed)?;
    let bar_range = pci_info.bar_range.start as usize..pci_info.bar_range.end as usize;
    memory.map_mmio_range(bar_range).map_err(PciError::PciBarMapFailed)?;
