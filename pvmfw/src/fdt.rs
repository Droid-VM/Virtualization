...
fn parse_vm_reference_dt(fdt: &Fdt) -> libfdt::Result<BTreeMap<CString, Vec<u8>>> {
...
/// when it exists both in vm_reference_dt and props_info. If the values mismatch, it returns error.
fn apply_vm_reference_dt(
    vm_dt: &mut Fdt,
    vm_reference_dt: &Fdt,
    ...
    let avf_vm_reference_dt = vm_reference_dt.node(cstr!("/avf"))?.ok_or(FdtError::NotFound)?;
    for (name, value) in props_info.iter() {
        if let Some(value_in_vm_reference_dt) = avf_vm_reference_dt.getprop(name)? {
            if value != value_in_vm_reference_dt {
                error!(
                    ...
                    name, value, value_in_vm_reference_dt
                );
                return Err(FdtError::BadValue);
            }
            avf_vm_dt.setprop(name, value_in_vm_reference_dt)?;
        }
    }
    Ok(())
}
...
pub struct DeviceTreeInfo {
    ...
    vm_reference_dt_props_info: BTreeMap<CString, Vec<u8>>,
}
...
pub fn sanitize_device_tree(
    ...
    if let Some(vm_reference_dt) = vm_reference_dt {
        let vm_reference_dt = Fdt::from_slice(vm_reference_dt).map_err(|e| {
            error!("Failed to load VM reference DT: {e}");
            RebootReason::InvalidFdt
        })?;

        apply_vm_reference_dt(fdt, vm_reference_dt, &info.vm_reference_dt_props_info).map_err(
            |e| {
...
fn parse_device_tree(fdt: &Fdt, vm_dtbo: Option<&VmDtbo>) -> Result<DeviceTreeInfo, RebootReason> {
    ...
    let vm_reference_dt_props_info = parse_vm_reference_dt(fdt).map_err(|e| {
