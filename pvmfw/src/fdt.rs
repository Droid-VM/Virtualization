...
fn patch_wdt_info(fdt: &mut Fdt, wdt_info: &WdtInfo) -> libfdt::Result<()> {
    let mut node = fdt.node_mut(cstr!("qemu,vcpu-stall-detector"))?.ok_or(libfdt::FdtError::NotFound)?;
    node.setprop_inplace(cstr!("interrupts"), &wdt_info.irq[2].to_be_bytes())?;
...
fn patch_device_tree(fdt: &mut Fdt, info: &DeviceTreeInfo) -> Result<(), RebootReason> {
    ...
    patch_wdt_info(fdt, &info.wdt_info).map_err(|e| {
