...
use vmbase::cstr;
use vmbase::fdt::{SwiotlbInfo, PVMFW_DICE_NODE_NAME, RESERVED_MEMORY_NODE_NAME};
...
fn patch_dice_node(fdt: &mut Fdt, addr: usize, size: usize) -> libfdt::Result<()> {
    ...
    let node = fdt.node_mut(cstr!(RESERVED_MEMORY_NODE_NAME))?.ok_or(libfdt::FdtError::NotFound)?;

    let mut node = node.next_compatible(cstr!(PVMFW_DICE_NODE_NAME))?.ok_or(FdtError::NotFound)?;
