...
pub fn modify_for_next_stage(
    ...
    if let Some(mut chosen) = fdt.chosen_mut()? {
        reset_or_clear_prop(&mut chosen, cstr!("avf,strict-boot"), strict_boot)?;
        reset_or_clear_prop(&mut chosen, cstr!("avf,new-instance"), new_instance)?;
    };
    if !debuggable {
