...
fn read_wdt_info_from(fdt: &Fdt) -> libfdt::Result<WdtInfo> {
    ...
    let mut interrupts = node.getprop_cells(cstr!("interrupts"))?.ok_or(FdtError::NotFound)?;
    if interrupts.next().is_some() {
        warn!("vmwdt has more than 1 interrupt nodes.");
    }

    let value: ArrayVec<[u32; WdtInfo::IRQ_CELLS]> = interrupts.take(WdtInfo::IRQ_CELLS).collect();
