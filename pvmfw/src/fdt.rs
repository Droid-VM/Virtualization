...
impl fmt::Display for FdtValidationError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::ParsingNodeFailed(fdt_node_name, e) => write!(f, "Failed to parse the node '{}': {}", fdt_node_name.str(), e),
        }
    }
}
...
fn read_and_validate_num_cpus_from(fdt: &Fdt) -> Result<usize, FdtValidationError> {
    let num_cpus = fdt.compatible_nodes(FdtNodeName::Cpu.cstr()).map_err(|e| {
        FdtValidationError::ParsingNodeFailed(FdtNodeName::Cpu, e)
    })?.count();
