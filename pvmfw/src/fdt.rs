...
fn apply_debug_policy(fdt: &mut Fdt, debug_policy: &mut [u8]) -> libfdt::Result<()> {
    let mut backup_fdt = Vec::new();
    backup_fdt.extend_from_slice(fdt.as_slice());
    ...
    let mut backup_overlay = Vec::new();
    backup_overlay.extend_from_slice(overlay.as_slice());

    // SAFETY - on failure, the corrupted fdts are discarded and are restored using the backups.
    let ret = unsafe { fdt.apply_overlay(overlay) };
    if let Err(e) = ret {
