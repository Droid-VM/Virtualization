...
fn patch_wdt_info(fdt: &mut Fdt, num_cpus: usize) -> libfdt::Result<()> {
    let node = fdt
        .compatible_nodes(cstr!("qemu,vcpu-stall-detector"))?
        .next()
        .ok_or(FdtError::NotFound)?;
    let interrupts = node.getprop_cells(cstr!("interrupts"))?.ok_or(FdtError::NotFound)?;
    let mut value: ArrayVec<[u32; WdtInfo::IRQ_CELLS]> =
        interrupts.take(WdtInfo::IRQ_CELLS).collect();

    let num_cpus: u32 = num_cpus.try_into().unwrap();
    let cpu_mask: u32 = (((0x1 << num_cpus) - 1) & 0xff) << 8;
    for v in value.iter_mut().skip(2) {
        *v |= cpu_mask;
    }
    ...
    let mut node =
        fdt.node_mut(cstr!("qemu,vcpu-stall-detector"))?.ok_or(libfdt::FdtError::NotFound)?;
