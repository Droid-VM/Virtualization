...
fn patch_gic(fdt: &mut Fdt, num_cpus: usize) -> libfdt::Result<()> {
    ...
    let size: u64 = DeviceTreeInfo::gic_patched_size(num_cpus).unwrap();
...
impl DeviceTreeInfo {
    ...
    fn gic_patched_size(num_cpus: usize) -> Option<u64> {
        Self::GIC_REDIST_SIZE_PER_CPU.checked_mul(num_cpus.try_into().unwrap())
    }
}
