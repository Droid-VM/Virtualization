...
fn patch_initrd_range(fdt: &mut Fdt, initrd_range: &Range<usize>) -> libfdt::Result<()> {
    let start: u32 = initrd_range.start as u32;
    let end: u32 = initrd_range.end as u32;
...
fn patch_bootargs(fdt: &mut Fdt, bootargs: &CStr) -> libfdt::Result<()> {
    let mut node = fdt.chosen_mut()?.ok_or(FdtError::NotFound)?;
    // TODO filter out dangerous options
...
fn patch_num_cpus(fdt: &mut Fdt, num_cpus: usize) -> libfdt::Result<()> {
    ...
    loop {
        if let Some(current) = next? {
            count += 1;
            // See the side note in libfdt/src/lib.rs for the reason of having
            // delete_and_next_compatible.
            next = if count > num_cpus {
                current.delete_and_next_compatible(cpu)
            } else {
                current.next_compatible(cpu) // keep current
            };
        } else {
            // There's no more compatible node in the DT, but we still have more nodes: this is an
            // error.
            if count < num_cpus {
...
fn patch_pci_info(fdt: &mut Fdt, pci_info: &PciInfo) -> libfdt::Result<()> {
    ...
    node.trimprop(cstr!("interruipt-map"), irq_maps_size)?;
...
fn patch_gic(fdt: &mut Fdt, num_cpus: usize) -> libfdt::Result<()> {
    ...
    node.setprop_inplace(cstr!("reg"), flatten(&[range0.to_cells(), range1.to_cells()]))
}
...
fn patch_timer(fdt: &mut Fdt, num_cpus: usize) -> libfdt::Result<()> {
    ...
    for i in (2..value.len()).step_by(CELLS_PER_INTERRUPT) {
        value[i] |= cpu_mask;
    }
    ...
    let value = unsafe {
        core::mem::transmute::<
            ...
            [u8; NUM_INTERRUPTS * CELLS_PER_INTERRUPT * 4],
        >(value.into_inner())
    };
