...
as a host process and as pVMs must not trust the host (see [_Why AVF?_][why-avf]),
the virtual machine it configures can't be trusted either.
...
tampering with it, loads it from its own protected memory into pVM memory and
sets it as the entry point of the pVM. This process takes place whenever the VMM
places a VM in protected mode and can’t be prevented by the host.
...
was set up as expected by validating the address space, the list of attached
virtual devices, and the interface with the hypervisor.
...
device-specific pvmfw configuration data by appending it to the pvmfw binary
...
- **Hidden Inputs**: Factory Reset Secret (FRS) or similar that changes as part
  of the device lifecycle (_e.g._ reset)
