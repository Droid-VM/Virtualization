...
```
avbtool add_hash_footer --image <guest.bin> \
    --partition_name boot \
...
avbtool add_hash_footer --image /tmp/<initrd.bin> \
    --partition_name initrd_normal \  # or initrd_debug
    --dynamic_partition_size \
    --key $KEY <...>
avbtool add_hash_footer --image <guest.bin> \
    --partition_name boot \
...
The name of the ramdisk "partition" (`initrd_normal` or `initrd_debug`) controls
whether pvmfw considers the guest to be debuggable or not, modifying how
strictly it validates inputs such as the DT, and affecting the secrets and
certificate (BCC) passed to the guest kernel.
