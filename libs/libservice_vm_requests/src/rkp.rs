...
use diced_open_dice::{
    derive_cdi_leaf_priv, kdf, sign, DiceArtifacts, PrivateKey, DEFAULT_KEY_ALGORITHM,
};
use log::{debug, error};
...
fn build_signed_data(payload: &Value, dice_artifacts: &dyn DiceArtifacts) -> Result<CoseSign1> {
    ...
    let protected = HeaderBuilder::new().algorithm(DEFAULT_KEY_ALGORITHM.into()).build();
