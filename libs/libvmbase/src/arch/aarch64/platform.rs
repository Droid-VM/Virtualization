...
pub type EmergencyUart = Option<Uart>;
...
pub fn emergency_uart(euart: &mut EmergencyUart) -> &mut Uart {
    let addr = UART_ADDRESSES[DEFAULT_EMERGENCY_CONSOLE_INDEX];

    if euart.is_none() {
        // SAFETY: Initialization of UART using dedicated const address.
        *euart = Some(unsafe { Uart::new(addr) });
    }

    // SAFETY: UART driver is initialised above so we can transmute it to ordinary uart reference
    return euart.as_mut().unwrap();
}
