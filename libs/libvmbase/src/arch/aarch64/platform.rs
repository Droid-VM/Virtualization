...
/// Type representing uninitialised emergency UART driver
pub type EmergencyUart = MaybeUninit<Uart>;
...
pub fn map_uarts_mmio() -> Result<(), hypervisor_backends::Error> {
    if let Some(mmio_guard) = hypervisor_backends::get_mmio_guard() {
        ...
        //
        // For 4KiB, that's only the UARTs. For 16KiB, it also cospin::mutex::SpinMutexGuard<vers
        // the RTC and watchdog but, as neither is used by vmbase clients (and as both are
        // outside of the UART page), they will never have valid stage-1 mappings to those
        // devices. As a result, this MMIO_GUARD_MAP isn't affected by the granule size in
        // any visible way. Larger granule sizes will need to be checked separately, if
        // needed.
        assert!({
...
pub fn init() {
    if map_uarts_mmio().is_err() {
        ...
        // Rebot to prevent printing any message.
        reboot()
    }
...
pub unsafe fn emergency_uart(euart: &mut EmergencyUart) -> &mut Uart {
    let addr = UART_ADDRESSES[DEFAULT_EMERGENCY_CONSOLE_INDEX];
    // SAFETY: Initialization of UART unisng dedicated const addres.
    euart.write(unsafe { Uart::new(addr) });
    // SAFETY: UART driver is already initialised so we can transmut it to ordinary uart reference
    return unsafe { euart.assume_init_mut() };
}
