...
pub fn ewriteln(format_args: Arguments) {
    let mut uninitialized_uart = platform::EmergencyUart::uninit();
    // SAFETY: This function always initializ UART driver so we can safely call it from unknown
    // context.
    let uart = unsafe { platform::emergency_uart(&mut uninitialized_uart) };
    let _ = write(uart, format_args);
    let _ = uart.write_str("\n");
}
