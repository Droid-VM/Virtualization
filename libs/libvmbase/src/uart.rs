...
pub trait UartBackend {
    ...
    fn write_volatile_u8(&self, offset: usize, byte: u8);
}
...
impl<Backend: UartBackend> Uart<Backend> {
    ...
    pub fn write_byte(&self, byte: u8) {
        self.backend.write_volatile_u8(0, byte)
    }
}
...
// SAFETY: Uart is a tiny wrapper over UartBackend if the UartBackend is safe to access from any
// context so is Uart.
unsafe impl<Backend> Send for Uart<Backend> where Backend: UartBackend + Send {}
