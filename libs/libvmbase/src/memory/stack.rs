...
pub(crate) fn max_stack_size() -> Option<usize> {
    // SAFETY: Function is either provided by client or defaults to default_max_stack_size().
    unsafe { vmbase_max_stack_size() }
}

#[no_mangle] // Weakly aliased to vmbase_max_stack_size() as user might not provide it.
fn default_vmbase_max_stack_size() -> Option<usize> {
    None
}
