...
pub fn sign_cose_sign1_multialg(
    ...
    private_key: &[u8; PRIVATE_KEY_SIZE],
    encoded_signature: &mut [u8],
    authority_algorithm: KeyAlgorithm,
    subject_algorithm: KeyAlgorithm,
) -> Result<usize> {
    let mut encoded_signature_actual_size = 0;
    let context = DiceContext_ {
        authority_algorithm: authority_algorithm.into(),
        subject_algorithm: subject_algorithm.into(),
    };
    check_result(
        // SAFETY: The function writes to `encoded_signature` and `encoded_signature_actual_size`
        // within the given bounds. It only reads `message`, `aad`, and `private_key` within their
...
pub fn sign_cose_sign1_with_cdi_leaf_priv_multialg(
    ...
    let private_key = derive_cdi_leaf_priv_multialg(dice_artifacts, subject_algorithm)?;
    sign_cose_sign1_multialg(
        message,
        aad,
        private_key.as_array(),
        encoded_signature,
        authority_algorithm,
        subject_algorithm,
    )
