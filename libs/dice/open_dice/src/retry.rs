...
//! of this buffer may fail and callers will see Error::NoMemory. When running
//! with std, allocation may fail.
    ...
    F: FnMut(&mut Vec<u8>) -> Result<usize>,
{
    ...
    match f(&mut buffer) {
        Err(DiceError::BufferTooSmall(actual_size)) => {
            if !cfg!(feature = "std") {
                buffer.try_reserve_exact(actual_size).map_err(|_| DiceError::NoMemory)?;
            }
