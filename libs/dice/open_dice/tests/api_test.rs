...
mod tests {
    ...
    use diced_open_dice::{
        derive_cdi_certificate_id, derive_cdi_private_key_seed, keypair_from_seed,
        keypair_from_seed_multialg, retry_sign_cose_sign1, retry_sign_cose_sign1_multialg,
        retry_sign_cose_sign1_with_cdi_leaf_priv,
        retry_sign_cose_sign1_with_cdi_leaf_priv_multialg, sign, verify, verify_multialg,
        DiceArtifacts, DiceContext, KeyAlgorithm, PrivateKey, CDI_SIZE, ID_SIZE,
    };
    use diced_open_dice::{hash, kdf, HASH_SIZE, PRIVATE_KEY_SEED_SIZE};
    ...
    fn get_test_key_pair() -> (Vec<u8>, PrivateKey) {
        ...
        let cdi_private_key_seed =
            derive_cdi_private_key_seed(dice_context, cdi_attest.try_into().unwrap()).unwrap();
        assert_eq!(cdi_private_key_seed.as_array(), EXPECTED_CDI_PRIVATE_KEY_SEED);
        let (pub_key, priv_key) =
            keypair_from_seed(dice_context, cdi_private_key_seed.as_array()).unwrap();
