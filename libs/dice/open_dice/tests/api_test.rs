...
mod tests {
    ...
    fn sign_cose_sign1_verify_multialg() {
        ...
        let (pub_key, priv_key) = get_test_key_pair_multialg(key_algorithm);
        ...
        let mut verify_result = cose_sign1.verify_signature(b"MyAad", |sign, data| {
            verify_multialg(data, sign, &pub_key, key_algorithm)
        });
        assert!(verify_result.is_ok());
    ...
    fn sign_cose_sign1_with_cdi_leaf_priv_verify_multialg() {
        ...
        let mut verify_result = cose_sign1.verify_signature(b"MyAad", |sign, data| {
            verify_multialg(data, sign, EXPECTED_EC_P256_PUB_KEY, key_algorithm)
        });
        assert!(verify_result.is_ok());
    ...
    #[cfg(feature = "multialg")]
    fn get_test_key_pair_multialg(key_algorithm: KeyAlgorithm) -> (Vec<u8>, PrivateKey) {
        let seed = hash(b"MySeedString").unwrap();
        assert_eq!(seed, EXPECTED_SEED);
        let cdi_attest = &seed[..CDI_SIZE];
        assert_eq!(cdi_attest, EXPECTED_CDI_ATTEST);
        ...
        let (pub_key, priv_key) =
            keypair_from_seed_multialg(cdi_private_key_seed.as_array(), key_algorithm).unwrap();

        // Assuming `key_algorithm` is `EcdsaP256`.
