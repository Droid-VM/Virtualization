...
final class DebianServiceImpl extends DebianServiceGrpc.DebianServiceImplBase {
    ...
    protected DebianServiceImpl(Context context, DebianServiceCallback callback) {
        super();
        mCallback = callback;
        mContext = context;
    ...
    protected void killForwarderHost() {
    ...
    private void updateListeningPorts() {
        Set<String> runningGuestPorts =
                mSharedPref.getStringSet(PREFERENCE_FORWARDING_PORTS, Collections.emptySet());
        ArrayList<Integer> listeningPorts = new ArrayList<>();
        for (String port : runningGuestPorts) {
