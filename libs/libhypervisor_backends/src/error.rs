...
pub enum Error {
    ...
    #[cfg(target_arch = "aarch64")]
    /// Failed to invoke Gunyah HVC function.
    GunyahError(GunyahError, u32),
}
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            #[cfg(target_arch = "aarch64")]
            Self::GunyahError(e, function_id) => {
                write!(
                    f,
                    "Failed to invoke Gunyah HVC function with function ID {function_id}: {e}"
                )
            }
            #[cfg(target_arch = "aarch64")]
