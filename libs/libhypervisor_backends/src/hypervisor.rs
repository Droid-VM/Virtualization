...
#[cfg(target_arch = "aarch64")]
use {
    super::Error, geniezone::GeniezoneHypervisor, gunyah::GunyahHypervisor, smccc::hvc64,
    uuid::Uuid,
};
...
#[cfg(target_arch = "x86_64")]
fn querry_x86_hyp() -> Result<HypervisorBackend> {
    kvm::determine_hyp_type()
}

fn detect_hypervisor() -> HypervisorBackend {
    #[cfg(target_arch = "aarch64")]
    return query_vendor_hyp_call_uid().try_into().expect("Failed to detect hypervisor");

    #[cfg(target_arch = "x86_64")]
    querry_x86_hyp().expect("Failed to detect hypervisor")
}
