...
impl Hypervisor for RegularKvmHypervisor {
    // TODO: remove in order to switch to default trait implementation
    // (libs/libhypervisor_backends/src/hypervisor/common.rs). For some unknown reason after
    // switching to default, build success but cause runtime error - x86 Trusty (when using this
    // lib) reaches 'u2d' instruction.
...
impl MemSharingHypervisor for ProtectedKvmHypervisor {
    fn share(&self, base_ipa: u64) -> Result<()> {
        ...
        if ret != 0 {
            return Err(Error::KvmError(KvmError::from(ret as i32), 0));
        }
