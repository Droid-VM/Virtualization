...
pub trait Hypervisor {
    /// Returns a handle to the hypervisor's MMIO_GUARD implementation, if any.
    fn get_mmio_guard(&self) -> Option<&dyn MmioGuardedHypervisor> {
        None
    }

    /// Returns a handle to the hypervisor's dynamic memory sharing implementation, if any.
    fn get_mem_share(&self) -> Option<&dyn MemSharingHypervisor> {
        None
    }
}
...
impl<T> Hypervisor for T
where
    T: MmioGuardedHypervisor + MemSharingHypervisor,
{
    fn get_mmio_guard(&self) -> Option<&dyn MmioGuardedHypervisor> {
        Some(self)
    }
