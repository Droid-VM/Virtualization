...
pub enum Error {
    ...
    QueryGranuleFailed(smccc::Error),
...
impl fmt::Display for Error {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Self::QueryGranuleFailed(e) => write!(f, "Failed to get the MMIO_GUARD granule: {e}"),
...
pub fn init() -> Result<()> {
    ...
    let mmio_granule = hyp.mmio_guard_granule().map_err(Error::QueryGranuleFailed)?;
