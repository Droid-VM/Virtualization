...
struct AndroidDisplayContext {
    ...
    AndroidDisplayContext(ErrorCallback cb) : error_callback(cb) {
        auto virt_service = IVirtualizationServiceInternal::fromBinder(
            ndk::SpAIBinder(AServiceManager_waitForService("android.system.virtualizationservice")));
...
LIBEXPORT
struct AndroidDisplayContext* create_android_display_context(
    const char*, ErrorCallback error_callback) {
    return new AndroidDisplayContext(error_callback);
}

LIBEXPORT
void destroy_android_display_context(struct AndroidDisplayContext* ctx) {
    delete ctx;
}

LIBEXPORT
ANativeWindow* create_android_surface(struct AndroidDisplayContext *ctx,
        uint32_t width, uint32_t height) {
    if (ctx->disp_service == nullptr) {
        ctx->errorf("Display service was not created");
        return nullptr;
    }
    ANativeWindow* surface = ctx->disp_service->getSurface()->get();
    if (ANativeWindow_setBuffersGeometry(surface, width, height, AHARDWAREBUFFER_FORMAT_R8G8B8A8_UNORM) != 0) {
...
LIBEXPORT
void destroy_android_surface(struct AndroidDisplayContext*, ANativeWindow*) {
    // NOT IMPLEMENTED
}

LIBEXPORT
void* get_android_surface_buffer(struct AndroidDisplayContext* ctx, ANativeWindow* surface, uint32_t* out_stride) {
    ANativeWindow_Buffer buffer;
    if (ANativeWindow_lock(surface, &buffer, nullptr) != 0) {
...
LIBEXPORT
void post_android_surface_buffer(struct AndroidDisplayContext* ctx, ANativeWindow* surface) {
