...
impl<'a, C: ByteChannel<E>, E> PrefixedByteChannel<'a, C, E> {
    ...
    /// Receives a byte buffer from the channel and returns the buffer without the length
    /// prefix.
    ///
    /// The received byte buffer should always have a header including the buffer length padded
    /// in front. This function can handle scenarios where a long buffer is broken down into
    /// several partial packets. It blocks the reading until the full length of the buffer is
    /// received.
