...
Result<uint32_t> getU32Attribute(const ResXMLTree& tree, size_t index) {
    ...
    switch (type) {
        ...
        case Res_value::TYPE_STRING: {
            ...
            auto result = getStringOnlyAttribute(tree, index);
            if (!result.ok()) return result.error();
            char* str_end = nullptr;
            // Note that by specifying base 0 we allow for octal, hex, or
            // decimal representations here.
            unsigned long value = std::strtoul(result->c_str(), &str_end, 0);
            if (str_end != result->c_str() + result->size() ||
...
uint32_t getVersionCode(const ApkManifestInfo* info) {
