...
impl VbMetaImage {
    ...
    pub fn verify_path<P: AsRef<Path>>(path: P) -> Result<Self, VbMetaImageVerificationError> {
        let file = File::open(path).map_err(VbMetaImageVerificationError::Io)?;
        let size = file.metadata().map_err(VbMetaImageVerificationError::Io)?.len();
    ...
    pub fn verify_reader_region<R: Read + Seek>(
        ...
        image.seek(SeekFrom::Start(offset + size)).map_err(VbMetaImageVerificationError::Io)?;
        let footer = read_avb_footer_from_image(&mut image)?;
        ...
        image
            ...
            .map_err(VbMetaImageVerificationError::Io)?;
        // Verify the image before examining it to check the size.
        let mut data = vec![0u8; vbmeta_size as usize];
        image.read_exact(&mut data).map_err(VbMetaImageVerificationError::Io)?;
...
#[cfg(feature = "std")]
fn read_avb_footer_from_image<R: Read + Seek>(
    image: &mut R,
) -> Result<Option<AvbFooter>, VbMetaImageVerificationError> {
    let footer_size = size_of::<AvbFooter>();
    image
        .seek(SeekFrom::Current(-(footer_size as i64)))
        .map_err(VbMetaImageVerificationError::Io)?;
    let mut raw_footer = vec![0u8; footer_size];
    image.read_exact(&mut raw_footer).map_err(VbMetaImageVerificationError::Io)?;
    Ok(read_avb_footer_from_bytes(
        raw_footer[..].try_into().map_err(|_| VbMetaImageVerificationError::InvalidFooter)?,
    ))
}

/// Reads AVB footer from bytes.
#[allow(dead_code)] // TODO(b/256148034): Remove this when pvmfw payload verification is code complete.
                    // TODO(b/256148034): Return Result<AvbFooter, VbMetaImageVerificationError> instead
fn read_avb_footer_from_bytes(bytes: &[u8; size_of::<AvbFooter>()]) -> Option<AvbFooter> {
    ...
    let mut footer = unsafe {
        ...
        footer_slice.copy_from_slice(bytes);
        footer.assume_init()
    };
