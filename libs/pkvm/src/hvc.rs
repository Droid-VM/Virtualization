...
use crate::smccc::{checked_hvc64, checked_hvc64_expect_zero, Error, Result};
...
pub fn kvm_hyp_meminfo() -> Result<u64> {
    let args = [0u64; 17];
...
pub fn kvm_mem_share(base_ipa: u64) -> Result<()> {
...
/// [`hyp_meminfo`].
pub fn kvm_mem_unshare(base_ipa: u64) -> Result<()> {
    let mut args = [0u64; 17];
    args[0] = base_ipa;
...
pub(crate) fn kvm_mmio_guard_info() -> Result<u64> {
    let args = [0u64; 17];

    checked_hvc64(VENDOR_HYP_KVM_MMIO_GUARD_INFO_FUNC_ID, args)
}

pub(crate) fn kvm_mmio_guard_enroll() -> Result<()> {
    let args = [0u64; 17];

    checked_hvc64_expect_zero(VENDOR_HYP_KVM_MMIO_GUARD_ENROLL_FUNC_ID, args)
}

pub(crate) fn kvm_mmio_guard_map(ipa: u64) -> Result<()> {
    let mut args = [0u64; 17];
    args[0] = ipa;
...
pub(crate) fn kvm_mmio_guard_unmap(ipa: u64) -> Result<()> {
    let mut args = [0u64; 17];
    args[0] = ipa;

    // TODO(b/251426790): pKVM currently returns NOT_SUPPORTED for SUCCESS.
    // Drop the workaround as the bug is supposed to be fixed.
