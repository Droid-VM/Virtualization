...
#[test]
<<<<<<< PATCH SET (6a1f36 libfdt: Test for mixed node name)
fn node() {
    let data = fs::read(TEST_NODE_NAME_MATCH_PATH).unwrap();
    let fdt = Fdt::from_slice(&data).unwrap();

    let node = fdt.node(cstr!("/mbox/a")).unwrap().unwrap();
    assert_eq!(Ok(Some(2_u32)), node.getprop_u32(cstr!("val")));
=======
...
>>>>>>> BASE      (1ec249 Merge "Treat APK path change as compatible" into main)
}
