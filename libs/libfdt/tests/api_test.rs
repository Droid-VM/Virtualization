...
fn max_phandle() {
    ...
    assert_eq!(fdt.max_phandle().unwrap(), Phandle(0xFF));
}
...
fn node_with_phandle() {
    ...
    // Test linux,phandle
    let node = fdt.node_with_phandle(Phandle(0xFF)).unwrap().unwrap();
    assert_eq!(node.name().unwrap().to_str().unwrap(), "node_zz");

    // Test phandle
    let node = fdt.node_with_phandle(Phandle(0x22)).unwrap().unwrap();
...
fn node_nop() {
    ...
    fdt.node_with_phandle(Phandle(0xFF)).unwrap().unwrap();
    let node = fdt.node_mut(cstr!("/node_z/node_zz")).unwrap().unwrap();

    node.nop().unwrap();

    assert!(fdt.node_with_phandle(Phandle(0xFF)).unwrap().is_none());
...
fn node_add_subnode_with_namelen() {
    ...
    for len in 1..subnode_name.to_bytes().len() {
        let name =
            CString::from_vec_with_nul([&subnode_name.to_bytes()[0..len], b"\0"].concat()).unwrap();
        let path = CString::from_vec_with_nul(
            [node_path.to_bytes(), b"/", name.to_bytes_with_nul()].concat(),
        )
        .unwrap();
        let subnode = fdt.node(&path).unwrap().unwrap();
        assert_eq!(subnode.name().unwrap().to_str().unwrap(), name.to_str().unwrap());
    }
}
