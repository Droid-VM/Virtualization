...
fn node_properties() {
    ...
    for (node, expect) in root.properties().unwrap().zip(expected) {
...
fn max_phandle() {
    ...
    assert_eq!(fdt.max_phandle().unwrap(), 0xFF);
}
...
#[test]
fn node_add_subnode_with_name_len() {
    let mut data = fs::read(TEST_TREE_PHANDLE_PATH).unwrap();
    ...
    for len in 1..subnode_name.to_bytes().len() {
        ...
        node.add_subnode_with_name_len(subnode_name, len).unwrap();

        let mut node = fdt.node_mut(node_path).unwrap().unwrap();
        assert!(node.subnode_with_name_len(subnode_name, len).unwrap().is_some());
    }
