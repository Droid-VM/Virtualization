...
pub fn create_empty_tree(fdt: &mut [u8]) -> Result<()> {
...
pub(crate) unsafe trait Libfdt {
    ...
    fn get_name(&self, node: c_int) -> Result<&[u8]> {
        ...
        get_slice_at_ptr(self.as_fdt_slice(), name, len).ok_or(FdtError::Internal)
    }
    ...
    fn string(&self, offset: c_int) -> Result<&CStr> {
        ...
        let bytes = get_slice_from_ptr(self.as_fdt_slice(), ptr).ok_or(FdtError::Internal)?;
