...
pub use ctypes::{NodeOffset, Phandle};
pub use iterators::{
...
impl<'a> FdtNodeMut<'a> {
    ...
    pub fn trimprop(&mut self, name: &CStr, new_size: usize) -> Result<()> {
        self.fdt.setprop_placeholder(self.offset, name, new_size).map(|_| ())
    }
    ...
    pub fn delete_and_next_subnode(self) -> Result<Option<Self>> {
        ...
        self.delete_and_return(next_offset)
    }
    ...
    pub fn delete_and_next_node(self, depth: usize) -> Result<Option<(Self, usize)>> {
        ...
        if let Some((offset, depth)) = next_node {
            let next_node = self.delete_and_return(Some(offset))?.unwrap();
    ...
    fn delete_and_return(self, next_offset: Option<NodeOffset>) -> Result<Option<Self>> {
