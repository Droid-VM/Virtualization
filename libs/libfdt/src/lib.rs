...
impl<'a> FdtProperty<'a> {
    fn new(fdt: &'a Fdt, offset: c_int) -> Result<Self> {
        ...
        let property = unsafe { &*prop.cast::<FdtPropertyStruct>() };
    ...
    pub fn value(&self) -> Result<&'a [u8]> {
        Ok(self.fdt.get_from_ptr(self.property.data_ptr(), self.property.data_len()).unwrap())
    }
...
impl<'a> FdtNodeMut<'a> {
    ...
    pub fn delete_and_next_compatible(mut self, compatible: &CStr) -> Result<Option<Self>> {
        ...
        // SAFETY: nop_self() only touches one byte at self.offset and doesn't alter any other blob
        // in the tree. self.fdt and next_offset would remain valid.
    ...
    /// Deletes this node effectively from DT, by setting it with FDT_NOP.
    /// This only changes one byte (fdt_node_header.tag) in the blob, and doesn't alter or move any
