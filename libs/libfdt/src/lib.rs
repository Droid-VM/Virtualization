...
fn fdt_err(val: i32) -> Result<i32> {
    ...
    Err(match -val as u32 {
        libfdt_bindgen::FDT_ERR_NOTFOUND => Error::NotFound,
        libfdt_bindgen::FDT_ERR_EXISTS => Error::Exists,
        libfdt_bindgen::FDT_ERR_NOSPACE => Error::NoSpace,
        libfdt_bindgen::FDT_ERR_BADOFFSET => Error::BadOffset,
        libfdt_bindgen::FDT_ERR_BADPATH => Error::BadPath,
        libfdt_bindgen::FDT_ERR_BADPHANDLE => Error::BadPhandle,
        libfdt_bindgen::FDT_ERR_BADSTATE => Error::BadState,
        libfdt_bindgen::FDT_ERR_TRUNCATED => Error::Truncated,
        libfdt_bindgen::FDT_ERR_BADMAGIC => Error::BadMagic,
        libfdt_bindgen::FDT_ERR_BADVERSION => Error::BadVersion,
        libfdt_bindgen::FDT_ERR_BADSTRUCTURE => Error::BadStructure,
        libfdt_bindgen::FDT_ERR_BADLAYOUT => Error::BadLayout,
        libfdt_bindgen::FDT_ERR_INTERNAL => Error::Internal,
        libfdt_bindgen::FDT_ERR_BADNCELLS => Error::BadNCells,
        libfdt_bindgen::FDT_ERR_BADVALUE => Error::BadValue,
        libfdt_bindgen::FDT_ERR_BADOVERLAY => Error::BadOverlay,
        libfdt_bindgen::FDT_ERR_NOPHANDLES => Error::NoPhandles,
        libfdt_bindgen::FDT_ERR_BADFLAGS => Error::BadFlags,
        libfdt_bindgen::FDT_ERR_ALIGNMENT => Error::Alignment,
        _ => Error::Unknown(val),
    })
}
...
fn getprop<'a>(fdt: &'a [u8], nodeoffset: FdtNodeOffset, name: &FdtCString) -> Result<&'a [u8]> {
    ...
    Ok(unsafe {
        core::slice::from_raw_parts(
            ret as *const u8,
            usize::try_from(lenp).map_err(|_| Error::BadValue)?,
        )
    })
}
