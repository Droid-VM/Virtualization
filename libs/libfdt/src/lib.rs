...
impl Fdt {
    ...
    ///
    /// SAFETY: The slice must be valid.
    pub fn from_slice(fdt: &[u8]) -> Result<&Self> {
        let fdt = Self::unchecked_from_slice(fdt);
        fdt.check_full()?;
        Ok(fdt)
    }
    ...
    pub fn from_mut_slice(fdt: &mut [u8]) -> Result<&mut Self> {
        let fdt = Self::unchecked_from_mut_slice(fdt);
        fdt.check_full()?;
        Ok(fdt)
    }
    ...
    ///
    /// SAFETY: The slice must be valid.
    pub fn unchecked_from_slice(fdt: &[u8]) -> &Self {
    ...
    ///
    /// SAFETY: The slice must be valid.
    pub fn unchecked_from_mut_slice(fdt: &mut [u8]) -> &mut Self {
