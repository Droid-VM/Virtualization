...
impl Fdt {
    ...
    /// Return the device tree as a mutable slice (may be smaller than the containing buffer).
    pub fn as_mut_slice(&mut self) -> &mut [u8] {
        let totalsize = self.totalsize();
        &mut self.buffer[..totalsize]
    }
    ...
    fn header(&self) -> &libfdt_bindgen::fdt_header {
        // SAFETY - A valid FDT (verified by new()) must contain a valid fdt_header.
