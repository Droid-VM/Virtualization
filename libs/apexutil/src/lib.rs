...
use avb::{
    verify_vbmeta_from_reader, Descriptor, VbMetaImage, VbMetaImageParseError,
    VbMetaImageVerificationError,
};
use std::fs::File;
...
pub fn verify(path: &str) -> Result<ApexVerificationResult, ApexVerificationError> {
    ...
    let vbmeta = verify_vbmeta_from_reader(apex_file, image_offset, image_size)
...
pub fn get_payload_vbmeta_image_hash(path: &str) -> Result<Vec<u8>, ApexVerificationError> {
    ...
    let vbmeta = verify_vbmeta_from_reader(apex_file, offset, size)
        .map_err(ApexVerificationError::PayloadVbmetaError)?;
