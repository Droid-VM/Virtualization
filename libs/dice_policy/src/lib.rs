...
pub enum ConstraintIdentifier {
    ...
    /// if the dice chain has exact same values for given keySpec.
    ExactMatch = 1,
    /// Enforce Greater than or equal to criteria - when applied to security_version, this
    /// is useful to set policy that matches dice chains with same of upgraded images.
    GreaterOrEqual = 2,
}
...
impl DicePolicy {
    ...
    /// Constraints on all but first node is applied using constraint_spec argument.
    ...
    ///
    /// `constraint_spec`: List of (ConstraintIdentifier, KeySpec) tuple. Each tuple specifies which constraint
    /// to apply and on which all entries in the dice node.
    /// The first item of the tuple is ConstraintIdentifier.
    /// The second item is KeySpec, which is a list of label/int. KeySpec identifies the `key` to be applying constraints on.
    /// Note: Dice node is treated as a nested map (& so the lookup is done in that fashion).
    ///
    /// The constraint is applied to this KeySpec of all the dice nodes (except the first one).
    ///
    /// Examples of constraint_spec:
    ///  1. For exact_match on auth_hash & greater_or_equal on security_version
    ///    constraint_spec =[
    ///     (ConstraintIdentifier::ExactMatch, vec![AUTHORITY_HASH]),
    ///     (ConstraintIdentifier::GreaterOrEqual, vec![CONFIG_DESC, COMPONENT_NAME]),
    ///    ];
    ///
    ...
    ///    constraint_spec =[
    ///     (ConstraintIdentifier::ExactMatch, vec![1]),             // exact_matches value 'a'
    ///     (ConstraintIdentifier::GreaterOrEqual, vec![2, 200]),   // matches any value >= 5
    ///    ];
    pub fn from_dice_chain(
        dice_chain: &[u8],
        constraint_spec: &[(ConstraintIdentifier, Vec<i64>)],
    ) -> Result<Self> {
        ...
        constraints_list.push(NodeConstraints(Box::new([(
            ConstraintIdentifier::ExactMatch as i64,
            Vec::new(),
            it.next().unwrap(),
        )])));
...
fn lookup_value_in_nested_map(map: &Value, key_spec: &[i64]) -> Result<Value> {
    ...
    let mut map = map;
    let mut explicit_map: Vec<(Value, Value)>;
    for key in key_spec.iter() {
        explicit_map = get_map_from_value(map)?;
        map = lookup_value_in_map(&explicit_map, key).ok_or(anyhow!(
            "Could not find value in map for nested key {:?}, in KeySpec {:?}",
            key,
            key_spec
        ))?;
    }
...
fn lookup_value_in_map<'a>(map: &'a [(Value, Value)], key: &'a i64) -> Option<&'a Value> {
    for (k, v) in map.iter() {
        if k.is_integer() && k.clone().into_integer().unwrap() == (*key).into() {
            return Some(v);
        }
    }
    None
...
mod tests {
    ...
    fn policy_dice_size_is_same() {
        ...
        let constraint_spec = [
            (ConstraintIdentifier::ExactMatch, vec![AUTHORITY_HASH]),
            (ConstraintIdentifier::ExactMatch, vec![KEY_MODE]),
            (ConstraintIdentifier::GreaterOrEqual, vec![CONFIG_DESC, COMPONENT_NAME]),
        ];
    ...
    fn policy_structure_check() {
        ...
        let constraint_spec = [
            (ConstraintIdentifier::ExactMatch, vec![1]),
            (ConstraintIdentifier::GreaterOrEqual, vec![3, 100]),
        ];
        let policy = DicePolicy::from_dice_chain(&input_dice, &constraint_spec).unwrap();
            ...
            DicePolicy {
                ...
                node_constraints_list: Box::new([
                    ...
                    NodeConstraints(Box::new([
                        ...
                        (
                            ConstraintIdentifier::GreaterOrEqual as i64,
                            vec![3, 100],
