...
/// The first item of the tuple is ConstraintType.
/// The second item is ConstraintPath, which is essentially a list of label/int.
/// > ConstraintPath identifies which entry (in a dice node) to be applying constraints on.
/// See documentation of `from_dice_chain()` for examples.
pub struct ConstraintSpec(ConstraintType, Vec<i64>);
...
fn payload_to_constraints(
    ...
    for constraint_item in constraint_spec {
        ...
        let constraint = Constraint(constraint_item.0 as u16, key_spec, val);
        node_constraints.push(constraint);
    }
    Ok(NodeConstraints(node_constraints.into_boxed_slice()))
}

fn lookup_value_in_nested_map(cbor_map: &Value, key_spec: &[i64]) -> Result<Value> {
    if key_spec.is_empty() {
        return Ok(cbor_map.clone());
    }
    let explicit_map = get_map_from_value(cbor_map)?;
    let val = lookup_value_in_map(&explicit_map, &key_spec[0]).ok_or(anyhow!(
        "Could not find value in map for nested key {:?}, in KeySpec {:?}",
        key_spec[0],
        key_spec
    ))?;
    lookup_value_in_nested_map(val, &key_spec[1..])
}
...
fn lookup_value_in_map<'a>(map: &'a [(Value, Value)], key: &i64) -> Option<&'a Value> {
    for (k, v) in map.iter() {
...
mod tests {
    ...
    fn policy_dice_size_is_same() {
        ...
        let constraint_spec = [
            ConstraintSpec(ConstraintType::ExactMatch, vec![AUTHORITY_HASH]),
            ConstraintSpec(ConstraintType::ExactMatch, vec![KEY_MODE]),
            ConstraintSpec(ConstraintType::GreaterOrEqual, vec![CONFIG_DESC, COMPONENT_NAME]),
        ];
