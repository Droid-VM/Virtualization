...
fn verify_having_valid_result<P: AsRef<Path>>(path: P) {
    ...
    let verified_public_key_path =
        format!("{}.verified_public_key", path.as_ref().to_str().unwrap());
    // Helps regenerate the verified public key when needed.
    // if fs::metadata(&verified_public_key_path).is_err() {
    //     assert_eq!(
    //         format!(
    //             "echo -en {}>{}",
    //             result
    ...
    assert_eq!(verified_public_key, result.unwrap().as_ref());
}
