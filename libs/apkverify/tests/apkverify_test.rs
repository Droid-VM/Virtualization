...
fn validate_apk_public_key<P: AsRef<Path>>(apk_path: P) {
    ...
    // (without verification) == public key from verification.
...
fn validate_apk_digest<P: AsRef<Path>>(apk_path: P, expected_algorithm_id: SignatureAlgorithmID) {
    // Validates apk digest directly extracted from apk (without computing) == expected apk digest
    ...
    assert_eq!(expected_algorithm_id.to_u32().unwrap(), signature_algorithm_id);
