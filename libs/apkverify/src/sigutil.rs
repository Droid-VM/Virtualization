        ...
        for data in &[
            ...
            while data.limit() > 0 {
                ...
                digests_of_chunks.put_slice(
                    digest_dispatch(slice, CHUNK_HEADER_MID, chunk_size as u32, signature_algorithm_id)?.as_ref(),
                );
                chunk_count += 1;
            }
        }
...
fn digest_dispatch(data: &[u8], header: &[u8], size: u32, signature_algorithm_id: SignatureAlgorithmID) -> Result<Vec<u8>> {
    match signature_algorithm_id{
        ...
        | SignatureAlgorithmID::VerityDsaWithSha256 => digest::<bssl_crypto::digest::Sha256>(data, header, size),
        SignatureAlgorithmID::RsaPssWithSha512
        | SignatureAlgorithmID::RsaPkcs1V15WithSha512
        | SignatureAlgorithmID::EcdsaWithSha512 => digest::<bssl_crypto::digest::Sha512>(data, header, size),
    }
}
