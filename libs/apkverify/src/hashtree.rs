...
fn hash_one_block<A: Algorithm>(input: &[u8], salt: &[u8], block_size: usize) -> Result<Vec<u8>> {
    let mut ctx = A::new();
    ctx.update(salt);
    ctx.update(input);
    let pad_size = block_size - input.len();
    ctx.update(&vec![0; pad_size]);
    Ok(ctx.digest_to_vec())
}
...
fn zero_pad_salt<A: Algorithm>(salt: &[u8]) -> Vec<u8> {
    if salt.is_empty() {
        ...
        let padded_len = round_to_multiple(salt.len(), A::BLOCK_LEN);
...
mod tests {
    ...
    fn compare_with_golden_output() -> Result<()> {
        ...
        for size in sizes.iter() {
            ...
            let ht =
                HashTree::from::<File, bssl_crypto::digest::Sha256>(&mut input, size, &salt, 4096)?;
