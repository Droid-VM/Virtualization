...
pub fn verify<P: AsRef<Path>>(apk_path: P) -> Result<Box<[u8]>> {
    ...
    let (signer, mut sections) = extract_v3_signer_and_apk_sections(apk)?;
...
pub fn get_public_key_der<P: AsRef<Path>>(apk_path: P) -> Result<Box<[u8]>> {
    let apk = File::open(apk_path.as_ref())?;
    let (signer, _) = extract_v3_signer_and_apk_sections(apk)?;
...
fn extract_v3_signer_and_apk_sections<R: Read + Seek>(
    apk: R,
) -> Result<(LengthPrefixed<Signer>, ApkSections<R>)> {
    ...
    let mut supported =
        signers.inner.into_iter().filter(|s| s.sdk_range().contains(&SDK_INT)).collect::<Vec<_>>();
    ensure!(
        supported.len() == 1,
        "APK Signature Scheme V3 only supports one signer: {} signers found.",
        supported.len()
    );
    Ok((supported.pop().context("Error when extracting Signer")?, sections))
}
