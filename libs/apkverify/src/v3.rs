...
fn extract_v3_signer_and_apk_sections<R: Read + Seek>(apk: R) -> Result<(Signer, ApkSections<R>)> {
    ...
    Ok((supported.pop().context("Error when extracting Signer")?.into_inner(), sections))
}
