...
fn process_least_recent_error() -> ReasonCode {
    ...
    error!(
        ...
        file.and_then(|s| s.to_str().ok()).unwrap_or("<unknown file>"),
        line,
...
fn lib_error_string(packed_error: u32) -> &'static str {
    // SAFETY: This function only reads the given error code and returns a
    // pointer to a static string.
    let p = unsafe { ERR_lib_error_string(packed_error) };
    // SAFETY: Any non-null result is expected to point to a global const C string.
    unsafe { as_static_cstr(p) }.and_then(|s| s.to_str().ok()).unwrap_or("<unknown library>")
}

fn reason_error_string(packed_error: u32) -> &'static str {
    // SAFETY: This function only reads the given error code and returns a
    // pointer to a static string.
    let p = unsafe { ERR_reason_error_string(packed_error) };
    // SAFETY: Any non-null result is expected to point to a global const C string.
    unsafe { as_static_cstr(p) }.and_then(|s| s.to_str().ok()).unwrap_or("<unknown reason>")
}
