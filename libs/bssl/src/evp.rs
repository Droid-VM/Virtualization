...
use bssl_ffi::{
    CBB_flush, CBB_len, EVP_PKEY_assign_EC_KEY, EVP_PKEY_new, EVP_marshal_public_key, EVP_PKEY,
};
use core::ptr::NonNull;

/// Wrapper of an `EVP_PKEY` object, representing a public or private key.
///
/// The `Drop`` trait is not implemented for this struct because `EvpPKey` does
/// not take ownership of its inner key object. Thus we don't want to free all
/// the data it references.
pub struct EvpPKey(NonNull<EVP_PKEY>);
...
impl EvpPKey {
    ...
    pub fn new() -> Result<Self> {
        ...
        NonNull::new(key).map(Self).ok_or(to_call_failed_error(ApiName::EVP_PKEY_new))
    }

    /// Sets the underlying public key in the inner `EVP_PKEY` object to
    /// point to the provided `EcKey`.
    pub fn set_ec_key(&mut self, ec_key: &mut EcKey) -> Result<()> {
        ...
        let ret = unsafe { EVP_PKEY_assign_EC_KEY(self.0.as_ptr(), ec_key.as_mut_ptr()) };
    ...
    pub fn subject_public_key_info(&self) -> Result<Vec<u8>> {
        ...
        // SAFETY: The function only write bytes to the buffer managed by
        // the valid `CBB`. The underlying key in `EVP_PKEY` is either null
        // or set to a valid `EC_KEY`, both are handled in this function.
        let ret = unsafe { EVP_marshal_public_key(cbb.as_mut(), self.0.as_ptr()) };
