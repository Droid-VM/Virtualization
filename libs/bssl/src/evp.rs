...
use bssl_ffi::{CBB_flush, CBB_len, EVP_PKEY_new, EVP_marshal_public_key, EVP_PKEY};
...
pub(crate) struct EvpPKey(NonNull<EVP_PKEY>);
...
impl EvpPKey {
    ...
    pub(crate) fn subject_public_key_info(&self) -> Result<Vec<u8>> {
    ...
    /// Returns the pointer to the inner `EVP_PKEY`.
    pub(crate) fn as_mut_ptr(&mut self) -> *mut EVP_PKEY {
        self.0.as_ptr()
    }
}
