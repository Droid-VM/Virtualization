...
impl EcKey {
    ...
    /// Constructs an `EcKey` instance from the provided CBOR encoded public key slice.
    ...
    ///
    /// The lifetime of the returned `EcKey` is not tied to the lifetime of the
    /// `cose_Key`, because the the affine coordinates stored in the `cose_Key`
    /// are copied into the `EcKey`.
