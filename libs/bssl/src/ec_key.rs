...
impl EcKey {
    ...
    pub fn new_p384() -> Result<Self> {
        // SAFETY: The returned pointer is checked below.
        let ec_key = unsafe {
            EC_KEY_new_by_curve_name(NID_secp384r1) // EC P-256 CURVE Nid
        };
        NonNull::new(ec_key)
            .map(Self)
            .ok_or(to_call_failed_error(ApiName::EC_KEY_new_by_curve_name))
    ...
    pub fn from_cose_public_key(cose_key: &[u8]) -> Result<Self> {
        ...
        let ec_key = match get_label_value(
            &cose_key,
            ...
            crv => {
                error!("Only EC P-256 and P-384 curves are supported. Curve type in the COSE Key: {crv:?}");
                return Err(Error::Unimplemented);
            }
        };
        ...
        ec_key.ec_group()?.check_affine_coordinate_size(x)?;
        ec_key.ec_group()?.check_affine_coordinate_size(y)?;
    ...
    /// by the pointer is owned by the `EC_KEY`.
    fn ec_group(&self) -> Result<EcGroup> {
        let group =
           // SAFETY: It is safe since the key pair has been generated and stored in the
           // `EC_KEY` pointer.
