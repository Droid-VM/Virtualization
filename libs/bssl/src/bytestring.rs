...
//! Helpers for using BoringSSL CBB (crypto byte builder) and CBS (crypto byte string)
//! objects.

use bssl_ffi::{CBB_init_fixed, CBS_init, CBB, CBS};
...
/// CRYPTO ByteString.
///
/// Wraps a `CBS` that references a existing fixed-sized buffer; no memory is allocated, but the
/// buffer cannot grow.
pub struct Cbs<'a> {
    cbs: CBS,
    /// The CBB contains a mutable reference to the buffer, disguised as a pointer.
...
impl<'a> Cbs<'a> {
    /// Create a new CBS that points to the given buffer.
