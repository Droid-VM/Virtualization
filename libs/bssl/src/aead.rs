...
pub struct Aead(pub(crate) &'static EVP_AEAD);
...
/// Represents an AEAD algorithm configured with a specific key and message-independent IV.
pub struct AeadCtx {
...
impl AeadCtx {
    ...
    /// Encrypts and authenticates `data` and returns the encrypted result.
    pub fn encrypt_to_vec(&self, data: &[u8]) -> Result<Vec<u8>> {
        let mut res = vec![0u8; data.len() + self.aead.max_overhead()];
        let len = self.encrypt(data, &mut res)?;
        res.truncate(len);
        Ok(res)
    }
    ...
    pub fn encrypt(&self, data: &[u8], out: &mut [u8]) -> Result<usize> {
    ...
    /// Authenticates `data` and decrypts it.
    /// Returns the decrypted result.
    pub fn decrypt_to_vec(&self, data: &[u8]) -> Result<Vec<u8>> {
        let mut res = vec![0u8; data.len()];
        let len = self.decrypt(data, &mut res)?;
        res.truncate(len);
        Ok(res)
    }

    /// Authenticates `data` and decrypts it to `out`.
    /// The `out` length should be at least the `data` length.
    pub fn decrypt(&self, data: &[u8], out: &mut [u8]) -> Result<usize> {
        let nonce = ptr::null_mut();
        let nonce_len = 0;
        let ad = ptr::null_mut();
        let ad_len = 0;
