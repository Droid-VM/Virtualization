...
use crate::ec_key::check_int_result;
...
impl AeadCtx {
    ...
    /// `aead`.
    ///
    /// Returns the length of data been writen to `out`.
    pub fn seal(&self, data: &[u8], out: &mut [u8]) -> Result<usize> {
    ...
    pub fn open(&self, data: &[u8], out: &mut [u8]) -> Result<usize> {
        let nonce = ptr::null_mut();
        let nonce_len = 0;
        let ad = ptr::null_mut();
        let ad_len = 0;
