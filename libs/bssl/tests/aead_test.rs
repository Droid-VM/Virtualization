...
fn aes_256_gcm_randnonce_encrypts_and_decrypts_successfully() -> Result<()> {
    let ciphertext = aes_256_gcm_randnonce_encrypt(MESSAGE)?;
    let tag_len = None;
    let nonce = &[];
    let ad = &[];
    ...
    let plaintext = aead_ctx.open(&ciphertext, nonce, ad, &mut out)?;
...
fn aes_256_gcm_randnonce_fails_to_decrypt_corrupted_ciphertext() -> Result<()> {
    let mut ciphertext = aes_256_gcm_randnonce_encrypt(MESSAGE)?;
...
fn aes_256_gcm_randnonce_encrypt(message: &[u8]) -> Result<Vec<u8>> {
    let tag_len = None;
    let aead_ctx = AeadCtx::new(Aead::aes_256_gcm_randnonce(), &KEY1, tag_len)?;
    let mut out = vec![0u8; message.len() + aead_ctx.aead().max_overhead()];
    let nonce = &[];
    assert_eq!(aead_ctx.aead().nonce_length(), nonce.len());
    let ad = &[];

    let ciphertext = aead_ctx.seal(message, nonce, ad, &mut out)?;
