...
#[test]
fn ecdsa_nist_signing_and_verification_succeed() -> Result<()> {
    let digest = sha256(MESSAGE1)?;
    let mut ec_key = EcKey::new_p256()?;
    ec_key.generate_key()?;

    let signature = ec_key.ecdsa_sign_nist(&digest)?;
    ec_key.ecdsa_verify_nist(&signature, &digest)?;
    assert_eq!(signature.len(), 64);
    Ok(())
}

#[test]
fn verifying_ecdsa_nist_signed_with_a_different_message_fails() -> Result<()> {
    let digest = sha256(MESSAGE1)?;
    let mut ec_key = EcKey::new_p256()?;
    ec_key.generate_key()?;
    ...
    let err = ec_key.ecdsa_verify_der(&signature, &sha256(MESSAGE2)?).unwrap_err();
    let expected_err = Error::CallFailed(ApiName::ECDSA_verify, EcdsaError::BadSignature.into());
    assert_eq!(expected_err, err);
    Ok(())
}
