...
pub unsafe extern "C" fn AVmPayload_readRollbackProtectedSecret(data: *mut u8) {
    initialize_logging();
    let stored_data = unwrap_or_abort(try_read_rollback_protected_data());
    if let Some(stored_data) = stored_data {
        // SAFETY: See the requirements on `data` above; `stored_data` is known to have length 32,
        // and cannot overlap `data` because we just allocated it.
        unsafe {
            ptr::copy_nonoverlapping(stored_data.as_ptr(), data, 32);
        }
    }
}
