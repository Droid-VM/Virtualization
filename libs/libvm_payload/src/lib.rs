...
use vm_payload_status_bindgen::{AVmAttestationStatus, AVmAccessRollbackProtectedSecretStatus};
use std::cmp::min;
...
/// Writes upto n bytes from buffer starting at `buf`, on behalf of the payload, to rollback
...
pub unsafe extern "C" fn AVmPayload_writeRollbackProtectedSecret(buf: *const u8, n: usize) -> i32 {
    ...
    if n < RP_DATA_SIZE {
        ...
        return AVmAccessRollbackProtectedSecretStatus::BAD_SIZE as i32;
    }
    ...
    match try_writing_payload_rollback_protected_data(buf.try_into().unwrap()) {
        ...
        Err(e) => {
            ...
            AVmAccessRollbackProtectedSecretStatus::ACCESS_FAILED as i32
        }
    }
}

/// Read upto n bytes into `buf`, written on behalf of the payload, to rollback detectable storage.
/// For this implementation, the backing storage is Secretkeeper HAL, which allows storing & reading
/// of 32 bytes secret!
///
/// # Safety
    ...
    match try_read_rollback_protected_data() {
        ...
        Ok(stored_data) => {
            if let Some(stored_data) = stored_data {
                ...
                AVmAccessRollbackProtectedSecretStatus::ENTRY_NOT_FOUND as i32
            }
        }
    }
}
