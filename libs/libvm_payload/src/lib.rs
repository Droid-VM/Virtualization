...
/// Get the accessor binder from Microdroid Manager. That process is the only process with
/// permissions to create the VSOCK connections required to communicate with host services.
///
...
pub unsafe extern "C" fn AVmPayload_getAccessorBinder(instance: *const c_char) -> *mut AIBinder {
    ...
    let instance_cstr = unsafe { CStr::from_ptr(instance) };
    let service = unwrap_or_abort(get_vm_payload_service());
    match service.getAccessorBinder(
        instance_cstr.to_str().expect("instance argument contains invalid UTF-8"),
    ) {
        Ok(accessor) => {
