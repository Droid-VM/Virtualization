...
__attribute__((constructor)) void injectServices(void) {
    ...
    ARpc_AccessorProvider* provider = ARpc_addAccessorProvider(accessorProvider, nullptr, nullptr);

    __android_log_write(ANDROID_LOG_INFO, TAG, "addAccessorProvider returned: ");
    if (provider) {
        __android_log_write(ANDROID_LOG_INFO, TAG, "success!");
    } else {
        __android_log_write(ANDROID_LOG_INFO, TAG, "failure!");
    }
    // drop the pointer to the ARpc_AccessorProvider since we never
