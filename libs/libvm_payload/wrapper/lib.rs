...
pub fn read_rollback_protected_secret(data: &[u8; 32]) {
    // SAFETY: The function only reads from`[data]` within its bounds.
    unsafe {
        AVmPayload_readRollbackProtectedSecret(data.as_ptr() as *mut c_void);
    }
}

/// Write `data`, on behalf of the payload, to Secretkeeper.
pub fn write_rollback_protected_secret(data: &[u8; 32]) {
    // SAFETY: The function only writes to `[data]` within its bounds.
    unsafe {
        AVmPayload_writeRollbackProtectedSecret(data.as_ptr() as *const c_void);
    }
}
