...
pub fn read_rollback_protected_secret(data: &mut [u8; 32]) -> AccessRollbackProtectedSecretStatus {
    // SAFETY: The function only reads from`[data]` within its bounds.
    unsafe { AVmPayload_readRollbackProtectedSecret(data.as_ptr() as *mut c_void, 32) }
}
...
pub fn write_rollback_protected_secret(data: &[u8; 32]) -> AccessRollbackProtectedSecretStatus {
    ...
    unsafe { AVmPayload_writeRollbackProtectedSecret(data.as_ptr() as *const c_void, 32) }
}
...
pub fn is_new_instance_status(is_new_instance: &mut bool) -> i32 {
    // SAFETY: The function only writes to `[is_new_instance]` within its bounds.
