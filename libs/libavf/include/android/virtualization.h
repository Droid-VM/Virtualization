...
/**
 * Use the specified fd as the device tree overlay blob for booting VM. Overlays applied in
 * insertion order.
 *
 * \param config a virtual machine config object.
 * \param fd a device tree blob for overlay
 *
 * \return If successful, it returns 0. It returns `-EBADF` if FD is invalid.
 */
