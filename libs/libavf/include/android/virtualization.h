 ...
 *
 * \return If successful, it returns 0. If `instanceIdSize` is not 64, it returns -EINVAL.
 */
 ...
 * \param config a virtual machine config object.
 * \param fd a readable, seekable, and sized (i.e. report a valid size using fstat()) file
 *   descriptor containing the disk. `AVirtualMachineRawConfig_addDisk` takes ownership of `fd`.
 *
...
int AVirtualMachineRawConfig_addCustomMemoryBackingFile(AVirtualMachineRawConfig* _Nonnull config,
                                                        int fd, size_t rangeStart, size_t rangeEnd)
        __INTRODUCED_IN(36);
 ...
 * @see AVirtualMachine_waitForStopWithTimeout
 */
 ...
 * machine is started or an error happens.
 *
 ...
 * virtual machine to stop by either `AVirtualMachine_waitForStop` or
 * `AVirtualMachine_waitForStopWithTimeout`.
 *
 ...
 *
 * `AVirtualMachine_waitForStop` waits indefinitely. Use `AVirtualMachine_waitForStopWithTimeout` if
 * you want to set a timeout.
 *
 * \param vm a handle on a virtual machine.
 *
 * \return The reason why the virtual machine stopped.
 */
enum AVirtualMachineStopReason AVirtualMachine_waitForStop(AVirtualMachine* _Nonnull vm)
        __INTRODUCED_IN(36);
 ...
 *
 * \return
 *   - If the virtual machine stops within the timeout, it sets `reason` and returns true.
 *   - If the timeout expired, it returns `false`.
 */
bool AVirtualMachine_waitForStopWithTimeout(AVirtualMachine* _Nonnull vm, struct timespec timeout,
