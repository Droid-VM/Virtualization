...
pub unsafe extern "C" fn AVirtualMachine_waitForStop(
    vm: *const VmInstance,
) -> AVirtualMachineStopReason {
    // SAFETY: `vm` is assumed to be a valid, non-null pointer returned by
    // AVirtualMachine_create. It's the only reference to the object.
    let vm = unsafe { &*vm };
    death_reason_to_stop_reason(vm.wait_for_death())
}

/// Wait until a virtual machine stops or the timeout elapses.
///
/// # Safety
/// `vm` must be a pointer returned by `AVirtualMachine_createRaw`. `reason` must be a valid,
/// non-null pointer to an AVirtualMachineStopReason object.
#[no_mangle]
pub unsafe extern "C" fn AVirtualMachine_waitForStopWithTimeout(
    vm: *const VmInstance,
    timeout: timespec,
