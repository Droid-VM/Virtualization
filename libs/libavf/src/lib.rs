...
/// Create a virtual machine raw config object. The caller takes ownership of the returned config
/// object, and is responsible for releasing it by calling `AVirtualMachineConfig_delete`.
#[no_mangle]
pub extern "C" fn AVirtualMachineConfig_createRaw() -> *mut *mut VirtualMachineConfig {
    let config = Box::new(VirtualMachineConfig::RawConfig(VirtualMachineRawConfig {
        platformVersion: "~1.0".to_owned(),
        ..Default::default()
    }));
    // SAFETY: `config` is the only reference here and the caller takes ownership.
    unsafe { Box::into_raw(config) }
}
...
/// and `log_fd`, and taken file descriptors will be closed upon `AVirtualMachine_delete`.
#[no_mangle]
