...
impl VmConfig {
    ...
    pub fn to_parcelable(&self) -> Result<VirtualMachineRawConfig, Error> {
        ...
        let cpu_topology = if let Some(cpu_topology) = &self.cpu_topology {
            match cpu_topology.as_str() {
                "one_cpu" => CpuTopology::ONE_CPU,
                "match_host" => CpuTopology::MATCH_HOST,
                _ => bail!("Invalid cpu topology {}", cpu_topology),
            }
        } else {
            CpuTopology::ONE_CPU
        };
