...
impl VmConfig {
    ...
    pub fn to_parcelable(&self) -> Result<VirtualMachineRawConfig, Error> {
        ...
        let usb_config = if let Some(usb_config) = &self.usb_config {
            usb_config.to_parcelable()?
        } else {
            AidlUsbConfig { controller: true, devices: Vec::new() }
        };
...
impl UsbConfig {
    fn to_parcelable(&self) -> Result<AidlUsbConfig> {
        ...
        self.devices.iter().for_each(|device| {
            devices.push(open_parcel_file(device, true).expect("failed to open usb device"))
        });
        Ok(AidlUsbConfig { controller: self.controller, devices })
    }
}
