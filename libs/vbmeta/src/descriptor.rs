    ...
    pub(super) fn from_image(data: &[u8]) -> Result<Descriptors<'_>, VbMetaImageParseError> {
        extern "C" fn desc_cb(descriptor: *const AvbDescriptor, user_data: *mut c_void) -> bool {
            ...
            descriptors.push(match desc.tag {
                AvbDescriptorTag::AVB_DESCRIPTOR_TAG_PROPERTY => Descriptor::Property(data),
                AvbDescriptorTag::AVB_DESCRIPTOR_TAG_HASHTREE => Descriptor::Hashtree(data),
                AvbDescriptorTag::AVB_DESCRIPTOR_TAG_HASH => Descriptor::Hash(data),
