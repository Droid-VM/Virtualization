...
fn try_main() -> Result<()> {
    ...
    let sign1 = CoseSign1Builder::new()
        ...
        .try_create_signature(&[], |m| Signer::new_without_digest(&attestation_key)?.sign_oneshot_to_vec(m)).context("Creating COSE_Sign1")?
        .build()
        .to_vec().map_err(|ce| anyhow!("Creating COSE_Sign1: {:?}", ce))?;

    let challenge = b"This is from the server";
    let _certificates = vm_service
        .getRemotelyAttestedCertificate(
            &bcc,
            &sign1,
            challenge,
        )
