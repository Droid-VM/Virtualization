...
pub extern "C" fn AVmPayload_enableTombstoneExport() {
    lazy_static! {
        static ref ENABLE_TOMBSTONE_EXPORT: () = unwrap_or_abort(try_enable_tombstone_export());
    }

    lazy_static::initialize(&ENABLE_TOMBSTONE_EXPORT);
}

fn try_enable_tombstone_export() -> Result<()> {
    initialize_logging();

    try_set_export_tombstone()?;
    try_ramdump_if_supported()?;
