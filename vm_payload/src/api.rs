...
pub unsafe extern "C" fn AVmPayload_getPrivateKeyFromResult(
    ...
    if size != 0 {
        // SAFETY: See the requirements on `data` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and `private_key` cannot overlap `data` because we just
        // allocated it. We allow data to be null, which is never valid, but only if size ==
        // 0 which is checked above.
        unsafe {
...
pub unsafe extern "C" fn AVmPayload_signWithAttestedKey(
    ...
    if size != 0 {
        // SAFETY: See the requirements on `data` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and `signature` cannot overlap `data` because we just
        // allocated it. We allow data to be null, which is never valid, but only if size ==
        // 0 which is checked above.
        unsafe {
...
pub unsafe extern "C" fn AVmPayload_getCertificateChainFromResult(
    ...
    if size != 0 {
        // SAFETY: See the requirements on `data` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and `cert_chain` cannot overlap `data` because we just
        // allocated it. We allow data to be null, which is never valid, but only if size ==
        // 0 which is checked above.
        unsafe {
            ptr::copy_nonoverlapping(
