...
/// Generates a certificate using the provided certificate signing request (CSR).
/// Panics on failure.
///
/// # Safety
///
...
/// [valid]: ptr#safety
#[no_mangle]
pub unsafe extern "C" fn AVmPayload_generateCertificate(
    csr: *const u8,
    csr_size: usize,
    buffer: *mut u8,
    size: usize,
    ...
    let certificate = unwrap_or_abort(try_generate_certificate(csr));

    if size != 0 {
        // SAFETY: See the requirements on `buffer` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and `certificate` cannot overlap `buffer` because we just allocated
        // it. We allow data to be null, which is never valid, but only if size == 0 which is
        // checked above.
        unsafe {
...
fn try_generate_certificate(csr: &[u8]) -> Result<Vec<u8>> {
