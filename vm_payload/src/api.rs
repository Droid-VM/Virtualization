...
/// These constants must be kept in sync with the enum `attestation_status_t` in vm_payload.h.
mod attestation_status_t {
    pub(super) const ATTESTATION_OK: i32 = 0;
    pub(super) const ATTESTATION_UNKNOWN_ERROR: i32 = -10000;
    pub(super) const ATTESTATION_ERROR_INVALID_CHALLENGE: i32 = ATTESTATION_UNKNOWN_ERROR - 1;
}
...
pub unsafe extern "C" fn AVmPayload_requestAttestation(
    ...
    // SAFETY: The validity of `res` is guaranteed by the caller.
    unsafe {
...
#[no_mangle]
pub extern "C" fn AAttestationResult_resultToString(status: i32) -> *const c_char {
    let message = match status {
        attestation_status_t::ATTESTATION_OK => {
            CStr::from_bytes_until_nul(b"The remote attestation completes successfully.\0").unwrap()
        }
        attestation_status_t::ATTESTATION_ERROR_INVALID_CHALLENGE => {
            CStr::from_bytes_until_nul(b"The challenge size is not between 0 and 64.\0").unwrap()
        }
        _ => CStr::from_bytes_until_nul(
...
/// Reads the COSE_Key encoded EC P-256 private key from the provided attestation result.
///
/// # Safety
///
/// Behavior is undefined if any of the following conditions are violated:
...
/// [valid]: ptr#safety
#[no_mangle]
pub unsafe extern "C" fn AVmPayload_getPrivateKeyFromResult(
    ...
    // SAFETY: Caller should ensure that `res` points to a valid `AttestationResult`.
    let private_key = unsafe { &(*res).privateKey };
    if size != 0 {
        // SAFETY: See the requirements on `data` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and `cdi` cannot overlap `data` because we just allocated
        // it. We allow data to be null, which is never valid, but only if size == 0 which is
...
/// Signs the given message using ECDSA with the attested EC P-256 private key in the
/// attestation result and returns a DER-encoded ECDSASignature.
///
/// # Safety
///
/// Behavior is undefined if any of the following conditions are violated:
...
/// [valid]: ptr#safety
#[no_mangle]
pub unsafe extern "C" fn AVmPayload_signWithAttestedKey(
    res: *const AttestationResult,
    message: *const u8,
    message_size: usize,
    data: *mut u8,
    ...
    let signature = unwrap_or_abort(try_ecdsa_sign(message, res));
    if size != 0 {
        // SAFETY: See the requirements on `data` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and `cdi` cannot overlap `data` because we just allocated
        // it. We allow data to be null, which is never valid, but only if size == 0 which is
        // checked above.
        unsafe {
            ptr::copy_nonoverlapping(signature.as_ptr(), data, std::cmp::min(signature.len(), size))
        };
    }
    signature.len()
}

fn try_ecdsa_sign(message: &[u8], res: &AttestationResult) -> Result<Vec<u8>> {
    get_vm_payload_service()?.ecdsaSign(message, res).context("Failed to sign with ECDSA")
}
