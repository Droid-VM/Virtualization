...
/// [valid]: ptr#safety
#[no_mangle]
pub unsafe extern "C" fn AVmPayload_getCertificate(
    csr: *const u8,
    csr_size: usize,
    buffer: *mut u8,
    size: usize,
    ...
    let certificate = unwrap_or_abort(try_get_certificate(csr));
...
fn try_get_certificate(csr: &[u8]) -> Result<Vec<u8>> {
    let certificate =
        get_vm_payload_service()?.getCertificate(csr).context("Failed to get certificate")?;
    Ok(certificate)
}
