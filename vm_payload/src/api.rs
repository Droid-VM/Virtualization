...
/// the process.
fn panic_on_error<T, E: Debug>(result: Result<T, E>) -> T {
...
pub extern "C" fn AVmPayload_notifyPayloadReady() {
    ...
    if !ALREADY_NOTIFIED.swap(true, Ordering::Relaxed) {
        panic_on_error(try_notify_payload_ready());
...
pub unsafe extern "C" fn AVmPayload_getVmInstanceSecret(
    ...
    let vm_secret = panic_on_error(try_get_vm_instance_secret(identifier, size));
...
pub unsafe extern "C" fn AVmPayload_getDiceAttestationChain(
    ...
    let chain = panic_on_error(try_get_dice_attestation_chain());
...
pub unsafe extern "C" fn AVmPayload_getDiceAttestationCdi(
    ...
    let cdi = panic_on_error(try_get_dice_attestation_cdi());
    total.write(cdi.len());
