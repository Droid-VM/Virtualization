...
pub unsafe extern "C" fn AVmPayload_getCertificateChainFromResult(
    ...
    // SAFETY: Caller should ensure that `res` points to a valid `AttestationResult`.
    let certificates = unsafe { &(*res).certificateChain };
    let cert_chain = certificates
        .iter()
        .flat_map(|certificate| &certificate.encodedCertificate)
        .cloned()
        .collect::<Vec<u8>>();
    if size != 0 {
        // SAFETY: See the requirements on `data` above. The number of bytes copied doesn't exceed
        // the length of either buffer, and the caller ensures that `cert_chain` cannot overlap
        // `data`. We allow data to be null, which is never valid, but only if size == 0
        // which is checked above.
        unsafe {
            ptr::copy_nonoverlapping(
