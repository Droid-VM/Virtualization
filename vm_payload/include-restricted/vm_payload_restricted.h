...
typedef enum {
    ...
    AATESTATION_UNKNOWN_ERROR = -10000,

    /** The challenge size is not between 0 and 64. */
    AATESTATION_ERROR_INVALID_CHALLENGE = AATESTATION_UNKNOWN_ERROR - 1,
 ...
 *          64 bytes. The status `attestation_status_t::AATESTATION_ERROR_INVALID_CHALLENGE`
 ...
 *
 * \return AATESTATION_OK on successful attestation.
 */
attestation_status_t AVmPayload_requestAttestation(const void* _Nonnull challenge,
                                                   size_t challenge_size,
                                                   struct AttestationResult** _Nonnull result)
