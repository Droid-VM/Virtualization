 ...
 *
 * \return `attestation_status_t::ATTESTATION_OK` upon successful attestation.
 */
 ...
 */
void AVmAttestationResult_free(struct AVmAttestationResult* _Nonnull result)
        __INTRODUCED_IN(__ANDROID_API_V__);
 ...
 * the attestation key's certificate chain. It starts with the Google-rooted certificate and
 ...
 */
size_t AVmAttestationResult_getCertificatesCount(const struct AVmAttestationResult* _Nonnull result)
        __INTRODUCED_IN(__ANDROID_API_V__);
 ...
 * \param result A pointer to the attestation result obtained from `AVmPayload_requestAttestation`
 *               when the attestation succeeds.
 * \param index Index of the certificate to retrieve.
