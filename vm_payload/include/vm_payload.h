 ...
 * For ex, decryption of any data is meaningless with the returned secret if `AVmPayload_isNewInstance`
 * returns true.
 *
 ...
 *
 * \return The result is true on the first boot of the instance (when secrets are created) and false on subsequent boot.
 */
