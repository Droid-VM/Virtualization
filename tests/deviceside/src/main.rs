...
const GOLDEN_DEVICE_TREE: &str = "dt_dump_golden.dts";
const GOLDEN_DEVICE_TREE_PROTECTED: &str = "dt_dump_protected_golden.dts";
...
fn run_test(protected: bool, golden_dt: &str) -> Result<(), Error> {
    ...
    let dump_dt = File::options()
        ...
        .write(true)
        .truncate(true)
        .open("/data/local/tmp/dump_dt.dtb")
    ...
    Command::new("./dtc_static")
        ...
        .arg("-s")
        .arg("-o")
        .arg("/data/local/tmp/dump_dt_golden.dtb")
        .arg(golden_dt)
        .output()
        .context("failed to execute dtc")?;
    let dtcompare_res = Command::new("./dtcompare")
        .arg("--dt1")
        .arg("/data/local/tmp/dump_dt_golden.dtb")
        .arg("--dt2")
        .arg("/data/local/tmp/dump_dt.dtb")
        .arg("--ignore-value")
    ...
    assert!(dtcompare_res.stderr.is_empty(), "{:?}", dtcompare_res.stderr);
