...
fn compare_device_trees(
    ...
    fn compare_subnodes(
        ...
        for subnode in node1.subnodes().unwrap() {
            subnodes_map.insert(subnode.name().unwrap().to_str()?.to_string(), subnode);
        }
        for sn2 in node2.subnodes().unwrap() {
            let name = sn2.name().unwrap().to_str()?;
