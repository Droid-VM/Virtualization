...
fn main() -> Result<()> {
    ...
    compare_device_trees(dt1.as_slice(), dt2.as_slice(), args.ignore_path_value, args.ignore_path)
}
...
fn compare_device_trees(
    ...
    let ignore_set = BTreeSet::from_iter(ignore_path_value);
    let skip_set = BTreeSet::from_iter(ignore_path);
...
fn compare_props(
    ...
    for prop in root1.properties().context("Error getting properties")? {
        ...
        if skip_set.contains(&prop_path) {
            continue;
        }
    ...
    for prop in root2.properties().context("Error getting properties")? {
        ...
        let Some(prop1_value) = prop_map.remove(&prop_path) else {
            return Err(anyhow!("Fdt2 added prop_path: {}", prop_path));
        };
        ...
        if !prop_compare {
            ...
            errors.push(prop_path);
        }
    }
    if !prop_map.is_empty() {
        return Err(anyhow!("Dt2 missing properties: {:?}", prop_map));
    }
    Ok(())
}
...
fn compare_subnodes(
    ...
    for sn2 in node2.subnodes().context("Error getting subnodes of second FDT")? {
        ...
        match sn1 {
            Some(sn) => {
                compare_props(&sn, &sn2, ignore_set, skip_set, &mut [sn_path.clone()], errors)?;
                compare_subnodes(&sn, &sn2, ignore_set, skip_set, &mut [sn_path.clone()], errors)?;
            }
            None => return Err(anyhow!("Fdt2 added node: {}", sn_path)),
        }
    }
    if !subnodes_map.is_empty() {
        return Err(anyhow!("Fdt2 missing nodes: {:?}", subnodes_map));
    }
    Ok(())
}
