...
fn fdterr_to_anyhow(err: FdtError) -> anyhow::Error {
    anyhow!("Fdt error: {:?}", err)
}

// Compare device trees by doing a depth-first traversal of the trees.
fn compare_device_trees(
    ...
    let fdt1 = Fdt::from_slice(dt1).map_err(fdterr_to_anyhow)?;
    let fdt2 = Fdt::from_slice(dt2).map_err(fdterr_to_anyhow)?;
    ...
    fn compare_props(
        ...
        for prop in root1.properties().map_err(fdterr_to_anyhow)? {
            let name = prop.name().map_err(fdterr_to_anyhow)?.to_str()?;
            // Do not add to prop map if skipping
            if skip_set.contains(name) {
                continue;
            }
            let value = prop.value().map_err(fdterr_to_anyhow)?;
            if prop_map.insert(name.to_string(), value).is_some() {
                return Err(anyhow!("Duplicate properties detected in subnode"));
            }
        }
        for prop in root2.properties().map_err(fdterr_to_anyhow)? {
            let name = prop.name().map_err(fdterr_to_anyhow)?.to_str()?;
            if skip_set.contains(name) {
                continue;
            }
            let prop1 = prop_map.remove(name);
            let prop_compare = match prop1 {
                Some(val) => *val == *prop.value().map_err(fdterr_to_anyhow)?,
                None => {
                    return Err(anyhow!(
            ...
            if !prop_compare {
                return Err(anyhow!(
                    ...
                    name,
                    prop1,
                    prop.value().map_err(fdterr_to_anyhow)?
                ));
            }
        }
    ...
    fn compare_subnodes(
        ...
        for subnode in node1.subnodes().map_err(fdterr_to_anyhow)? {
            subnodes_map
                .insert(subnode.name().map_err(fdterr_to_anyhow)?.to_str()?.to_string(), subnode);
        }
        for sn2 in node2.subnodes().map_err(fdterr_to_anyhow)? {
            let name = sn2.name().map_err(fdterr_to_anyhow)?.to_str()?;
            let sn1 = subnodes_map.remove(name);
            match sn1 {
                Some(sn) => {
                    compare_subnodes(&sn, &sn2, ignore_set, skip_set)?;
                    compare_props(&sn, &sn2, ignore_set, skip_set)?;
                }
                None => return Err(anyhow!("Missing node {} from Dt1", name)),
            }
        }
        if !subnodes_map.is_empty() {
            return Err(anyhow!("Dt2 missing nodes that exist in Dt1: {:?}", subnodes_map));
        }
        Ok(())
    }
