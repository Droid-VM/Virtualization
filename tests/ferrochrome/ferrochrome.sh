...
FECR_DEFAULT_IMAGE="${FECR_BASE_IMAGE}" # Match this with AndroidTest.xml and assets/vm_config.json
...
FECR_BOOT_COMPLETED_LOG_PATTERN="Chrome started, our work is done, exiting\|Have fun and send patches!"
...
while [[ $((EPOCHSECONDS - fecr_start_time)) -lt ${FECR_BOOT_TIMEOUT} ]]; do
  adb shell grep -se \""${FECR_BOOT_COMPLETED_LOG_PATTERN}"\" "${log_path}" && exit 0 || true
  sleep 10
done
