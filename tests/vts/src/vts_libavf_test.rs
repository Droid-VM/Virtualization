...
fn run_rialto(protected_vm: bool) -> Result<()> {
    ...
    if unsafe { AVirtualizationService_create(&mut service, false) } < 0 {
        bail!("AVirtualizationService_create failed");
    }
    ...
    {
        bail!("AVirtualMachine_createRaw failed");
    }
    ...
    let mut vsock_stream = listen_from_guest(vm_type.port())?;
    ...
    if reversed_data != expected_data {
        bail!("Expected {expected_data:?} but was {reversed_data:?}");
    }
    ...
    // SAFETY: vm is the only reference to a valid object
    if !unsafe { AVirtualMachine_waitForStop(vm, &STOP_TIMEOUT, &mut stop_reason) } {
        bail!("AVirtualMachine_waitForStop failed");
    }
