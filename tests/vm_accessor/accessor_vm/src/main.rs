...
impl IAccessorVmService for AccessorVmService {
    ...
    fn tryGetHostStatsService(&self) -> binder::Result<()> {
        ...
        let stats: Strong<dyn IStats> = binder::wait_for_interface(&stats_service_descriptor)
            .expect("Failed to get IStats host service");
        stats.reportVendorAtom(&VendorAtom::default()).expect("Failed to call reportVendorAtom");
