...
public class MicrodroidHostTests extends MicrodroidHostTestCaseBase {
    ...
    private VmInfo runMicrodroidWithResignedImages(
        ...
        Process process = newRunUtil().runCmdInBackground(args, new PipedOutputStream(pis));
    ...
    private boolean isLz4(String path) throws Exception {
        ...
        CommandResult result =
                newRunUtil().runTimedCmd(5000, lz4tool.getAbsolutePath(), "-t", path);
    ...
    private String avbInfo(String image_path) throws Exception {
        ...
        CommandResult result =
                newRunUtil().runTimedCmd(5000, "/bin/bash", "-c", String.join(" ", command));
        String out = result.getStdout();
        String err = result.getStderr();
        assertWithMessage(
                        "Command "
    ...
    private static RunUtil newRunUtil() {
