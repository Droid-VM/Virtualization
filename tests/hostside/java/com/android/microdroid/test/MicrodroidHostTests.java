...
public class MicrodroidHostTests extends MicrodroidHostTestCaseBase {
    ...
    public void testDeviceAssignment() throws Exception {
        ...
        try {
            ...
            CLog.e("jwkim which dtc" + yyy.toString());

            CommandResult xxx = RunUtil.getDefault().runTimedCmd(500, "dtc", "--help");
            CLog.e("jwkim dtc --help" + xxx.toString());
        } catch (Exception e) {
            CLog.e("jwkim dtc failed... " + e);
        }
                ...
                result =
                        RunUtil.getDefault()
                                .runTimedCmd(
                                        ...
                                        baseFdtFile.getPath(),
                                        fdtFile.getPath());
                CLog.e("jwkim dtdiff v1 " + result);
            } catch (Exception e) {
                CLog.e("jwkim failed to launch dtdiff -- v1 " + e);
            }

            result = dtdiff(baseFdtFile.getPath(), fdtFile.getPath());
            CLog.e("jwkim dtdiff v2 " + result);
