...
public final class AuthFsTestCase extends VirtualizationTestCaseBase {
    ...
    private static DeviceRunner sAndroidDeviceRunner;
    ...
    public static void beforeClassWithDevice(TestInformation testInfo)
        ...
        sAndroidDeviceRunner = new DeviceRunner(androidDevice);
    ...
    public static void afterClassWithDevice(TestInformation testInfo)
            throws DeviceNotAvailableException {
        assertNotNull(sAndroidDeviceRunner);

        if (sCid != null) {
            shutdownMicrodroid(sAndroidDeviceRunner.getDevice(), sCid);
            sCid = null;
        }

        cleanUpVirtualizationTestSetup(sAndroidDeviceRunner.getDevice());
        sAndroidDeviceRunner = null;
    }

    @After
    public void tearDown() throws DeviceNotAvailableException {
        sAndroidDeviceRunner.tryRun("killall fd_server");
        sAndroidDeviceRunner.tryRun("rm -f " + TEST_DIR + "/output");
    ...
    public void testWriteFailedIfDetectsTampering()
        ...
        sAndroidDeviceRunner.run("dd if=/dev/zero of=" + backendPath + " bs=1 count=8192");
    ...
    private String computeFileHashOnAndroid(String path) throws DeviceNotAvailableException {
        String result = sAndroidDeviceRunner.run("sha256sum " + path);
        String[] tokens = result.split("\\s");
        if (tokens.length > 0) {
            return tokens[0];
        } else {
        ...
        mThreadPool.submit(
                () -> {
                    try {
                        ...
                        CommandResult result = sAndroidDeviceRunner.runForResult(cmd);
