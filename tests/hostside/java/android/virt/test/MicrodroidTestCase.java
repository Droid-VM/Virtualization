...
public class MicrodroidTestCase extends BaseHostJUnit4Test {
    ...
    private String executeCommand(String cmd) {
        final long defaultCommandTimeoutMillis = 10000; // 1 sec
    ...
    @Before
    public void enableAdbRoot() throws Exception {
        mWasAdbRoot = getDevice().isAdbRoot();
        getDevice().enableAdbRoot();
        assertThat(getDevice().isAdbRoot(), is(true));
    }

    @After
    public void recoverAdbRoot() throws Exception {
        if (!mWasAdbRoot) {
    ...
    @Before
    public void deleteTestRoot() throws Exception {
        getDevice().executeShellCommand("rm -rf " + TEST_ROOT);
    }

    @Before
    @After
    public void disconnectFromMicrodroid() throws Exception {
        executeCommand("adb disconnect " + MICRODROID_SERIAL);
    }

    @After
    public void killStaleCrosvmProcesses() throws Exception {
        getDevice().executeShellV2Command("pidof crosvm && kill -9 $(pidof crosvm)");
    }
}
