...
static constexpr const char MODULES_LOAD_LIST[] = "modules.load";

bool LoadKernelModules(bool want_console) {
    struct utsname uts;
    if (uname(&uts)) {
    ...
    for (const auto& module_dir : module_dirs) {
        ...
        Modprobe m({dir_path}, MODULES_LOAD_LIST);
    ...
    Modprobe m({MODULE_BASE_DIR}, MODULES_LOAD_LIST);
    bool retval = m.LoadListedModules(!want_console);
    int modules_loaded = m.GetModuleCount();
    if (modules_loaded > 0) {
        return retval;
