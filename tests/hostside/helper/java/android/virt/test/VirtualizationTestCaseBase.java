...
public class VirtualizationTestCaseBase extends BaseHostJUnit4Test {
    ...
    // Same as runOnMicrodroid, but returns a boolean for success or error.
    public boolean tryRunOnMicrodroid(String... cmd) {
        CommandResult result = runOnMicrodroidInternal(cmd);
        if (result.getStatus() == CommandStatus.SUCCESS) {
            return true;
        } else {
