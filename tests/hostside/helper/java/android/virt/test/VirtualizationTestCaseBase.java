...
public abstract class VirtualizationTestCaseBase extends BaseHostJUnit4Test {
    ...
    // Same as runOnHost, but keeps retrying on error for maxTimeMs time
    // Each attempt with timeoutMs
    public static String runOnHostRetryingOnFailure(long timeoutMs, long maxTimeMs, String... cmd)
            throws InterruptedException {
        long start = System.currentTimeMillis();
        while (true) {
            CommandResult result = RunUtil.getDefault().runTimedCmd(timeoutMs, cmd);
    ...
    // Same as runOnMicrodroid, but keeps retrying on error till maxTimeMs
    public static String runOnMicrodroidRetryingOnFailure(long timeout, long maxTime,
            String... cmd)
            throws InterruptedException {
        return runOnHostRetryingOnFailure(timeout, maxTime,
    ...
    public static void rootMicrodroid() throws InterruptedException {
        runOnHostRetryingOnFailure(10000, MICRODROID_ADB_CONNECT_TIMEOUT_MINUTES,
                "adb", "-s", MICRODROID_SERIAL, "root");
        // adbd reboots after root. Some commands (including wait-for-device) following this fails
        // with with error: closed. Hence, we run adb shell true in microdroid with retries
        // before returning.
        runOnMicrodroidRetryingOnFailure(30000, MICRODROID_ADB_CONNECT_TIMEOUT_MINUTES, "true");
    }
