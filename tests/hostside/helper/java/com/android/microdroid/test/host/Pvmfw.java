...
public final class Pvmfw {
    ...
    private static final int HEADER_MAX_SIZE = 512; // must be SIZE_8B aligned.
    ...
    public void serialize(@NonNull File outFile) throws IOException {
        ...
        int bccOffset = HEADER_MAX_SIZE;
        int bccSize = (int) mBccFile.length();

        int debugPolicyOffset = alignTo(bccOffset + bccSize, SIZE_8B);
        int debugPolicySize = (int) mDebugPolicyFile.length();

        int totalSize = debugPolicyOffset + debugPolicySize;

        ByteBuffer header = ByteBuffer.allocate(HEADER_MAX_SIZE).order(LITTLE_ENDIAN);
    ...
    private void appendFile(FileOutputStream out, File inFile) throws IOException {
    ...
    private void padTo(FileOutputStream out, int size) throws IOException {
