...
public final class KvmHypTracer {
    ...
    public String run(String payload_cmd) throws Exception {
        ...
        mRunner.run("echo 1 | tee " + HYP_TRACING_ROOT + "events/*/enable");
        mRunner.run("echo 0 > " + HYP_TRACING_ROOT + "events/host_mem_abort/enable");
        //for (String event: HYP_EVENTS)
        //    setNode("events/" + event + "/enable", 1);
        setNode("tracing_on", 1); /* Toogle the tracing_on knob to get the buffer allocated */
        setNode("tracing_on", 0);
        ...
        for (File trace: mTraces) {
            ...
            while ((l = br.readLine()) != null) {
                ...
                if (event.equals(prev_event))
                    throw new ParseException("Hyp event found twice in a row: " + trace + " - " + l, 0);
