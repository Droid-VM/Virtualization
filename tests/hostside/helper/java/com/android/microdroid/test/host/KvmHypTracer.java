...
class KvmHypDurationStats {
    ...
    public void add(KvmHypEvent start, KvmHypEvent end) throws Exception {
        if (start.cpu != end.cpu || start.timestamp > end.timestamp ||
            !isValidEvent(start) || !isValidEvent(end)) {
        ...
        if (maxSection == null ||
            sectionDuration(maxSection) < duration) {
...
public final class KvmHypTracer {
    ...
    private void updateDurationStats() throws Exception {
        ...
        for (File trace : mTraces) {
            ...
            while ((event = getNextEvent(br)) != null) {
                ...
                if (prevEvent!= null && prevEvent.timestamp > cur) throw new ParseException(trace + ": Time must not go backward: " + cur, 0);

                if (prevEvent != null && prevEvent.name.equals(event.name)) {
                    throw new ParseException(
                            "Hyp event found twice in a row: file=" + trace + " event=" + event, 0);
                }
    ...
    public Pair<Double, Double> getMaxDurationSection() throws Exception {
        updateDurationStats();
        return Pair.create(mDurationStats.maxSection.first.timestamp,
    ...
    public Map<String, Double> getFuncDurations(double start, double end, int cpu) throws Exception {
                    ...
                    case "func_ret":
                        ...
                        if (!prev.first.equals(func)) {
                            throw new Exception("Event " + event +
                                                " does not match '" + prev.first + "'");
                        }

                        funcDurations.put(func,
                                funcDurations.getOrDefault(func, Double.valueOf(0)) +
                                event.timestamp - prev.second);
