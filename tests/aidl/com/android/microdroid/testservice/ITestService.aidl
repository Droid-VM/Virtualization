...
interface ITestService {
    ...
    /**
     * Read payload's rollback protected data. The `AccessRollbackProtectedSecretStatus` is wrapped
     * as service_specific error in case of failure. This is _only_ used for testing.
     */
    byte[32] insecurelyReadPayloadRpData();

    /**
     * Request VM to write payload's rollback protected data. The
     * `AccessRollbackProtectedSecretStatus` is wrapped as service_specific error in case of
