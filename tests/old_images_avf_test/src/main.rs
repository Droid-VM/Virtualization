...
fn run_vm(image_path: &str, test_name: &str, protected_vm: bool) -> Result<()> {
    ...
    unsafe {
        AVirtualMachineRawConfig_setName(
            config,
            CStr::from_bytes_with_nul(test_name.as_bytes()).unwrap().as_ptr(),
        );
...
fn test_run_rialto_protected() -> Result<()> {
    if hypervisor_props::is_protected_vm_supported()? {
        run_vm(
            "/data/local/tmp/android15_rialto.bin", /* image_path */
            "android15_test_rialto\0",              /* test_name */
            true,                                   /* protected_vm */
        )
    } else {
