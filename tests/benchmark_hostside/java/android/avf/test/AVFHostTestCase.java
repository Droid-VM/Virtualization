...
public final class AVFHostTestCase extends MicrodroidHostTestCaseBase {
    ...
    // Number of vCPUs for testing purposes
    ...
    public void testAppStartupTimeAfterVm() throws Exception {
        assumeTrue("Skip on non-protected VMs", hypervisor_protected_vm_supported());

        StartupTimeMetricCollection mCollection =
                new StartupTimeMetricCollection(SETTINGS_PACKAGE_NAME, ROUND_COUNT);
        final String suffix_total_time = "total_time";
        final String suffix_wait_time = "wait_time";
        for (int round = 0; round < ROUND_COUNT; ++round) {
            getAppStartupTime(mCollection);
        }

        reportMetric(mCollection.mAppBeforeVmRunTotalTime,
                mCollection.getPkgName() + "_before_vm" + suffix_total_time,
                "ms");
        reportMetric(mCollection.mAppBeforeVmRunWaitTime,
                mCollection.getPkgName() + "_before_vm" + suffix_wait_time,
                "ms");
        reportMetric(mCollection.mAppDuringVmRunTotalTime,
                mCollection.getPkgName() + "_during_vm" + suffix_total_time,
                "ms");
        reportMetric(mCollection.mAppDuringVmRunWaitTime,
                mCollection.getPkgName() + "_during_vm" + suffix_wait_time,
                "ms");
        reportMetric(mCollection.mAppAfterVmRunTotalTime,
                mCollection.getPkgName() + "_after_vm" + suffix_total_time,
                "ms");
        reportMetric(mCollection.mAppAfterVmRunWaitTime,
                mCollection.getPkgName() + "_after_vm" + suffix_wait_time,
                "ms");
    }
    ...
    private boolean hypervisor_protected_vm_supported()
        ...
        return Boolean.parseBoolean(
                android.run("getprop ro.boot.hypervisor.protected_vm.supported"));
    }
    ...
    private AmStartupTimeCmdParser getColdRunStartupTimes(String pkgName)
        ...
        assertTrue(vmStartAppLog != null && !vmStartAppLog.isEmpty());
