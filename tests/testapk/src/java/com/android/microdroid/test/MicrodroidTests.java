...
public class MicrodroidTests extends MicrodroidDeviceTestBase {
    ...
    public void connectToVmService() throws VirtualMachineException, InterruptedException {
        ...
        VirtualMachineConfig.Builder builder =
                newVmConfigBuilder("assets/vm_config_extra_apk.json");
        ...
        VirtualMachine vm = createNewVirtualMachine("test_vm_extra_apk", config);
    ...
    public void changingDebugLevelInvalidatesVmIdentity()
        ...
        VirtualMachineConfig.Builder builder = newVmConfigBuilder("assets/vm_config.json");
        VirtualMachineConfig normalConfig = builder.debugLevel(DebugLevel.NONE).build();
        VirtualMachine vm = createNewVirtualMachine("test_vm", normalConfig);
        VmEventListener listener =
                new VmEventListener() {
                    @Override
                    public void onPayloadReady(VirtualMachine vm) {
        ...
        VirtualMachine newVm = createNewVirtualMachine("test_debug_vm", debugConfig);
        ...
        newVm.delete();
        vm = mVmm.get("test_vm"); // re-load with the copied-in config file.
        final CompletableFuture<Boolean> payloadStarted = new CompletableFuture<>();
        listener =
                new VmEventListener() {
                    @Override
    ...
    private VmCdis launchVmAndGetCdis(String instanceName)
        ...
        VirtualMachineConfig normalConfig = newVmConfigBuilder("assets/vm_config.json")
                .debugLevel(DebugLevel.NONE)
                .build();
        VirtualMachine vm = mVmm.getOrCreate(instanceName, normalConfig);
    ...
    public void bccIsSuperficiallyWellFormed()
        ...
        VirtualMachineConfig normalConfig = newVmConfigBuilder("assets/vm_config.json")
                .debugLevel(DebugLevel.NONE)
                .build();
        VirtualMachine vm = createNewVirtualMachine("bcc_vm", normalConfig);
        final VmCdis vmCdis = new VmCdis();
    ...
    private RandomAccessFile prepareInstanceImage(String vmName)
            throws VirtualMachineException, InterruptedException, IOException {
        VirtualMachineConfig config = newVmConfigBuilder("assets/vm_config.json")
                .debugLevel(DebugLevel.FULL)
                .build();

        VirtualMachine vm = createNewVirtualMachine(vmName, config);
    ...
    private void assertThatBootFailsAfterCompromisingPartition(UUID partitionUuid)
        ...
        assertThat(tryBootVm(TAG, mVmm.get("test_vm_integrity")).payloadStarted).isFalse();
    }
    ...
    public void bootFailsWhenConfigIsInvalid()
            throws VirtualMachineException, InterruptedException, IOException {
        VirtualMachineConfig.Builder builder =
                newVmConfigBuilder("assets/vm_config_no_task.json");
        VirtualMachineConfig normalConfig = builder.debugLevel(DebugLevel.NONE).build();
        VirtualMachine vm = createNewVirtualMachine("test_vm_invalid_config", normalConfig);
