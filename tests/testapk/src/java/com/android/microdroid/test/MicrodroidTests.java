...
public class MicrodroidTests extends MicrodroidDeviceTestBase {
    ...
    public void testShareVmWithAnotherApp() throws Exception {
        ...
        // Send the VM descriptor to the other app. When received, it will reconstruct the VM from
        // the descriptor, start it, connect to the ITestService in it, creates a "proxy"
        // ITestService binder that delegates all the calls to the VM, and share it with this app.
        // It will allow us to verify assertions on the running VM in the other app.
        IBinder startVmBinder = service.startVm(vmDesc);
        assertThat(startVmBinder).isNotNull();
        ITestService testServiceProxy = ITestService.Stub.asInterface(startVmBinder);
    ...
    public void testShareVmWithAnotherApp_encryptedStorage() throws Exception {
        ...
        runVmTestService(
                vm,
                (ts, tr) -> {
                    ts.writeToFile("the content", "/mnt/encryptedstore/private.key");
                });
        // Get a descriptor that we will share with another app (VM_SHARE_APP_PACKAGE_NAME)
        VirtualMachineDescriptor vmDesc = vm.toDescriptor();
        ...
        assertThat(result).isEqualTo("the content");
    }

    private class VmShareServiceConnection implements ServiceConnection {
