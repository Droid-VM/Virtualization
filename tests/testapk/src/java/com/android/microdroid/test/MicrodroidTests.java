...
public class MicrodroidTests extends MicrodroidDeviceTestBase {
    ...
    public void concurrentVms() throws Exception {
        final long vmSize = minMemoryRequired(); // in MB
        final int numVMs = 8;
        final long availableMem = getAvailableMemoryMB();

        // Let's not use more than half of the available memory
        assume().withMessage("Available memory (" + availableMem + " MB) too small")
    ...
    private long getAvailableMemoryMB() {
        ...
        return memoryInfo.availMem / ONE_MEBI;
    }
