...
public class MicrodroidTests extends MicrodroidDeviceTestBase {
    ...
    @Test
    @CddTest(requirements = {"9.17/C-1-1", "9.17/C-2-1"})
    public void vmAttestationWhenDeviceDoesNotSupportRemoteAttestation() throws Exception {
        ...
        assume().withMessage("Device should not support Remote Attestation for this test")
    ...
    @Test
    @CddTest(requirements = {"9.17/C-1-1", "9.17/C-2-1"})
    public void vmAttestationWhenDeviceSupportsRemoteAttestation() throws Exception {
        // pVM remote attestation is only supported on protected VMs.
        assumeProtectedVM();
        assumeFeatureEnabled(VirtualMachineManager.FEATURE_REMOTE_ATTESTATION);
        assume().withMessage("Device should support Remote Attestation for this test")
                .that(getVirtualMachineManager().isRemoteAttestationSupported())
                .isTrue();
        VirtualMachineConfig config =
                newVmConfigBuilderWithPayloadBinary(VM_ATTESTATION_PAYLOAD_PATH)
