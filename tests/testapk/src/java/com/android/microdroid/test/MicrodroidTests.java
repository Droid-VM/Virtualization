...
public class MicrodroidTests extends MicrodroidDeviceTestBase {
    ...
    public void encryptedStorageSupportsExpansion() throws Exception {
        ...
        Log.e(TAG, "Encrypted storage size: " + testResults.mEncryptedStorageSize);
        testResults.assertNoException();
        assertThat(testResults.mEncryptedStorageSize).isWithin(TOLERANCE_BYTES).of(ENCRYPTED_STORAGE_BYTES);
        ...
        Log.e(TAG, "Encrypted storage size: " + testResults.mEncryptedStorageSize);
        testResults.assertNoException();
        assertThat(testResults.mEncryptedStorageSize).isWithin(TOLERANCE_BYTES).of(ENCRYPTED_STORAGE_BYTES * 2);
    }
    ...
    public void encryptedStorageShrinkFails() throws Exception {
        ...
        // Re-run the VM with more storage size & verify the file persisted.
        // Note, the previous `runVmTestService` stopped the VM
        VirtualMachineConfig newConfig = newVmConfigBuilderWithPayloadBinary("MicrodroidTestNativeLib.so")
                    .setEncryptedStorageBytes(ENCRYPTED_STORAGE_BYTES / 2)
                    .build();
        assertThrowsVmExceptionContaining(() -> vm.setConfig(newConfig), "Must be incremental config update");
    }
