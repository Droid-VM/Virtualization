...
public class MicrodroidTests extends MicrodroidDeviceTestBase {
    ...
    private void copyVmDirectory(String sourceVmName, String targetVmName) throws IOException {
        ...
        for (Path f : Files.list(sourceVm).collect(toList())) {
    ...
    // Create a fresh VM with the given `vmName`. This function creates a Vm with a different
    // temporary name &  copies it to target VM directory. This ensures this VM is not in cache of
    // `VirtualMachineManager`
    // which makes it possible to modify underlying files.
