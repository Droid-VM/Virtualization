...
extern "C" int AVmPayload_main() {
    ...
    ndk::SpAIBinder binder(
            AServiceManager_waitForService("android.hardware.light.ILights/default"));

    if (binder.get()) {
        __android_log_write(ANDROID_LOG_ERROR, TAG, "The binder is not null!");
    } else {
        __android_log_write(ANDROID_LOG_ERROR, TAG, "The binder is null");
    }
