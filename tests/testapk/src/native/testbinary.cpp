...
Result<void> start_test_service() {
    ...
    auto vsock_server = setup_vsock_server();
    if (vsock_server == nullptr) {
        return Error() << "Cannot setup vsock";
    }
    if (!notify_payload_ready(vsock_server)) {
        return Error() << "Failed to notify payload ready";
    }
