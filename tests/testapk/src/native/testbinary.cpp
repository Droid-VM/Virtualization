...
Result<void> start_test_service() {
    ...
    public:
        ...
        ScopedAStatus insecurelyReadPayloadRpData(std::array<uint8_t, 32>* out) override {
            AccessRollbackProtectedSecretStatus status =
                    AVmPayload_readRollbackProtectedSecret(out->data(), 32);
            if (status != AccessRollbackProtectedSecretStatus::ACCESS_OK) {
        ...
        ScopedAStatus insecurelyWritePayloadRpData(
            ...
            AccessRollbackProtectedSecretStatus status =
                    AVmPayload_writeRollbackProtectedSecret(inputData.data(), 32);
            if (status != AccessRollbackProtectedSecretStatus::ACCESS_OK) {
                return ScopedAStatus::fromServiceSpecificError(status);
            }
            return ScopedAStatus::ok();
        ...
        ScopedAStatus isNewInstance(bool* is_new_instance_out) override {
            auto ret = AVmPayload_isNewInstanceStatus(is_new_instance_out);
            if (ret != 0) {
