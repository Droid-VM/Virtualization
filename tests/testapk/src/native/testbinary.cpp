...
Result<void> start_test_service() {
    ...
    // TODO(inseob): we have to listen - notify - join. Refactor when rpc becomes stable
    ndk::SpAIBinder binder(
    ...
    if (virtualMachineService == nullptr) {
        return Error() << "failed to connect VirtualMachineService";
    }
    if (auto res = get_local_cid(); !res.ok()) {
        return res.error();
    } else if (!virtualMachineService->notifyPayloadReady(res.value()).isOk()) {
        return Error() << "failed to notify payload ready to virtualizationservice";
    }

    if (!RunRpcServer(testService->asBinder().get(), testService->SERVICE_PORT)) {
