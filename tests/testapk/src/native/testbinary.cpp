...
Result<void> start_test_service() {
    class TestService : public aidl::com::android::microdroid::testservice::BnTestService {
        ...
        ndk::ScopedAStatus insecurelyExposeAttestationCdi(std::vector<uint8_t>* out) override {
            size_t cdi_size;
            AVmPayload_getDiceAttestationCdi(nullptr, 0, &cdi_size);
            out->resize(cdi_size);
            AVmPayload_getDiceAttestationCdi(out->data(), out->size(), &cdi_size);
            return ndk::ScopedAStatus::ok();
        }

        ndk::ScopedAStatus getBcc(std::vector<uint8_t>* out) override {
            size_t bcc_size;
            AVmPayload_getDiceAttestationChain(nullptr, 0, &bcc_size);
            out->resize(bcc_size);
            AVmPayload_getDiceAttestationChain(out->data(), out->size(), &bcc_size);
            return ndk::ScopedAStatus::ok();
        }
