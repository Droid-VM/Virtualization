...
public class MicrodroidBenchmarks extends MicrodroidDeviceTestBase {
    ...
    public void testVsockTransferFromHostToVM() throws Exception {
        ...
        reportMetrics(transferRates, "vsock/transfer_host_to_vm", "_mb_per_sec");
    }
    ...
    private static class VsockVmEventListener extends VmEventListener {
        ...
        public void onPayloadReady(VirtualMachine vm) {
            try {
                ...
                new Thread(() -> {
                    try {
                        ParcelFileDescriptor parcelFd = vm.connectVsock(mPort);
                        int rawFd = parcelFd.getFd();
                        assertThat(rawFd).isGreaterThan(-1);
                        double transferRate =
                                IoVsockHostNative.sendDataFromHostToVM(
                                        rawFd, mPort);
                    } catch (Exception e) {
                        throw new RuntimeException(e);
                    }
                ...
                benchmarkService.recvData(1, mPort, serverFd);
            } catch (Exception e) {
                throw new RuntimeException(e);
            }
            forceStop(vm);
