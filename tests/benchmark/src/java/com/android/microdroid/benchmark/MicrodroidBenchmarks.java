...
public class MicrodroidBenchmarks extends MicrodroidDeviceTestBase {
    ...
    private boolean canBootMicrodroidWithMemory(int mem)
        ...
        for (int i = 0; i < trialCount; i++) {
            VirtualMachineConfig.Builder builder =
                    newVmConfigBuilder("assets/vm_config.json");
            VirtualMachineConfig normalConfig =
                    builder.debugLevel(DebugLevel.FULL).memoryMib(mem).build();
            VirtualMachine vm = createNewVirtualMachine("test_vm_minimum_memory", normalConfig);

            if (tryBootVm(TAG, vm).payloadStarted) return true;
        }
    ...
    public void testMicrodroidBootTime()
        ...
        for (int i = 0; i < trialCount; i++) {
            ...
            VirtualMachine vm = createNewVirtualMachine("test_vm_boot_time", normalConfig);

            BootResult result = tryBootVm(TAG, vm);
