...
Result<double> measure_send_rate(int fd, const int num_mb_to_send) {
    std::string data;
    data.assign(kNumBytesPerMB * num_mb_to_send, 'a');
    clock_t start = clock();
    ...
    double send_rate = num_mb_to_send / elapsed_seconds;
    return {send_rate};
}
...
Java_com_android_microdroid_benchmark_IoVsockHostNative_measureSendRate(__unused JNIEnv *env,
                                                                        ...
                                                                        int fd,
                                                                        int num_mb_to_send) {
    if (auto res = measure_send_rate(fd, num_mb_to_send); res.ok()) {
        return res.value();
    } else {
