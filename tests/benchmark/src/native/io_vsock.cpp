...
namespace io_vsock {
constexpr size_t kBlockSizeBytes = 4096;

android::base::Result<int> init_vsock_server(unsigned int port) {
    int server_fd(TEMP_FAILURE_RETRY(socket(AF_VSOCK, SOCK_STREAM, 0)));
    if (server_fd < 0) {
...
Result<double> run_vsock_server_and_receive_data(int server_fd) {
    ...
    LOG(INFO) << "VM:Connection from CID " << client_sa.svm_cid << " on port "
              << client_sa.svm_port;
    // std::string message = "VM is ready to receive data.", msg;
    // LOG(INFO) << "VM:" << message;
    // if (!ReadFdToString(client_fd, &msg)) {
    //     return Error() << "ReadFdToString";
    // }
    // LOG(INFO) << "VM:Received " << msg;
    
    // if (!WriteStringToFd(message, client_fd)) {
    ...
    double recv_time = ts.tv_sec + 1e-9*ts.tv_nsec;
    LOG(INFO) << "VM:Received '" << data << "'";
    LOG(INFO) << "VM:Received data at '" << recv_time << "'";
    return {recv_time};
}
} // namespace io_vsock