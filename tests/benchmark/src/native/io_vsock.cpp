...
Result<void> run_vsock_server_and_receive_data(int server_fd, int num_mb_to_receive) {
    ...
    if (data.length() != num_mb_to_receive * kNumBytesPerMB) {
        return Error() << "Received data length(" << data.length() << ") is not equal to "
                       << num_mb_to_receive * kNumBytesPerMB;
    }
