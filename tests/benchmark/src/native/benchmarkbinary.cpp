...
public:
    ...
    ndk::ScopedAStatus runVsockServerAndReceiveData(int32_t server_fd, double* out) override {
        if (auto res = io_vsock::run_vsock_server_and_receive_data(server_fd); res.ok()) {
            *out = res.value();
        } else {
            std::stringstream error;
            error << "Cannot run vsock server: " << res.error();
            return ndk::ScopedAStatus::fromExceptionCodeWithMessage(EX_ILLEGAL_ARGUMENT,
                                                                    error.str().c_str());
        }
        return ndk::ScopedAStatus::ok();
