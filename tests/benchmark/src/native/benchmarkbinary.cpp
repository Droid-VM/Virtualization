...
Result<void> run_virtio_blk_benchmark_tests() {
    ...
    public:
        ...
        Result<void> run_tests() {
            ...
            // const char* filename = "/dev/block/vda";
            ...
            for (int i = 0; i < n; ++i) {
                if (auto res = read_file(fd, block_count, /*is_rand=*/false); res.ok()) {
                    elapsed_seconds += res.value();
                } else {
                    return res.error();
                }
            }
            _seq_read_rate = mb / elapsed_seconds * n;

            // Test random read.
            elapsed_seconds = 0.;
            for (int i = 0; i < n; ++i) {
                if (auto res = read_file(fd, block_count, /*is_rand=*/true); res.ok()) {
                    elapsed_seconds += res.value();
                } else {
                    return res.error();
                }
            }
            _rand_read_rate = mb / elapsed_seconds * n;
            return {};
        }
