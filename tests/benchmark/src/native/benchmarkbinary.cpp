...
Result<void> run_io_benchmark_tests() {
    class IOBenchmarkService
          : public aidl::com::android::microdroid::testservice::BnBenchmarkService {
    public:
        ndk::ScopedAStatus readFile(const std::string& filename, int64_t fileSize, bool isRand,
                                    double* out) override {
            if (auto res = read_file(filename, fileSize, isRand); res.ok()) {
                *out = res.value();
            } else {
    ...
    private:
        ...
        Result<double> read_file(const std::string& filename, int64_t fileSize, bool is_rand) {
            const int64_t block_count = fileSize / kBlockSize;
            std::vector<uint64_t> offsets;
            if (is_rand) {
                std::mt19937 rd{std::random_device{}()};
                offsets.reserve(block_count);
                for (auto i = 0; i < block_count; ++i) offsets.push_back(i * kBlockSize);
