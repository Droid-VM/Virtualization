...
Result<void> run_io_benchmark_tests() {
    ...
    public:
        ...
        Result<void> run_tests() {
            if (auto res = init_block_count(); !res.ok()) {
                return res.error();
            }
            if (auto res = run_read_test(/*is_rand=*/false); !res.ok()) {
                return res.error();
            }
            if (auto res = run_read_test(/*is_rand=*/true); !res.ok()) {
                return res.error();
            }
            return {};
        }
    ...
    private:
        ...
        Result<void> run_read_test(bool is_rand) {
        ...
        Result<double> read_file(bool is_rand) {
            ...
            unique_fd fd(open(_filename, O_RDONLY));
            if (fd.get() == -1) {
                return ErrnoError() << "Read: opening " << _filename << " failed";
            }
        ...
        void flush_cache() {
            unique_fd fd(open("/proc/sys/vm/drop_caches", O_WRONLY));
            write(fd.get(), "3", sizeof(char)); // Clear PageCache, dentries and inodes.
        }
