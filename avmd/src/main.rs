...
use apexutil::{get_payload_vbmeta_image_hash, to_hex_string};
...
/// Iterate over a set of argument values, that could be empty, and come in
/// (<namespace>, <name>, <file>) triplets due to value_names() implicitly calling
/// number_of_values(3).
    ...
    for descriptor in avmd.descriptors() {
        match descriptor {
            Descriptor::VbMeta(data) => {
                ...
                println!("    vbmeta digest:         {}", to_hex_string(&data.vbmeta_digest));
            }
            Descriptor::Apk(data) => {
                ...
                println!("    APK digest:            {}", to_hex_string(&data.apk_digest));
            }
        }
    }
    Ok(())
...
mod tests {
    ...
    fn test_create_command() {
        let avmdtool = Command::new("avmdtool").arg("create");
    }
}
