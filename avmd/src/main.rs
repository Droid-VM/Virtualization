...
fn get_vbmeta_digest(file: &str) -> Result<Vec<u8>> {
    ...
    if img.descriptors()?.iter().any(|a| matches!(a, vbmeta::Descriptor::ChainPartition(_))) {
        bail!("Haven't figured out how to handle AVB chaining yet");
    }
    Ok(sha512(img.data()).to_vec())
}
