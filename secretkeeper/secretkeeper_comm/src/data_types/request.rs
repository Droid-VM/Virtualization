...
pub trait Request {
    ...
    /// the args' type is adhered to the the HAL spec.
    ///
    /// # Arguments
    ...
    fn new(args: Vec<Value>) -> Result<Box<Self>, InternalError>;
    ...
    fn deserialize_from_packet(packet: RequestPacket) -> Result<Box<Self>, InternalError> {
        let mut req = packet.get_content();
        if req.is_empty() {
            return Err(InternalError::RequestMalformed);
        }
        let num: u16 = value_to_integer(&req[0])?.try_into()?;
        let opcode_packet = Opcode::from_u16(num).ok_or(InternalError::RequestMalformed)?;
