...
/// Set of all possible Opcode supported by Secretkeeper HAL.
...
/// Corresponds to GetVersionRequestPacket.
...
impl Request for GetVersionRequest {
    ...
    fn init(opcode: Opcode, args: Vec<Value>) -> Result<Box<Self>, InternalError> {
        if opcode != Opcode::GetVersion || !args.is_empty() {
...
impl Response for GetVersionResponse {
    fn init(res: Vec<Value>) -> Result<Box<Self>, InternalError> {
        if res.len() != 2 {
