...
mod tests {
    ...
    fn request_serialization_deserialization() {
        ...
        assert_eq!(packet.get_opcode().unwrap(), Opcode::GetVersion);
        assert_eq!(
            RequestPacket::deserialize_from_bytes(&packet.clone().serialize_to_bytes().unwrap())
                .unwrap(),
            packet
        );
    ...
    fn success_response_serialization_deserialization() {
        ...
        assert_eq!(
            ResponsePacket::deserialize_from_bytes(&packet.clone().serialize_to_bytes().unwrap())
                .unwrap(),
            packet
        );
