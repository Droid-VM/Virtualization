...
impl DicePolicy {
    ...
    ///    ];
    ///
    /// 2. For hypothetical (and highly simplified) dice chain: [ROT_KEY, [{1 : 'a', 2 : {200 : 5,
    ///    201 : 'b'}}]] The following can be used constraint_spec =[
...
mod tests {
    ...
    impl TestArtifacts {
        ...
        fn get_example() -> Self {
            ...
            let input_dice = get_dice_chain_helper(
                rot_key.clone(),
                EXAMPLE_NUM_1,
                EXAMPLE_STRING,
            ...
            let updated_input_dice = get_dice_chain_helper(
                rot_key.clone(),
                EXAMPLE_NUM_2,
                EXAMPLE_STRING,
    ...
    fn policy_matches_original_dice_chain() {
        let example = TestArtifacts::get_example();
        let policy_res = DicePolicy::from_dice_chain(&example.input_dice, &example.constraint_spec);
        assert!(policy_res.is_ok(), "Failed to construct DicePolicy");
        assert!(
    ...
    fn policy_matches_updated_dice_chain() {
        ...
        assert!(policy_res.is_ok(), "Failed to construct DicePolicy");
        assert!(
            policy_res.unwrap().matches_dice_chain(&example.updated_input_dice).is_ok(),
    ...
    fn policy_mismatch_downgraded_dice_chain() {
        ...
        assert!(policy_res.is_ok(), "Failed to construct DicePolicy");
        assert!(
            policy_res.unwrap().matches_dice_chain(&example.input_dice).is_err(),
