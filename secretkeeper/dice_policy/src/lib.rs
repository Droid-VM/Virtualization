    ...
    impl TestExample {
        fn init() -> Result<Self> {
            ...
            let constraint_spec = vec![
                ConstraintSpec::new(ConstraintType::ExactMatch, vec![1])?,
                ConstraintSpec::new(ConstraintType::GreaterOrEqual, vec![3, 100])?,
            ];
            ...
            Ok(TestExample { input_dice, constraint_spec, expected_dice_policy })
        }
    }

    test!(policy_structure_check);
    fn policy_structure_check() {
        let example = TestExample::init().unwrap();
        let policy =
