...
fn main() -> Result<(), Error> {
    ...
    let virtualization_service = get_interface(VIRTUALIZATION_SERVICE_BINDER_SERVICE_IDENTIFIER)
        .context("Failed to find VirtualizationService")?;

    match opt {
        Opt::Run { config, daemonize } => command_run(virtualization_service, &config, daemonize),
        Opt::Stop { cid } => command_stop(virtualization_service, cid),
        Opt::List => command_list(virtualization_service),
    }
}

/// Retrieve reference to a previously daemonized VM and stop it.
fn command_stop(
    virtualization_service: Strong<dyn IVirtualizationService>,
    cid: u32,
) -> Result<(), Error> {
...
fn command_list(virtualization_service: Strong<dyn IVirtualizationService>) -> Result<(), Error> {
    let vms = virtualization_service.debugListVms().context("Failed to get list of VMs")?;
