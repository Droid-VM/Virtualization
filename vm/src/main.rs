...
use binder::{BinderFeatures, Interface, ProcessState, Strong};
...
fn main() -> Result<(), Error> {
    ...
    let vm_services = match opt {
        ...
        Opt::RunApp { ref config } => config.common.vm_services,
        Opt::RunMicrodroid { ref config } => config.common.vm_services,
        Opt::Run { ref config } => config.common.vm_services,
    };
    ...
    let vm = match opt {
        ...
        opt => bail!("Unhandled option"),
    }?;

    if !vm_services.is_empty() {
        for service in vm_services {
            let accessor = Accessor::new(&vm, &service.service_name, service.port);
            let accessor_binder = BnAccessor::new_binder(accessor, BinderFeatures::default());

            // TODO: Handle error?
            binder::register_lazy_service(&service.service_name, accessor.as_binder()).unwrap();
        }
