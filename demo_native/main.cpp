...
// APK file's content
Result<ScopedFileDescriptor> create_or_update_idsig_file(
        std::shared_ptr<IVirtualizationService> service, const std::string& work_dir,
...
// it's own identity information in an encrypted form.
Result<ScopedFileDescriptor> create_instance_image_file_if_needed(
        std::shared_ptr<IVirtualizationService> service, const std::string& work_dir) {
    ...
    ScopedAStatus ret = service->initializeWritablePartition(instance, size,
...
// Construct VirtualMachineAppConfig for a VM named `vm_name` that executes a shared library named
// `paylaod_binary_name` in the apk `main_apk_path`.
Result<VirtualMachineAppConfig> create_vm_config(std::shared_ptr<IVirtualizationService> service,
                                                 const std::string& work_dir,
                                                 const std::string& vm_name,
                                                 const std::string& main_apk_path,
                                                 const std::string& payload_binary_name,
                                                 bool debuggable, bool protected_vm,
                                                 int32_t memory_mib) {
...
Result<std::shared_ptr<IVirtualMachine>> create_virtual_machine(
        std::shared_ptr<IVirtualizationService> service, VirtualMachineAppConfig& app_config) {
    std::shared_ptr<IVirtualMachine> vm;

    VirtualMachineConfig config = std::move(app_config);
    ScopedFileDescriptor console_fd(dup(fileno(stdout)));
    ScopedFileDescriptor log_fd(dup(fileno(stdout)));
...
// When a VM lifecyle changes, a corresponding method in this class is called. This also provides
...
public:
    Callback(std::shared_ptr<IVirtualMachine> vm) : mVm(vm) {}
    ...
    void wait_for_payload_ready() {
        std::unique_lock lock(mMutex);
        mCv.wait(lock, [this] {
            auto state = get_vm_state();
            return state.ok() && *state == VirtualMachineState::READY;
        });
    }

    void wait_for_vm_death() {
        std::unique_lock lock(mMutex);
        mCv.wait(lock, [this] {
            auto state = get_vm_state();
            return state.ok() && *state == VirtualMachineState::DEAD;
        });
    }
...
Result<void> do_something(std::shared_ptr<ITestService> payload) {
    int32_t result;
