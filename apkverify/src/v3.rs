...
use crate::bytes_ext::{BytesExt, LengthPrefixed, ReadBytes};
use crate::sigutil::{
    find_signature, is_supported_signature_algorithm, rank_signature_algorithm, SignatureInfo,
};
...
// TODO(jooyung) add derive macro: #[derive(ReadBytes)]
...
pub fn verify<P: AsRef<Path>>(path: P) -> Result<()> {
    ...
    verify_signature(&mut signature.signature_block.slice(..))?;
    Ok(())
}

/// Verifies the contents of the provided APK file against the provided APK Signature Scheme v3
/// Block.
fn verify_signature(block: &mut Bytes) -> Result<()> {
