...
impl<R: Read + Seek> ApkSections<R> {
    ...
    fn contents(&mut self) -> Result<DataSource<'_, R>> {
        DataSource::from_read(&mut self.inner, 0, self.signing_block_offset)
    }
    fn central_directory(&mut self) -> Result<DataSource<'_, R>> {
        DataSource::from_read(
            &mut self.inner,
            self.central_directory_offset,
            self.central_directory_size,
        )
    }
    fn eocd_for_verification(&mut self) -> Result<DataSource<'_, R>> {
        let mut eocd = self.bytes(self.eocd_offset, self.eocd_size)?;
        // use "the offset of siging block" as "the offset of central directory"
        set_central_directory_offset(&mut eocd, self.signing_block_offset)?;
        DataSource::from_vec(eocd)
    }
...
enum DataSource<'a, R: Read + Seek> {
    FromRead(Take<&'a mut R>),
    FromVec(Take<Cursor<Vec<u8>>>),
}

impl<'a, R: Read + Seek> DataSource<'a, R> {
    fn from_read(inner: &'a mut R, offset: u32, size: u32) -> Result<DataSource<'a, R>> {
        inner.seek(SeekFrom::Start(offset as u64))?;
        Ok(Self::FromRead(inner.take(size as u64)))
    }
    fn from_vec(inner: Vec<u8>) -> Result<DataSource<'a, R>> {
        let sz = inner.len() as u64;
        Ok(Self::FromVec(Read::take(Cursor::new(inner), sz)))
    }
    fn limit(&self) -> u64 {
        match self {
            DataSource::FromRead(inner) => inner.limit(),
            DataSource::FromVec(inner) => inner.limit(),
        }
    }
}

impl<'a, R: Read + Seek> Read for DataSource<'a, R> {
    fn read(&mut self, buf: &mut [u8]) -> std::io::Result<usize> {
        match self {
            DataSource::FromRead(inner) => inner.read(buf),
            DataSource::FromVec(inner) => inner.read(buf),
        }
    }
}
