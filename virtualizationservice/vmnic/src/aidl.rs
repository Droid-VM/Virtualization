...
use libc::{close, c_int, c_short, ifreq, socket, AF_INET, IFF_NO_PI, IFF_TAP, IFF_UP, IFNAMSIZ, SOCK_DGRAM};
use log::info;
...
fn bring_up_interface(sockfd: c_int, ifname: &[u8]) -> Result<()> {
    // SAFETY: All-zero is a valid value for the ifreq type.
    let mut ifr: ifreq = unsafe { std::mem::zeroed() };
    ifr.ifr_name[..ifname.len()].copy_from_slice(ifname);
    // SAFETY: `ioctl` is copied into the kernel. It modifies the state in the kernel, not the
...
impl IVmnic for Vmnic {
    fn createTapInterface(&self, iface_name_suffix: &str) -> binder::Result<ParcelFileDescriptor> {
        ...
        if sockfd < 0 {
            return Err(anyhow!("Failed to create socket")).or_service_specific_exception(-1)?;
        }
        let result = bring_up_interface(sockfd, null_terminated_ifname);
        // SAFETY: This is just closing file descriptor opened right above.
        unsafe { close(sockfd) };
        if result.is_err() {
            result
