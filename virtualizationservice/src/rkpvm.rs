...
use service_vm_comm::{
    GenerateCertificateRequestParams, Request, RequestCertificateParams, Response,
};
use service_vm_manager::ServiceVm;

pub(crate) fn request_attestation(csr: &[u8], client_vm_cid: i32) -> Result<Vec<u8>> {
    let mut vm = ServiceVm::start()?;

    let attestation_key = get_attestation_key(client_vm_cid)?;
    let params = RequestCertificateParams {
        csr: csr.to_vec(),
        remotely_provisioned_key_blob: attestation_key.keyBlob,
        remotely_provisioned_cert_chain: attestation_key.encodedCertChain,
    };
    let request = Request::RequestCertificate(params);
