...
/// Create the `GptPartitionEntry` and one or more `ComponentDisk` proto messages for the given
/// partition.
fn create_partition(
    partition: &PartitionInfo,
    offset: u64,
    header_path: &str,
) -> Result<(GptPartitionEntry, Vec<ComponentDisk>), Error> {
...
pub fn create_composite_disk(
    ...
    for partition in partitions {
        let (gpt_entry, component_disks) =
            create_partition(partition, next_disk_offset, &header_path)?;
        gpt_entry.write_bytes(&mut writer)?;
        for component_disk in component_disks {
