...
use virtualizationmaintenance::IVirtualizationMaintenance::{
    AppsForUser::AppsForUser, BnVirtualizationMaintenance, IVirtualizationMaintenance,
};
...
impl IVirtualizationMaintenance for VirtualizationMaintenanceService {
    ...
    fn currentAppList(&self, _current_user_apps: &[AppsForUser]) -> binder::Result<()> {
        Err(anyhow!("currentAppList not supported"))
            .or_binder_exception(ExceptionCode::UNSUPPORTED_OPERATION)
    }
}
