...
use crate::aidl::TEMPORARY_DIRECTORY;
...
fn vm_instance(service: &dyn IVirtualizationService) -> Result<VmInstance> {
    ...
    VmInstance::create(service, &config, None, None, None, None)
        .context("Failed to create service VM")
}

fn instance_img(service: &dyn IVirtualizationService) -> Result<ParcelFileDescriptor> {
    let instance_img_path = instance_img_path();
...
pub fn instance_img_path() -> PathBuf {
    format!("{TEMPORARY_DIRECTORY}/{INSTANCE_IMG_NAME}").into()
}
