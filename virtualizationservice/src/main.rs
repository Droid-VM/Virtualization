...
fn main() {
    ...
    let virtualization_service = VirtualizationService::default();
    let virtualization_service = BnVirtualizationService::new_binder(
        virtualization_service,
        BinderFeatures { set_requesting_sid: true, ..BinderFeatures::default() },
    );
    add_service(BINDER_SERVICE_IDENTIFIER, virtualization_service.as_binder()).unwrap();
