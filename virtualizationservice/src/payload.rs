...
impl PackageManager {
    ...
    fn get_apex_list(&self, prefer_staged: bool) -> Result<ApexInfoList> {
        ...
        if prefer_staged {
            let pm = get_interface::<dyn IPackageManagerNative>(PACKAGE_MANAGER_NATIVE_SERVICE);
            match pm {
                Ok(pm) => {
                ...
                Err(err) => {
                    // Even when "package_native" is not ready, keep running with active apexes
                    error!("prefer_staged is ignored because package_native is not ready: {}", err);
                }
            }
        }
        Ok(list)
