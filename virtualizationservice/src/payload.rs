...
fn get_path(package_name: &str) -> Result<PathBuf, Error> {
...
fn make_size_filler(size: u64, filler_path: &Path) -> Result<bool, Error> {
...
fn make_zero_filler(size: u64, filler_path: &Path) -> Result<bool, Error> {
...
fn make_no_filler(_size: u64, _filler_path: &Path) -> Result<bool, Error> {
    Ok(false)
}
...
pub fn make_disk_image(
    ...
    let mut partitions = vec![compositediskconfig::Partition {
