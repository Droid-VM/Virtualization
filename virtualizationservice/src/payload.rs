...
impl ApexInfoList {
    ...
    fn load() -> Result<&'static ApexInfoList> {
        ...
        INSTANCE.get_or_try_init(|| {
            ...
            // For active APEXes, we refer env variables to see if it conributes to classpath
    ...
    fn collect(&self, predicate: fn(&ApexInfo) -> bool) -> Vec<String> {
...
fn collect_apex_names(apex_list: &ApexInfoList, apexes: &[ApexConfig]) -> Vec<String> {
    ...
    let mut apex_names: Vec<String> = apexes
        ...
        .flat_map(|apex| match apex.name.as_str() {
            "{BOOTCLASSPATH}" => apex_list.collect(|apex| apex.boot_classpath),
            "{DEX2OATBOOTCLASSPATH}" => apex_list.collect(|apex| apex.boot_classpath),
            "{SYSTEMSERVERCLASSPATH}" => apex_list.collect(|apex| apex.boot_classpath),
