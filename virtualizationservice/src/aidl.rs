...
fn assemble_disk_image(
    ...
    if disk.qcow2 {
        let qcow2_image_filename =
            make_qcow2_image_filename(temporary_directory, next_temporary_image_id);
        let qcow2_image =
...
fn make_qcow2_image_filename(
    temporary_directory: &Path,
    next_temporary_image_id: &mut u64,
) -> PathBuf {
    let id = *next_temporary_image_id;
    *next_temporary_image_id += 1;
    temporary_directory.join(format!("qcow2-{}.qcow2", id))
}
