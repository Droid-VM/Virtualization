...
impl GlobalVmInstance {
    fn get_temp_dir(&self) -> PathBuf {
        format!("{}/{}", TEMPORARY_DIRECTORY, self.cid).into()
    }
}
...
impl GlobalState {
    fn held_cids(&self) -> Vec<Arc<GlobalVmInstance>> {
        // Attempt to upgrade the weak pointers to strong pointers.
        self.held_cids.iter().map(|(_, inst)| inst).filter_map(Weak::upgrade).collect()
    }
    ...
    fn allocate_vm_context(
        ...
        self.held_cids.insert(cid, Arc::downgrade(&instance));
