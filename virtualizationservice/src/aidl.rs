...
impl IVirtualizationServiceInternal for VirtualizationServiceInternal {
    ...
    fn enableTestAttestation(&self) -> binder::Result<()> {
        ...
        // Wait until the service VM shuts down to ensure that the key blob generated in the
        // current Service VM session can be properly decrypted in the next session.
