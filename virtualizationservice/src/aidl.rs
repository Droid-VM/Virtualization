...
impl Interface for VirtualizationServiceInternal {
    pub fn init() -> VirtualizationServiceGlobal {
        let service = VirtualizationServiceGlobal::default();
...
impl IVirtualizationServiceInternal for VirtualizationServiceInternal {
    fn setRlimitMemlock(&self) -> binder::Result<bool> {
        const SZ_1G: libc::rlim_t = 1024 * 1024 * 1024;

        let caller = get_calling_pid();
        let new_limit = libc::rlimit { rlim_cur: SZ_1G, rlim_max: SZ_1G };
...
impl VirtualMachineCallbacks {
    ...
    /// Call all registered callbacks to notify that the payload has finished.
    pub fn notify_stdio(&self, cid: Cid, fd: ParcelFileDescriptor) {
        let callbacks = &*self.0.lock().unwrap();
        for callback in callbacks {
            if let Err(e) = callback.onStdioReady(cid as i32, &fd) {
                error!("Error notifying payload finish event from VM CID {}: {:?}", cid, e);
            }
        }
    }
...
impl GlobalState {
    ...
    fn find_available_cid(&mut self) -> Result<Cid> {
        ...
        loop {
            ...
            if !cids.iter().any(|inst| inst.cid == cid) {
                break;
            }
            if cid == last_cid {
                bail!("Could not find an available CID"); // this must be a bug
            }
        }
...
impl IVirtualMachineService for VirtualMachineService {
    ...
    fn notifyStdioServerReady(&self, port: i32) -> binder::Result<()> {
        let cid = self.cid;
        if let Some(vm) = self.state.lock().unwrap().get_vm(cid) {
            info!("VM having CID {} encountered a stdio", cid);
            ...
            vm.callbacks.notify_stdio(cid, vsock_stream_to_pfd(stream));
            Ok(())
        } else {
            error!("notifyCurrentStatus is called from an unknown CID {}", cid);
            Err(Status::new_service_specific_error_str(
