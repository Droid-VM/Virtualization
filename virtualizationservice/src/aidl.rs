...
impl VirtualizationService {
    ...
    fn create_vm_internal(
        ...
        let state = &mut *self.state.lock().unwrap();
        let console_fd = if let Some(fd) = console_fd {
            Some(clone_file(fd)?)
        } else {
            prepare_logger_fd(config, format!("Console({})", cid))?
        };

        let log_fd = if let Some(fd) = log_fd {
            Some(clone_file(fd)?)
        } else {
            prepare_logger_fd(config, format!("Log({})", cid))?
        };
...
fn prepare_logger_fd(config: &VirtualMachineConfig, tag: String) -> Result<Option<File>, Status> {
