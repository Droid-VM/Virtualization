...
const fn is_valid_guest_cid(cid: &Cid) -> bool {
    (GUEST_CID_MIN..=GUEST_CID_MAX).contains(cid)
}

const fn next_guest_cid(cid: Cid) -> Cid {
...
impl GlobalState {
    ...
    fn allocate_cid(&mut self) -> Result<Arc<Cid>> {
        ...
        let first_cid = next_guest_cid(last_cid)?;
...
fn handle_stream_connection_tombstoned() -> Result<()> {
    ...
    assert!(!is_valid_guest_cid(VM_TOMBSTONES_SERVICE_PORT));
    let listener =
