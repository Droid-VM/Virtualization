...
fn load_app_config(
    ...
    let storage_backing_file =
        config.encryptedStorageImage.as_ref().map(|file| clone_file(file).unwrap());
    ...
    add_microdroid_system_images(config, instance_file, storage_backing_file, &mut vm_config)?;

    // Microdroid payload image
