...
fn handle_stream_connection_from_vm(state: Arc<Mutex<State>>) -> Result<()> {
    ...
    for stream in listener.incoming() {
        ...
        if let Ok(SockAddr::Vsock(addr)) = stream.peer_addr() {
            ...
            if state.get_vm(cid).is_some() {
                state.add_stream(cid, stream);
            } else {
...
struct State {
    ...
    /// THe map of streams for input/output of payload.
    streams: HashMap<Cid, VsockStream>,
...
impl State {
    ...
    fn add_stream(&mut self, cid: Cid, stream: VsockStream) {
        self.streams.insert(cid, stream);
    }

    fn get_stream(&mut self, cid: Cid) -> Option<VsockStream> {
...
impl IVirtualMachineService for VirtualMachineService {
    fn notifyPayloadStarted(&self, cid: i32) -> binder::Result<()> {
        let cid = cid as Cid;
        let state = &mut *self.state.lock().unwrap();
        if let Some(vm) = state.get_vm(cid) {
            info!("VM having CID {} started payload", cid);
            vm.callbacks.notify_payload_started(cid, state.get_stream(cid));
            Ok(())
        } else {
