...
fn create_vm_directory(client_uid: Uid, cid: Cid) -> Result<PathBuf> {
    let path: PathBuf = format!("{}/{}@{}", TEMPORARY_DIRECTORY, client_uid, cid).into();
    chown_and_remove_dir(&path, Uid::current()).unwrap_or_else(|e| {
        warn!("Could not delete temporary directory {:?}: {}", path, e);
    });
    create_and_chown_dir(&path, client_uid)
        .with_context(|| format!("Could not create temporary directory {:?}", path))?;
    Ok(path)
}

/// Creates a directory and makes it accessible only to the given UID.
fn create_and_chown_dir(path: &PathBuf, uid: Uid) -> Result<(), Error> {
    create_dir(path)?;
...
/// Removes a directory owned by a different user by first changing its owner to the given UID.
pub fn chown_and_remove_dir(path: &PathBuf, uid: Uid) -> Result<(), Error> {
    // Change owner to the give UID and permissions to owner RWX.
    chown(path, Some(uid), None)?;
    set_permissions(path, Permissions::from_mode(0o700))?;
    ...
    for dir_entry in read_dir(path)? {
        ...
        if dir_entry.file_type()?.is_dir() {
            chown_and_remove_dir(&dir_entry.path(), uid)?;
        } else {
