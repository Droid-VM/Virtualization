...
impl IVirtualizationServiceInternal for VirtualizationServiceInternal {
    ...
    fn requestAttestation(
        ...
        if cfg!(remote_attestation) {
            ...
            let mut certificate_chain = parse_certificate_chain(&attestation_key.encodedCertChain)
...
fn parse_certificate_chain(cert_chain: &[u8]) -> Result<Vec<Certificate>> {
    let mut out = Vec::new();
    let mut remaining = cert_chain;
    while !remaining.is_empty() {
        let (to_read, _) = X509Certificate::from_der(remaining)?;
        let end = remaining.len() - to_read.len();
        out.push(Certificate { encodedCertificate: remaining[..end].to_vec() });
        remaining = to_read;
    }
    Ok(out)
}
...
mod tests {
    ...
    #[test]
    fn parse_certificate_chain_succeeds() -> Result<()> {
        let bytes = fs::read(TEST_RKP_CERT_CHAIN_PATH)?;
        let cert_chain = parse_certificate_chain(&bytes)?;
