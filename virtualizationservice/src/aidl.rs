...
/// The smallest CID for guest VMs
const VMADDR_CID_FIRST_GUEST: u32 = 10;
...
impl VirtualMachineCallbacks {
    ...
    pub fn notify_payload_started(&self, cid: Cid, stream: VsockStream) {
        ...
        // Consume the stream if there is no external consumer, otherwise the payload in the VM
        // might be blocked on stdio.
        if callbacks.is_empty() {
            let mut buf = [0; 1024];
            loop {
                match pfd.as_ref().read(&mut buf) {
                    Ok(0) => break,
                    _ => continue,
                };
            }
