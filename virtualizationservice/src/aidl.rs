...
impl IVirtualizationServiceInternal for VirtualizationServiceInternal {
    ...
    fn getAssignableDevices(&self) -> binder::Result<Vec<AssignableDevice>> {
        ...
        let devices: Devices = serde_xml_rs::from_str(&xml)
            .context("can't parse assignable_device.xml")
            .with_log()
            .or_service_specific_exception(-1)?;
        ...
        for device in devices.device.into_iter() {
            if device_set.contains(&device.node) {
                warn!("duplicated assignable device {device:?}; ignoring...")
            } else if Path::new(&device.node).exists() {
                device_set.insert(device.node.clone());
                ret.push(AssignableDevice { kind: device.kind, node: device.node });
            } else {
