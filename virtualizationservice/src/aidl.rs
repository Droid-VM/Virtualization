...
fn create_vm_internal(
    ...
    let config = match config {
        VirtualMachineConfig::AppConfig(config) => {
            BorrowedOrOwned::Owned(load_app_config(config, &temporary_directory).map_err(|e| {
...
impl IVirtualizationService for VirtualizationService {
    ...
    fn createVm(
        ...
        let ret = create_vm_internal(state, config, console_fd, log_fd, &mut is_protected);
        match ret {
            Ok(_) => {
