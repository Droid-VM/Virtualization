...
impl VmIdDb {
    ...
    pub fn new(db_dir: &str) -> Result<(Self, bool)> {
        ...
        let mut result = Self {
            conn: Connection::open_with_flags(db_path, flags)
                .context("failed to open/create DB with {flags:?}")?,
        };
    ...
    /// Remove the given VM IDs from the database.
    pub fn delete_vm_ids(&mut self, mut vm_ids: &[&VmId]) -> Result<()> {
        while !vm_ids.is_empty() {
            let len = std::cmp::min(vm_ids.len(), MAX_VARIABLES);
            let batch = &vm_ids[..len];

            let mut vars = "?,".repeat(batch.len());
            ...
            let _rows = stmt.execute(params_from_iter(batch)).context("failed to delete VM IDs")?;
