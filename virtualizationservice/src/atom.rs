...
fn get_main_apk_path(config: &VirtualMachineAppConfig) -> Result<String> {
    let apk = config.apk.as_ref().ok_or_else(|| anyhow!("APK is none"))?;
    let raw_fd = apk.as_raw_fd();
    let link = read_link(format!("/proc/self/fd/{}", raw_fd))?;
    let path = link.into_os_string().into_string().unwrap_or_default();
    Ok(path)
}
...
pub fn write_vm_creation_stats(
    ...
    match config {
        VirtualMachineConfig::AppConfig(config) => {
            ...
            main_apk = get_main_apk_path(config).unwrap_or_else(|_| "INFO: APK is none".into());
            if let Ok(vm_payload_config) = vm_payload_config {
                extra_apks = vm_payload_config
                    .extra_apks
                    .iter()
                    .map(|x| x.path.clone())
                    .collect::<Vec<String>>()
                ...
                extra_apks = "INFO: Can't get VmPayloadConfig".into();
...
pub fn write_vm_booted_stats(name: &String) {
    let vm_booted = vm_booted::VmBooted { vm_name: name };
...
pub fn write_vm_exited_stats(name: &String, reason: DeathReason) {
