...
/// Print arguments of the crosvm command. In doing so, /proc/self/fd/XX is replaced with the path
/// to the actual file if the FD is backed by a regular file. If not, the /proc path is printed
/// unmodified.
            ...
            .map(|s| {
                re.replace_all(&s, |caps: &Captures| {
                    ...
                    if let Ok(realpath) = std::fs::canonicalize(path) {
                        format!("{}(fd:{})", realpath.to_string_lossy(), fd)
                    } else {
                        path.to_owned()
                    }
                })
