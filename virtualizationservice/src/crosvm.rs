        ...
        loop {
            {
                ...
                if let Ok(guest_time) = get_guest_time(pid) {
                    vm_metric.cpu_guest_time = match vm_metric.cpu_guest_time {
                        Some(x) => Some(max(x, guest_time)),
                        None => Some(guest_time),
                    }
                } else {
                ...
                // TODO : get maximum value, not latest
                if let Ok(rss) = get_rss(pid) {
                    vm_metric.rss = Some(rss);

                    vm_metric.rss = match vm_metric.rss {
                        Some(x) => {
                            Some(Rss { vm: max(x.vm, rss.vm), crosvm: max(x.crosvm, rss.crosvm) })
                        }
                        None => Some(rss),
                    }
                } else {
