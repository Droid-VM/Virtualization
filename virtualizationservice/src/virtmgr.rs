...
fn register_pdeathsig() -> Result<(), Error> {
    ...
    // SAFETY - Only constants are passed to the C call.
    let ret = unsafe { libc::prctl(libc::PR_SET_PDEATHSIG, libc::SIGTERM) };
    if ret < 0 {
        bail!("prctl: {}", std::io::Error::last_os_error());
    }

    // Check if the parent died during prctl().
...
fn wait_for_pdeathsig() -> Result<(), Error> {
...
fn main() {
    ...
    register_pdeathsig().expect("Could not register parent death notification");
    ...
    server.start();
    wait_for_pdeathsig().expect("Failed to wait for parent death notification");
