...
fn parse_fd_arg(arg: &str) -> Result<OwnedFd, clap::Error> {
    ...
    nix::fcntl::fcntl(raw_fd, nix::fcntl::F_GETFD).map_err(|e| {
        clap::Error::with_description(
            format!("Invalid file descriptor {raw_fd}: {e}"),
            clap::ErrorKind::InvalidValue,
        )
    })?;
    ...
    if owned_fds.contains(&raw_fd) {
        return Err(clap::Error::with_description(
            format!("File descriptor {raw_fd} already owned"),
            clap::ErrorKind::InvalidValue,
        ));
    }
...
fn main() {
    ...
    let _ = File::from(args.shutdown_fd).read(&mut [0]);
