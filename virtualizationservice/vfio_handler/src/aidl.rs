...
fn get_dtbo_img_path() -> binder::Result<PathBuf> {
    ...
    let Some(slot_suffix) = slot_suffix.as_deref() else {
        return Err(anyhow!("slot_suffix is none"))
            .or_binder_exception(ExceptionCode::SERVICE_SPECIFIC);
    };
...
fn get_dt_table_header(file: &mut File) -> binder::Result<DtTableHeader> {
    file.seek(SeekFrom::Start(0))
        .context("Cannot seek the offset of dt_table_header")
        .or_service_specific_exception(-1)?;
    ...
    for value in &mut values {
        ...
        file.read_exact(&mut buffer)
            .context("Failed to read dt_table_header")
            .or_service_specific_exception(-1)?;
        // DtTableHeader uses big endian.
        // See system/libufdt/utils/src/dt_table.c
    ...
    if dt_table_header._magic != DT_TABLE_MAGIC {
        return Err(anyhow!("Invalid dt_table magic"))
            .or_binder_exception(ExceptionCode::SERVICE_SPECIFIC);
    }
    Ok(dt_table_header)
}
...
fn get_dt_table_entry(
    ...
    if index > header._dt_entry_count {
        return Err(anyhow!("Invalid dtbo index"))
            .or_binder_exception(ExceptionCode::SERVICE_SPECIFIC);
    }
    let dt_entry_offset = header._dt_entries_offset + header._dt_entry_size * index;
    file.seek(SeekFrom::Start(dt_entry_offset.into()))
    ...
    for value in &mut values {
        let mut buffer = [0; size_of::<u32>()];
        file.read_exact(&mut buffer)
            .context("Failed to read dt_table_entry")
            .or_service_specific_exception(-1)?;
        // DtTableEntry uses big endian.
        // See system/libufdt/utils/src/mkdtimg_core.c
        *value = u32::from_be_bytes(buffer);
    }
...
fn filter_dtbo_from_img(
    ...
    buffer.resize(entry._dt_size.try_into().unwrap(), 0);
    dtbo_img_file
