...
struct DtTableEntry {
    /// size of each DT
    _dt_size: U32<BigEndian>,
    /// offset from head of dt_table_header
    _dt_offset: U32<BigEndian>,
...
fn read_values(file: &mut File, size: u64, offset: u64) -> binder::Result<Vec<u8>> {
...
fn read_u32_values(file: &mut File, size_u32: u64, offset: u64) -> binder::Result<Vec<u8>> {
    let Some(rate) = size_of::<u32>().checked_div(size_of::<u8>()) else {
    ...
    let rate = rate
        ...
        .context("Failed to convert type")
        .or_binder_exception(ExceptionCode::ILLEGAL_STATE)?;
    let Some(size_u8) = size_u32.checked_mul(rate) else {
        return Err(anyhow!("Unexpected arithmetic result"))
            .or_binder_exception(ExceptionCode::ILLEGAL_STATE);
    };
    read_values(file, size_u8, offset)
}

fn get_dt_table_header(file: &mut File) -> binder::Result<DtTableHeader> {
    let values = read_u32_values(file, 8, 0)?;
    let dt_table_header = DtTableHeader::read_from(values.as_slice())
        .context("Failed to convert type into DtTableHeader")
        .or_service_specific_exception(-1)?;
    if dt_table_header._magic.get() != DT_TABLE_MAGIC {
...
fn get_dt_table_entry(
    ...
    if index >= header._dt_entry_count.get() {
        return Err(anyhow!("Invalid dtbo index")).or_service_specific_exception(-1)?;
    }
    let Some(prev_dt_entry_total_size) = header._dt_entry_size.get().checked_mul(index) else {
        return Err(anyhow!("Unexpected arithmetic result"))
            .or_binder_exception(ExceptionCode::ILLEGAL_STATE);
    };
    let Some(dt_entry_offset) = prev_dt_entry_total_size.checked_add(header._dt_entries_offset.get())
    else {
        return Err(anyhow!("Unexpected arithmetic result"))
            .or_binder_exception(ExceptionCode::ILLEGAL_STATE);
    };
    let values = read_u32_values(file, 8, dt_entry_offset.into())?;
    let dt_table_entry = DtTableEntry::read_from(values.as_slice())
        .context("Failed to convert type into DtTableEntry")
        .or_service_specific_exception(-1)?;
    Ok(dt_table_entry)
}
...
fn filter_dtbo_from_img(
    ...
    let buffer =
        read_values(dtbo_img_file, entry._dt_size.get().into(), entry._dt_offset.get().into())?;
