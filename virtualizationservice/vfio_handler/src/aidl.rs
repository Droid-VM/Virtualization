...
impl IVfioHandler for VfioHandler {
    fn bindDeviceToVfioDriver(&self, device: &str) -> binder::Result<()> {
        ...
        bind_device(Path::new(device))
    }

    fn unbindDeviceFromVfioDriver(&self, device: &str) -> binder::Result<()> {
        // permission check is already done by IVirtualizationServiceInternal.
        if !*IS_VFIO_SUPPORTED {
            return Err(anyhow!("VFIO-platform not supported"))
                .or_binder_exception(ExceptionCode::UNSUPPORTED_OPERATION);
        }
        unbind_device(Path::new(device))
    }
