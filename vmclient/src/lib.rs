...
fn start(
    ...
    let state = Arc::new(VmState::default());
...
impl VmInstance {
    ...
    /// why it died.
    pub fn wait_for_completion(&self) -> DeathReason {
        self.state.wait()
    }
}
...
fn wait_for_death(binder: &mut impl IBinder, state: Arc<VmState>) -> BinderResult<DeathRecipient> {
...
impl VmState {
    pub fn wait(&self) -> DeathReason {
        self.death_ready
            .wait_while(self.death_reason.lock().unwrap(), |death_reason| death_reason.is_none())
            .unwrap()
            .unwrap()
    }

    fn notify_death(&self, reason: DeathReason) {
        let death_reason = &mut *self.death_reason.lock().unwrap();
        // In case this method is called more than once, ignore subsequent calls.
        if death_reason.is_none() {
            death_reason.replace(reason);
            self.death_ready.notify_all();
        }
    }
}
