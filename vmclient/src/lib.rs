...
use log::warn;
use rpcbinder::{get_preconnected_rpc_interface, get_unix_domain_bootstrap_rpc_interface};
...
pub fn connect() -> Result<Strong<dyn IVirtualizationService>, StatusCode> {
    let (wait_fd, ready_fd) = create_pipe()?;
    ...
    get_unix_domain_bootstrap_rpc_interface(client_fd)
}
