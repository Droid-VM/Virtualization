...
//! AuthFsServce facilitates authfs mounting (which is a privilegd operation) for the client. The
//! client will provide an `AuthFsConfig` which includes the backend address (cid, port) and the
//! filesystem configuration. It is up to the client to ensure the backend server is running. On a
//! successful mount, the client receives an `IAuthFs`, and through the binder object, the client is
//! able to retrieve "remote file descriptors".
...
impl IAuthFsService for AuthFsService {
    ...
    fn mount(&self, config: &AuthFsConfig) -> binder::Result<Strong<dyn IAuthFs>> {
        ...
        let mountpoint = get_mount_point(config.cid, config.port);
...
impl AuthFsService {
    fn new_binder(debuggable: bool) -> Strong<dyn IAuthFsService> {
        BnAuthFsService::new_binder(AuthFsService { debuggable }, BinderFeatures::default())
    }
...
fn get_mount_point(cid: i32, port: i32) -> OsString {
    OsString::from(format!("{}/{}_{}", SERVICE_ROOT, cid, port))
}
