...
fn try_main() -> Result<()> {
    ...
    debug!("{} is starting as a rpc service.", AUTHFS_SERVICE_NAME);
    let retval = run_unix_domain_rpc_server(service, AUTHFS_SERVICE_NAME, || {
        info!("{} is ready", AUTHFS_SERVICE_NAME);
    });
    if retval {
        debug!("{} has shut down gracefully", AUTHFS_SERVICE_NAME);
        Ok(())
    } else {
        bail!("Premature termination of {}", AUTHFS_SERVICE_NAME)
    }
}
