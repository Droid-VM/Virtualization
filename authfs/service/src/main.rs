...
use authfs_aidl_interface::aidl::com::android::virt::fs::IAuthFsService::{
    BnAuthFsService, IAuthFsService, AUTHFS_SERVICE_NAME,
};
use binder::{self, BinderFeatures, ExceptionCode, Interface, ProcessState, Status, Strong};
...
fn try_main() -> Result<()> {
    ...
    if run_unix_domain_rpc_server(service, AUTHFS_SERVICE_NAME, || {
        info!("{} is ready", AUTHFS_SERVICE_NAME);
    }) {
        debug!("{} is running", AUTHFS_SERVICE_NAME);
        Ok(())
    } else {
        bail("Failed to start service {}", AUTHFS_SERVICE_NAME);
    }
}
