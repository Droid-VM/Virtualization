...
impl IAuthFs for AuthFs {
    fn openFile(&self, basename: i64, writable: bool) -> binder::Result<ParcelFileDescriptor> {
...
impl AuthFs {
    ...
    pub fn mount_and_wait(
        ...
        let cid = config.cid.try_into().context("CID for authfs backend")?;
        let child = run_authfs(
            cid,
            &mountpoint,
            &config.input_fd_annotations,
            &config.output_fd_annotations,
            debuggable,
        )?;
...
fn run_authfs(
    ...
    for conf in in_fds {
        ...
        args.push(OsString::from(format!("{}:{}:{}", conf.fd, conf.fd, conf.file_size)));
    }
    for conf in out_fds {
