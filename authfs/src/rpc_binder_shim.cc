...
AIBinder_Weak* RpcClient(unsigned int cid, unsigned int port) {
    auto session = RpcSession::make();
    session->setMaxThreads(8); // arbitrary
    if (!session->setupVsockClient(cid, port)) {
        LOG(ERROR) << "Failed to set up vsock client with CID " << cid << " and port " << port;
        return nullptr;
    }
    return AIBinder_Weak_new(AIBinder_fromPlatformBinder(session->getRootObject()));
}
