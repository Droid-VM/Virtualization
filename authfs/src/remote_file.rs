...
type VirtFdService = Box<dyn IVirtFdService::IVirtFdService>;

pub mod server {
    // TODO(victorhsieh): use remote binder.
    pub fn get_local_service() -> super::VirtFdService {
        let service_name = "virt_fd_server";
        binder::get_interface(&service_name).expect("Cannot reach virt_fd_server binder service")
    }
}
...
pub struct RemoteFsverityMerkleTreeReader {
    ...
    // TODO(victorhsieh): change to Sp<> once binder supports it.
    service: Arc<Mutex<VirtFdService>>,
    file_fd: i32,
}
