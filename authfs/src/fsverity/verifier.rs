...
mod tests {
    ...
    impl<M: ReadByChunk> MerkleTreeReader<M> {
        pub fn new(
            metadata: M,
            merkle_tree_index: u64,
        ) -> Result<MerkleTreeReader<M>, FsverityError> {
            Ok(MerkleTreeReader { metadata, merkle_tree_index })
        }
    }
    ...
    fn new_reader_with_fsverity(
        ...
        let metadata = parse_fsverity_metadata(&metadata_file)?;
        ...
        Ok((
            VerifiedFileReader::new(
                ...
                MerkleTreeReader::new(
                    LocalFileReader::new(metadata_file)?,
                    metadata.merkle_tree_index,
                )?,
            )?,
            file_size,
        ))
