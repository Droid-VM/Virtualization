...
fn verity_check<T: ReadByChunk>(
    ...
    fsverity_walk(chunk_index, file_size, merkle_tree)?.try_fold(
        ...
        |actual_hash, result| {
            ...
            let expected_hash = &merkle_chunk[hash_offset_in_chunk..hash_offset_in_chunk + 32];
...
fn fsverity_walk<T: ReadByChunk>(
    ...
    let hash_size = 32;
    let hashes_per_node = CHUNK_SIZE / hash_size as u64;
    ...
    let root_to_leaf_steps = (0..=max_level)
        ...
        .map(|x| {
            ...
            let offset_in_level = (chunk_index / leaves_per_hash) * hash_size as u64;
