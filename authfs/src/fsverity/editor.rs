...
impl<F: ReadOnlyDataByChunk + RandomWrite> VerifiedFileEditor<F> {
    ...
    fn new_hash_for_incomplete_write(
        ...
        // The buffer is initialized to 0 purposely. To calcuate the block hash, the data is
    ...
    fn write_at(&self, buf: &[u8], offset: u64) -> io::Result<usize> {
            ...
            ChunkedSizeIter::new(buf.len(), offset, CHUNK_SIZE as usize)
        {
            ...
            let new_hash = if current_size as u64 == CHUNK_SIZE {
                ...
                // any).
                let result = self.new_hash_for_incomplete_write(
                    source,
                    offset_from_alignment,
                    output_chunk_index,
                    &mut merkle_tree,
                );
                // Even if the hash is not consistent for the current chunk, we can still consider
                // the writes to the earlier chunks as a success.
                if result.is_err() {
