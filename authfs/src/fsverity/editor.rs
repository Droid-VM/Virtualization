...
impl<F: ReadByChunk + RandomWrite> VerifiedFileEditor<F> {
    ...
    fn read_backing_chunk(&self, chunk_index: u64, buf: &mut ChunkBuffer) -> io::Result<usize> {
    ...
    fn read_backing_chunk_verified(
        ...
        let size = self.read_backing_chunk(chunk_index, buf)?;
    ...
    fn new_hash_for_incomplete_write(
        ...
        if merkle_tree.is_index_valid(output_chunk_index) {
            self.read_backing_chunk(output_chunk_index as u64, &mut orig_data)?;
    ...
    fn resize(&self, size: u64) -> io::Result<()> {
        ...
        if size < merkle_tree.file_size() && size % CHUNK_SIZE > 0 {
            ...
            if new_tail_size > 0 {
                ...
                let s = self.read_backing_chunk(chunk_index, &mut buf)?;
