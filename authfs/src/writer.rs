    ...
    fn write_at(&self, buf: &[u8], offset: u64) -> io::Result<usize> {
            ...
            ChunkedSizeIter::new(buf.len(), offset, CHUNK_SIZE as usize)
        {
            ...
            let new_hash = if current_size as u64 == CHUNK_SIZE {
                ...
                if merkle_tree.is_index_valid(output_chunk_index as usize) {
                    self.read_chunk(output_chunk_index as u64, &mut orig_data)?;

                    // Verify original content
                    let hash = Sha256Hasher::new()?.update(&orig_data)?.finalize()?;
                    if !merkle_tree.is_consistent(output_chunk_index as usize, &hash) {
                        // Even if the hash is not consistent for the current chunk, we can still
                        // consider the writes to the earlier chunks as a success.
            ...
            merkle_tree.update_hash(output_chunk_index as usize, &new_hash, size_at_least);
        }
        Ok(buf.len())
    }
}
