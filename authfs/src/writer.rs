...
impl<F: ReadOnlyDataByChunk + RandomWrite> VerifiedFileEditor<F> {
    ...
    fn write_chunk(
        ...
        // hash. Nothing can be done within this editor (unless the caller later overrides the full
        // chunk, if it happens to still hold the data), but at least it still maintains the
        // (original) integrity for the file.
...
impl<F: ReadOnlyDataByChunk + RandomWrite> RandomWrite for VerifiedFileEditor<F> {
    fn write_at(&self, buf: &[u8], offset: u64) -> io::Result<usize> {
        ...
        {
            let result = self.write_chunk(buf, offset, output_offset, current_size);

            // Return early when any error happens during the writes. Even if the hash is not
