...
pub trait RandomWrite {
    /// Writes `buf` to the destination at `offset`. Returns the written size.
...
/// VerifiedFileRW provides an integrity layer to an underlying read-writable file, which may not be
/// stored in a trusted environment. Only new, empty file is currently supported.
...
impl<F: ReadOnlyDataByChunk + RandomWrite> VerifiedFileRW<F> {
    ...
    /// Calulates the fs-verity digest of the current file.
