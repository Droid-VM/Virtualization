...
//! location will propogate the hash update from the leaf to the root node. Unlike fs-verity, which
//! assumes static files, to support write operation, we need to allow the file (thus tree) to
//! update.
//!
//! Another difference to fs-verity is that the storage is not assumed to be trusted. For the
//! trusted world to generate a large file with random write and hash it, the writer needs to hold
//! some private information and update the Merkle tree during a file write (or even when the Merkle
//! tree needs to be stashed to the untrusted storage).
//!
...
//!
//! Rollback attack is another possible attack, but can be addressed with something like RPBM.

use std::cmp::{max, min};
use std::io;
use std::sync::{Arc, RwLock};

use crate::common::{ChunkedSizeIter, COMMON_PAGE_SIZE as CHUNK_SIZE};
...
pub struct VerifiedFileRW<F: ReadOnlyDataByChunk + RandomWrite> {
    file: F,
    file_size: Arc<RwLock<u64>>,
    merkle_tree: MerkleLeaves,
}

#[allow(dead_code)]
impl<F: ReadOnlyDataByChunk + RandomWrite> VerifiedFileRW<F> {
    pub fn new(file: F) -> Self {
        Self { file, file_size: Arc::new(RwLock::new(0)), merkle_tree: MerkleLeaves::new() }
    }

    pub fn calculate_fsverity_digest(&self) -> io::Result<Sha256Hash> {
        self.merkle_tree
            .calculate_fsverity_digest(*self.file_size.read().unwrap())
            .map_err(|e| io::Error::new(io::ErrorKind::Other, e))
    }

    fn read_full_chunk_unless_eof(
        &self,
        index: u64,
        buf: &mut [u8; CHUNK_SIZE as usize],
    ) -> io::Result<()> {
        let file_size = *self.file_size.read().unwrap();
        let mut remaining = min(buf.len(), file_size.saturating_sub(index * CHUNK_SIZE) as usize);
        while remaining > 0 {
            let size = self.file.read_chunk(index as u64, &mut buf[..])?;
            remaining -= size;
        }
        Ok(())
    }
...
impl<F: ReadOnlyDataByChunk + RandomWrite> RandomWrite for VerifiedFileRW<F> {
    fn write_at(&self, buf: &[u8], offset: u64) -> io::Result<usize> {
        ...
        //  2. A write of incomplete chunk, possibly beyond the original EOF.
        //
        ...
        // Now iteratate on the input data, considering the alignment at the destination.
            ...
            ChunkedSizeIter::new(buf.len(), offset, CHUNK_SIZE as usize)
        {
            ...
            let output_chunk_index = output_offset / CHUNK_SIZE;
            let offset_from_alignment = (output_offset % CHUNK_SIZE) as usize;

            let new_hash = if current_size as u64 == CHUNK_SIZE {
                // Case 1: If the chunk is a complete one, just calculate the hash, regardless of
                // write location.
                Sha256Hasher::new()?.update(source)?.finalize()
            } else {
                // Case 2: For an incomplete, calculate the hash based on previous data (if any) and
                // zero padding (beyond EOF).

                // Original data may be beyond EOF. The buffer is initialized to 0 because
                // 1) if hole is created by the write, those bytes are 0.
                // 2) if the last file chunk is incomplete, the hash is 0-padded to chunk size.
                ...
                if self.merkle_tree.is_index_valid(output_chunk_index as usize) {
                    self.read_full_chunk_unless_eof(output_chunk_index as u64, &mut orig_data)?;

                    // Verify original content
                    let hash = Sha256Hasher::new()?.update(&orig_data)?.finalize()?;
                    if !self.merkle_tree.is_consistent(output_chunk_index as usize, &hash) {
            ...
            self.merkle_tree.update_hash(output_chunk_index as usize, &new_hash);
            self.file.write_all_at(&source, output_offset)?;

            let mut file_size = self.file_size.write().unwrap();
            *file_size = max(*file_size, offset + buf.len() as u64);
        }
        Ok(buf.len())
    }
}

impl<F: ReadOnlyDataByChunk + RandomWrite> ReadOnlyDataByChunk for VerifiedFileRW<F> {
...
#[cfg(test)]
mod tests {
    // Test data below can be geenrated by:
    ...
    impl InMemoryWriter {
        pub fn new() -> InMemoryWriter {
            InMemoryWriter { data: RefCell::new(Vec::new()) }
        }
    }
    ...
    fn test_verified_writer_no_write() -> Result<()> {
        ...
        let file = VerifiedFileRW::new(InMemoryWriter::new());
        assert_eq!(
