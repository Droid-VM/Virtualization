...
struct Args {
    ...
    /// directory is integrity-protected in the same way as --remote-new-verified-file. Can be
    /// multiple.
    ///
...
fn prepare_root_dir_entries(args: &Args) -> Result<HashMap<PathBuf, AuthFsEntry>> {
    ...
    for config in &args.remote_ro_file {
        root_entries.insert(
            PathBuf::from(config.remote_fd.to_string()),
    ...
    for remote_fd in &args.remote_ro_file_unverified {
        ...
        root_entries.insert(
            PathBuf::from(remote_fd.to_string()),
