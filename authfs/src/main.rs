...
struct Args {
    ...
    remote_new_rw_file: Vec<OptioRemoteRwFile>,
...
fn parse_remote_new_rw_file_option(option: &str) -> Result<OptioRemoteRwFile> {
    let strs: Vec<&str> = option.split(':').collect();
    if strs.len() != 2 {
        bail!("Invalid option: {}", option);
    }
    Ok(OptioRemoteRwFile {
