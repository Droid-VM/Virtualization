...
use binder::{AIBinder, FromIBinder};
use std::io;

use crate::common::CHUNK_SIZE;
use authfs_aidl_interface::aidl::com::android::virt::fs::IVirtFdService::IVirtFdService;
use authfs_aidl_interface::binder::{get_interface, SpIBinder, Strong};
...
fn get_rpc_binder(cid: u32) -> io::Result<VirtFdService> {
    // SAFETY: TODO
    let aibinder = unsafe { rpc_binder_bindgen::RpcClient(cid, RPC_SERVICE_PORT) } as *mut AIBinder;
    if let Some(raw_aibinder) = unsafe { SpIBinder::from_raw_experimental(aibinder) } {
        Ok(IVirtFdService::try_from(raw_aibinder).map_err(|e| {
            io::Error::new(
                io::ErrorKind::AddrNotAvailable,
