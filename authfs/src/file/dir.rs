...
impl RemoteDirEditor {
    ...
    pub fn create_file(
        ...
        self.entries.insert(basename.to_path_buf(), Entry { inode, is_dir: false });
        Ok(new_remote_file)
    }
    ...
    pub fn mkdir(&mut self, basename: &Path, inode: Inode) -> io::Result<RemoteDirEditor> {
        ...
        self.entries.insert(basename.to_path_buf(), Entry { inode, is_dir: true });
        Ok(new_remote_dir)
    }

    /// Deletes a file
    pub fn force_delete_file(&mut self, basename: &Path) -> io::Result<Inode> {
