...
/// `Attr` maintains the local truth for attribites (e.g. mode and type) while allowing setting the
/// remote attribute for the file descriptro.
pub struct Attr {
...
impl Attr {
    ...
    pub fn new_file_with_mode(service: VirtFdService, remote_fd: i32, mode: libc::mode_t) -> Attr {
        Attr { service, mode: Mode::from_bits_truncate(mode), remote_fd, is_dir: false }
    }

    pub fn new_dir_with_mode(service: VirtFdService, remote_fd: i32, mode: libc::mode_t) -> Attr {
