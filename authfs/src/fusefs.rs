...
struct AuthFs {
    ...
    /// Root directory entry table for path to `Inode` lookup. This needs to be `Sync` to be used
    /// in `fuse::worker::start_message_loop`.
    root_entries: Arc<Mutex<HashMap<PathBuf, Inode>>>,
...
impl AuthFs {
    pub fn new(root_entries_by_path: HashMap<PathBuf, AuthFsEntry>, max_write: u32) -> AuthFs {
        ...
        root_entries_by_path.into_iter().for_each(|(path_buf, entry)| {
            next_inode += 1; // checked_add overflow
            root_entries.insert(path_buf, next_inode);
            inode_table.insert(next_inode, entry);
        });

        AuthFs {
            inode_table: Arc::new(Mutex::new(inode_table)),
            root_entries: Arc::new(Mutex::new(root_entries)),
            max_write,
        }
    }
    ...
    /// handler returns.
    fn handle_inode<F, R>(&self, inode: &Inode, handler: F) -> io::Result<R>
    where
    ...
    /// Inserts a new inode and corresponding `AuthFsEntry` created by `creator` to the inode table,
    /// then returns the new inode number.
    fn insert_new_inode<F>(&self, inode: &Inode, creator: F) -> io::Result<Inode>
    where
        F: FnOnce(&mut AuthFsEntry) -> io::Result<(Inode, AuthFsEntry)>,
    {
        ...
        let (new_inode, new_file_config) = creator(&mut config)?;
...
cfg_if::cfg_if! {
    // On Android, only x86_64 uses unsigned 64 bits integer for nlink. Also, it's safe to cast
    // from usize since authfs limits directory entries to small-ish number.
    if #[cfg(target_arch = "x86_64")] {
        fn to_nlink(num: usize) -> libc::c_ulong { num as libc::c_ulong }
    } else {
        fn to_nlink(num: usize) -> libc::c_uint { num as libc::c_uint }
    }
}
...
fn create_dir_stat(
    ...
    st.st_ino = ino;
    // Until needed, let's just grant the owner access.
    const COMMON_DIR_MODE: u32 =
        libc::S_IFDIR | libc::S_IXUSR | libc::S_IXOTH | libc::S_IXGRP | libc::S_IRUSR;
    st.st_mode = match access_mode {
        AccessMode::ReadOnly => COMMON_DIR_MODE,
        AccessMode::ReadWrite => COMMON_DIR_MODE | libc::S_IWUSR,
    };

    // 2 extra for . and ..
    if file_number + 2 > u32::MAX as usize {
        return Err(io::Error::from_raw_os_error(libc::EMLINK));
    }
    st.st_nlink = to_nlink(file_number + 2);
...
impl FileSystem for AuthFs {
    ...
    fn lookup(&self, _ctx: Context, parent: Inode, name: &CStr) -> io::Result<Entry> {
        if parent == ROOT_INODE {
            let root_entries = self.root_entries.lock().unwrap();
            let inode = *root_entries
            ...
            let st = self.handle_inode(&inode, |config| match config {
                ...
                AuthFsEntry::VerifiedNewDirectory { dir } => {
                    create_dir_stat(inode, dir.number_of_entries(), AccessMode::ReadWrite)
                }
            })?;
            Ok(Entry {
                inode,
    ...
    fn mkdir(
        ...
        Ok(Entry {
            inode: new_inode,
            generation: 0,
            attr: create_dir_stat(new_inode, /* file_number */ 0, AccessMode::ReadWrite)?,
            entry_timeout: DEFAULT_METADATA_TIMEOUT,
            attr_timeout: DEFAULT_METADATA_TIMEOUT,
        })
    }
