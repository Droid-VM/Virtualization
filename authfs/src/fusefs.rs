...
impl AuthFs {
    ...
    ///
    /// The operation involes two updates: adding the name with a new allocated inode to the parent
    /// directory, and insert the new inode and the actual `AuthFsEntry` to the global inode table.
    ///
    /// `create_fn` recevies the parent directory, through which it can create the new entry at and
...
impl FileSystem for AuthFs {
    ...
    fn lookup(&self, _ctx: Context, parent: Inode, name: &CStr) -> io::Result<Entry> {
        ...
        let inode = self.handle_inode(&parent, |parent_entry| match parent_entry {
            AuthFsEntry::ReadonlyDirectory { dir } => Ok(dir
                .lookup_inode(cstr_to_path(name))
                .ok_or_else(|| io::Error::from_raw_os_error(libc::ENOENT))?),
