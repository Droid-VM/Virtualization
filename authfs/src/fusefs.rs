...
impl AuthFs {
    pub fn new(root_entries_by_path: HashMap<PathBuf, AuthFsEntry>, max_write: u32) -> AuthFs {
        ...
        AuthFs { inode_table: Arc::new(Mutex::new(inode_table)), root_entries, max_write }
    }
...
fn create_dir_stat(ino: libc::ino_t, file_number: u16) -> io::Result<libc::stat64> {
    ...
    debug_assert!(size_of_val(&st.st_nlink) > size_of_val(&file_number)); // won't overflow below
    st.st_nlink = (file_number + 2).into(); // for . and ..
