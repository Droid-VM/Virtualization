...
mod tests {
    ...
    fn test_reading_more_than_4kb_data<T: ReadOnlyDataByChunk>(reader: T) -> Result<()> {
        ...
        let last_index = (reader.size() + 4095) / 4096 - 1;
        assert_eq!(reader.read_chunk(last_index, &mut buf)?, (reader.size() % 4096) as usize);
        Ok(())
    }
    ...
    fn test_read_in_memory_data() -> Result<()> {
        test_reading_more_than_4kb_data(&include_bytes!("../testdata/test.apk")[..])
    }
    ...
    fn test_read_beyond_file_size() {
        let data = &include_bytes!("../testdata/test.apk")[..];
        let mut buf = [0u8; 4096];
        let last_index_plus_1 = (data.size() + 4095) / 4096;
        data.read_chunk(last_index_plus_1, &mut buf); // should panic
    }
}
