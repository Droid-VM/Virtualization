...
impl ReadOnlyDataByChunk for ChunkedFileReader {
    ...
    fn read_chunk(&self, chunk_index: u64, buf: &mut [u8]) -> Result<usize> {
        ...
        let (start, end) = chunk_index_to_range(self.len(), Self::CHUNK_SIZE, chunk_index)?;
...
impl ReadOnlyDataByChunk for ChunkedDataReader {
    ...
    fn read_chunk(&self, chunk_index: u64, buf: &mut [u8]) -> Result<usize> {
        debug_assert!(buf.len() as u64 >= Self::CHUNK_SIZE);
        let chunk = &self.data.chunks(Self::CHUNK_SIZE as usize).nth(chunk_index as usize).unwrap();
...
mod tests {
    ...
    fn test_reading_more_than_4kb_data<T: ReadOnlyDataByChunk>(reader: T) -> Result<()> {
        let mut buf = [0u8; 4096];
        assert_eq!(reader.read_chunk(0, &mut buf)?, 4096);
        assert!(reader.len() > 0);
        let last_index = (reader.len() + 4095) / 4096 - 1;
        assert_eq!(reader.read_chunk(last_index, &mut buf)?, (reader.len() % 4096) as usize);
        Ok(())
    }
    ...
    fn test_read_in_memory_data() -> Result<()> {
        test_reading_more_than_4kb_data(ChunkedDataReader::new(
            include_bytes!("../testdata/test.apk").to_vec(),
        )?)
    }
    ...
    fn test_read_in_memory_empty_data() {
        if let Ok(reader) = ChunkedDataReader::new(Vec::new()) {
            let mut buf = [0u8; 4096];
            reader.read_chunk(0, &mut buf);
        }
    }
    ...
    fn test_read_beyond_file_size() {
        if let Ok(reader) = ChunkedDataReader::new(include_bytes!("../testdata/test.apk").to_vec())
        {
            let mut buf = [0u8; 4096];
            let last_index_plus_1 = (reader.len() + 4095) / 4096;
            reader.read_chunk(last_index_plus_1, &mut buf); // should panic
        }
    }
}
