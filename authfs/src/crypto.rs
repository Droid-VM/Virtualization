...
impl Sha256Hasher {
    pub fn new() -> Result<Sha256Hasher, CryptoError> {
        // unsafe because of an uninitialized instance, plus the FFI that operates on its raw
        // pointer.
        unsafe {
    ...
    pub fn update(&mut self, data: &[u8]) -> Result<&mut Self, CryptoError> {
        // unsafe because the FFI operates on the raw pointer of slice `data`.
        let retval = unsafe {
    ...
    pub fn finalize(&mut self) -> Result<[u8; SHA256_HASH_SIZE], CryptoError> {
        let mut md = [0u8; SHA256_HASH_SIZE];
        // unsafe because the FFI operates on the raw pointer of the Vec.
