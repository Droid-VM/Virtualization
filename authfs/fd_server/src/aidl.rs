...
pub struct FdService {
    /// A pool of opened files and directories.
...
impl FdService {
    ...
    /// Inserts a new FD and corresponding `FdConfig` created by `creator` to the FD pool, then
    /// returns the new FD number.
    fn insert_new_fd<F>(&self, fd: &i32, creator: F) -> BinderResult<i32>
    where
        F: FnOnce(&mut FdConfig) -> BinderResult<(i32, FdConfig)>,
    {
        ...
        let (new_fd, new_fd_config) = creator(&mut fd_config)?;
...
impl IVirtFdService for FdService {
    ...
    fn createFileInDirectory(&self, fd: i32, basename: &str) -> BinderResult<i32> {
        if basename.starts_with(MAIN_SEPARATOR) {
            return Err(new_errno_error(Errno::EINVAL));
        }
    ...
    fn mkdir(&self, dir_fd: i32, basename: &str) -> BinderResult<i32> {
        if basename.starts_with(MAIN_SEPARATOR) {
            return Err(new_errno_error(Errno::EINVAL));
        }
