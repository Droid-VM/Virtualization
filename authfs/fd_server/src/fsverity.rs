...
fn read_metadata(fd: i32, metadata_type: u64, offset: u64, buf: &mut [u8]) -> io::Result<usize> {
    ...
    Ok(unsafe { read_verity_metadata(fd, &mut arg) }.map_err(|e| {
        if let nix::Error::Sys(errno) = e {
            ...
            io::Error::new(io::ErrorKind::Other, "never reached") // never returned by ioctl_readwrite
        }
    })? as usize)
}

/// Read the raw Merkle tree from the fd (if exists). The API semantics is similar to a regular
