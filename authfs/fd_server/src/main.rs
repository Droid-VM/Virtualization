...
use binder::IBinder; // TODO(178852354): use exposed API and remove libbinder_rs dependency.
use log::{debug, error};

use authfs_aidl_interface::aidl::com::android::virt::fs::IVirtFdService::{
    BnVirtFdService, IVirtFdService, ERROR_FILE_NOT_FOUND, ERROR_IO,
};
...
impl FdService {
    ...
    fn get_file_config(&self, id: i32) -> BinderResult<&ReadonlyFdConfig> {
        self.fd_pool.get(&id).ok_or_else(|| Status::from(ERROR_FILE_NOT_FOUND))
    }
...
impl IVirtFdService for FdService {
    fn readFile(&self, id: i32, offset: i64, size: i32) -> BinderResult<Vec<u8>> {
        if size > Self::MAX_REQUESTING_DATA {
            return Err(new_binder_exception(
                ExceptionCode::ILLEGAL_ARGUMENT,
                "Unexpected large size",
            ));
        }
        let size: usize = size
            .try_into()
    ...
    fn readFsveritySignature(&self, id: i32) -> BinderResult<Vec<u8>> {
        if let Some(file) = &self.get_file_config(id)?.alt_signature_file {
            ...
            let size = Self::MAX_REQUESTING_DATA as usize;
...
fn main() -> Result<()> {
    ...
    add_service(SERVICE_NAME, service.as_binder()).unwrap_or_else(|e| {
