...
use binder::{public_api::Result as BinderResult, IBinder, Interface, Status};
use log::{debug, error};

use authfs_aidl_interface::aidl::com::android::server::virt::fs::IVirtFdService::{
    BnVirtFdService, IVirtFdService,
};

static SERVICE_NAME: &str = "virt_fd_server";
...
impl IVirtFdService for FdService {
    fn GetFileSize(&self, id: i32) -> BinderResult<i64> {
        if let Some(file_config) = self.fd_pool.get(&id) {
            let metadata = file_config.file.metadata().map_err(|e| {
                error!("GetFileSize: failed to get metadata of file {}: {}", id, e);
                Status::from(ErrorCode::Io)
            })?;
            Ok(metadata.len().try_into().map_err(|_| {
                error!("GetFileSize: invalid data");
                Status::from(ErrorCode::TryInto)
            })?)
        } else {
            Err(Status::from(ErrorCode::FileNotFound))
        }
    }

    fn ReadFile(&self, id: i32, offset: i64, size: i32) -> BinderResult<Vec<u8>> {
        if size > MAX_REQUESTING_DATA {
            return Err(Status::from(ErrorCode::InvalidArgument));
        }
        let size: usize = size.try_into().map_err(|_| {
            error!("ReadFile: invalid size: {}", size);
            Status::from(ErrorCode::TryInto)
        })?;
        let offset: u64 = offset.try_into().map_err(|_| {
            error!("ReadFile: invalid offset: {}", offset);
            Status::from(ErrorCode::TryInto)
        })?;

        Ok(read_exact_into_buf(self.get_file(id)?, size, offset).map_err(|e| {
    ...
    fn ReadFsverityMerkleTree(&self, id: i32, offset: i64, size: i32) -> BinderResult<Vec<u8>> {
        ...
        let size: usize = size.try_into().map_err(|_| {
            error!("ReadFsverityMerkleTree: invalid size: {}", size);
            Status::from(ErrorCode::TryInto)
        })?;
        let offset: u64 = offset.try_into().map_err(|_| {
            error!("ReadFsverityMerkleTree: invalid offset: {}", offset);
            Status::from(ErrorCode::TryInto)
        })?;

        if let Some(file) = &self.get_file_config(id)?.alt_merkle_file {
            Ok(read_exact_into_buf(&file, size, offset).map_err(|e| {
                error!("ReadFsverityMerkleTree: read error: {}", e);
                Status::from(ErrorCode::Io)
            })?)
        } else {
            // TODO(victorhsieh) retrieve from the fd when the new ioctl is ready
            Err(Status::from(ErrorCode::NotSupported))
        }
    }
    ...
    fn ReadFsveritySignature(&self, id: i32) -> BinderResult<Vec<u8>> {
        if let Some(file) = &self.get_file_config(id)?.alt_signature_file {
            ...
            Ok(read_exact_into_buf(&file, size, 0).map_err(|e| {
