...
enum FdConfig {
    ...
    /// associcated fs-verity metadata.
    ///
    /// Data:
    /// 1. The file to read from. fs-verity metadata can be retrieved from this file's FD.
    /// 2. Alternative Merkle tree stored in another file.
    /// 3. Alternative signature stored in another file.
    Readonly(File, Option<File>, Option<File>),
...
impl IVirtFdService for FdService {
    fn readFile(&self, id: i32, offset: i64, size: i32) -> BinderResult<Vec<u8>> {
        ...
        match self.get_file_config(id)? {
            FdConfig::Readonly(file, _, _) => read_into_buf(&file, size, offset).map_err(|e| {
                error!("readFile: read error: {}", e);
                Status::from(ERROR_IO)
            }),
            FdConfig::ReadWrite(file) => read_into_buf(&file, size, offset).map_err(|e| {
                error!("readFile: read error: {}", e);
                Status::from(ERROR_IO)
            }),
        }
    ...
    fn readFsverityMerkleTree(&self, id: i32, offset: i64, size: i32) -> BinderResult<Vec<u8>> {
        ...
        match &self.get_file_config(id)? {
            FdConfig::Readonly(_, alt_merkle_file, _) => {
                if let Some(file) = &alt_merkle_file {
    ...
    fn readFsveritySignature(&self, id: i32) -> BinderResult<Vec<u8>> {
        match &self.get_file_config(id)? {
            FdConfig::Readonly(_, _, alt_signature_file) => {
                if let Some(file) = &alt_signature_file {
    ...
    fn writeFile(&self, id: i32, buf: &[u8], offset: i64) -> BinderResult<i32> {
        match &self.get_file_config(id)? {
            FdConfig::Readonly(_, _, _) => Err(StatusCode::INVALID_OPERATION.into()),
...
fn parse_arg_ro_fds(arg: &str) -> Result<(i32, FdConfig)> {
    ...
    Ok((
        ...
        FdConfig::Readonly(
            ...
            fds.get(1).map(|fd| fd_to_file(*fd)).transpose()?,
            // Alternative signature, if provided
            fds.get(2).map(|fd| fd_to_file(*fd)).transpose()?,
        ),
    ))
}
