...
fn parse_values<F, T>(values: Option<Values<'_>>, opener: F) -> Result<Vec<(PseudoRawFd, T)>>
where
    F: Fn(&str) -> Result<T>,
{
    if let Some(options) = values {
        options
            ...
            .collect::<Result<Vec<(PseudoRawFd, T)>, Error>>()
    } else {
        Ok(Vec::new())
    }
}
...
fn parse_args() -> Result<Args> {
    ...
    let ro_files = parse_values(matches.values_of("open-ro"), |path| {
        OpenOptions::new().read(true).open(path).with_context(|| format!("Open {} read-only", path))
    })?;

    let rw_files = parse_values(matches.values_of("open-rw"), |path| {
        OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
    ...
    let dir_files = parse_values(matches.values_of("open-dir"), |path| {
