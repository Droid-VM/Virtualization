...
// with this alias is to improve readability by distuiguishing from actual RawFd.
...
fn as_fd_mapping<T: AsRawFd>(config: &FileMapping<T>) -> FdMapping {
    FdMapping { parent_fd: config.file.as_raw_fd(), child_fd: config.target_fd }
}
