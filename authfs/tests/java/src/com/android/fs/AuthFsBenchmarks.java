...
public class AuthFsBenchmarks extends BaseHostJUnit4Test {
    ...
    @Test
    public void seqReadWithFsverityVerification() throws Exception {
        ...
        reportMetrics(transferRates, "seq_read_with_fsverity", "mb_per_sec");
    }

    @Test
    public void seqReadWithoutFsverityVerification() throws Exception {
        List<Double> transferRates = new ArrayList<>(TRIAL_COUNT);
        for (int i = 0; i < TRIAL_COUNT; ++i) {
            mAuthFsTestRule.runFdServerOnAndroid("--open-ro 3:input.4m", "--ro-fds 3");
            mAuthFsTestRule.runAuthFsOnMicrodroid("--remote-ro-file-unverified 3");
            double elapsedSeconds = measureSeqReadOnMicrodroid("3");
            transferRates.add(INPUT_SIZE_IN_MB / elapsedSeconds);
        }
        reportMetrics(transferRates, "seq_read_without_fsverity", "mb_per_sec");
    }
