...
public class AuthFsBenchmarks extends BaseHostJUnit4Test {
    ...
    /** Name of the measure_read binary on host. */
    private static final String MEASURE_READ_BIN_NAME = "measure_read";

    /** Path to measure_read on Microdroid. */
    private static final String MEASURE_READ_BIN_PATH = "/data/local/tmp/measure_read";
    ...
    private void readRemoteFile(String readMode) throws DeviceNotAvailableException {
        pushMeasureReadBinToMicrodroid();
        ...
        String cmd =
                MEASURE_READ_BIN_PATH + " " + filePath + " " + INPUT_SIZE_IN_MB + " " + readMode;
        List<Double> readRates = new ArrayList<>(TRIAL_COUNT);
        for (int i = 0; i < TRIAL_COUNT + 1; ++i) {
            ...
            readRates.add(Double.parseDouble(readRate));
        }
        reportMetrics(readRates, readMode + "_read", "mb_per_sec");
    }

    private void pushMeasureReadBinToMicrodroid() throws DeviceNotAvailableException {
        File measureReadBin = mAuthFsTestRule.findTestFile(getBuild(), MEASURE_READ_BIN_NAME);
        assertThat(measureReadBin.exists()).isTrue();
        mAuthFsTestRule.getMicrodroidDevice().pushFile(measureReadBin, MEASURE_READ_BIN_PATH);
        assertThat(mAuthFsTestRule.getMicrodroid().run("ls " + MEASURE_READ_BIN_PATH))
                .isEqualTo(MEASURE_READ_BIN_PATH);
    }
