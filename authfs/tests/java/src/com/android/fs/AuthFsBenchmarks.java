    ...
    public void testReadWithFsverityVerification() throws Exception {
        ...
        for (int i = 0; i < TRIAL_COUNT; ++i) {
            ...
            mAuthFsTestRule.runAuthFsOnMicrodroid(
                    "--remote-ro-file 3:" + DIGEST_4M + " --cid " + VMADDR_CID_HOST);
            double elapsedSeconds = (System.nanoTime() - startTime) / NANO_SECS_PER_SEC;
            transferRates.add(INPUT_SIZE_IN_MB / elapsedSeconds);
        }
        reportMetrics(transferRates, "read_with_fsverity", "mb_per_sec");
    }

    @Test
    public void testReadWithoutFsverityVerification() throws Exception {
        List<Double> transferRates = new ArrayList<>(TRIAL_COUNT);
        for (int i = 0; i < TRIAL_COUNT; ++i) {
            double startTime = System.nanoTime();
            mAuthFsTestRule.runFdServerOnAndroid("--open-ro 3:input.4m", "--ro-fds 3");
            mAuthFsTestRule.runAuthFsOnMicrodroid(
                    "--remote-ro-file-unverified 3" + " --cid " + VMADDR_CID_HOST);
            double elapsedSeconds = (System.nanoTime() - startTime) / NANO_SECS_PER_SEC;
            transferRates.add(INPUT_SIZE_IN_MB / elapsedSeconds);
        }
        reportMetrics(transferRates, "read_without_fsverity", "mb_per_sec");
    }
        ...
        for (Map.Entry<String, Double> entry : stats.entrySet()) {
            Metric transferRate =
                    Metric.newBuilder()
                            ...
                            .build();
            mTestMetrics.addTestMetric(entry.getKey(), transferRate);
        }
    }
}
