...
public final class AuthFsHostTest extends VirtualizationTestCaseBase {
    ...
    private static final String MOUNT_DIR = "/data/local/tmp/authfs/mnt"; // TODO /data/local/tmp
    ...
    private static final int AUTHFS_INIT_TIMEOUT_MS = 5500;

    /** FUSE's magic from statfs(2) */
    private static final String FUSE_SUPER_MAGIC_HEX = "65735546";

    private static final long MICRODROID_ADB_CONNECT_TIMEOUT_MINUTES = 1;
    ...
    public void setUp() throws Exception {
        ...
        mCid = startMicrodroid(apkName, packageName, configPath);
        adbConnectToMicrodroid(mCid, MICRODROID_ADB_CONNECT_TIMEOUT_MINUTES);

        // Root because authfs currently require root to open /dev/fuse and mount the FUSE.
        rootMicrodroid();

        runOnMicrodroid("mkdir -p " + MOUNT_DIR);
    }
    ...
    public void tearDown() throws Exception {
        ...
        tryRunOnMicrodroid("rm -rf " + MOUNT_DIR);
    ...
    public void testReadWithFsverityVerification_TamperedMerkleTree()
        ...
        // Verify
        assertFalse(copyFileInMicrodroid(MOUNT_DIR + "/10", "/dev/null"));
    }
    ...
    public void testWriteThroughCorrectly()
        ...
        assertTrue(copyFileInMicrodroid(srcPath, destPath));
    ...
    private boolean copyFileInMicrodroid(String src, String dest)
            throws DeviceNotAvailableException {
    ...
    private void resizeFileOnMicrodroid(String path, long size) throws DeviceNotAvailableException {
    ...
    private void runAuthFsOnMicrodroid(String flags) throws DeviceNotAvailableException {
        String cmd = "/system/bin/authfs" + " " + MOUNT_DIR + " " + flags;
    ...
    private boolean isMicrodroidDirectoryOnFuse(String path) throws DeviceNotAvailableException {
        String fs_type = runOnMicrodroid("stat -f -c '%t' " + path);
