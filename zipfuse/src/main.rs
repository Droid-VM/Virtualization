...
use crate::inode::{DirectoryEntry, Inode, InodeData, InodeTable, InodeType};
...
pub fn run_fuse(zip_file: &Path, mount_point: &Path) -> Result<()> {
    ...
    const MAX_WRITE: u32 = 1 << 20; // This is a read-only filesystem
...
// `ZipFile` is in general not seekable due to the compression.
...
impl ZipFuse {
    ...
    fn stat_from(&self, inode: Inode) -> io::Result<libc::stat64> {
        ...
        st.st_gid = 0;
        st.st_rdev = 0;
        st.st_size = inode_data.size as i64;
        Ok(st)
    }
}
...
impl fuse::filesystem::FileSystem for ZipFuse {
    ...
    fn open(
        ...
        match open_files.get_mut(&handle) {
            Some(ofb) => {
                assert!(ofb.open_count >= 1);
                ofb.open_count += 1;
            }
            None => {
                let inode_data = self.find_inode(inode)?;
                ...
                open_files
                    .insert(handle, OpenFileBuf { open_count: 1, buf: buf.into_boxed_slice() });
            }
        }
        // TODO(jiyong): Should we return `OpenOptions::DIRECT_IO` here? Doing so seems to make
        // sense because we keep the buffer in memory. However, then any I/O on this file system
        // will require an IPC between the kernel and this FUSE process.
    ...
    fn next(&mut self) -> Option<fuse::filesystem::DirEntry> {
        ...
        Some(fuse::filesystem::DirEntry {
            ...
            type_: match entry.type_ {
                InodeType::Directory => libc::DT_DIR.into(),
                InodeType::File => libc::DT_REG.into(),
            },
            name,
        })
    }
...
mod tests {
    ...
    fn start_fuse(zip_path: &Path, mnt_path: &Path) {
        ...
        std::thread::spawn(move || {
            //assert!(crate::run_fuse(&zip_path, &mnt_path).is_ok());
    ...
    fn run_test(add: fn(&mut zip::ZipWriter<File>), check: fn(&std::path::Path)) {
        ...
        // TODO(jiyong): wait by monitoring if something is mounted at mnt_dir.
        std::thread::sleep(std::time::Duration::from_secs(2));

        // Run the check routine, and do the clean up.
        let mnt_path = test_dir.path().join("mnt");
        check(&mnt_path);
        assert!(nix::mount::umount2(&mnt_path, nix::mount::MntFlags::MNT_FORCE).is_ok());
    }
