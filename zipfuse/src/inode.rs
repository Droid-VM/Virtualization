...
const READ_MODE: u32 = if cfg!(release_avf_enable_multi_tenant_microdroid_vm) {
    libc::S_IRUSR | libc::S_IRGRP
} else {
    libc::S_IRUSR
};
const EXECUTE_MODE: u32 = if cfg!(release_avf_enable_multi_tenant_microdroid_vm) {
    libc::S_IXUSR | libc::S_IXGRP
} else {
    libc::S_IXUSR
};
