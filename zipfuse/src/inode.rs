...
impl InodeData {
    ...
    fn new_dir(mode: u32, size: u64) -> InodeData {
        InodeData { mode, size, data: InodeDataData::Directory(HashMap::new()) }
    }

    fn new_file(zip_index: usize, zip_file: &zip::read::ZipFile) -> InodeData {
        InodeData {
...
impl InodeTable {
    ...
    fn find(&self, parent: Inode, name: &OsStr) -> Option<Inode> {
        ...
        assert!(data.is_some());
        assert!(data.unwrap().is_dir());

        match data.unwrap().get_directory().unwrap().get(name) {
    ...
    fn add(&mut self, parent: Inode, name: &OsStr, data: InodeData) -> Inode {
        ...
        let type_ = if data.is_dir() { InodeType::Directory } else { InodeType::File };
        ...
        self.get_mut(parent)
            .unwrap()
            .add_to_directory(OsString::from(name), DirectoryEntry { inode, type_ });
        inode
    }
    ...
    pub fn from_zip<R: io::Read + io::Seek>(
        ...
        assert_eq!(INVALID, table.put(InodeData::new_dir(0, 0)));
        assert_eq!(ROOT, table.put(InodeData::new_dir(0, 0)));
        ...
        for i in 0..archive.len() {
            ...
            while let Some(name) = iter.next() {
                ...
                let inode = if is_file {
                    ...
                    InodeData::new_dir(file.unix_mode().unwrap_or(0), file.size())
                } else {
                    InodeData::new_dir(0, 0)
                };
