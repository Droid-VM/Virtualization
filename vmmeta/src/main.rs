...
use std::fmt::Write;
use vmmeta::{ApexDescriptor, ApkDescriptor, Descriptor, ResourceIdentifier, VbMetaDescriptor, VmMeta};
...
fn create(args: &ArgMatches) -> Result<()> {
    ...
    for (i, namespace, name, file) in NamespaceNameFileIterator::new(args, "apex-payload") {
        descriptors.insert(
            i,
            Descriptor::Apex(ApexDescriptor {
                resource: ResourceIdentifier {
                    namespace: namespace.to_string(),
                    name: name.to_string(),
                },
                root_digest: verify(file)?.root_digest,
            }),
        );
    }
...
fn dump(args: &ArgMatches) -> Result<()> {
    ...
    for descriptor in vmmeta.descriptors() {
        match descriptor {
            ...
            Descriptor::Apex(data) => {
                println!("  APEX descriptor:");
                println!("    namespace:             {}", data.resource.namespace);
                println!("    name:                  {}", data.resource.name);
                println!("    root digest:           {}", encode_hex(&data.root_digest))
            },
        }
    }
    Ok(())
...
*vmmetatool create \
   --vbmeta u-boot.bin \
   --vbmeta disk1/vbmeta.imb \
   --vbmeta micordoid/vbmeta.img \
   --apk compos.apk \
   --apk extra_apk.apk \
   --apex-payload art.apex
*/
fn main() -> Result<()> {
