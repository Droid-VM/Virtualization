...
show_help() {
	echo "Usage: $0 [OPTION]..."
	echo "Fetches a debian image from Placer and releases it to Lorry"
	echo "Options:"
	...
	echo "-d DIR        Directory where image is release. Defaults to BUILD_ID."
}

parse_opt() {
	while getopts "ha:b:d:" option; do
		case ${option} in
			h)
				show_help
				exit;;
...
do_release() {
	...
	local dir=${dir:-${build_id}}
	local serving_url=/android/ferrochrome/${arch}/${dir}/${image_filename}
