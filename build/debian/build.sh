...
package_custom_kernel() {
	...
	# NOTE: 6.1 is the latest LTS kernel for which the Debian build scripts still work on Python 3.10,
	#       which is installed on our Ubuntu 22.04 builders.
...
EOF
	...
	popd > /dev/null
	cp "linux-headers-${abi_kver}-${custom_flavour}-${debian_arch}_${debian_kver}_${debian_arch}.deb" \
	   "linux-image-${abi_kver}-${custom_flavour}-${debian_arch}-unsigned_${debian_kver}_${debian_arch}.deb" \
	   "${debian_cloud_image}/localdebs/"
	popd > /dev/null
	cat >> "${config_space}/package_config/AVF" <<EOF
linux-headers-${abi_kver}-${custom_flavour}-${debian_arch}
linux-image-${abi_kver}-${custom_flavour}-${debian_arch}-unsigned
EOF
}
