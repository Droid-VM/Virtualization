...
async fn send_active_ports_report(
    listening_ports: HashMap<i32, String>,
    ...
    let res = client
        .report_vm_active_ports(Request::new(ReportVmActivePortsRequest {
            ports: listening_ports
                .into_iter()
                .map(|(port, comm)| ActivePort { port, comm })
                .collect(),
        }))
        .await?
        .into_inner();
...
async fn report_active_ports(
    ...
    let mut listening_ports: HashMap<_, _> = listeners
        ...
        .map(|x| (x.socket.port().into(), x.process.name.to_string()))
    ...
    while let Some(record) = records.next().await {
        ...
        match row.newstate.as_str() {
            TCPSTATES_STATE_LISTEN => {
                listening_ports.insert(row.lport, row.c_comm);
            }
