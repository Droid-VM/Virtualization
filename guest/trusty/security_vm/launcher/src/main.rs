...
fn main() -> Result<()> {
    ...
    // If --load-kernel-as-bootloader option is present, then threat kernel as bootloader
    let (kernel, bootloader) =
        if !args.load_kernel_as_bootloader { (Some(kernel), None) } else { (None, Some(kernel)) };
