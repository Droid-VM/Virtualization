...
fn main() -> Result<()> {
    ...
    if args.cpu_topology != "one-cpu" && args.cpu_topology != "match-host" {
        args_cpu_topology = "one-cpu".to_owned(); 
    }

    let cpu_topology_config =
        if args.cpu_topology == "one-cpu" { CpuTopology::ONE_CPU } else { CpuTopology::MATCH_HOST };
