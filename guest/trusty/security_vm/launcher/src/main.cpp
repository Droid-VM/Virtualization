...
int main(int argc, char** argv) {
    ...
    AVirtualMachineRawConfig* config = AVirtualMachineRawConfig_create();
    AVirtualMachineRawConfig_setName(config, name.c_str());
    AVirtualMachineRawConfig_setKernel(config, kernel_fd);
    AVirtualMachineRawConfig_setProtectedVm(config, protected_vm);
    AVirtualMachineRawConfig_setMemoryMib(config, memory_size_mib);

    printf("creating VM with LLNDK\n");

    AVirtualMachine* vm;
    if (AVirtualMachine_createRaw(service, config,
                                  // console_in, console_out, and log will be redirected to the
                                  // kernel log by virtmgr
                                  -1, // console_in
                                  -1, // console_out
                                  -1, // log
                                  &vm) != 0) {
