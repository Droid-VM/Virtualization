...
fn check_csr(csr: Vec<u8>, vm_type: VmType) -> Result<()> {
    ...
    let build_type = system_properties::read("ro.build.type").unwrap_or("").unwrap_or("");
    session.set_allow_any_mode(non_protected_vm || (!build_type.is_empty && build_type != "user"));
