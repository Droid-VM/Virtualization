...
fn check_csr(csr: Vec<u8>, vm_type: VmType) -> Result<()> {
    ...
    // Allow any mode for (1) non-protected VMs because they use a fake DICE chain with the mode set
    // to debug, and (2) debuggable builds.
    session.set_allow_any_mode(
        vm_type == VmType::NonProtectedVm
            || system_properties::read_bool("ro.debuggable", false).unwrap_or(false),
    );
