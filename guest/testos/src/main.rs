...
use vmbase::memory::resize_available_memory;
...
fn main(arg0: u64, arg1: u64, arg2: u64, arg3: u64) {
    ...
    let fdt = Fdt::from_slice(fdt).expect("Fdt::From_slice failed");

    let memory_range = fdt.first_memory_range().expect("first_memory_range failed");
    debug!("memory_range: {memory_range:#x?}");
    resize_available_memory(&memory_range).unwrap_or_else(|e| {
        panic!("Failed to use memory range value from DT: {memory_range:#x?}: {e:?}");
    });

    let swiotlb_range = SwiotlbInfo::new_from_fdt(fdt)
        .expect("SwiotlbInfo::new_from_fdt failed")
        .and_then(|info| info.fixed_range());
    init_shared_pool(swiotlb_range).expect("Failed to initialize shared pool.");
    ...
    loop {
        ...
        match request {
            testos_protocol::Request::ReadRange(start, size) => {
                ...
                // SAFETY: `map_rodata` just made the range readable and verified the it hadn't
                ...
                vsock_stream.flush().expect("failed to flush response");
            }
