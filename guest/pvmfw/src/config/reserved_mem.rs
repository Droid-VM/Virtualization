...
pub fn parse_reserved_mem(
    ...
    for hdr in cfg_header.into_iter() {
        if hdr.vm_name == name {
            ...
            // SAFETY: hdr.blob_size is non-zero and ptr is checked for null after.
...
mod tests {
    ...
    fn update_fdt_match_all() {
        ...
        parse_reserved_mem(fdt, &bytes, 4096, c"Fry").unwrap();
        fdt.pack().unwrap();
    ...
    fn update_fdt_match_some() {
        ...
        parse_reserved_mem(fdt, &bytes, 4096, c"Leela").unwrap();
        fdt.pack().unwrap();
    ...
    fn update_fdt_match_none() {
        ...
        parse_reserved_mem(fdt, &bytes, 4096, c"Zoidberg").unwrap();
        fdt.pack().unwrap();
    ...
    fn missing_reserved_mem() {
        ...
        assert_eq!(parse_reserved_mem(fdt, &bytes, 4096, c"Fry"), Err(Error::MissingNode));
    }
    ...
    fn zero_size() {
        ...
        assert_eq!(parse_reserved_mem(fdt, &bytes, 4096, c"Fry"), Err(Error::ZeroSize));
    }
    ...
    fn bad_align() {
        ...
        assert_eq!(parse_reserved_mem(fdt, &bytes, 4095, c"Fry"), Err(Error::BadGuestAlign));
    }
}
