...
impl Entry {
    ...
    const ALL_ENTRIES: [Entry; Self::COUNT] =
        [Self::Bcc, Self::DebugPolicy, Self::VmDtbo, Self::VmBaseDtbo, Self::ReservedMem];
}
...
impl<'a> Config<'a> {
    ...
    pub fn get_entries(self) -> Entries<'a> {
        ...
        let [bcc, debug_policy, vm_dtbo, vm_ref_dt, reserved_mem] = entries;
        ...
        let reserved_mem = reserved_mem.map(|x| &*x);

        Entries { bcc, debug_policy, vm_dtbo, vm_ref_dt, reserved_mem }
    }
}
