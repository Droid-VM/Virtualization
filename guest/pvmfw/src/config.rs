...
impl Header {
    ...
    pub fn entry_count(&self) -> Result<usize> {
        let last_entry = match self.version {
            ...
            Self::VERSION_1_3 => Entry::ReservedMemConfig,
            v @ Version { major: 1, .. } => {
...
pub enum Entry {
    ...
    VmDtbo,
    VmBaseDtbo,
    /// Depthcharge simply refers to this as "ConfigEntry".
    ReservedMemConfig,
...
impl Entry {
    ...
    const ALL_ENTRIES: [Entry; Self::COUNT] =
        [Self::Bcc, Self::DebugPolicy, Self::VmDtbo, Self::VmBaseDtbo, Self::ReservedMemConfig];
}
...
#[derive(Debug)]
pub struct ReservedMemConfig<'a> {
    body: &'a mut [u8],
    ranges: [Option<NonEmptyRange>; Entry::COUNT],
}

impl<'a> ReservedMemConfig<'a> {
    ...
    pub fn get_entries(self) -> Entries<'a> {
        ...
        Entries { bcc, debug_policy, vm_dtbo, vm_ref_dt, config }
    }
}
