...
impl Bcc {
    ...
    pub fn new(received_bcc: Option<&[u8]>) -> Result<Bcc> {
        ...
        // Decode all the payloads to make sure they are well-formed.
        let payloads = bcc
            .into_iter()
            .skip(1)
        ...
        let is_debug_mode = is_any_entry_debug_mode(&payloads)?;
        // Safe to unwrap because we checked the length above.
        let leaf_subject_public_key_alg = payloads.last().unwrap().subject_public_key()?.alg()?;
...
fn is_any_entry_debug_mode(payloads: &[BccPayload]) -> Result<bool> {
...
impl BccPayload {
    ...
    fn subject_public_key(&self) -> Result<PublicKey> {
        ...
        self.value_from_key(SUBJECT_PUBLIC_KEY)
            ...
            .as_bytes()
            .map_or(Err(BccError::MalformedBcc("Subject public key is not a byte string")), |v| {
...
impl PublicKey {
    fn from_slice(slice: &[u8]) -> Result<Self> {
        Ok(Self(CoseKey::from_slice(slice)?))
    }

    fn alg(&self) -> Result<iana::Algorithm> {
        if let Some(Algorithm::Assigned(alg)) = self.0.alg {
            Ok(alg)
        } else {
            Err(BccError::MalformedBcc("Invalid algorithm in public key"))
        }
    }
}
