...
impl Bcc {
    ...
    pub fn new(received_bcc: Option<&[u8]>) -> Result<Bcc> {
        ...
        let leaf_public_key =
            entries.last().ok_or(BccError::MalformedBcc("No leaf entry"))?.subject_public_key()?;
        let Some(Algorithm::Assigned(leaf_subject_public_key_alg)) = leaf_public_key.alg else {
