...
impl PartialInputs {
    pub fn new(data: &VerifiedBootData) -> Result<Self> {
        ...
        let rkp_vm_marker = rkp_server_may_provision(data);
...
fn rkp_server_may_provision(data: &VerifiedBootData) -> bool {
    #[cfg(not(test))]
    use pvmfw_embedded_key::PUBLIC_KEY;
    #[cfg(test)]
    const PUBLIC_KEY: &[u8] = b"public key";

    if data.has_capability(Capability::TrustedWithRemoteKeys) {
        // This check is currently redundant with our rollback protection solution.
        // However, given that the RKP server will blindly trust the RKP VM DICE marker if found
        // in a DICE chain generated by a genuine pvmfw rooted in a known UDS, it doesn't hurt to
        // ensure that a future change relaxing RBP checks on the public key doesn't compromise the
        // RKP server.
...
mod tests {
    ...
    fn rkp_vm_conversion() {
        let vb_data = VerifiedBootData {
            ...
            capabilities: vec![Capability::TrustedWithRemoteKeys],
            ..BASE_VB_DATA
        };
        let inputs = PartialInputs::new(&vb_data).unwrap();
