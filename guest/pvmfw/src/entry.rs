...
// pub static ALLOC_POINTER: OnceBox<usize> = OnceBox::new();

pub const ALLOC_POINTER: usize = 0x8ce46000;
pub const KERNEL_START: u64 = 0x80000000;
pub const KERNEL_COUNT: u64 = 0x22a3;
pub const INITRD_START: u64 = 0x83000000;
pub const INITRD_COUNT: u64 = 0x206;
pub const FREE_START: u64 = 0x83206000;
pub const FREE_COUNT: u64 = 0x9c40;

use crate::uefi::CONFIGURATION_TABLE;
...
fn main_wrapper(
    ...
    // assert_eq!(KERNEL_START, slices.kernel.as_ptr() as _);
    // assert_eq!(KERNEL_COUNT, slices.kernel.len().div_ceil(SIZE_4KB));
    // assert_eq!(INITRD_START, initrd.as_ptr() as usize);
    // assert_eq!(INITRD_COUNT, initrd.len().div_ceil(SIZE_4KB));
    // assert_eq!(FREE_START, range_free_start);
    // assert_eq!(FREE_COUNT, range_free_size);
    ...
    // Find the start of region available for allocation after INITRD and before FDT.
    let alloc_region_start = r_free.end;
    let alloc_region_end = fdt;
    let alloc_range = alloc_region_start..alloc_region_end;
    let _alloc_r = MEMORY.lock().as_mut().unwrap().alloc_range_mut(&alloc_range).unwrap();

    // ALLOC_POINTER.set(Box::new(alloc_region_start)).unwrap();
...
fn jump_to_payload_with_efi_stub(payload_start: usize, payload_size: usize) {
    ...
    // // SAFETY: TODO(nikolinailic).
    // unsafe {
    //     info!("Configuration Table Contents:");
