...
pub fn start(fdt_address: u64, payload_start: u64, payload_size: u64, _arg3: u64) {
    ...
    let reboot_reason = match main_wrapper(fdt_address, payload_start, payload_size) {
        Err(r) => r,
        Ok((next_stage, slices, use_uefi)) => match next_stage {
            NextStage::LinuxBootWithUart(ep) => {
                if use_uefi {
                    let payload = slices.kernel.as_ptr() as usize;
                    jump_to_payload_with_efi_stub(payload, slices.kernel.len());
            ...
            NextStage::LinuxBoot(ep) => {
                if use_uefi {
                    ...
                    jump_to_payload_with_efi_stub(payload, slices.kernel.len());
                    RebootReason::InternalError
                } else if let Err(e) = unshare_uart() {
...
fn main_wrapper<'a>(
    ...
    payload: usize,
    payload_size: usize,
) -> Result<(NextStage, MemorySlices<'a>, bool), RebootReason> {
    ...
    let _ = vmbase::memory::map_data(r_free.start, r_free.len().try_into().unwrap());

    // Find the start of region in memory available for allocation after INITRD and before FDT.
    let alloc_region_start = r_free.end;
    let alloc_region_end = fdt;
    let alloc_range = alloc_region_start..alloc_region_end;
    let _ = vmbase::memory::map_data(alloc_range.start, alloc_range.len().try_into().unwrap());
