...
pub fn parse_reserved_mem(fdt: &mut Fdt, config: &[u8], name: &CStr) -> Result<(), Error> {
...
mod tests {
    ...
    fn make_header(
        ...
        RMemHeader { vm_name_offset, blob_offset, blob_size, compat_offset, flags: 0 }
    }
    ...
    fn update_fdt_match_all() {
        ...
        parse_reserved_mem(fdt, &bytes, c"Fry").unwrap();
        fdt.pack().unwrap();
    ...
    fn update_fdt_match_some() {
        ...
        parse_reserved_mem(fdt, &bytes, c"Leela").unwrap();
        fdt.pack().unwrap();
    ...
    fn update_fdt_match_none() {
        ...
        parse_reserved_mem(fdt, &bytes, c"Zoidberg").unwrap();
        fdt.pack().unwrap();
    ...
    fn missing_reserved_mem() {
        ...
        assert_eq!(parse_reserved_mem(fdt, &bytes, c"Fry"), Err(Error::MissingNode));
    }
}
