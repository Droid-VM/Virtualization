...
#[repr(C)]
#[derive(Debug, Eq, Immutable, KnownLayout, PartialEq)]
...
fn check_compat(buffer: &[u8], expected_len: usize) -> Result<(), Error> {
    ...
    let prev_compat =
        core::ffi::CStr::from_bytes_until_nul(buffer).map_err(|_| Error::InvalidCStr)?;
    // count_bytes does not include the nul, so add 1
    if prev_compat.count_bytes() + 1 != expected_len {
        Err(Error::InvalidCStr)
    } else {
        Ok(())
    }
}
...
impl<'a> Iterator for ConfigHeaderIterator<'a> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.next < self.hdr.hdrs.len() {
            ...
            self.next += 1;
            Some((
                &self.hdr.buffer[hdr.blob_offset()..hdr.blob_offset() + hdr.blob_size()],
                compat,
                &hdr.vm_uuid,
            ))
        } else {
...
pub fn parse_reserved_mem(fdt: &mut Fdt, config: &[u8], uuid: &[u8; 16]) -> Result<(), Error> {
    ...
    for (blob, compat, hdr_uuid) in cfg_header.into_iter() {
        if hdr_uuid == uuid {
            let mem_node = fdt
            ...
            node.appendprop_addrrange(c"reg", blob.as_ptr() as u64, blob.len() as u64)
                .map_err(|_| Error::AppendFailure)?;
        }
    }
    Ok(())
