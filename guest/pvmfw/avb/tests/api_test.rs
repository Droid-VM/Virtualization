...
fn payload_with_all_capabilities() -> Result<()> {
    ...
    assert!(verified_boot_data.has_capability(Capability::RemoteAttest));
    assert!(verified_boot_data.has_capability(Capability::SecretkeeperProtection));
    assert!(verified_boot_data.has_capability(Capability::UefiSupport));
    /// Verify that the number of supported capabilities is exactly 3 - this should not exceed the
    /// maximum number of capabilites.
