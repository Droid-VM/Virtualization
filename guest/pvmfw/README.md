...
- entry 4 if present contains reserved memory nodes to add to the device tree
  ...
  headeers is data blob reserved memory headers reference.
...
- A reserved memory header containes a vm_uuid for identifying which VM should
  be alloced access to the memory, a 4 byte offset from the base of the data
  blob where this node's data blob begins, a 4 byte size for the data portion
  of the blob, a 4 byte offset of a C string containing the name of the blob,
  and a 4 byte [flags](guest/pvmfw/src/reserved_mem.rs) field
