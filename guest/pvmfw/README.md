...
A pVM is a VM managed by a hypervisor, running in the non-secure or realm world and is isolated from the Android host OS so that access to its memory is restricted even for a compromised Android host. pVMs support rich environments, including Linux-based distributions.
