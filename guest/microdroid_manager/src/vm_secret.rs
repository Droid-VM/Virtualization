...
impl VmSecret {
    pub fn new(
        ...
        // sealing_policy will building will break for nonprotected VM - but it isnt. Looks fishy!
    ...
    pub fn write_payload_data_rp(&self, data: &[u8; SECRET_SIZE]) -> Result<()> {
        ...
        let Self::V2 { dice_artifacts, instance_id, skp_secret: _, secretkeeper_session } = self
        else {
            return Err(anyhow!("Rollback protected data support is not available for V1 secrets"));
        };
        let payload_id = sha::sha512(instance_id);
        let explicit_dice_chain = dice_artifacts
            .explicit_key_dice_chain()
            .ok_or(anyhow!("Missing explicit dice chain, this is unusual"))?;

        // Consider adding this to `Self::V2{}` instead of recreating it on every write/read.
        let policy = sealing_policy(explicit_dice_chain)
            .map_err(|e| anyhow!("Failed to build a sealing_policy: {e}"))?;
        secretkeeper_session.store_secret(payload_id, data, policy)
    }
