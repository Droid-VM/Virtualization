...
// The VM instance can be in following state:
// 1. Newly created - which can happen if this is a new VM instance (or a malicious Android has
//    deleted relevant secrets) OR
// 2. Read from previously stored values or (if missing) maybe created new.
// 3. The above 2 can  - This happen in case of legacy secret mechanism(V1), where secrets are
//    deterministically obtained from DICE.
#[derive(PartialEq, Eq)]
...
fn try_run_payload(
    ...
    let is_new_instance = match state {
        ...
        VmInstanceState::Unknown => {
            ...
            ensure!(!should_defer_rollback_protection(), "VmInstanceState is Unknown whilst guest is expected to use V2 based secrets. This should've never happened");
