...
pub fn run_vm() -> Result<VmInstance, Error> {
    ...
    let idsig = File::create(work_dir.join("apk.idsig")).context("Failed to create idsig file")?;
    ...
    let vm_config = VirtualMachineConfig::AppConfig(VirtualMachineAppConfig {
        ...
        protectedVm: false,
        memoryMib: 0, // 0 means use the VM default
