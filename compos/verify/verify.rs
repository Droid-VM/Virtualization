...
use compos_common::{
    ...
    IDSIG_MANIFEST_EXT_APK_FILE, ID_FILE, INSTANCE_IMAGE_FILE, TEST_INSTANCE_DIR,
};
use log::error;
use std::fs;
use std::fs::File;
...
fn try_main() -> Result<()> {
    ...
    let id_file = instance_dir.join(ID_FILE);
    ...
    let id: [u8; 64] = fs::read(id_file)?.try_into().map_err(|_| anyhow!("Failed to get Id"))?;
