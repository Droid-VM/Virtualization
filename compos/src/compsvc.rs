...
pub fn new_binder(rpc_binder: bool) -> Result<Strong<dyn ICompOsService>> {
    // NOTE: duplicated, not yet used.
    let key_service = CompOsKeyService::new(rpc_binder)?;

    let service = CompOsService { dex2oat_path: PathBuf::from(DEX2OAT_PATH), key_service };
