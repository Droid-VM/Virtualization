...
impl CompOsService {
    fn generate_fsverity_signature(
        &self,
        key_blob: &[u8],
        ...
        match output {
            CompilerOutput::Digests { oat, vdex, image } => {
                ...
                if key.is_empty() {
                    ...
                    let oat_signature = self.generate_fsverity_signature(key, &oat);
                    let vdex_signature = self.generate_fsverity_signature(key, &vdex);
                    let image_signature = self.generate_fsverity_signature(key, &image);
