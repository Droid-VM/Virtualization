...
fn set_classpaths(android_root: &Path) -> Result<()> {
    let classpath_vars = run_derive_classpath(android_root)?;
    load_classpath_vars(&classpath_vars)
}
...
fn load_classpath_vars(classpath_vars: &str) -> Result<()> {
    ...
    for line in classpath_vars.lines() {
        if let Some(captures) = pattern.captures(line) {
            ...
            // TODO(b/200808307) Don't modify our env, construct a fresh one for odrefresh
