    ...
    F: FnOnce(PathBuf) -> Result<()>,
{
    ...
    if context.system_ext_dir_fd >= 0 {
        input_dir_fd_annotations.push(InputDirFdAnnotation {
            fd: context.system_ext_dir_fd,
            // 1 is the index of extra_apks in vm_config_*.json
    ...
    if context.system_ext_dir_fd >= 0 {
        ...
        system_ext_root.push(context.system_ext_dir_fd.to_string());
