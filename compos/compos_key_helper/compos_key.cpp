...
Result<Ed25519KeyPair> compos_key::deriveKeyFromSecret(const uint8_t* secret, size_t secret_size) {
...
Result<Signature> compos_key::sign(const PrivateKey& private_key, const uint8_t* data,
                                   size_t data_size) {
    Signature result;
...
bool compos_key::verify(const PublicKey& public_key, const Signature& signature,
