...
KeyResult generate_key_pair() {
    ...
    // Public exponent is always 65537, which is what odsign is expecting.
...
SignResult sign(rust::Slice<const uint8_t> private_key, rust::Slice<const uint8_t> data) {
    ...
    signature.resize(RSA_size(key.get()));
    unsigned signature_len = 0;

    if (!RSA_sign(NID_sha256, digest, sizeof(digest), signature.data(), &signature_len,
                  key.get())) {
    ...
    std::copy(signature.data(), signature.data() + signature_len,
