...
public class ComposBenchmark extends MicrodroidDeviceTestBase {
    ...
    private String executeCommand(String command) {
        try {
            ...
            Log.w(TAG, "Get SecurityException when execute: " + command + " , Exception: " + e);
            return "";
        } catch (Exception e) {
        ...
        public void run() {
            while (!mStop.get()) {
                try {
                    ...
                    // Sometimes occur SecurityException: Calling from not trusted UID!
                    // Skip it because it will not affect result.
                    Log.w(TAG, "Get SecurityException when update process memory: " + e);
        ...
        for (Map.Entry<Integer, String> process :
            ...
            if (pName.equalsIgnoreCase(processName)) {
                for (Map.Entry<String, Long> stat :
                        ProcessUtil.getProcessSmapsRollup(pId, this::executeCommand)
                                .entrySet()) {
                    Log.i(
                            TAG,
                            "Get running process "
