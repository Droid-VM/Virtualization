...
public class ComposBenchmark extends MicrodroidDeviceTestBase {
    ...
    /** Test for compilation in vm guest*/
    @Test
    ...
    private void reportProcessMetric(String prefix, Map<String, List<Long>> processMemory) {
        processMemory.forEach((k, v) -> reportMetric(prefix + k, "kb", v));
    }
    ...
    /** Callback shell executor for ProcessUtil */
    private String executeCommandWrapper(String command) {
        try {
    ...
    private void updateProcessMemory(String processName, Map<String, List<Long>> processMemory)
        ...
        for (Map.Entry<Integer, String> process :
                ProcessUtil.getProcessMap(this::executeCommandWrapper).entrySet()) {
            int pId = process.getKey();
            String pName = process.getValue();
            if (pName.equalsIgnoreCase(processName)) {
                for (Map.Entry<String, Long> stat :
                        ProcessUtil.getProcSmapsRollup(pId, this::executeCommandWrapper)
                                .entrySet()) {
                    Log.i(
                            TAG,
                            "Get running process "
