...
public class ComposBenchmark extends MicrodroidDeviceTestBase {
     ...
     */
    public String executeCommandWrapper(String command) {
        try {
    ...
    private void updateProcessMemory(String processName,
        ...
        for (Map.Entry<Integer, String> process : ProcessUtil.getProcessMap(
                command -> this.executeCommandWrapper(command)).entrySet()) {
            int pId = process.getKey();
            String pName = process.getValue();
            if (pName.equalsIgnoreCase(processName)) {
                for (Map.Entry<String, Long> stat : ProcessUtil.getProcSmapsRollup(pId,
                        command -> this.executeCommandWrapper(command)).entrySet()) {
