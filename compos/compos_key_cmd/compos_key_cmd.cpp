...
const char* const kConfigApkPath =
        "/apex/com.android.compos/app/CompOSPayloadApp/CompOSPayloadApp.apk";
const char* const kConfigApkIdsigPath = "/apex/com.android.compos/etc/CompOSPayloadApp.apk.idsig";

// This is a path inside the APK
const char* const kConfigFilePath = "assets/key_service_vm_config.json";
...
class TargetVm {
public:
    TargetVm(int cid, const std::string& logFile, const std::string& imageFile)
          : mCid(cid), mLogFile(logFile), mImageFile(imageFile) {}
    ...
    Result<int> resolveCid() {
        ...
        ScopedFileDescriptor imageFd(
                TEMP_FAILURE_RETRY(open(mImageFile.c_str(), O_RDONLY | O_CLOEXEC)));
        if (imageFd.get() == -1) {
        ...
        appConfig.instanceImage = std::move(imageFd);
        appConfig.configPath = kConfigFilePath;
        appConfig.debug = true;
        appConfig.memory_mib = 0; // Use default
...
static Result<void> makeImage(const std::string& image_path) {
...
int main(int argc, char** argv) {
    ...
    if (argc == 4 && argv[1] == "generate"sv) {
    ...
    } else if (argc == 3 && argv[1] == "make-image"sv) {
        auto result = makeImage(argv[2]);
        if (result.ok()) {
            return 0;
        } else {
            std::cerr << result.error() << '\n';
        ...
        std::cerr << "Usage: compos_key_cmd [OPTIONS] generate|verify|sign|make-image\n"
                  ...
                  << "  make-image <image file> Create an empty instance image file for a VM.\n"
                  << "\n"
