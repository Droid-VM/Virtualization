...
impl VmInstance {
    ...
    pub fn start(
        ...
        let extra_apk_fd = File::open("/system/etc/security/fsverity/BuildManifest.apk")
            .context("Failed to open build manifest APK file")?;
        let extra_apk_fd = ParcelFileDescriptor::new(extra_apk_fd);
        let idsig_extra_fd = prepare_idsig(service, &extra_apk_fd, idsig_extra)?;
        ...
        let config = VirtualMachineConfig::AppConfig(VirtualMachineAppConfig {
            ...
            extraIdsigs: vec![idsig_extra_fd],
            ..Default::default()
        });
