...
impl InstanceManager {
    ...
    pub fn start_current_instance(&self) -> Result<Arc<CompOsInstance>> {
        let mut state = self.0.lock().unwrap();
        state.mark_starting()?;
        drop(state); // Don't hold the lock while we start the instance

        let instance = self.try_start_current_instance();

        let mut state = self.0.lock().unwrap();
        if let Ok(ref instance) = instance {
            state.set_running_instance(instance)?;
        } else {
...
impl State {
    ...
    fn clear_starting(&mut self) {
        if !self.is_starting || self.running_instance.is_some() {
            panic!("Tried to clear starting when not starting");
        }
        self.is_starting = false;
    }

    // Move from Starting to Started.
    fn set_running_instance(&mut self, instance: &Arc<CompOsInstance>) -> Result<()> {
        if !self.is_starting {
            panic!("Can't go to started when not starting")
        }
        if self.running_instance.is_some() {
            panic!("Attempted to start a second instance");
        }
        self.is_starting = false;
