...
impl ICompilationTask for CompilationTask {
    fn cancel(&self) -> BinderResult<()> {
        let task = self.running_task.lock().unwrap().take();
...
impl CompilationTask {
    pub fn start_test_compile(
        ...
        warn!("Started compilation");

        task.clone().wait_for_completion(odrefresh);

        warn!("Started waiting thread");

        Ok(task)
    }

    fn wait_for_completion(self, odrefresh: Arc<Odrefresh>) {
        thread::spawn(move || {
