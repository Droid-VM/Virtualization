...
use crate::odrefresh;
use crate::service::open_dir_path;
...
impl OdrefreshTask {
    ...
    pub fn start(
        ...
        task.clone().start_thread(service, staging_dir_path);

        Ok(task)
    }

    fn start_thread(self, service: Strong<dyn ICompOsService>, staging_dir_path: PathBuf) {
        thread::spawn(move || {
            let exit_code = try_odrefresh(service, staging_dir_path);
