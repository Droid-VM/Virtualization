/*
 * Copyright (C) 2021 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include "libcompos_client.h"

#include <android-base/logging.h>
#include <android-base/strings.h>
#include <android/binder_auto_utils.h>
#include <binder/IInterface.h>
#include <stdlib.h>
#include <sys/types.h>
#include <unistd.h>

#include <binder_rpc_unstable.hpp>
#include <memory>

#include "aidl/com/android/compos/FdAnnotation.h"
#include "aidl/com/android/compos/ICompOsService.h"

using aidl::com::android::compos::FdAnnotation;
using aidl::com::android::compos::ICompOsService;
using android::base::Join;

namespace {

constexpr unsigned int kCompsvcRpcPort = 6432;

struct FileSharingSession {
    pid_t fd_server_pid;
};

FileSharingSession* CreateFileSharingSession(const int* ro_fds, size_t ro_fds_num,
                                             const int* rw_fds, size_t rw_fds_num) {
    pid_t pid;
    if ((pid = vfork()) > 0) {
        auto process = new FileSharingSession();
        process->fd_server_pid = pid;
        return process;
    } else if (pid == 0) {
        // Construct fd_server flags to exec.

        // Holder of C Strings, with enough memory reserved to avoid reallocation. Otherwise,
        // `holder.rbegin()->c_str()` may become invalid.
        std::vector<std::string> holder;
        holder.reserve(ro_fds_num + rw_fds_num);

        std::vector<char const*> args = {"/apex/com.android.virt/bin/fd_server"};
        for (int i = 0; i < ro_fds_num; ++i) {
            args.emplace_back("--ro-fds");
            holder.emplace_back(std::to_string(*(ro_fds + i)));
            args.emplace_back(holder.rbegin()->c_str());
        }
        for (int i = 0; i < rw_fds_num; ++i) {
            args.emplace_back("--rw-fds");
            holder.emplace_back(std::to_string(*(rw_fds + i)));
            args.emplace_back(holder.rbegin()->c_str());
        }
        LOG(DEBUG) << "Starting fd_server, args: " << Join(args, ' ');
        args.emplace_back(nullptr);
        if (execv(args[0], const_cast<char* const*>(args.data())) < 0) {
            PLOG(ERROR) << "execv failed";
        }
        exit(EXIT_FAILURE);
    } else {
        PLOG(ERROR) << "vfork error";
        return nullptr;
    }
}

void DeleteFileSharingSession(FileSharingSession* session) {
    if (!session) {
        LOG(ERROR) << "Invalid argument: session is nullptr";
        return;
    }
    if (kill(session->fd_server_pid, SIGTERM) < 0) {
        PLOG(ERROR) << "Cannot kill fd_server (pid " << std::to_string(session->fd_server_pid)
                    << ") with SIGTERM. Retry with SIGKILL.";
        if (kill(session->fd_server_pid, SIGKILL) < 0) {
            PLOG(ERROR) << "Still cannot terminate with SIGKILL. Give up.";
            // TODO: it may be the safest if we turn fd_server into a library to run in a thread.
        }
    }
}

} // namespace

__BEGIN_DECLS

struct AComposClient {
    std::shared_ptr<ICompOsService> service;
};

AComposClient* AComposClient_Connect(int cid) {
    ndk::SpAIBinder binder(RpcClient(cid, kCompsvcRpcPort));
    auto client = std::make_unique<AComposClient>();
    client->service = ICompOsService::fromBinder(binder);
    return client.release();
}

void AComposClient_Disconnect(AComposClient* client) {
    delete client;
}

int AComposClient_Request(AComposClient* client, const uint8_t* marshaled, size_t size,
                          const int* ro_fds, size_t ro_fds_num, const int* rw_fds,
                          size_t rw_fds_num) {
    if (!client || !client->service) {
        LOG(ERROR) << "Client is not connected";
        return -1;
    }

    auto session_raii = std::unique_ptr<
            FileSharingSession,
            decltype(&DeleteFileSharingSession)>(CreateFileSharingSession(ro_fds, ro_fds_num,
                                                                          rw_fds, rw_fds_num),
                                                 DeleteFileSharingSession);

    // Since the input from the C API are raw pointers, we need to duplicate them into vectors in
    // order to pass to the binder API.
    std::vector<uint8_t> duplicated_buffer(marshaled, marshaled + size);
    FdAnnotation fd_annotation = {
            .input_fds = std::vector<int>(ro_fds, ro_fds + ro_fds_num),
            .output_fds = std::vector<int>(rw_fds, rw_fds + rw_fds_num),
    };
    int8_t exit_code;
    ndk::ScopedAStatus status =
            client->service->compile(duplicated_buffer, fd_annotation, &exit_code);
    if (!status.isOk()) {
        LOG(ERROR) << "Compilation failed (exit " << std::to_string(exit_code)
                   << "): " << status.getDescription();
        return -1;
    }
    return 0;
}

__END_DECLS
