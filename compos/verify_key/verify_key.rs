...
use compos_common::{
    COMPOS_DATA_ROOT, CURRENT_INSTANCE_DIR, IDSIG_EXTRA_FILE, IDSIG_FILE, INSTANCE_IMAGE_FILE,
    PENDING_INSTANCE_DIR, PRIVATE_KEY_BLOB_FILE, PUBLIC_KEY_FILE, TEST_INSTANCE_DIR,
};
...
fn verify(debug_mode: bool, instance_dir: &Path) -> Result<()> {
    ...
    let idsig_extra = instance_dir.join(IDSIG_EXTRA_FILE);
