...
const MAX_FILE_SIZE: u64 = 8 * 1024;
...
fn main() -> Result<()> {
    let matches = clap::App::new("compos_verify_key")
        .arg(
            clap::Arg::with_name("pending")
                .long("pending")
                .help("Verify pending instance rather than current"),
        )
        .get_matches();
    let do_pending = matches.is_present("pending");
...
fn verify(instance_dir: &Path) -> Result<()> {
    ...
    let blob = read_file(blob).context("Failed to read key blob")?;
    let public_key = read_file(public_key).context("Failed to read public key")?;

    let instance = File::open(instance).context("Failed to open instance image file")?;
    let vm_instance = VmInstance::new(instance)?;
...
fn promote_to_current(instance_dir: &Path) -> Result<()> {
    ...
    fs::remove_dir_all(&current_dir).context("Failed to remove current instance")?;
    fs::rename(&instance_dir, &current_dir).context("Renaming pending to current failed")?;
    Ok(())
}
...
impl VmInstance {
    fn new(instance_file: File) -> Result<VmInstance> {
...
impl IVirtualMachineCallback for VmCallback {
    fn onDied(&self, _arg_cid: i32) -> BinderResult<()> {
        self.0.set_died();
        println!("VM died");
        Ok(())
    }
