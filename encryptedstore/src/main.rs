...
fn set_root_dir_permissions(mountpoint: &Path) -> Result<()> {
    // mke2fs hardwires the root dir permissions as 0o700 which doesn't match what we want.
    // We want to allow full access by both root and the payload group. And we want the
    // sticky bit set, so different payload UIDs can create sub-directories that other payloads
    // can't delete.
