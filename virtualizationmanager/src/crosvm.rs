...
fn vfio_argument_for_platform_device(path: &Path) -> Result<String, Error> {
    ...
    let dev_sysfs_path = path.canonicalize()?;
    if !dev_sysfs_path.starts_with(SYSFS_PLATFORM_DEVICES) {
        bail!("{dev_sysfs_path:?} is not a platform device");
    }

    // Check platform device is bound to VFIO driver
    let dev_driver_path = dev_sysfs_path.join("driver").canonicalize()?;
    if dev_driver_path != Path::new(VFIO_PLATFORM_DRIVER) {
        bail!("{dev_sysfs_path:?} is not bound to VFIO-platform driver");
    }

    if let Some(p) = dev_sysfs_path.to_str() {
        Ok(format!("--vfio={p},iommu=viommu"))
    } else {
        bail!("invalid path {dev_sysfs_path:?}");
    }
}
