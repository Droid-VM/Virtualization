...
impl VmInstance {
    ...
    fn monitor_vm_exit(
        ...
        *vm_state = VmState::Dead;
        ...
        // TODO: clean up assigned devices.
...
fn check_platform_device(path: &str) -> Result<String, Error> {
    ...
    let dev_sysfs_path = std::fs::canonicalize(path)?;
    if !dev_sysfs_path.as_path().starts_with(SYSFS_PLATFORM_DEVICES) {
        bail!("{} is not a platform device", dev_sysfs_path.display());
    }

    // Check platform device is bound to VFIO driver
    let dev_driver_path = std::fs::canonicalize(dev_sysfs_path.as_path().join("driver"))?;
    if dev_driver_path != Path::new(VFIO_PLATFORM_DRIVER) {
        bail!("{} is not bound to VFIO-platform driver", dev_sysfs_path.display());
    }

    if let Some(p) = dev_sysfs_path.to_str() {
        Ok(p.into())
    } else {
        bail!("invalid path {}", dev_sysfs_path.display());
    }
}

fn append_platform_devices(command: &mut Command, config: &CrosvmConfig) -> Result<(), Error> {
    for device in &config.vfio_devices {
        let dev_sysfs_path = check_platform_device(device)?;
        command.arg(format!("--vfio={},iommu=viommu", dev_sysfs_path));
    }
    if let Some(_dtbo) = &config.vfio_dtbo {
