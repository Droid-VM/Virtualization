...
fn make_payload_disk(
    ...
    check_apexes_are_from_allowed_partitions(&apex_infos, &apex_list)?;
...
fn check_apexes_are_from_allowed_partitions(
    ...
    for apex in requested_apexes {
        if apex_list
            .list
            .iter()
            // The starts_with("/system") checks covers both /system and /system_ext
            .any(|ai| ai.name == apex.name && !ai.preinstalled_path.starts_with("/system"))
        {
