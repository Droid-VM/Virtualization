...
pub(crate) const HOST_CONTROLLED_NODE_NAME: &CStr = cstr!("avf-host-controlled");
...
pub(crate) enum DtAddition<'a> {
    ...
    FromPath(&'a Path),
    /// Include a property in /avf-host-controlled node. This node is used to specify host provided
    /// properties such as `Id`. pVM firmware does minimal validation of properties in this node.
    HostControlledProp(&'a CStr, &'a [u8]),
}
...
pub(crate) fn create_device_tree_overlay<'a>(
    ...
    let (additional_properties, additional_paths): (Vec<_>, _) =
        dt_additions.iter().partition(|o| matches!(o, DtAddition::HostControlledProp(_, _)));
    ...
    if !additional_properties.is_empty() {
        let mut node = node
            .add_subnode(HOST_CONTROLLED_NODE_NAME)
            .map_err(|e| anyhow!("Failed to add host-controlled-node: {e:?}"))?;
        for prop in additional_properties {
            if let DtAddition::HostControlledProp(name, value) = prop {
...
mod tests {
    ...
    fn nonsecure_prop_test() {
        ...
        let fdt = create_device_tree_overlay(
            &mut buffer,
            &[DtAddition::HostControlledProp(name, value_in)],
        )
        .unwrap();

        let value_dt = fdt
            .node(cstr!("/fragment@0/__overlay__/avf-host-controlled"))
            .unwrap()
            .expect("avf-host-controlled node doesn't exist")
            .getprop(name)
            .unwrap()
            .expect("Prop not found!");
        assert_eq!(value_dt, value_in, "Unexpected property value");
    }
}
