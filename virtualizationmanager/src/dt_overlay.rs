...
pub(crate) const AVF_UNTRUSTED_NODE_NAME: &CStr = cstr!("avf-untrusted");
...
pub(crate) fn create_device_tree_overlay<'a>(
    ...
    if !additional_properties.is_empty() {
        let mut node = node
            .add_subnode(AVF_UNTRUSTED_NODE_NAME)
            .map_err(|e| anyhow!("Failed to add avf-untrusted: {e:?}"))?;
...
mod tests {
    ...
    fn nonsecure_prop_test() {
        ...
        let fdt = create_device_tree_overlay(
            &mut buffer,
            &[DtAddition::AvfUntrustedProp(name, value_in)],
        )
        .unwrap();
