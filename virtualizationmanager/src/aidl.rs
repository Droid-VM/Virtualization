...
impl IVirtualizationService for VirtualizationService {
    ...
    fn createVm(
        ...
        let converted_config: VirtualMachineConfig;
        let config = if let VirtualMachineConfig::RawConfigFromPath(raw_config_from_path) = config {
            ...
            converted_config = VirtualMachineConfig::RawConfig(raw_config);
            &converted_config
        } else {
            config
        };
