    ...
    fn create_vm_internal(
        ...
        *is_protected = config.protectedVm;
        ...
        if config.protectedVm {
            ...
            check_protected_vms_are_supported()?;
        }
...
fn check_protected_vms_are_supported() -> binder::Result<()> {
    let are_pvms_supported =
        !hypervisor_props::is_protected_vm_supported().or_service_specific_exception(-1)?;
    if are_pvms_supported {
        Ok(())
    } else {
        Err(anyhow!("pVMs are not supported"))
            .or_binder_exception(ExceptionCode::UNSUPPORTED_OPERATION)
    }
}
