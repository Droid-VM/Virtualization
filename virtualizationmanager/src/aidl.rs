...
mod tests {
    ...
    #[test]
    fn test_extract_os_name_from_config_path() -> Result<()> {
        let expected_results = vec![
            (
                Path::new("/apex/com.android.virt/etc/microdroid.json"),
                Some("microdroid".to_owned()),
            ),
            (
                Path::new("/apex/com.android.virt/etc/microdroid_gki-android14-6.1.json"),
                Some("microdroid_gki-android14-6.1".to_owned()),
            ),
            (Path::new("/apex/com.android.virt/etc/fs/microdroid.img"), None),
        ];
        for (path, expected_result) in expected_results {
