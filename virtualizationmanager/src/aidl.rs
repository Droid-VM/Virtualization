...
use std::fs::{canonicalize, read, read_dir, remove_file, File, OpenOptions};
...
fn create_dtbo_for_vendor_image(vendor_public_key: &[u8], dtbo: &PathBuf) -> Result<()> {
    ...
    node.setprop(vendor_public_key_name.as_c_str(), vendor_public_key)
...
mod tests {
    ...
    fn test_create_dtbo_for_vendor_image() -> Result<()> {
        ...
        let data = read(dtbo_path)?;
        let fdt = Fdt::from_slice(&data).unwrap();

        let avf_node_name = CString::new("/avf")?;
        let avf_node = fdt.node(avf_node_name.as_c_str()).unwrap();
        ...
        let Some(key_from_dtbo) = key_from_dtbo else {
            bail!("vendor_public_key shouldn't be None.");
        };
        assert_eq!(key_from_dtbo, vendor_public_key);
