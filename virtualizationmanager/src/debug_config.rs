...
mod tests {
    ...
    // Read avf_debug_policy_with_adb.dtbo.
    fn test_new_internal() -> Result<()> {
    ...
    fn test_new() -> Result<()> {
        if !can_set_sysprop() {
            // Skip test if set sysprop isn't possible.
            return Ok(());
        }
        ...
        let test_result = test_new_internal();

        // Clean up system properties.
        system_properties::write(CUSTOM_DEBUG_POLICY_OVERLAY_SYSPROP, "")
            .context("Failed to reset sysprop")?;
