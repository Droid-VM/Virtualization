...
/// Debug policy device tree path that can be also compatible with file path.
#[derive(Debug)]
struct DPPath {
    node_path: &'static CStr,
    prop_name: &'static CStr,
}

impl DPPath {
    /// Creates new instance with static literals.
    fn new(node_path: &'static str, prop_name: &'static str) -> Self {
        Self {
            node_path: CStr::from_bytes_with_nul(node_path.as_bytes()).unwrap_or_default(),
            prop_name: CStr::from_bytes_with_nul(prop_name.as_bytes()).unwrap_or_default(),
        }
    }
...
lazy_static! {
    static ref DEBUG_POLICY_LOG_PATH: DPPath = DPPath::new("/avf/guest/common\0", "log\0");
    static ref DEBUG_POLICY_RAMDUMP_PATH: DPPath = DPPath::new("/avf/guest/common\0", "ramdump\0");
    static ref DEBUG_POLICY_ADB_PATH: DPPath = DPPath::new("/avf/guest/microdroid\0", "adb\0");
}
...
fn get_debug_policy_bool(path: &Path) -> Result<Option<bool>> {
    let value = match fs::read(path) {
        ...
        value.len() == 4,
        format!(
            "Malformed data in {path:?}. Must be 4 bytes for bool, but size was {}",
            value.len()
        ),
    );
...
fn get_fdt_prop_bool(fdt: &Fdt, path: &DPPath) -> Result<Option<bool>> {
...
impl OwnedFdt {
    fn from_overlay_onto_new_fdt(overlay_file_path: &Path) -> Result<Self> {
        let mut overlay_buf = match fs::read(overlay_file_path) {
            Ok(fdt) => fdt,
            Err(error) if error.kind() == ErrorKind::NotFound => Default::default(), // nothing to overlay.
            Err(error) => {
        ...
        if !overlay_buf.is_empty() {
            ...
            // SAFETY - Return immediately if error happen, so damaged fdt_buf and fdt are discarded.
            unsafe {
...
impl DebugConfig {
    ...
    fn from_custom_debug_overlay_policy(debug_level: DebugLevel, path: &Path) -> Result<Self> {
        match OwnedFdt::from_overlay_onto_new_fdt(path) {
            Ok(fdt) => Ok(Self {
                debug_level,
                debug_policy_log: get_fdt_prop_bool(fdt.as_fdt(), &DEBUG_POLICY_LOG_PATH)?
                    .unwrap_or_default(),
                debug_policy_ramdump: get_fdt_prop_bool(fdt.as_fdt(), &DEBUG_POLICY_RAMDUMP_PATH)?
                    .unwrap_or_default(),
                debug_policy_adb: get_fdt_prop_bool(fdt.as_fdt(), &DEBUG_POLICY_ADB_PATH)?
                    .unwrap_or_default(),
            }),
            Err(err) => Err(err),
        }
    ...
    fn from_host(debug_level: DebugLevel) -> Result<Self> {
        Ok(Self {
            debug_level,
            debug_policy_log: get_debug_policy_bool(&DEBUG_POLICY_LOG_PATH.to_path())?
                .unwrap_or_default(),
            debug_policy_ramdump: get_debug_policy_bool(&DEBUG_POLICY_RAMDUMP_PATH.to_path())?
                .unwrap_or_default(),
            debug_policy_adb: get_debug_policy_bool(&DEBUG_POLICY_ADB_PATH.to_path())?
                .unwrap_or_default(),
        })
    }
}
