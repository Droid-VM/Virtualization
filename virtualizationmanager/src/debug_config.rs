...
mod tests {
    ...
    fn test_new_without_custom_policy_internal() -> Result<()> {
    ...
    #[test]
    fn test_new_without_custom_policy() -> Result<()> {
        let old_sysprop = system_properties::read(CUSTOM_DEBUG_POLICY_OVERLAY_SYSPROP)
            .context("Failed to read existing sysprop")?
            .unwrap_or_default();

        if !can_set_sysprop() && !old_sysprop.is_empty() {
            // Skip test if we can't clear custom debug policy overlay sysprop.
            return Ok(());
        }

        // Setup
        if !old_sysprop.is_empty() {
            system_properties::write(CUSTOM_DEBUG_POLICY_OVERLAY_SYSPROP, "")
                .context("Failed to set sysprop")?;
        }

        // Run test
        let test_result = test_new_without_custom_policy_internal();
