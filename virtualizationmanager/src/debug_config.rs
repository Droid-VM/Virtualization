...
impl DebugConfig {
    pub fn new(config: &VirtualMachineConfig) -> Self {
        let debug_level = get_debug_level(config);

        let dp_sysprop = system_properties::read(CUSTOM_DEBUG_POLICY_OVERLAY_SYSPROP);
        let debug_policy = match dp_sysprop.unwrap_or_default() {
            Some(path) if !path.is_empty() => {
                match DebugPolicyConfig::from_overlay(Path::new(&path)) {
                    Ok(debug_config) => {
            ...
            _ => match DebugPolicyConfig::from_host() {
                Ok(debug_config) => {
        ...
        if debug_policy.is_none() {
            info!("Debug policy is disabled");
        }
        let debug_policy = debug_policy.unwrap_or_default();

        let dump_device_tree = match system_properties::read_bool(DUMP_DT_SYSPROP, false) {
            Ok(value) => value,
            Err(e) => {
...
mod tests {
    ...
    fn test_read_avf_debug_policy_with_ramdump() -> Result<()> {
        let debug_policy =
            DebugPolicyConfig::from_overlay("avf_debug_policy_with_ramdump.dtbo".as_ref()).unwrap();
    ...
    fn test_read_avf_debug_policy_without_ramdump() -> Result<()> {
        let debug_policy =
            DebugPolicyConfig::from_overlay("avf_debug_policy_without_ramdump.dtbo".as_ref())
                .unwrap();
    ...
    fn test_read_avf_debug_policy_with_adb() -> Result<()> {
        let debug_policy =
            DebugPolicyConfig::from_overlay("avf_debug_policy_with_adb.dtbo".as_ref()).unwrap();
    ...
    fn test_read_avf_debug_policy_without_adb() -> Result<()> {
        let debug_policy =
            DebugPolicyConfig::from_overlay("avf_debug_policy_without_adb.dtbo".as_ref()).unwrap();
    ...
    fn test_invalid_sysprop_disables_debug_policy() -> Result<()> {
        let debug_policy =
            DebugPolicyConfig::from_overlay("/a/does/not/exist/path.dtbo".as_ref()).unwrap();
