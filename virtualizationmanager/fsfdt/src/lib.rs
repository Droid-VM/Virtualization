...
impl<'a> FsFdt<'a> for Fdt {
    fn from_fs(fs_path: &Path, fdt_buffer: &'a mut [u8]) -> Result<&'a mut Fdt> {
        ...
        // Recursively travels fs_path with DFS algorithm.
        let mut stack = vec![fs_path.to_path_buf()];
        while let Some(dir_path) = stack.pop() {
            let relative_path =
                dir_path.strip_prefix(fs_path).context("Internal error while traveling")?;
            let fdt_path = CString::from_vec_with_nul(
                [b"/", relative_path.to_str().unwrap().as_bytes(), b"\0"].concat(),
            )
            .unwrap();
