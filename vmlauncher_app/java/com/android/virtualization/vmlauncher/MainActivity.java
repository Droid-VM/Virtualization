        ...
        while (true) {
            try {
                ...
                ICrosvmAndroidDisplayService service =
                        ICrosvmAndroidDisplayService.Stub.asInterface(vs.getDisplayService());
                if (service != null) {
                    func.apply(service);
                    return;
                }
            } catch (Exception e) {
                Log.d(TAG, "will retry it 1000ms later", e);
            }
            Log.d(TAG, "wait 1000ms for ICrosvmAndroidDisplayService");
            getMainThreadHandler().postDelayed(() -> runWithDisplayService(func), 1000);
        }
    }
