...
1. Secretkeeper protected secret: These is a random 64 bytes generated on first boot & stored in
   Secretkeeper.
...
instance, Microdroid Manager (on behalf of the VM payload) generates a secret and stores it in
...
this is Instance Id, allocated to by host and relayed to VM via device tree property (`instance-id`)
...
- For each DiceChainEntry:
  ...
  1. ExactMatch on MODE (Required) - Secret should be inaccessible if any of the runtime
  ...
  1. GreaterOrEqual on SECURITY_VERSION (Optional): The secrets will be accessible if version of any
     image is greater or equal to the set version. This is an optional field, certain components may
     chose to prevent booting of rollback images for ex, ABL is expected to provide rollback
     protection of pvmfw. Such components may chose to not put SECURITY_VERSION in the corresponding
     DiceChainEntry.
...
VMs fallback to Secretkeeper based rollback protection.
