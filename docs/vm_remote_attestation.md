...
-   It is running on a valid device trusted by the Remote Key Provisioning (RKP)
    backend, such as Google.
...
environment and certifies the payload executed within each pVM.
...
For detailed information and usage examples, please refer to [demo app](demo).
