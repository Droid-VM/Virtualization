...
intervention. AVF uses `vfio-platform` for device assignment.
...
When the host boots, the bootloader provides VM DTBO to both host and pvmfw.
...
assigning devices, and wouldn't be applied to VM DT. Here are details:
...
* Physical device nodes
  ...
  * Physical device nodes can optionally contain `<iommus>` property. The
    ...
    * IOMMU can be shared among devices, but should use distinct SIDs. Sharing
      the same IOMMU-SID pair between multiple devices isn't supported for now.
...
dtbo.img. See [DTB/DTBO Paritions] for partition format.
