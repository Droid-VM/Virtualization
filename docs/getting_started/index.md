 ...
 * [yukawa: Khadas VIM3L](vim3l.md) (arm64)
...
It is not currently part of the base system image and needs to be installed separately.

### With 'adb sync'

This is the preferred option, but requires root and remounting the system image as writable.
Due to broken overlayfs, it also currently does not work on the Goldfish emulator.

It is handy if you're developing CrosVM and want to quickly sync your system files instead of
flashing the device each time.
...
Now (re-)build CrosVM, stop the world and sync:
```
$ m crosvm
$ adb shell stop
$ adb sync
...
### Including CrosVM in the system image

Insert `crosvm` into the `PRODUCT_PACKAGES_DEBUG` list in
`build/make/target/product/base_system.mk`.

Now flash your device or restart the emulator to start using the new system image.
...
the "target" (the device connected via USB) using ADB. The tests spawn guest VMs and test different
...
You can spawn your own VMs by run CrosVM directly on a rooted device. If your device is attached
over ADB, you can run:
```
