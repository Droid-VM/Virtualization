...
fn generate_hash_tree<R: Read>(
    ...
    let tree_size = levels.iter().map(|r| r.size).sum();
...
fn calc_hash_levels(input_size: usize, block_size: usize, digest_size: usize) -> Vec<Range> {
    ...
    //
    // This is done by accumulating the sizes in reverse order
    let mut level_offsets = level_sizes.iter().rev().fold(
    ...
    level_offsets.pop(); // we don't need the offset for level "-1", which is the input file.
    level_offsets.reverse(); // reverse again so that index N is for level N

    level_offsets
        .iter()
        .zip(level_sizes.iter())
        .map(|(offset, size)| Range { start: *offset, size: *size })
        .collect()
}
