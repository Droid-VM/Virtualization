...
pub fn get_cpu_time() -> Result<CpuTime> {
    ...
    let binding = re.replace_all(&line, "");
    let data_list: Vec<_> = binding.split(' ').collect();

    let ticks_per_sec = unsafe { sysconf(_SC_CLK_TCK) } as i64;
    let cpu_time = CpuTime {
        user: data_list[0].parse::<i64>()? * MILLIS_PER_SEC / ticks_per_sec,
        nice: data_list[1].parse::<i64>()? * MILLIS_PER_SEC / ticks_per_sec,
        sys: data_list[2].parse::<i64>()? * MILLIS_PER_SEC / ticks_per_sec,
        idle: data_list[3].parse::<i64>()? * MILLIS_PER_SEC / ticks_per_sec,
    };
    Ok(cpu_time)
}
...
pub fn get_mem_info() -> Result<MemInfo> {
    let re = Regex::new(r"(^.*?:)(\s*)|(\skB$)").unwrap();

    let proc_mem_info = fs::read_to_string("/proc/meminfo")?;
    let data_list: Vec<_> =
        proc_mem_info.trim().split('\n').map(|s| re.replace_all(s, "").to_string()).collect();
