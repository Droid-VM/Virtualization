...
pub fn get_cpu_time() -> Result<CpuTime> {
    ...
    let proc_stat = BufReader::new(File::open("/proc/stat")?);
    let first_line =
        proc_stat.lines().next().ok_or_else(|| anyhow!("unexpected format: /proc/stat"))??;
    let binding = re.replace_all(&first_line, "");
    let data_list: Vec<_> = binding.split(' ').collect();

    let cpu_time = CpuTime {
        user: data_list[0].parse()?,
        nice: data_list[1].parse()?,
        sys: data_list[2].parse()?,
        idle: data_list[3].parse()?,
    };
    Ok(cpu_time)
}
