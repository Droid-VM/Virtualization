...
fn build_signed_data(
    ...
    let cdi_leaf_sig = build_empty_signature(iana::Algorithm::EdDSA);
    let attestation_key_sig = build_empty_signature(ATTESTATION_KEY_ALGO);
    let aad = &[];
...
/// Builds an empty signature with the provided algorithm.
/// The signature data will be filled later when building the signed data.
fn build_empty_signature(alg: iana::Algorithm) -> CoseSignature {
