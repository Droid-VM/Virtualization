...
impl IVmPayloadService for VmPayloadService {
    ...
    fn requestCertificate(&self, challenge: &[u8]) -> binder::Result<Vec<u8>> {
        self.check_restricted_apis_allowed()?;
...
mod tests {
    ...
    fn csr_format_is_correct() -> Result<()> {
        ...
        let csr_payload = cose_sign.payload.unwrap();
        let csr_payload: CsrPayload = cbor_util::deserialize(&csr_payload).unwrap();
        let expected_csr_payload = CsrPayload::V1(CsrPayloadV1 {
            challenge: CHALLENGE.to_vec(),
