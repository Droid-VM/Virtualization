...
fn build_csr(
    ...
    let public_key = to_cose_public_key(attestation_key)?.to_vec()?;
    ...
    let signed_csr_payload =
        build_signed_data(csr_payload, &cdi_leaf_priv, attestation_key)?.to_vec()?;
