...
impl IVmPayloadService for VmPayloadService {
    ...
    fn getAccessorBinder(&self, instance: &str) -> binder::Result<SpIBinder> {
        ...
        let get_connection_info = move || {
            let connection_info = vm_service.getServiceConnectionInfo(&instance_string).unwrap();
            // TODOasdf match and handle error
            info!("Getting connection info from virtmgr");
            Some(ConnectionInfo {
                port: connection_info.port.try_into().unwrap(),
                cid: VMADDR_CID_HOST,
            })
        };

        // Create the accessor in libbinder
        let accessor = Accessor::new(instance, get_connection_info);

        // Get and return the binder
        match accessor.as_binder() {
            Some(binder) => {
                info!("returning Accessor::as_binder() for {instance}");
                Ok(binder)
            }
            None => Err(anyhow!("Failed to get a binder from Accessor::as_binder"))
                .with_log()
                .or_binder_exception(ExceptionCode::NULL_POINTER),
        }
    }
}
