...
use ciborium::Value;
use coset::{iana::{self, EnumI64}, Algorithm, KeyType, Label, CoseSign, CoseSignBuilder, HeaderBuilder, CborSerializable, CoseSignatureBuilder,
    CoseKey, CoseKeyBuilder, CoseSignature};
use service_vm_comm::{Csr, CsrPayload};
use std::os::unix::io::OwnedFd;
use openssl::{
    ec::{EcGroup, EcKey, EcKeyRef, EcPoint}, nid::Nid, bn::{BigNumContext, BigNum}, sha::sha512,
...
const ATTESTATION_PRIVATE_KEY_SIZE: i32 = 32;
...
impl IVmPayloadService for VmPayloadService {
    ...
    fn requestAttestation(&self, challenge: &[u8]) -> binder::Result<AttestationResult> {
        self.check_restricted_apis_allowed()?;
        let (private_key, csr) =
            match generate_attestation_key_and_csr(challenge, self.secret.dice()) {
                Ok((private_key, csr)) => (private_key, csr),
                Err(e) => {
    ...
    fn ecdsaSign(&self, message: &[u8], res: &AttestationResult) -> binder::Result<Vec<u8>> {
        if !cfg!(remote_attestation) {
            return Err(Status::new_exception_str(
                ExceptionCode::UNSUPPORTED_OPERATION,
                Some("ecdsaSign is not supported with the remote_attestation feature disabled"),
            ))
            .with_log();
        }
        self.check_restricted_apis_allowed()?;
        try_ecdsa_sign(message, &res.privateKey)
            .context("Failed to sign with ECDSA")
            .or_binder_exception(ExceptionCode::ILLEGAL_STATE)
    }
}

fn try_ecdsa_sign(message: &[u8], private_key: &[u8]) -> Result<Vec<u8>> {
    let private_key = CoseKey::from_slice(private_key)
        .map_err(|e| anyhow!("Failed to deserialize private key: {e}"))?;
    let private_key = to_ec_private_key(&private_key)?;
...
fn build_signed_data(
    ...
    let cdi_leaf_sig = build_signature(iana::Algorithm::EdDSA);
    let attestation_key_sig = build_signature(ATTESTATION_KEY_ALGO);
    let aad = &[];
...
fn build_signature(alg: iana::Algorithm) -> CoseSignature {
    let protected = HeaderBuilder::new().algorithm(alg).build();
    CoseSignatureBuilder::new().protected(protected.clone()).build()
}
...
fn to_cose_private_key(key: &EcKeyRef<Private>) -> Result<CoseKey> {
    ...
    let private_key = key.private_key().to_vec_padded(ATTESTATION_PRIVATE_KEY_SIZE)?;
...
fn check_ec_key_params(cose_key: &CoseKey) -> Result<()> {
    ...
    assert_eq!(Some(Algorithm::Assigned(ATTESTATION_KEY_ALGO)), cose_key.alg);
