...
pub enum VmSecret {
    ...
    V2 { dice: OwnedDiceArtifacts, rp_secret: ZVec },
...
impl VmSecret {
    pub fn new(
        ...
        if is_rp_secrets_supported() {
            ...
            // TODO: This need not be constant size.
            let auth_det = vec![0u8; RP_SECRET_SIZE];
            let sk_service =
            ...
            // On first boot -> create a secret
            // let uuid = read_instance_id();
            if super::is_new_instance() {
                rp_secret = Vec::with_capacity(RP_SECRET_SIZE);
                rp_secret.try_fill(&mut rand::thread_rng())?;
                sk_service.store(&rp_secret)?;
            } else {
                rp_secret = sk_service.read(&auth_det)?;
            }
            return Ok(Self::V2 { dice: dice_artifacts, rp_secret: ZVec::try_from(rp_secret)? });
        }
