...
use secretkeeper_client::dice::ExplicitAndOwnedDiceArtifacts;
...
pub enum VmSecret {
    ...
    V2 { dice: ExplicitAndOwnedDiceArtifacts, skp_secret: ZVec },
...
impl VmSecret {
    pub fn new(
        ...
        // TODO: Convert dice chian to explicit key chain

        if let Some(sk_service) = is_sk_supported(vm_service)? {
            let explicit_owned_dice =
                ExplicitAndOwnedDiceArtifacts::from(dice_artifacts).map_err(|e| {
                    anyhow!("Failed to convert to explicit key dice cert chain format: {:?}", e)
                })?;
            let id = get_id();
            ...
            if super::is_strict_boot() {
                if super::is_new_instance() {
                    *skp_secret = rand::random();
                    store_secret(sk_service.clone(), id, skp_secret.clone(), &explicit_owned_dice)?;
                } else {
                    // Subsequent run of the pVM -> get the secret stored in Secretkeeper.
                    *skp_secret = get_secret(sk_service.clone(), id, &explicit_owned_dice)?;
                }
            } else {
    ...
    pub fn dice(&self) -> &dyn DiceArtifacts {
        match self {
            Self::V2 { dice, .. } => dice,
            Self::V1 { dice } => dice,
        }
...
fn store_secret(
    ...
    dice_artifacts: &ExplicitAndOwnedDiceArtifacts,
) -> Result<()> {
    // Start a new secretkeeper session using dice chain as the identity!
    let chain =
        dice_artifacts.dice_chain_in_explicit_key_format().ok_or(anyhow!("Dice chain missing"))?;

    let session = SkSession::new(secretkeeper, dice_artifacts).map_err(anyhow_err)?;
    let sealing_policy = sealing_policy(chain).map_err(anyhow_err)?;
...
fn get_secret(
    ...
    let chain =
        dice_artifacts.dice_chain_in_explicit_key_format().ok_or(anyhow!("Dice chain missing"))?;
    let session = SkSession::new(secretkeeper, dice_artifacts).map_err(anyhow_err)?;
    let _sealing_policy = sealing_policy(chain).map_err(anyhow_err)?;
...
// Construct a sealing policy for the secret. VMs uses the following containtspec for rollback protection
// Mode Decision: This captures configuration signal
// SECURITY_VERSION
fn sealing_policy(dice: &[u8]) -> Result<Vec<u8>, String> {
    let constraint_spec = [
        ConstraintSpec::new(ConstraintType::ExactMatch, vec![AUTHORITY_HASH])?,
        ConstraintSpec::new(ConstraintType::ExactMatch, vec![KEY_MODE])?,
        ConstraintSpec::new(ConstraintType::ExactMatch, vec![CONFIG_DESC, COMPONENT_NAME])?,
        ConstraintSpec::new(ConstraintType::GreaterOrEqual, vec![CONFIG_DESC, SECURITY_VERSION])?,
        ConstraintSpec::new(ConstraintType::GreaterOrEqual, vec![CONFIG_DESC, COMPONENT_VERSION])?,
    ];

    let policy = DicePolicy::from_dice_chain(dice, &constraint_spec)?; //.to_vec()
    log::info!("Sealing policy: {:?}", policy);
    policy.to_vec().map_err(|e| format!("DicePolicy matching failed {e:?}"))
}
