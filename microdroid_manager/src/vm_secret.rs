...
impl VmSecret {
    pub fn new(
        ...
        if is_sk_supported(sk_service.is_some()) {
            ...
            let id = get_id();
            let mut secret: [u8; SECRET_SIZE];
            if super::is_strict_boot() {
                if super::is_new_instance() {
                    secret = rand::random();
                    store_secret(sk_service.clone(), &id, &secret, &dice_artifacts)?;
                } else {
                    // Subsequent run of the pVM -> get the secret stored in Secretkeeper.
                    secret = get_secret(sk_service.clone(), &id, &dice_artifacts)?;
                }
            } else {
            ...
            let skp_secret = ZVec::try_from(secret.to_vec())?;
            secret.zeroize();

            return Ok(Self::V2 { dice: dice_artifacts, skp_secret });
        }
