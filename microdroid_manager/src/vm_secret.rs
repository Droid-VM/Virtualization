...
use dice_policy_builder::{ArrayIndex, ConstraintSpec, ConstraintType, policy_for_dice_chain, MissingAction, WILDCARD_FULL_ARRAY};
...
//     microdroid_manager/src/vm_config.cddl): a) GreaterOrEqual on SECURITY_VERSION (Required) b)
...
fn sealing_policy(dice: &[u8]) -> Result<Vec<u8>, String> {
    let constraint_spec = vec![
        ...
        ConstraintSpec::new(
            ...
            MissingAction::Fail,
            ArrayIndex::FromEnd(PAYLOAD_INDEX_FROM_END),
        ),
    ];
