        ...
        if let Some(sk_service) = is_sk_supported(vm_service)? {
            let explicit_dice =
                ExplicitAndOwnedDiceArtifacts::from(dice_artifacts).map_err(|e| {
                    anyhow!("Failed to convert to ExplicitAndOwnedDiceArtifacts: {:?}", e)
                })?;
            let mut session = SkSession::new(sk_service).map_err(anyhow_err)?;
            let id = get_id();
            ...
            if super::is_strict_boot() {
                if super::is_new_instance() {
                    *skp_secret = rand::random();
                    ...
                    let _policy = sealing_policy(explicit_bcc);
                    store_secret(
                        &mut session,
                        id,
                        skp_secret.clone(),
...
// 1. ExactMatch on AUTHORITY_HASH (non-optional ie, each DiceChainEntry must have it).
// 2. ExactMatch on KEY_MODE (non-optional) - Secret should be inaccessible if any of the boot image
