...
fn dice_derivation(verified_data: MicrodroidData, payload_config_path: &str) -> Result<()> {
    ...
    diced
        .demoteSelf(&[InputValues {
            ...
            // TODO: figure out policy for debug mode
            // TODO: "1", "y", "yes", "on", or "true" are all positive booleans
            mode: if system_properties::read("ro.boot.adb.enabled")? == "1" {
                Mode::DEBUG
            } else {
                Mode::NORMAL
            },
            hidden: [0; 64], // TODO: a random value but no 100% necessary?
        }])
