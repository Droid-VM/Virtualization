...
fn verify_payload(
    ...
    let extra_apks: Vec<_> =
        sorted(glob(EXTRA_APK_PATH_PATTERN)?.map(|x| x.expect("Failed to read extra APKs")))
            .collect();
    let extra_idsigs: Vec<_> =
        sorted(glob(EXTRA_IDSIG_PATH_PATTERN)?.map(|x| x.expect("Failed to read extra idsigs")))
            .collect();
    ...
    let (extra_apk_names, extra_root_hashes_from_idsig): (Vec<_>, Vec<_>) = extra_idsigs
        ...
        .map(|(i, extra_idsig)| {
            (
                format!("extra-apk-{}", i),
                get_apk_root_hash_from_idsig(extra_idsig.to_str().unwrap()).unwrap(),
            )
        })
        .unzip();
    ...
    let extra_root_hashes_trustful: Vec<_> = extra_root_hashes_from_idsig
        ...
        .map(|(i, root_hash_from_idsig)| {
            let saved_root_hash =
                saved_extra_apks_data.and_then(|d| d.get(i)).map(|d| &d.root_hash);
            saved_root_hash == Some(root_hash_from_idsig)
        })
        .collect();
