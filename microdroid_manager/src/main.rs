...
fn main() -> Result<()> {
    ...
    let saved_roothash = saved_roothash.as_ref().map(|h| h.as_ref());

    // Verify the payload before using it.
    let trusted_roothash = verify_payload(saved_roothash).context("Payload verification failed")?;
    if saved_roothash != Some(trusted_roothash.as_ref()) {
