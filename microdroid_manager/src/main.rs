...
fn try_run_payload(service: &Strong<dyn IVirtualMachineService>) -> Result<i32> {
    ...
    // Wait for encryptedstore to finish mounting the storage (if enabled) setting dev.bootcomplete
    // Reason being init stops uneventd after bootcomplete. Encryptedstore, however requires ueventd
