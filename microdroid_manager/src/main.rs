...
fn main() -> Result<()> {
    ...
    if let Err(err) = verify_payloads(&metadata) {
...
fn verify_payloads(metadata: &Metadata) -> Result<()> {
    // APEXes
    for apex in metadata.apexes.iter() {
        let path = format!("/dev/block/by-name/{}", &apex.partition_name);
        verify(&path).context(format!("failed to verify {}", &path))?;
    }
