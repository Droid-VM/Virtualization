...
// TODO: migrate into vsock crate
// https://github.com/rust-vsock/vsock-rs/issues/18
...
fn exec_task(task: &Task, service: &Strong<dyn IVirtualMachineService>) -> Result<()> {
    ...
    let stream = match VsockStream::connect_with_cid_port(VMADDR_CID_HOST, PORT_VIRT_SVC) {
        Ok(stream) => {
            ...
            Some(unsafe { File::from_raw_fd(stream.into_raw_fd()) })
        }
        Err(e) => {
    ...
    let mut child = build_command(task, stream)?.spawn()?;
...
fn build_command(task: &Task, stream: Option<File>) -> Result<Command> {
