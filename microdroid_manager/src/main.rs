...
const DM_VERITY_APK_NAME: &str = "microdroid-apk";
const DM_MOUNTED_APK_PATH: &str = "/dev/block/mapper/microdroid-apk";
const MICRODROID_APK_PATH: &str = "/dev/block/by-name/microdroid-apk";
const MICRODROID_APK_IDSIG_PATH: &str = "/dev/block/by-name/microdroid-apk-idsig";
...
fn verify_payload(
    ...
    let arguments = vec![ApkDmverityArgument {
        apk: PathBuf::from(MICRODROID_APK_PATH),
        idsig: PathBuf::from(MICRODROID_APK_IDSIG_PATH),
        name: String::from(DM_VERITY_APK_NAME),
    }];
    let mut apkdmverity_child = run_apkdmverity(&arguments)?;
