...
const LINKERCONFIG_READY_PROP: &str = "linkerconfig.ready";
...
fn main() -> Result<()> {
    ...
    let trusted_roothash = verify_payload(
        // Convert Option<Box<[u8]>> into Option<&[u8]>
        saved_roothash.as_ref().map(|h| h.as_ref()),
    )
    .context("Payload verification failed")?;
    if saved_roothash != Some(trusted_roothash.clone()) {
        info!("Updating APK roothash: {}", to_hex_string(trusted_roothash.as_ref()));
        // TODO(jiyong): the data should have an internal structure.
        instance
            .write_microdroid_data(trusted_roothash.as_ref())
            .context("Failed to write identity data")?;
    } else {
        info!("Saved roothash is trustful. Not updating");
    }

    wait_for_linkerconfig_ready()?;
...
// trusted roothash that can be saved to the instance disk.
...
fn wait_for_linkerconfig_ready() -> Result<()> {
    let mut prop = PropertyWatcher::new(LINKERCONFIG_READY_PROP)?;
    loop {
        prop.wait()?;
        let val = system_properties::read(LINKERCONFIG_READY_PROP)?;
        if val == "true" {
            break;
        }
    }
