...
fn dice_derivation(
    ...
    let mut config_desc = vec![
        ...
        0x3a, 0x00, 0x01, 0x15, 0x57, // -71000
    ];
    ...
    match payload_metadata {
        ...
        PayloadMetadata::config(payload_config) => {
            let mut config_hash = Sha512::new();
            config_hash.update(&[if payload_config.export_tombstones { 1 } else { 0 }]);
            config_hash.update(payload_config.payload_binary_path.as_bytes());
            let config_hash = config_hash.finish();
            encode_header(2, config_hash.len().try_into().unwrap(), &mut config_desc)?;
            config_desc.extend_from_slice(&config_hash);
    ...
    match payload_metadata {
        ...
        PayloadMetadata::config(payload_config) => {
            ...
            Ok(VmPayloadConfig {
                ...
                extra_apks: vec![],
                prefer_staged: false,
                export_tombstones: payload_config.export_tombstones,
                enable_authfs: false,
            })
        }
    }
