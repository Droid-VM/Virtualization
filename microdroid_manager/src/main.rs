...
use microdroid_metadata::{write_metadata, Metadata, Payload};
...
fn dice_derivation(
    ...
    let config_descriptor = format_payload_config_descriptor(payload)?;
...
fn load_config(payload: Payload) -> Result<VmPayloadConfig> {
    match payload {
        ...
        _ => bail!("Failed to match config against as config type.")
    }
}
