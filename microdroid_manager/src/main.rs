...
const DEBUG_MICRODROID_IGNORE_IDENTITY: &str =
    "/sys/firmware/devicetree/base/virtualization/guest/debug-microdroid,ignore-identity";
...
fn is_identity_ignored() -> bool {
    Path::new(DEBUG_MICRODROID_IGNORE_IDENTITY).exists()
}
...
fn try_run_payload(service: &Strong<dyn IVirtualMachineService>) -> Result<i32> {
    ...
    // when the payload is changed. This is to keep the device secret same as before.
    let verified_data = if let Some(saved_data) = saved_data {
        if is_identity_ignored() && saved_data != verified_data {
