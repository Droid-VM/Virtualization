...
fn is_debuggable() -> Result<bool> {
    ...
    // TODO: "1", "y", "yes", "on", or "true" are all positive booleans.
    let adb = system_properties::read(ADBD_ENABLED_PROP)? == "1";
    let logd = system_properties::read(LOGD_ENABLED_PROP)? == "1";
    let debuggable = system_properties::read(DEBUGGABLE_PROP)? == "1";
...
fn try_run_payload(service: &Strong<dyn IVirtualMachineService>) -> Result<i32> {
    ...
    // TODO(b/215747811): remove after migrating from keystore to diced
