...
fn verify_payload(
    ...
    // Use the salt from a verified instance, or a default one as is_new_instance() keeps us in
    // sync with new salts being generated by previous stages, affecting our CDIs.
    let salt = saved_data.map_or_else(|s| s.salt.clone(), vec![0u8; 64]);
