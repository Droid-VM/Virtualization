...
impl fmt::Display for MicrodroidError {
...
fn try_main() -> Result<()> {
    ...
    if let Err(err) = try_main2(&service) {
...
fn try_main2(service: &Strong<dyn IVirtualMachineService>) -> Result<()> {
...
fn verify_payload(
    ...
    if let Some(saved_data) = saved_data.map(|d| &d.apex_data) {
        // We don't support APEX updates. (assuming that update will change root digest)
            ...
            MicrodroidError::PayloadChanged(String::from("APEX payloads has changed."))
        );
