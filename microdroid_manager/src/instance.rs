...
/// Size of a header in the instance disk
...
struct PartitionHeader {
    uuid: Uuid,
    header_offset: u64, // in bytes. should be multiple of 512
...
impl InstanceDisk {
    ...
    pub fn read_microdroid_data(&mut self) -> Result<Option<Box<[u8]>>> {
        let header = self.locate_microdroid_header()?;
        if header.payload_size == 0 {
            return Ok(None);
        }

        // Read the encrypted payload
        let payload_offset = header.header_offset + HEADER_SIZE;
        ...
        let key = get_key();
        let mut key = OpeningKey::new(UnboundKey::new(&AES_256_GCM, &key)?, NonceGen::new());
        let plaintext_len = key.open_in_place(Aad::from(&header), &mut data)?.len();
    ...
    pub fn write_microdroid_data(&mut self, data: &[u8]) -> Result<()> {
        ...
        // the encrypted payload which will be written.
        let payload_size = (data.len() + AES_256_GCM.tag_len()) as u64;

        // If the partition was empty, write the header. If not, make sure we don't change the
        // partition size.
        if header.payload_size == 0 {
            let microdroid_uuid = Uuid::parse_str(MICRODROID_PARTITION_UUID)?;
            self.file.seek(SeekFrom::Start(header.header_offset))?;
            self.file.write_all(microdroid_uuid.as_bytes())?;
            self.file.write_u64::<LittleEndian>(payload_size)?;
        } else if header.payload_size != payload_size {
        ...
        // Encrypt and sign the data. The non-encrypted input data is copied to a vector because
        // it is encrypted in place, and also the tag is appended.
        let key = get_key();
        let mut key = SealingKey::new(UnboundKey::new(&AES_256_GCM, &key)?, NonceGen::new());
        let mut data = data.to_vec();
        key.seal_in_place_append_tag(Aad::from(&header), &mut data)?;

        // Persist the encrypted payload data
        self.file.seek(SeekFrom::Start(header_offset + HEADER_SIZE))?;
        self.file.write_all(&data)?;

        Ok(())
    }

    /// Read a block at `header_offset` and parse it into a `PartitionHeader`.
    fn read_header_at(&mut self, header_offset: u64) -> Result<PartitionHeader> {
        ...
        Ok(PartitionHeader { uuid, header_offset, payload_size })
    }

    /// Locate the header of the partition for microdroid manager.
    fn locate_microdroid_header(&mut self) -> Result<PartitionHeader> {
        let microdroid_uuid = Uuid::parse_str(MICRODROID_PARTITION_UUID)?;

        let mut header_offset = HEADER_SIZE; // the first partition header
        loop {
            ...
            if header.uuid == microdroid_uuid {
                ...
                return Ok(PartitionHeader {
                    uuid: microdroid_uuid,
                    header_offset,
                    payload_size: 0,
                });
            }
            // Move to the next partition. Be careful about overflow.
            let payload_size = round_to_multiple(header.payload_size, HEADER_SIZE);
...
fn round_to_multiple(n: u64, unit: u64) -> u64 {
    (n + unit - 1) & !(unit - 1)
}

/// Nonce generator that is used when decrypting and encrypting the payload data. Currently, it's a
/// simple counter-based generator.
/// TODO(jiyong): do we need a better nonce generator?
struct NonceGen {
    count: u8,
}
...
impl NonceSequence for NonceGen {
    fn advance(&mut self) -> Result<Nonce, Unspecified> {
        self.count += 1;
        Ok(Nonce::assume_unique_for_key([self.count; 12]))
    }
}
...
fn get_key() -> [u8; 32] {
    ...
    let salt = Salt::new(HKDF_SHA256, &SEALING_CDI); // Sealing CDI as IKM
    let prk = salt.extract(&[]); // use 0 as salt
