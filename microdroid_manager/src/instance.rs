...
impl InstanceDisk {
    ...
    pub fn read_microdroid_data(&mut self) -> Result<Option<Box<[u8]>>> {
        ...
        let plaintext_len =
            get_key().as_ref().open_in_place(nonce, Aad::from(&header), &mut data)?.len();
    ...
    pub fn write_microdroid_data(&mut self, data: &[u8]) -> Result<()> {
        ...
        get_key().as_ref().seal_in_place_append_tag(nonce, Aad::from(&header), &mut data)?;
...
struct DropOnZeroKey(LessSafeKey);

impl Drop for DropOnZeroKey {
    fn drop(&mut self) {
...
impl AsRef<LessSafeKey> for DropOnZeroKey {
    fn as_ref(&self) -> &LessSafeKey {
        &self.0
    }
}
...
fn get_key() -> DropOnZeroKey {
    ...
    // this is safe enough.
    DropOnZeroKey(LessSafeKey::new(UnboundKey::new(ENCRYPT_ALG, &key).unwrap()))
}
