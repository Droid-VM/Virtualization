...
impl InstanceDisk {
    ...
    pub fn read_microdroid_data(&mut self) -> Result<Option<Box<[u8]>>> {
        ...
        // Read the 8 bytes seed (unencrypted)
        let mut seed = [0; 8];
        self.file.read_exact(&mut seed)?;
        let seed = u64::from_le_bytes(seed);

        // Read the encrypted payload
        let payload_size = header.payload_size - 8; // we already have read the seed
        ...
        let key = get_key();
        let mut key = OpeningKey::new(UnboundKey::new(ENCRYPT_ALG, &key)?, NonceGen::new(seed));
    ...
    pub fn write_microdroid_data(&mut self, data: &[u8]) -> Result<()> {
        ...
        // the encrypted payload which will be written. In addition, a 8byte seed will be prepended
        // (non-encrypted).
        let payload_size = (data.len() + ENCRYPT_ALG.tag_len() + 8) as u64;
        ...
        let mut key = SealingKey::new(UnboundKey::new(ENCRYPT_ALG, &key)?, NonceGen::new(seed));
        let mut data = data.to_vec();
        key.seal_in_place_append_tag(Aad::from(&header), &mut data)?;

        // Persist the encrypted payload data along with the seed
...
/// simple counter-based generator where seed is expected to be randomized
struct NonceGen {
    nonce: u64,
    count: u64,
}
...
impl NonceGen {
    fn new(seed: u64) -> Self {
        NonceGen { nonce: seed, count: 0 }
    }
}

impl NonceSequence for NonceGen {
    fn advance(&mut self) -> Result<Nonce, Unspecified> {
        // Increment the count and check for overflow to ensure that the sequence is unique
        self.count = self.count.checked_add(1).ok_or(Unspecified)?;
        self.nonce += 1; // overflow here is okay

        // Expand the 8 bytes nonce into 12 bytes by padding zeros and return it.
        let mut val = Vec::from(self.nonce.to_be_bytes());
        val.resize(12, 0);
        Ok(Nonce::assume_unique_for_key(val.try_into().unwrap()))
    }
}
