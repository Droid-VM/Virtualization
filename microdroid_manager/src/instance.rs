...
/// Encrytion algorithm used to cipher payload
...
impl InstanceDisk {
    ...
    pub fn read_microdroid_data(&mut self) -> Result<Option<Box<[u8]>>> {
        ...
        let key = get_key();
        let mut key = OpeningKey::new(UnboundKey::new(ENCRYPT_ALG, &key)?, NonceGen::new());
    ...
    pub fn write_microdroid_data(&mut self, data: &[u8]) -> Result<()> {
        ...
        let payload_size = (data.len() + ENCRYPT_ALG.tag_len()) as u64;
        ...
        let key = get_key();
        let mut key = SealingKey::new(UnboundKey::new(ENCRYPT_ALG, &key)?, NonceGen::new());
    ...
    fn locate_microdroid_header(&mut self) -> Result<(PartitionHeader, PartitionOffset)> {
        ...
        loop {
            ...
            if header.uuid == microdroid_uuid {
                ...
                return Ok((header, header_offset));
            } else if header.uuid == Uuid::nil() {
                // found an empty space
                return Ok((
                    PartitionHeader { uuid: microdroid_uuid, payload_size: 0 },
                    header_offset,
                ));
            }
