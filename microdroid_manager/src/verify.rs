...
fn get_data_from_apk(
    ...
    let cert_hash = get_cert_hash_from_apk(apk_path, root_hash_trustful)?;
...
fn get_cert_hash_from_apk(apk: &str, root_hash_trustful: bool) -> Result<Box<[u8]>> {
    ...
    Ok(Box::new(sha512(signed_data.first_certificate_der()?)))
}
