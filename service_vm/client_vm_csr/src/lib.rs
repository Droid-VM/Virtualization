...
fn build_signed_data(
    ...
    let signed_data = CoseSignBuilder::new()
        ...
        .try_add_created_signature(attestation_key_sig_headers, aad, |message| {
            ecdsa_sign_nist(message, attestation_key)
        })?
        .build();
    Ok(signed_data)
}
...
fn ecdsa_sign_nist(message: &[u8], key: &EcKeyRef<Private>) -> Result<Vec<u8>> {
    ...
    ecda_sig_to_nist(&sig)
}

fn ecda_sig_to_nist(signature: &EcdsaSig) -> Result<Vec<u8>> {
...
mod tests {
    ...
    fn csr_and_private_key_have_correct_format() -> Result<()> {
        ...
        cose_sign
            ...
            .context("Verifying first signature")?;
        ...
        cose_sign
            .verify_signature(1, aad, |signature, message| {
                ecdsa_verify_nist(signature, message, &ec_public_key)
            })
            .context("Verifying second signature")?;

        // Verifies that private key and the public key form a valid key pair.
        let message = b"test message";
        let signature = ecdsa_sign_nist(message, &ec_private_key)?;
        ecdsa_verify_nist(&signature, message, &ec_public_key)
