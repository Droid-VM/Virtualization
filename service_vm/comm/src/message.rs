...
pub enum Request {
    ...
    RequestCertificate(RequestCertificateParams),
}

/// Represents the params passed to `Request::RequestCertificate`.
#[derive(Clone, Debug, Serialize, Deserialize)]
pub struct RequestCertificateParams {
    /// The CBOR-encoded CSR signed by the CDI_Leaf_Priv of the client VM's DICE chain
    /// and the private key to be attested.
    pub csr: Vec<u8>,

    /// The key blob retrieved from RKPD.
    pub remotely_provisioned_key_blob: Vec<u8>,

    /// Sequence of DER-encoded X.509 certificates that make up the remotely provisioned
    /// key's certificate chain retrieved from RKPD.
    pub remotely_provisioned_cert_chain: Vec<u8>,
}
...
pub enum Response {
    ...
    /// Returns a sequence of DER-encoded X.509 certificates that make up the attested key's
    /// certificate chain.
    ///
    /// The certificate chain starts with the root certificate that is trusted by Google and
    /// ends with the leaf certificate covering the attested public key. The leaf certificate
    /// also includes an extension that describes the attested client VM.
