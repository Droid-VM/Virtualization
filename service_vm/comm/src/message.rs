...
/// Represents the params passed to RequestCertificate
#[derive(Clone, Debug, Serialize, Deserialize)]
...
pub enum Response {
    ...
    /// Returns a Google-rooted X.509 encoded certificate chain, where the leaf
    /// certificate covers the attested public key specified in the CSR sent by
    /// the client VM. The leaf certificate also includes an extension that
    /// describes the attested client VM.
