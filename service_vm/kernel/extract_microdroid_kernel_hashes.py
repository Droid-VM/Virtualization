...
def main(args):
    ...
    print("pub const KERNEL_HASHES: &[&[u8]; " + str(num_kernel_images) +
          "] = &[" + to_rust_arrays(hashes[PARTITION_NAME_BOOT])
          + "\n];\n")
    print("pub const INITRD_NORMAL_HASHES: &[&[u8]; " + str(num_kernel_images) +
          "] = &[" + to_rust_arrays(hashes[PARTITION_NAME_INITRD_NORMAL])
          + "\n];\n")
    print("pub const INITRD_DEBUG_HASHES: &[&[u8]; " + str(num_kernel_images) +
          "] = &[" + to_rust_arrays(hashes[PARTITION_NAME_INITRD_DEBUG])
          + "\n];")

def to_rust_arrays(hex_strings: List[str]) -> str:
    return ",\n".join(f"&[{format_hex_string(h)}]" for h in hex_strings)
