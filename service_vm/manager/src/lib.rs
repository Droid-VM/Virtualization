...
lazy_static! {
    static ref NEW_REQUEST_NUM: NewRequestNum = NewRequestNum::default();
    static ref SERVICE_VM: Mutex<Option<ServiceVm>> = Mutex::new(None);
}

/// This struct wraps the number of new requests.
#[derive(Debug, Default)]
struct NewRequestNum {
    num: Mutex<usize>,
    condvar: Condvar,
}

impl NewRequestNum {
    fn is_zero_within_timeout(&self, timeout: Duration) -> bool {
        let (guard, _wait_request) = self
            .condvar
...
pub fn process_request(request: Request) -> Result<Response> {
    NEW_REQUEST_NUM.increment();
    let mut service_vm = SERVICE_VM.lock().unwrap();
    NEW_REQUEST_NUM.decrement();
...
fn stop_service_vm_if_idle() {
    if NEW_REQUEST_NUM.is_zero_within_timeout(Duration::from_secs(2)) {
