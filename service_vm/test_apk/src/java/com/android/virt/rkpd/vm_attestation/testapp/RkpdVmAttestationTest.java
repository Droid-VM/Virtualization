...
public class RkpdVmAttestationTest extends MicrodroidDeviceTestBase {
    ...
    private void checkAvfAttestationExtension(X509Certificate cert, byte[] challenge)
        ...
        assertExtensionContainsPayloadApk(payloadSeq);
    }

    private void assertExtensionContainsPayloadApk(ASN1Sequence payloadSeq) throws Exception {
        DERUTF8String payloadApk = new DERUTF8String("apk:" + TEST_APP_PACKAGE_NAME);
        boolean found = false;
        for (ASN1Encodable encodable : payloadSeq) {
            ASN1Sequence seq = ASN1Sequence.getInstance(encodable);
            assertThat(seq).hasSize(4);
            if (payloadApk.equals(seq.getObjectAt(0))) {
                assertWithMessage("Payload APK should not be found twice.").that(found).isFalse();
                found = true;
            }
        }
        assertWithMessage("The payload sequence should contain the payload APK.")
                .that(found)
                .isTrue();
    }
}
