...
public class RkpdVmAttestationTest extends MicrodroidDeviceTestBase {
    ...
    private void checkAvfAttestationExtension(X509Certificate cert, byte[] challenge)
        ...
        // The VM is unsecure as it is debuggable.
        ASN1Boolean expectedIsVmSecure = ASN1Boolean.FALSE;
        assertThat(seq.getObjectAt(1)).isEqualTo(expectedIsVmSecure);

        ASN1Sequence payloadSeq = ASN1Sequence.getInstance(seq.getObjectAt(2));
        assertExtensionContainsCurrentApk(payloadSeq);
    }

    private void assertExtensionContainsCurrentApk(ASN1Sequence payloadSeq) throws Exception {
        DERUTF8String currentApk = new DERUTF8String("apk:" + TEST_APP_PACKAGE_NAME);
        boolean found = false;
        for (ASN1Encodable encodable : payloadSeq) {
            ASN1Sequence seq = ASN1Sequence.getInstance(encodable);
            assertThat(seq.size()).isEqualTo(4);
            if (currentApk.equals(seq.getObjectAt(0))) {
                assertThat(found).isFalse();
                found = true;
            }
        }
        assertWithMessage("The payload sequence should contain the current APK.")
                .that(found)
                .isTrue();
    }
}
