...
impl AttestationResult {
    ...
    fn certificate_chain(&self) -> Result<Box<[u8]>> {
...
fn get_certificate_chain(res: &AVmAttestationResult) -> Result<Box<[u8]>> {
    let size =
    ...
    ensure!(size == cert_chain.len());
    Ok(cert_chain.into_boxed_slice())
}
