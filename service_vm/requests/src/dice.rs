...
impl ClientVmDiceChain {
    /// Validates the integrity of the `client_vm_dice_chain` by performing the following checks:
    ///
    ...
    pub(crate) fn validate_integrity(
        ...
        // After successfully calling `verify_client_vm_dice_chain_prefix_match`, we can be certain
        // that the client VM's DICE chain must contain at least three entries that describe:
        // - pvmfw
        // - Microdroid kernel
        // - Apk/Apexes
        assert!(
...
fn to_mode(value: Value) -> Result<DiceMode> {
    ...
    let mode = match mode {
        x if x == (DiceMode::kDiceModeNormal as u8).into() => DiceMode::kDiceModeNormal,
