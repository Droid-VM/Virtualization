...
impl ClientVmDiceChain {
    /// Verifies the integrity of the `client_vm_dice_chain` by performing the following checks:
    ///
    /// - The `client_vm_dice_chain` must match the `service_vm_dice_chain` up to the pvmfw entry.
    ...
    pub(crate) fn verify(
        ...
        verify_client_vm_dice_chain_prefix_match(&client_vm_dice_chain, &service_vm_dice_chain)?;
        ...
        for (i, value) in client_vm_dice_chain.into_iter().enumerate() {
            let payload = DiceChainEntryPayload::verify_cose_signature_and_extract_payload(
                value,
                previous_public_key,
            )
            .map_err(|e| {
...
fn verify_client_vm_dice_chain_prefix_match(
...
impl DiceChainEntryPayload {
    ...
    fn verify_cose_signature_and_extract_payload(
        value: Value,
...
fn to_mode(value: Value) -> Result<Integer> {
    match value {
        ...
        v => Err(CoseError::UnexpectedItem(cbor_value_type(&v), "bstr or int").into()),
    }
}
...
impl PayloadBuilder {
    ...
    fn mode(&mut self, mode: Integer) -> Result<()> {
    ...
    fn build(mut self) -> Result<DiceChainEntryPayload> {
        ...
        // If Mode is omitted, it should be treated as if it was NotConfigured, according to
        // the Open Profile for DICE spec.
        let mode = self.mode.take().unwrap_or((DiceMode::kDiceModeNotInitialized as i64).into());
        let is_secure_mode = mode == (DiceMode::kDiceModeNormal as i64).into();
