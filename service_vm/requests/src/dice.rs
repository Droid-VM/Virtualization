...
/// Represents a partially decoded `DiceCertChain`. The whole chain is defined as following:
///
/// DiceCertChain = [
...
impl Chain {
    /// Verifies `client_vm_dice_chain` against the provided `service_vm_dice_chain` and creates
    /// a partially decoded DICE chain from the client VM's DICE chain.
    pub(crate) fn verify_cbor_slice(
        ...
        // The client VM's DICE chain must contain at least three payloads after the validation:
        // - pvmfw payload
        // - Microdroid kernel payload
        // - Apk/Apexes payload
        if payloads.len() < 3 {
            error!("The client VM's DICE chain must contain at least three payloads");
...
fn verify_dice_chain_up_to_pvmfw_payload(
    ...
    if service_vm_dice_chain.len() < 3 {
        ...
        //   - The root public key
        //   - The pvmfw payload
        //   - The service VM payload
        error!("The service VM DICE chain must contain at least three entries");
        return Err(RequestProcessingError::InternalError);
    }
    // Ignores the last payload that describes service VM
...
impl PublicKey {
    ...
    pub(crate) fn verify(&self, _signature: &[u8], _message: &[u8]) -> Result<()> {
        // TODO(b/310931749): Implement the verification function for different types of keys.
        Ok(())
    }
}
...
impl Payload {
    /// Verifies the signature of the provided CBOR value with the provided public key and creates
    /// a partially decoded payload from the CBOR value.
...
fn build_payload(entries: Vec<(Value, Value)>) -> Result<Payload> {
    ...
    for (key, value) in entries.into_iter() {
        ...
        match key {
            ...
            MODE => builder.is_secure_mode(is_secure_mode(value)?)?,
...
fn is_secure_mode(value: Value) -> Result<bool> {
    let mode = match value {
        ...
        Value::Integer(i) => i,
        v => {
            return Err(CoseError::UnexpectedItem(cbor_value_type(&v), "bstr or int").into());
        }
    };
    Ok(mode == (DiceMode::kDiceModeNormal as i64).into())
}

#[derive(Default, Debug, Clone)]
struct PayloadBuilder {
    subject_public_key: OnceCell<PublicKey>,
    is_secure_mode: OnceCell<bool>,
...
impl PayloadBuilder {
    ...
    fn is_secure_mode(&mut self, is_secure_mode: bool) -> Result<()> {
        set_once(&self.is_secure_mode, is_secure_mode, "is_secure_mode")
    }
    ...
    fn build(mut self) -> Result<Payload> {
        ...
        let is_secure_mode = take_value(&mut self.is_secure_mode, "is_secure_mode")?;
