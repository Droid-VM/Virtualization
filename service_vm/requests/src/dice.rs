...
pub(crate) fn validate_client_vm_dice_chain_prefix_match(
    ...
    let client_vm_dice_chain = Value::from_slice(client_vm_dice_chain)?
        .into_array()
        .map_err(|e| to_unexpected_item_error(&e, "array", "client_vm_dice_chain"))?;
    let service_vm_dice_chain = Value::from_slice(service_vm_dice_chain)?
        .into_array()
        .map_err(|e| to_unexpected_item_error(&e, "array", "service_vm_dice_chain"))?;
...
fn build_payload(entries: Vec<(Value, Value)>) -> Result<DiceChainEntryPayload> {
    ...
    for (key, value) in entries.into_iter() {
        ...
        match key {
            ...
            CODE_HASH => builder.code_hash(value_to_byte_array(value, "code_hash")?)?,
            AUTHORITY_HASH => {
                builder.authority_hash(value_to_byte_array(value, "authority_hash")?)?
            }
            CONFIG_DESC => {
