...
impl Chain {
    ...
    pub fn leaf(&self) -> &Payload {
        // There is always at least one payload as checked when building the chain.
...
fn verify_dice_chain_up_to_pvmfw_payload(
    ...
    if entries_up_to_pvmfw != &client_vm_dice_chain[0..entries_up_to_pvmfw.len()] {
        error!(
            "The client VM's DICE chain does not match service VM's DICE chain up to the pvmfw payload"
        );
        return Err(RequestProcessingError::InvalidDiceChain);
    }
    Ok(())
