...
pub(crate) struct ClientVmDiceChain {
    ...
    /// The index of the vendor module entry in the DICE chain if it exists.
    vendor_module_index: Option<usize>,
...
impl ClientVmDiceChain {
    ...
    pub(crate) fn validate_signatures_and_parse_dice_chain(
        ...
        let has_vendor_module =
            vendor_module_exists(client_vm_dice_chain.len(), service_vm_dice_chain_len)?;
        ...
        Self::build(payloads, has_vendor_module)
    }

    fn build(
        dice_entry_payloads: Vec<DiceChainEntryPayload>,
        has_vendor_module: bool,
    ) -> Result<Self> {
        ...
        if MICRODROID_PAYLOAD_COMPONENT_NAME != microdroid_payload_name {
            error!(
                "The microdroid payload entry in the client VM DICE chain must describe the \
                    Microdroid payload. Got '{}'.",
                microdroid_payload_name,
            );
            return Err(RequestProcessingError::InvalidDiceChain);
        }

        let (vendor_module_index, kernel_index) = if has_vendor_module {
            let index = dice_entry_payloads.len() - 2;
            let vendor_module_name = &dice_entry_payloads[index].config_descriptor.component_name;
        ...
        if KERNEL_COMPONENT_NAME != kernel_name {
            error!(
                "The microdroid kernel entry in the client VM DICE chain must describe the Microdroid \
                 kernel. Got '{}'.",
                    kernel_name,
            );
            return Err(RequestProcessingError::InvalidDiceChain);
        ...
        Ok(Self { payloads: dice_entry_payloads, vendor_module_index, kernel_index })
    }
    ...
    pub(crate) fn vendor_module(&self) -> Option<&DiceChainEntryPayload> {
        self.vendor_module_index.map(|i| &self.payloads[i])
    }
...
fn vendor_module_exists(
    ...
    //    + Vendor module entry (exists only when the vendor module is present)
    ...
    match client_vm_dice_chain_len.checked_sub(entries_up_to_pvmfw_len) {
        Some(2) => {
            debug!("The vendor module entry is not present in the client VM's DICE chain");
            Ok(false)
        }
        Some(3) => {
            info!("The vendor module entry is present in the client VM's DICE chain");
            Ok(true)
        }
        _ => {
            error!(
