...
impl ClientVmDiceChain {
    ...
    pub(crate) fn validate_signatures_and_parse_dice_chain(
        ...
        info!("Vendor module offset: {}", vendor_module_offset);
...
/// entry.
///
/// Returns `Ok(true)` if the verification succeeds and the vendor module entry is present in the
/// client VM's DICE chain. Otherwise, returns `Ok(false)` if the vendor module entry is not
/// present.
pub(crate) fn validate_client_vm_dice_chain_prefix_match(
    ...
    let vendor_module_exists =
        match client_vm_dice_chain.len().checked_sub(entries_up_to_pvmfw.len()) {
            Some(2) => false,
            Some(3) => {
