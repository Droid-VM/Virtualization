...
impl ClientVmDiceChain {
    ...
    fn validate_microdroid_components_names(&self) -> Result<()> {
        ...
        if MICRODROID_KERNEL_COMPONENT_NAME != microdroid_kernel_name {
            error!("The second to last entry in the client VM DICE chain must describe the Microdroid kernel. Got {}", microdroid_kernel_name);
        ...
        if MICRODROID_PAYLOAD_COMPONENT_NAME != microdroid_payload_name {
            error!("The last entry in the client VM DICE chain must describe the Microdroid payload. Got {}", microdroid_payload_name);
            return Err(RequestProcessingError::InvalidDiceChain);
        }
        Ok(())
    }
...
impl SubComponentBuilder {
    ...
    fn build(mut self) -> Result<SubComponent> {
        let name = take_value(&mut self.name, "name")?;
        let version = take_value(&mut self.version, "version")?;
...
fn try_as_num<T: TryFrom<Integer>>(v: Value, context: &'static str) -> Result<T> {
    if let Value::Integer(data) = v {
        T::try_from(data).map_err(|_| {
            error!("The provided value '{context}' is not a valid number");
            RequestProcessingError::InvalidDiceChain
        })
    } else {
