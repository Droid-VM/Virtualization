...
use alloc::vec::Vec;
use bssl_avf::{ed25519_verify, get_label_value, get_label_value_as_bytes, Digester, EcKey, PKey};
    ...
    pub(crate) fn verify(&self, signature: &[u8], message: &[u8]) -> Result<()> {
        match &self.0.kty {
            KeyType::Assigned(iana::KeyType::EC2) => {
                let pkey: PKey = EcKey::from_cose_public_key(&self.0)?.try_into()?;
                ...
                Ok(pkey.verify(signature, message, Some(digester))?)
            }
            KeyType::Assigned(iana::KeyType::OKP) => {
                ...
                let public_key = x.try_into().map_err(|e| {
                    error!("Invalid ED25519 public key size: {:?}", e);
                    RequestProcessingError::InvalidDiceChain
                })?;
                let signature = signature.try_into().map_err(|e| {
                    error!("Invalid ED25519 signature size: {:?}", e);
                    RequestProcessingError::InvalidDiceChain
                })?;
