...
mod tests {
    ...
    #[test]
    fn decrypting_encrypted_keyblob_succeeds_with_the_same_kek() -> Result<()> {
        let mut rng = FakeRng::default();
        let encrypted_key_blob = EncryptedKeyBlob::new(&TEST_KEY, &TEST_SECRET1, &mut rng)?;
    ...
    #[test]
    fn decrypting_encrypted_keyblob_fails_with_a_different_kek() -> Result<()> {
        let mut rng = FakeRng::default();
        let encrypted_key_blob = EncryptedKeyBlob::new(&TEST_KEY, &TEST_SECRET1, &mut rng)?;
