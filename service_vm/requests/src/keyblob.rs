...
impl EncryptedKeyBlob {
    pub(crate) fn new<R: Rng>(private_key: &[u8], kek_secret: &[u8], rng: &mut R) -> Result<Self> {
        let mut kek_salt = [0u8; 32];
        rng.fill_bytes(&mut kek_salt).map_err(|e| {
    ...
    #[cfg(test)]
    pub(crate) fn from_cbor(slice: &[u8]) -> coset::Result<Self> {
        from_cbor(slice)
    }

    pub(crate) fn into_cbor(self) -> coset::Result<Vec<u8>> {
        to_cbor(&self)
    }
}
...
mod tests {
    ...
    impl Rng for FakeRng {
        ...
        fn fill_bytes(&mut self, dest: &mut [u8]) -> result::Result<(), ()> {
            let mut current_byte = self.0;
    ...
    fn decrypting_keyblob_succeeds_with_the_same_kek() -> Result<()> {
        ...
        let encrypted_key_blob =
            EncryptedKeyBlob::new(&TEST_KEY, &TEST_SECRET1, &mut rng)?.into_cbor()?;
        let encrypted_key_blob = EncryptedKeyBlob::from_cbor(&encrypted_key_blob)?;
    ...
    #[test]
    fn decrypting_corrupted_keyblob_fails() -> Result<()> {
        let mut rng = FakeRng::default();
        let mut encrypted_key_blob =
            EncryptedKeyBlob::new(&TEST_KEY, &TEST_SECRET1, &mut rng)?.into_cbor()?;
        encrypted_key_blob[15] = !encrypted_key_blob[15];
        let encrypted_key_blob = EncryptedKeyBlob::from_cbor(&encrypted_key_blob)?;
        let err = encrypted_key_blob.decrypt_private_key(&TEST_SECRET2).unwrap_err();
