...
use crate::util::{from_cbor, to_cbor};
...
pub(super) fn generate_ecdsa_p256_key_pair<R: Rng>(
    ...
    let key_pair = EcdsaP256KeyPair { maced_public_key, key_blob: key_blob.into_cbor()? };
    Ok(key_pair)
}
...
pub(super) fn generate_certificate_request(
    ...
    let dice_cert_chain: Value =
        dice_artifacts.bcc().map(from_cbor).ok_or(RequestProcessingError::MissingDiceChain)??;
    let auth_req = cbor!([
