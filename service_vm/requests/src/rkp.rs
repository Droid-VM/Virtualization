...
pub(super) fn generate_ecdsa_p256_key_pair<R: Rng>(
    ...
    let key_pair = EcdsaP256KeyPair { maced_public_key, key_blob: key_blob.into_cbor_vec()? };
    Ok(key_pair)
}
...
pub(super) fn generate_certificate_request(
    ...
    let csr_payload = cbor::to_vec(&csr_payload)?;
    ...
    let dice_cert_chain: Value = cbor::from_slice(dice_cert_chain)?;
    let auth_req = cbor!([
