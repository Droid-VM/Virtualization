...
pub(crate) fn to_cbor<T: ?Sized + Serialize>(v: &T) -> Result<Vec<u8>> {
...
pub(crate) fn from_cbor<T: DeserializeOwned>(mut data: &[u8]) -> Result<T> {
    let value = ciborium::from_reader(&mut data)?;
    if data.is_empty() {
        Ok(value)
    } else {
