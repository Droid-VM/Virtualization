...
use log::error;
use service_vm_comm::{RequestCertificateParams, RequestProcessingError};

type Result<T> = result::Result<T, RequestProcessingError>;

pub(super) fn request_certificate(
    params: RequestCertificateParams,
    dice_artifacts: &dyn DiceArtifacts,
) -> Result<Vec<u8>> {
    // TODO(b/303807447): Verify the signatures in the csr.
    ...
    // TODO(b/293871876): Extend remotely_provisioned_cert_chain and returns the new certificate
    // chain once all the verification succeeds.
    let res = params.remotely_provisioned_cert_chain;
    Ok(res)
}
