...
pub(super) fn request_attestation(
    ...
    cose_sign.verify_signature(DICE_CDI_LEAF_SIGNATURE_INDEX, aad, |signature, message| {
        // It is safe to unwrap here because the DICE chain must contain at least three payloads.
        let leaf = client_vm_dice_chain.payloads.last().unwrap();
        leaf.subject_public_key.verify(signature, message)
    })?;
    ...
    let attestation_ext = cert::AttestationExtension::new(
        ...
        client_vm_dice_chain.is_secure_mode(),
    )
    .to_vec()?;
