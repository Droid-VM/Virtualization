...
/// Validates the client VM DICE chain against the the reference service VM DICE chain and
/// the vendor_hashtree_root_digest from the device tree.
///
...
fn validate_client_vm_dice_chain(
    ...
    validate_vendor_module_code_hash_if_exists(&client_vm_dice_chain, vendor_hashtree_root_digest)?;
...
fn validate_vendor_module_code_hash_if_exists(
    client_vm_dice_chain: &ClientVmDiceChain,
    vendor_hashtree_root_digest: Option<&[u8]>,
) -> Result<()> {
    let Some(vendor_module) = client_vm_dice_chain.vendor_module() else {
        debug!("The vendor module is not present in the Client VM DICE chain");
        return Ok(());
    };
