...
pub(super) fn request_attestation(
    ...
    cose_sign.verify_signature(DICE_CDI_LEAF_SIGNATURE_INDEX, aad, |signature, message| {
        // The client VM DICE chain contains at least three entries.
        let leaf_certificate = client_vm_dice_chain.payloads.last().unwrap();
        leaf_certificate.subject_public_key.verify(signature, message)
    })?;
