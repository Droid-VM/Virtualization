...
//! Provides functions to build a test chain for client VM in end-to-end tests.
...
/// Generates a fake DICE artifacts for client VM.
pub fn fake_client_vm_dice_artifacts() -> Result<OwnedDiceArtifacts> {
    ...
    // the pvmfw entry.
    let dice_artifacts = service_vm::fake_dice_artifacts_up_to_pvmfw()?;
    ...
    let input_values = InputValues::new(
        ...
        service_vm::AUTHORITY_HASH_SERVICE_VM,
    ...
    // Adds an entry describing the Microdroid payloads.
    ...
    .map_err(|e| {
        error!("Failed to run BCC main flow for Microdroid: {e}");
        e
    })
}
...
pub fn fake_sub_components() -> Vec<Subcomponent> {
    vec![
        Subcomponent {
            name: "com.android.apk.apk1".to_string(),
            version: 1,
            code_hash: APK1_CODE_HASH.to_vec(),
            authority_hash: APK1_AUTHORITY_HASH.to_vec(),
        },
        Subcomponent {
            name: "com.android.apex.apex1".to_string(),
            version: 1,
