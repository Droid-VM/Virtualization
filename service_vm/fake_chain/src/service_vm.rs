...
//! Provides functions to build a test chain for non-protected rialto.
...
pub(crate) fn fake_dice_artifacts_up_to_pvmfw() -> Result<OwnedDiceArtifacts> {
    ...
    let bcc = dice_chain.to_vec().map_err(|e| {
        error!("Failed to serialize the DICE chain to CBOR data: {e}");
        DiceError::InvalidInput
    })?;
    Ok(OwnedDiceArtifacts { cdi_values, bcc })
}

/// Generates a fake DICE artifacts with a DICE chain up to the certificate
/// describing service_vm.
///
...
pub fn fake_service_vm_dice_artifacts() -> Result<OwnedDiceArtifacts> {
    let dice_artifacts = fake_dice_artifacts_up_to_pvmfw()?;
    let config_values = DiceConfigValues {
