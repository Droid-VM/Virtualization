...
use cstr::cstr;
use diced_open_dice::{
    derive_cdi_private_key_seed, hash, keypair_from_seed, retry_bcc_format_config_descriptor,
    retry_bcc_main_flow, retry_dice_main_flow, CdiValues, Config, DiceConfigValues, DiceError,
    DiceMode, Hash, InputValues, OwnedDiceArtifacts, Result, CDI_SIZE, HASH_SIZE, HIDDEN_SIZE,
};
use log::error;
...
pub(crate) fn authority_hash_service_vm() -> Result<Hash> {
    hash(pvmfw_embedded_key::PUBLIC_KEY)
}
