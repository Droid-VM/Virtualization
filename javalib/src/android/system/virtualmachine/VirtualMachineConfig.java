...
public final class VirtualMachineConfig {
    ...
    public static class Builder {
        ...
        public @NonNull VirtualMachineConfig build() throws IllegalArgumentException {
            ...
            if (!mCpuAffinity.isEmpty()
                    && !Pattern.matches("[\\d]+(,[\\d]+(-[\\d]+)?)*", mCpuAffinity)
                    && !Pattern.matches("[\\d]+=[\\d]+(:[\\d]+=[\\d]+)*", mCpuAffinity)) {
                throw new IllegalArgumentException("CPU affinity " + mCpuAffinity + " is invalid");
            }
