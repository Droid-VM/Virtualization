...
use android_system_virtmanager::aidl::android::system::virtmanager::Partition::Partition as AidlPartition;
...
pub fn make_composite_image(
...
) -> Result<(File, Vec<FdMapping>), Error> {
    ...
    command
        ...
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    command.fd_mappings(fd_mappings.clone())?;
    ...
    Ok((composite_image, fd_mappings))
}
