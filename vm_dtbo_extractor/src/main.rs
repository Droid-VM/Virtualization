...
const DTBO_PATH_PREFIX: &str = "/dev/block/by-name/dtbo";
...
impl DtTableEntry {
    fn new(values: &[u32]) -> Result<DtTableEntry> {
        ...
        let mut custom = [0; 4];
        custom.copy_from_slice(&values[4..]);
...
fn get_dtbo_img_path() -> Result<String> {
    ...
    Ok(DTBO_PATH_PREFIX.to_string() + slot_suffix)
}
...
fn main() -> Result<()> {
    // TODO(291191362): Read DTBO partition
    ...
    let _dt_table_entries = get_dt_table_entries(&mut dtbo_img, &dt_table_header)?;
    Ok(())
}
