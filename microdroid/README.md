...
$ adb push $ANDROID_PRODUCT_OUT/system/etc/microdroid.cdisk /data/local/tmp
$ dd if=/dev/zero of=userdata.img bs=4k count=600
$ mkfs.ext4 -F userdata.img
$ dd if=/dev/zero of=metadata.img bs=4k count=4k
$ dd if=/dev/zero of=misc.img bs=4k count=256
$ adb push userdata.img metadata.img misc.img /data/local/tmp/
```
...
```
$ adb shell 'cd /data/local/tmp; /apex/com.android.virt/bin/mk_cdisk microdroid.cdisk os_header.img os_footer.img os_composite.img'
