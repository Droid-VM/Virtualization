...
microdroid. The above four are essential ones and therefore shouldn't be
omitteed. In the future, you wouldn't need to add the default ones manually. If
more APEXes are required for you app, add their names too.

Embedd the shared library and the VM configuration file in an APK:
...
`path_to_keystore` should be replaced with the actual path to the keystore. It
...
This is a temporary step that needs to be done manually for now. Eventually,
this will be done by a service named `virtualizationservice` which is part of
...
The disabling of SELinux is a temporary step. The restriction will eventually be
removed.
