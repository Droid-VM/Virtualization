...
// The microdroid vendor partition doesn't contribute to the debuggability of the VM, and it is a
// bit tricky to propagate the info on whether the VM is debuggable to derive_microdroid_dice_node
// binary. Given these, we just always set `is_debuggable: false` for the "Microdroid vendor" dice
// node. The adjacent dice nodes (pvmfw & payload) provide the accurate information on whether VM
// is debuggable.
const IS_DEBUGGABLE: bool = false;
...
fn dice_derivation(dice: DiceDriver, vbmeta: &VbMetaImage) -> Result<OwnedDiceArtifacts> {
    ...
    dice.derive(code_hash, &desc, authority_hash, IS_DEBUGGABLE, hidden)
}
