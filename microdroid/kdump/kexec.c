...
int main(int argc, const char* argv[]) {
    ...
    unsigned long cmdline_len = strlen(cmdline) + 1; // include null terminator

    long ret = syscall(SYS_kexec_file_load, open_checked(kernel), open_checked(initrd), cmdline_len,
                       cmdline, KEXEC_FILE_ON_CRASH);
    if (ret == -1) {
