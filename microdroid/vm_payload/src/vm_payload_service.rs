...
fn try_setup_stdio_proxy() -> Result<()> {
    ...
    dup2(fd.as_ref(), io::stdin().as_fd()).context("Failed to dup2 stdin")?;
    dup2(fd.as_ref(), io::stdout().as_fd()).context("Failed to dup2 stdout")?;
    dup2(fd.as_ref(), io::stderr().as_fd()).context("Failed to dup2 stderr")?;
    Ok(())
}
