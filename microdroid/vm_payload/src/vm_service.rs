...
use binder::{wait_for_interface, Strong, unstable_api::AIBinder};
use log::{error, info, Level};
use std::os::raw::c_void;

// The minimum vsock port number given by payload.
const MIN_VSOCK_PORT: u32 = 1024;
...
pub extern "C" fn AVmPayload_runVsockRpcServer(
    ...
    if port <= MIN_VSOCK_PORT {
        error!("The port '{}' should be bigger than {}.", port, MIN_VSOCK_PORT);
        return false;
    }
    // SAFETY: Service ownership is transferring to the server and won't be valid afterward.
    // The binder objects are threadsafe.
    unsafe { binder_rpc_unstable_bindgen::RunRpcServerCallback(service, port, on_ready, param) }
}
