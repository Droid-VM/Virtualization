...
```bash
tools/bazel clean
tools/bazel run --config=fast //common:microdroid_aarch64_dist -- --dist_dir=out/dist
```

For x86\_64,
```bash
tools/bazel clean
tools/bazel run --config=fast //common:microdroid_x86_64_dist -- --dist_dir=out/dist
```
...
For ARM64
```bash
tools/bazel run //common:microdroid_aarch64_config -- menuconfig
```

For x86\_64
```bash
tools/bazel run //common:microdroid_x86_64_config -- menuconfig
```
