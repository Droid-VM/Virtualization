...
use diced_utils::{make_bcc_handover, DiceArtifacts};
...
static AVF_STRICT_BOOT: &str = "/sys/firmware/devicetree/base/chosen/avf,strict-boot";
...
impl UpdatableArtifacts {
            ...
            Self::Driver(driver_path) => {
                ...
                dice::OpenDiceCborContext::new().bcc_main_flow(
                    artifacts.cdi_attest()[..]
                        .try_into()
                        .context("Trying to convert cdi_attest.")?,
                    artifacts.cdi_seal()[..].try_into().context("Trying to convert cdi_seal.")?,
                    &artifacts.bcc(),
                    &input_values as &dyn dice::InputValues,
                )
            }
            Self::Updated(artifacts) => dice::OpenDiceCborContext::new().bcc_main_flow(
                artifacts.cdi_attest()[..].try_into().context("Trying to convert cdi_attest.")?,
                artifacts.cdi_seal()[..].try_into().context("Trying to convert cdi_seal.")?,
                &artifacts.bcc(),
                &input_values as &dyn dice::InputValues,
            ),
    ...
    fn update(self, inputs: &BinderInputValues) -> Result<Self> {
        ...
        if let Self::Driver(ref driver_path) = self {
            // Writing to the device wipes the artifcats. The string is ignored
