...
fn extend_dice_artifacts_with_microdroid_payload(
    ...
    let config_values = DiceConfigValues {
        component_name: Some(CStr::from_bytes_with_nul(b"Microdroid payload\0").unwrap()),
...
fn check_certificate_for_client_vm(
    ...
    // The VM shouldn't be secure as the last payload we added is in Debug mode.
    const EXPECTED_IS_VM_SECURE: bool = false;
    assert_eq!(EXPECTED_IS_VM_SECURE, attestation_ext[1].as_bool()?);
