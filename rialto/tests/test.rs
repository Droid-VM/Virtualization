...
use x509_parser::{
    ...
    der_parser::{
        der::{parse_der_octetstring, parse_der_sequence_defined},
        oid,
        oid::Oid,
    },
    prelude::FromDer,
...
fn check_certificate_for_client_vm(
    ...
    let (remaining, attestation_ext) = parse_der_sequence_defined(|content| {
        let (remaining, challenge) = parse_der_octetstring(content)?;
        Ok((remaining, vec![challenge]))
    })(extension.value)?;
    assert!(remaining.is_empty());
