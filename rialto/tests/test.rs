...
use log::info;
use service_vm_comm::{host_port, ByteChannel, PrefixedByteChannel};
use std::fs::File;
...
fn try_check_socket_connection(port: u32) -> Result<(), Error> {
    ...
    let mut prefix_channel = PrefixedByteChannel::new(&mut byte_channel);
    ...
    prefix_channel.send_bytes(message.as_bytes())?;
    info!("Sent message: {:?}.", message);

    let mut res = prefix_channel.recv_bytes()?;
