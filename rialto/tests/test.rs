...
fn check_processing_requests(vm_type: VmType) -> Result<()> {
    ...
    check_attestation_request(&mut vm, &key_pair.key_blob, &key_pair.maced_public_key)?;
    Ok(())
}
...
fn check_attestation_request(
    ...
    match response {
        Response::RequestClientVmAttestation(certificate) => {
            check_certificate_for_client_vm(&certificate, maced_public_key)?;
            Ok(())
        }
        _ => bail!("Incorrect response type: {response:?}"),
    }
...
fn check_certificate_for_client_vm(certificate: &[u8], maced_public_key: &[u8]) -> Result<()> {
