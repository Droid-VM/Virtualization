...
impl<H: Hal, T: Transport> VsockStream<H, T> {
    ...
    fn wait_for_connect(&mut self) -> virtio_drivers::Result {
        loop {
            let Some(event) = self.wait_for_event_matches_peer_address()? else {
                continue;
            };
            match event {
                ...
                | VsockEventType::CreditUpdate => return Err(SocketError::InvalidOperation.into()),
            }
        }
    }

    fn wait_for_event_matches_peer_address(
        &mut self,
    ) -> virtio_drivers::Result<Option<VsockEventType>> {
    ...
    fn wait_for_recv(&mut self) -> virtio_drivers::Result<usize> {
        loop {
            ...
            match event {
                ...
                | VsockEventType::ConnectionRequest => return Err(SocketError::InvalidOperation.into()),
                // We can safely ignore `CreditRequest` as `VsockConnectionManager` already
