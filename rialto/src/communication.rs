...
impl<H: Hal, T: Transport> VsockStream<H, T> {
    ...
    fn wait_for_connect(&mut self) -> virtio_drivers::Result {
        match self.wait_for_event_from_peer()? {
            VsockEventType::Connected => Ok(()),
            VsockEventType::Disconnected { .. } => Err(SocketError::ConnectionFailed.into()),
            // We shouldn't receive the following event before the connection is established.
            VsockEventType::Received { .. }
            | VsockEventType::ConnectionRequest
            | VsockEventType::CreditRequest
            | VsockEventType::CreditUpdate => Err(SocketError::InvalidOperation.into()),
        }
    }
    ...
    /// Polls the rx queue to receive data or update credit in the peer.
    fn poll(&mut self) -> virtio_drivers::Result {
    ...
    fn wait_for_event_from_peer(&mut self) -> virtio_drivers::Result<VsockEventType> {
        loop {
            if let Some(event) = self.poll_event_from_peer()? {
                return Ok(event);
            } else {
