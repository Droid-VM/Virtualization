...
impl<H: Hal, T: Transport> Write for VsockStream<H, T> {
    ...
    fn write_all(&mut self, mut data: &[u8]) -> result::Result<(), Self::Error> {
        if data.len() >= self.write_buf.capacity() {
            self.wait_for_send(data)?;
            return Ok(());
        }
        while !data.is_empty() {
            let spare_capacity = self.write_buf.capacity() - self.write_buf.len();
            if data.len() <= spare_capacity {
                self.write_buf.extend_from_slice(data);
                break;
            } else {
                let (first_chunk, rest) = data.split_at(spare_capacity);
                self.write_buf.extend_from_slice(first_chunk);
                self.flush()?;
                data = rest;
            }
        }
