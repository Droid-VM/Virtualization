...
unsafe fn try_main(fdt_addr: usize) -> Result<()> {
    ...
    let bcc_handover = match vm_type() {
        VmType::ProtectedVm => {
            ...
            unsafe {
                MEMORY
                    ...
                    .as_mut()
                    .unwrap()
                    .alloc_range_outside_main_memory(&dice_range)
                    .map_err(|e| {
            ...
            bcc_handover_parse(bcc_handover)?.into()
        }
        // Currently, a sample DICE data is used for non-protected VMs, as these VMs are solely
        // intended for debugging purposes.
        VmType::NonProtectedVm => diced_sample_inputs::make_sample_bcc_and_cdis()?.into(),
    };
    ...
    while let ServiceVmRequest::Process(req) = vsock_stream.read_request()? {
        let response = requests::process_request(req, &bcc_handover)?;
