...
pub(super) fn generate_ecdsa_p256_key_pair(
    ...
    let key_blob = keyblob::encrypt(ec_key.private_key()?.as_slice(), dice_artifacts.cdi_seal())?;

    let key_pair = EcdsaP256KeyPair { maced_public_key, key_blob: write_to_vec(&key_blob)? };
    Ok(key_pair)
}
...
pub(super) fn generate_certificate_request(
    ...
    let csr_payload = write_to_vec(&csr_payload)?;
...
fn write_to_vec<T: ?Sized + Serialize>(v: &T) -> Result<Vec<u8>> {
