...
pub(super) fn generate_certificate_request(
    ...
    for key_to_sign in params.keys_to_sign {
        let public_key = validate_public_key(&key_to_sign)?;
    ...
    // TODO(b/287233786): Retrieve the DICE chain in RKP VM.
...
fn sign_data(_data: &[u8]) -> Result<Vec<u8>> {
    // TODO(b/287233786): Implement the signing function.
    Ok(Vec::new())
}

fn cbor_to_vec(v: &Value) -> Result<Vec<u8>> {
    let mut data = Vec::new();
    match ciborium::into_writer(v, &mut data) {
        Ok(()) => Ok(data),
        Err(e) => Err(coset::CoseError::from(e).into()),
    }
}
