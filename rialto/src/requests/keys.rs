...
impl EcKey {
    ...
    pub fn public_key(&self) -> Result<PublicKey> {
        let mut public_key_ptr = MaybeUninit::zeroed();
        let ctx = BnCtx::new()?;
        ...
        let len = unsafe {
            ...
            EC_KEY_key2buf(
                self.0 as *const EC_KEY,
                conversion_form,
                public_key_ptr.as_mut_ptr(),
                ctx.0,
            )
        };
        if len > 0 {
            // SAFETY: `public_key_ptr` should have been initialized now.
            let public_key_ptr = unsafe { public_key_ptr.assume_init() };
            Ok(PublicKey::new(public_key_ptr, len))
        } else {
...
pub struct PublicKey {
    /// Pointer to the buffer containing a public key serialized in the X9.62 form.
    /// The uncompressed form is prefixed with 0x04.
    ptr: *mut u8,
    len: usize,
}
...
impl Drop for PublicKey {
    fn drop(&mut self) {
        ...
        // instance is created. We can free this pointer when the public key is
        // no longer needed.
