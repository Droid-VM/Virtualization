...
impl EcKey {
    ...
    fn public_key_coordinates(&self) -> Result<(Coordinate, Coordinate)> {
        ...
        Ok((
            x.padded_array::<P256_AFFINE_COORDINATE_SIZE>()?,
            y.padded_array::<P256_AFFINE_COORDINATE_SIZE>()?,
        ))
    }
...
impl Cbb {
    fn new(initial_capacity: usize) -> Result<Self> {
        ...
        check_int_result(ret, BoringSSLApiName::CBB_init)?;
    ...
    fn finish(mut self) -> Result<ZVec> {
        ...
        check_int_result(ret, BoringSSLApiName::CBB_finish)?;
...
impl BigNum {
    ...
    /// Converts the current BigNum to a padded array of given size.
    /// The integer is padded with leading zeros up to size `N`.
    fn padded_array<const N: usize>(&self) -> Result<[u8; N]> {
        let mut num = [0u8; N];
        // SAFETY: The `BIGNUM` pointer has been created with `BN_new`.
        let ret = unsafe { BN_bn2bin_padded(num.as_mut_ptr(), N, self.0.as_ptr()) };
