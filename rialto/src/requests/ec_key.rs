...
impl EcKey {
    ...
    fn public_key_coordinates(&self) -> Result<(Vec<u8>, Vec<u8>)> {
        ...
        if ret != 1 {
            return Err(RequestProcessingError::InternalError("EC_POINT_get_affine_coordinates"));
        }
        Ok((x.bytes()?, y.bytes()?))
    }
    ...
    fn public_key_ec_point(&self) -> Result<*const EC_POINT> {
        ...
        if ec_point.is_null() {
            Err(RequestProcessingError::GettingPublicKey)
        } else {
            Ok(ec_point)
        }
    }
...
impl Bn {
    fn new() -> Result<Self> {
        ...
        // SAFETY: We assume that the non-null pointer points to a valid `BIGNUM` object.
        unsafe {
            BN_init(bn);
        }
        Ok(Self(bn))
