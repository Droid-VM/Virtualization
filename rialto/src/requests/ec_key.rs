...
impl EcKey {
    ...
    pub fn private_key(&self) -> Result<ZVec> {
        ...
        let ret =
            ...
            unsafe { EC_KEY_marshal_private_key(cbb.as_mut_ptr(), self.0.as_ptr(), enc_flags) };
        ...
        // `CBB_init_fixed()`.
        check_int_result(unsafe { CBB_flush(cbb.as_mut_ptr()) }, BoringSSLApiName::CBB_flush)?;
        ...
        let len = unsafe { CBB_len(cbb.as_ptr()) };
        Ok(buf
            .get(0..len)
