...
fn get_num_vcpus(cpu_options: &CpuOptions) -> i32 {
    let num_cpus = get_num_cpus().and_then(|v| v.try_into().ok()).unwrap_or_else(|| {
        warn!("Failed to determine the number of CPUs in the host");
        INVALID_NUM_CPUS
    });
    let cpu_count = cpu_options.cpuCount;
    if cpu_options.matchHost {
        num_cpus
    } else if cpu_count > 1 && cpu_count <= num_cpus {
        cpu_count
    } else {
        warn!("invalid Vcpu count: {cpu_count}");
        INVALID_NUM_CPUS
    }
}
