...
fn get_partition_size(file: &File) -> Result<u64, Error> {
    match detect_image_type(file).context("failed to detect partition image type")? {
        ...
        ImageType::Qcow2 => bail!("qcow2 partitions not supported"),
        ImageType::CompositeDisk => bail!("composite disk partitions not supported"),
        ImageType::AndroidSparse => {
            ...
            let len = u64::from(header.total_blks.check)
