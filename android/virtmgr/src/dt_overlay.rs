...
impl DeviceTreeOverlay {
    ...
    pub(crate) fn create_fdt<'a>(&self, buffer: &'a mut [u8]) -> Result<&'a mut Fdt> {
        let (trusted, untrusted): (Vec<_>, Vec<_>) =
            self.overlays.iter().partition(|overlay| overlay.is_trusted());
        ...
        let mut trusted_avf_node = overlay
            .add_subnode(AVF_NODE_NAME)
            .map_err(|e| anyhow!("Failed to add avf node: {e:?}"))?;

        for overlay in trusted {
            overlay.setprop_on_node(&mut trusted_avf_node)?;
        }

        if !untrusted.is_empty() {
            let mut untrusted_node = trusted_avf_node
                .add_subnode(UNTRUSTED_NODE_NAME)
                .map_err(|e| anyhow!("Failed to add untrusted node: {e:?}"))?;

            for overlay in untrusted {
                overlay.setprop_on_node(&mut untrusted_node)?;
            }
        }
