...
pub fn check_host_service_permission(
    ...
    for service in services {
        servicemanager_delegate_check_access(caller_ctx, service, "find", caller_pid, caller_uid)
...
fn servicemanager_delegate_check_access(
    ...
    let res = check_service_access(selinux_type, caller_pid, caller_uid, service_name, permission);
    match res {
        Ok(check) => {
            if !check {
        ...
        Err(err) => {
            return Err(anyhow!(
                "{selinux_type} failed to check access for {service_name} with error: {err}"
            ));
        }
    }
    Ok(())
