...
fn run_vm(
    ...
    for shared_path in &config.shared_paths {
        if shared_path.app_domain {
            if let Some(socket) = &shared_path.socket {
                let socket_path = add_preserved_fd(&mut preserved_fds, socket.try_clone().unwrap());
                let raw_fd_str = socket_path.split('/').last().unwrap();
                let raw_fd: i32 = raw_fd_str.parse().unwrap();
                command
                    .arg("--vhost-user-fs")
