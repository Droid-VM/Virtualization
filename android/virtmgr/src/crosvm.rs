...
fn run_vm(
    ...
    if let Some(cpus) = config.cpus {
        let cpus_args = {
            ...
            {
                if config.sve.enabled == SveEnabled::TRUE {
            ...
            #[cfg(target_arch = "aarch64")]
            {
                let sve_opt = match config.sve.enabled {
                    SveEnabled::AUTO => ",sve=[auto=true]",
                    SveEnabled::TRUE => ",sve=[enable=true]",
                    SveEnabled::FALSE => "",
                    _ => {
                        bail!("Undefined SVE enable value passed");
                    }
                };
                cpus.to_string() + sve_opt
            }
        };
