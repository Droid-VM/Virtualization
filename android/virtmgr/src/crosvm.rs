...
fn get_guest_time(pid: u32) -> Result<i64> {
    ...
    if guest_time_ticks == 0 {
        return Err(anyhow!("zero value is measured on elapsed CPU guest_time"));
    }
...
fn get_rss(pid: u32) -> Result<Rss> {
    ...
    if rss_crosvm_total == 0 {
        return Err(anyhow!("zero value is measured on RSS of crosvm"));
    }
    if rss_vm_total == 0 {
        return Err(anyhow!("zero value is measured on RSS of VM"));
    }
