...
fn run_vm(
    ...
    if let Some(dump_dt) = config.dump_dt {
        add_preserved_fd(&mut preserved_fds, dump_dt);
        let dump_dt_fd = preserved_fds.last().unwrap().as_raw_fd();
        command.arg("--dump-device-tree-blob").arg(format!("{dump_dt_fd}"));
    }
