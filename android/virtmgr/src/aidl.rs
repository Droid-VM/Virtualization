...
use crate::dt_overlay::{extract_sk_public_key_from_host_ref_dt, ExtraDeviceTreeProperties, VM_DT_OVERLAY_MAX_SIZE, VM_DT_OVERLAY_PATH};
...
impl VirtualizationService {
    ...
    fn create_vm_internal(
        ...
        let extra_device_tree_properties = ExtraDeviceTreeProperties::new(
            cfg!(llpvm_changes) && extract_want_updatable(config) && is_secretkeeper_supported(),
            if cfg!(llpvm_changes) { Some(instance_id) } else { None },
            extract_secretkeeper_public_key(config).or_service_specific_exception(-1)?,
            extract_vendor_hashtree_digest(config).or_service_specific_exception(-1)?,
        );
        if let Some(dt_overlay) =
            maybe_create_reference_dt_overlay(extra_device_tree_properties, &temporary_directory)?
        {
...
fn maybe_create_reference_dt_overlay(
    extra_device_tree_properties: ExtraDeviceTreeProperties,
    temporary_directory: &Path,
) -> binder::Result<Option<File>> {
    let mut data = [0_u8; VM_DT_OVERLAY_MAX_SIZE];

    let device_tree_overlay = if let Some(fdt) = extra_device_tree_properties
        .maybe_create_device_tree_overlay(&mut data)
        .or_service_specific_exception(-1)?
    {
