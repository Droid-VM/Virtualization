...
public class VmLauncherService extends Service implements DebianServiceImpl.DebianServiceCallback {
    ...
    private void preparePortForwardingNotifications() {
        ...
        sharedPref.registerOnSharedPreferenceChangeListener(new PortForwardingListener(this));
    }
    ...
    private final class PortForwardingListener
        ...
        PortForwardingListener(Context context) {
            mContext = context;
        ...
        public void onSharedPreferenceChanged(SharedPreferences sharedPref, String key) {
            ...
            Set<String> union = new HashSet<>(activePorts);
            union.addAll(mPrevActivePorts);
            for (String portStr : union) {
                try {
                    if (!mPrevActivePorts.contains(portStr)) {
                        showPortForwardingNotification(Integer.parseInt(portStr));
                    } else if (!activePorts.contains(portStr)) {
        ...
        private PendingIntent getPortForwardingPendingIntent(int port, boolean enabled) {
            Intent intent = new Intent(mContext, PortForwardingRequestReceiver.class);
            intent.setAction(PortForwardingRequestReceiver.ACTION_PORT_FORWARDING);
        ...
        private void showPortForwardingNotification(int port) {
            ...
            String notificationTitle =
                    getString(R.string.settings_port_forwarding_notification_title);
                    ...
                    new Notification.Builder(mContext, mContext.getPackageName())
                            ...
                            .addAction(
                                    new Notification.Action.Builder(
                                                    ...
                                                    getPortForwardingPendingIntent(port, true))
                                            .build())
                            .addAction(
                                    new Notification.Action.Builder(
                                                    icon,
                                                    notificationDenyButtonText,
                                                    getPortForwardingPendingIntent(port, false))
                                            .build())
                            .build();
