...
class SettingsRecoveryActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
                            ...
                            if (keepRootfs) {
                                try {
                                    ...
                                    Snackbar.make(
                                        ...
                                        R.string.settings_recovery_error_due_to_backup,
                                        Snackbar.LENGTH_SHORT
                                    )
                                        .show();
                                    Log.e(TAG, "cannot backup", e)
        ...
        resetBackupCard.setOnClickListener {
            val dialog = MaterialAlertDialogBuilder(this)
                ...
                .setPositiveButton(R.string.settings_recovery_reset_dialog_confirm) { _, _ ->
                    runInBackground {
                        InstallUtils.getBackupFile(this@SettingsRecoveryActivity).delete()
                    }
                }
                .setNegativeButton(R.string.settings_recovery_reset_dialog_cancel) { dialog, _ -> dialog.dismiss() }
                .create()
    ...
    private fun runInBackground(backgroundWork: suspend CoroutineScope.() -> Unit): Unit {
        findViewById<LinearLayout>(R.id.setting_recovery_card_container).visibility = View.INVISIBLE
        findViewById<LinearLayout>(R.id.recovery_boot_progress).visibility = View.VISIBLE
        lifecycleScope.launch(Dispatchers.IO) {
            backgroundWork()
        }.invokeOnCompletion {
            runOnUiThread {
                findViewById<LinearLayout>(R.id.setting_recovery_card_container).visibility =
                    View.VISIBLE
                findViewById<LinearLayout>(R.id.recovery_boot_progress).visibility = View.INVISIBLE
                // Restart terminal
                val intent =
