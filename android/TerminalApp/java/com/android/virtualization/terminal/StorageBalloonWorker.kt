...
class StorageBalloonWorker(appContext: Context, workerParams: WorkerParameters) :
    ...
    override fun doWork(): Result {
        ...
        val debianService = (applicationContext as Application).debianService

        if (debianService == null) {
            val retryPeriod = 5L
            Log.e(TAG, "debianService is not ready. Try again in $retryPeriod seconds")
            scheduleNextTask(retryPeriod)
