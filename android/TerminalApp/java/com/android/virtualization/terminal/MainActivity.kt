...
public class MainActivity :
    ...
    val terminalViews: MutableMap<String, TerminalView> = mutableMapOf<String, TerminalView>()
    ...
    private fun initializeTab() {
        ...
        val rootView = findViewById<RelativeLayout>(R.id.terminal_container).parent as ViewGroup
        modifierKeysController = ModifierKeysController(this, rootView)
        ...
        TabLayoutMediator(
            tabLayout, viewPager, false, false
        ) { _: TabLayout.Tab?, _: Int -> }.attach()

        val firstTerminalTab = tabLayout.newTab()
        tabLayout.addTab(firstTerminalTab)
        addAddButtonTab()
        changeToTerminalTab(firstTerminalTab)
        modifierKeysController.changeTerminalView(
            terminalTabAdapter.tabs[firstTerminalTab.position].id
        )

        tabLayout.addOnTabSelectedListener(object : OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                if (tab.position == tabLayout.tabCount - 1) {
                    addAddButtonTab()
                    changeToTerminalTab(tab)
                } else {
                    tab.customView!!.setBackgroundResource(R.drawable.background_tabitem_selected)
                }
                modifierKeysController.changeTerminalView(terminalTabAdapter.tabs[tab.position].id)
            }

            override fun onTabUnselected(tab: TabLayout.Tab) {
                tab.customView!!.setBackground(null)
            }
    ...
    private fun changeToTerminalTab(tab: TabLayout.Tab) {
        viewPager.offscreenPageLimit += 1
        val id = terminalTabAdapter.addTab()
        tab.setCustomView(R.layout.tabitem_terminal)
        tab.customView!!.findViewById<Button>(R.id.tab_close_button).setOnClickListener(
            View.OnClickListener { _: View? ->
                viewPager.offscreenPageLimit -= 1
                modifierKeysController.removeTerminalView(id)
                terminalTabAdapter.deleteTab(tab.position)
                tabLayout.removeTab(tab)
            })
    }

    private fun addAddButtonTab() {
        val addButtonTab = tabLayout.newTab()
        addButtonTab.setCustomView(R.layout.tabitem_add_button)
        tabLayout.addTab(addButtonTab, false)
    }

    fun addTerminalView(id: String, terminalView: TerminalView) {
        modifierKeysController.addTerminalView(id, terminalView)
    }
    ...
    override fun onAccessibilityStateChanged(enabled: Boolean) {
        terminalViews.forEach({ (_, terminalView) ->
