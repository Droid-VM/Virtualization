...
class SettingsPortForwardingInactiveAdapter(val portsStateManager: PortsStateManager) :
    SettingsPortForwardingBaseAdapter<SettingsPortForwardingInactiveAdapter.ViewHolder>(
        portsStateManager
    ) {

    override fun getCurrentSettingsPortForwardingItem(): ArrayList<SettingsPortForwardingItem> {
        val activePorts = mPortsStateManager.getActivePorts()
        return mPortsStateManager
            .getEnabledPorts()
            .filter { !activePorts.contains(it) }
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.closeButton.contentDescription = viewHolder.port.text
