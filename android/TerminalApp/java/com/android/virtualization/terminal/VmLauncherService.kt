        ...
        override fun run() {
            synchronized(mLock) {
                if (currentBalloonPercent < INITIAL_BALLOON_PERCENT
                    ...
                    Log.e(
                        TAG, "currentBalloonPercent=$currentBalloonPercent is invalid," +
                                " should be in range: $INITIAL_BALLOON_PERCENT~$MAX_BALLOON_PERCENT"
                    )
                    return
                }
                // Increases the balloon size by 5% every time
        ...
        when (event) {
            ...
            ApplicationLifeCycleEvent.APP_ON_STOP -> {
                ...
                synchronized(mLock) {
                    ...
                    virtualMachine?.setMemoryBalloonByPercent(currentBalloonPercent)
                    inflateBalloonHandler.postDelayed(
                        inflateBalloonTask,
                        BALLOON_INFLATE_INTERVAL_MILLIS
                    );
                }
            }
            else -> {
