...
class VmLauncherService : Service() {
    ...
    override fun onStartCommand(intent: Intent, flags: Int, startId: Int): Int {
        ...
        if (mVirtualMachine != null) {
            Log.d(MainActivity.TAG, "VM instance is already started")
            return START_NOT_STICKY
        }
        ...
        nsdManager.registerServiceInfoCallback(
            ...
            object : NsdManager.ServiceInfoCallback {
                ...
                override fun onServiceUpdated(info: NsdServiceInfo) {
                    ...
                    Log.i(MainActivity.TAG, "Service found: $info")
    ...
    private fun startDebianServer(ipAddress: String?) {
        val interceptor: ServerInterceptor =
            object : ServerInterceptor {
                override fun <ReqT, RespT> interceptCall(
                    ...
                    Log.d(MainActivity.TAG, "blocked grpc request from $remoteAddr")
        ...
        try {
            ...
            Log.d(MainActivity.TAG, "grpc server error", e)
            return
        }
        ...
        mExecutorService!!.execute(
            Runnable {
                ...
                val debianServicePortFileName = "debian_service_port"
                val debianServicePortFile = File(filesDir, debianServicePortFileName)
                try {
                    FileOutputStream(debianServicePortFile).use { writer ->
                        writer.write(mServer!!.port.toString().toByteArray())
                    }
                } catch (e: IOException) {
                    Log.d(MainActivity.TAG, "cannot write grpc port number", e)
                }
            }
        )
    }
        ...
        if (mVirtualMachine != null) {
            if (mVirtualMachine!!.getStatus() == VirtualMachine.STATUS_RUNNING) {
                try {
                    ...
                    Log.e(MainActivity.TAG, "failed to stop a VM instance", e)
                }
            }
