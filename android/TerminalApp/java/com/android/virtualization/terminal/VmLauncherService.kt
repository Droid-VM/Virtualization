...
class VmLauncherService : Service() {
    ...
    private val inflateBalloonTask: Runnable = object : Runnable {
        override fun run() {
            if (currentBalloonPercent < 0 || currentBalloonPercent > MAX_BALLOON_PERCENT) {
                Log.e(TAG, "currentBalloonPercent=$currentBalloonPercent" +
                        " is invalid, should be in range of 0~$MAX_BALLOON_PERCENT")
                return
            }
