...
public class CertificateUtils {
    ...
    private static final String ALAIS = "ttyd";
    ...
    public static KeyStore.PrivateKeyEntry createOrGetKey() {
        try {
            ...
            if (!ks.containsAlias(ALAIS)) {
                Log.d(TAG, "there is no keypair, will generate it");
                createKey();
            } else if (!(ks.getCertificate(ALAIS) instanceof X509Certificate)) {
                Log.d(TAG, "certificate isn't X509Certificate or it is invalid");
                createKey();
            } else {
                try {
                    ((X509Certificate) ks.getCertificate(ALAIS)).checkValidity();
                } catch (CertificateExpiredException | CertificateNotYetValidException e) {
                    Log.d(TAG, "certificate is invalid", e);
                    createKey();
                }
            }
            return ((KeyStore.PrivateKeyEntry) ks.getEntry(ALAIS, null));
        } catch (Exception e) {
    ...
    private static void createKey()
        ...
        kpg.initialize(
                new KeyGenParameterSpec.Builder(
                                ALAIS, KeyProperties.PURPOSE_SIGN | KeyProperties.PURPOSE_VERIFY)
