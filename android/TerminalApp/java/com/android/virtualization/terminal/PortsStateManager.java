...
public class PortsStateManager {
    ...
    void updateActivePorts(Set<Integer> ports) {
        Set<Integer> prevPorts = mActivePorts;
        synchronized (mLock) {
            mActivePorts = ports;
        }
        notifyActivePortsUpdated(prevPorts, ports);
    }
    ...
    void updateEnabledPort(int port, boolean enabled) {
        ...
        notifyEnabledPortUpdated(port, enabled);
    }
    ...
    private void notifyEnabledPortUpdated(int port, boolean enabled) {
        Set<Listener> listeners;
        synchronized (mLock) {
            listeners = new HashSet<>(mListeners);
        }
        for (Listener listener : listeners) {
            listener.onEnabledPortUpdated(port, enabled);
        }
    }

    interface Listener {
        default void onActivePortsUpdated(Set<Integer> oldPorts, Set<Integer> newPorts) {}

        default void onEnabledPortUpdated(int port, boolean enabled) {}
    }
}
