...
fn parse_arg_ro_fds(arg: &str) -> Result<(RawFd, FdConfig)> {
    let fds: Result<Vec<RawFd>, _> = arg.split(':').map(|s| s.parse::<RawFd>()).collect();
    let fds: Result<Vec<OwnedFd>, _> = fds?.into_iter().map(take_fd_ownership).collect();
    let fds = fds?;
    if fds.len() > 2 {
    ...
    let file: File = iter.next().unwrap().into();
    // if provided
    let alt_metadata = iter.next().and_then(|fd| parse_fsverity_metadata(fd.into()).ok());
