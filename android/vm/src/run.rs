...
pub fn command_run_app(config: RunAppConfig) -> Result<(), Error> {
    ...
    if config.microdroid.enable_earlycon() {
        if config.debug.debug != DebugLevel::FULL {
            bail!("earlycon is only supported for debuggable VMs")
        }
        if cfg!(target_arch = "aarch64") {
            custom_config.kernelCmdlineParams.push(String::from("earlycon=uart8250,mmio,0x3f8"));
        } else if cfg!(target_arch = "x86_64") {
            custom_config.kernelCmdlineParams.push(String::from("earlycon=uart8250,io,0x3f8"));
        } else {
