...
pub fn command_run_app(config: RunAppConfig) -> Result<(), Error> {
    ...
    let storage = if let Some(ref path) = config.microdroid.storage {
        if !path.exists() {
            ...
            maybe_extend_encrypted_storage(service.as_ref(), path, config.microdroid.storage_size)?;
        }
...
fn maybe_extend_encrypted_storage(
    service: &dyn IVirtualizationService,
    image_path: &Path,
    size: Option<u64>,
) -> Result<(), Error> {
    if let Some(size) = size {
        ...
        service.maybeExtendEncryptedStorage(&ParcelFileDescriptor::new(image), size.try_into()?)?;
    }
    Ok(())
}
