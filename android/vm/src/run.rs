...
pub fn command_run_app(config: RunAppConfig) -> Result<(), Error> {
    ...
    let storage = if let Some(ref path) = config.microdroid.storage {
        if !path.exists() {
            ...
            ensure_partition_capacity(service.as_ref(), path, config.microdroid.storage_size)?;
        }
...
fn ensure_partition_capacity(
    ...
    if let Some(size) = size {
        ...
        service.ensurePartitionCapacity(&ParcelFileDescriptor::new(image), size.try_into()?)?;
    }
    Ok(())
}
