...
pub fn command_run_app(config: RunAppConfig) -> Result<(), Error> {
    ...
    let storage = if let Some(ref path) = config.microdroid.storage {
        if !path.exists() {
            ...
            extend_encrypted_storage(service.as_ref(), path, storage_size)?;
        }
...
fn extend_encrypted_storage(
    ...
    service.extendEncryptedStorage(&ParcelFileDescriptor::new(image), size.try_into()?)?;
    Ok(())
}
