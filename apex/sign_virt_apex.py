...
def SignVirtApex(args):
    ...
    def resign_kernel(kernel, initrd_normal, initrd_debug):
        ...
        if is_lz4(args, kernel_file):
            return Async(resign_compressed_kernel, kernel_file, initrd_normal_file, initrd_debug_file)
        else:
...
def update_initrd_digests_in_rialto(original_descriptors, args, files):
    ...
    print(f'original_descriptors: {original_descriptors}')
    print(f'updated_descriptors : {updated_descriptors}')
