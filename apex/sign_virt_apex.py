...
def UpdateVbmetaBootConfig(args, initrds, vbmeta_img):
    ...
    def append_bootconfigs(initrd_bc, initrd, bc):
        cmd = ['initrd_bootconfig',
                'append-bootconfig',
                initrd,
                bc,
    ...
    def calc_vbmeta_digest():
        cmd = ['avbtool', 'calculate_vbmeta_digest',
                                '--image', vbmeta_img,
    ...
    def update_bc_digest(bootconfigs):
        result = re.search(r"(androidboot.vbmeta.digest = \"[^\"]*\")", bootconfigs)
        if not result:
            raise ValueError("Failed to find androidboot.vbmeta.digest")

        return bootconfigs.replace(result.group(), "androidboot.vbmeta.digest = \""
                + calc_vbmeta_digest()+"\"")

    def update_bc_size(bootconfigs):
        result = re.search(r"(androidboot.vbmeta.size = [0-9]+)", bootconfigs)
        if not result:
            raise ValueError("Failed to find androidboot.vbmeta.size")
        return bootconfigs.replace(result.group(), "androidboot.vbmeta.size = "
                + str(calc_vbmeta_size()))
    ...
    with tempfile.TemporaryDirectory() as work_dir:
        ...
        for initrd in initrds:
            ...
            bc_data = update_bc_digest(bc_data)
            bc_data = update_bc_size(bc_data)
            bc_file.close()
            bc_file = open(tmp_bc, "wt", encoding="utf-8")
            bc_file.write(bc_data)
            bc_file.flush()
            append_bootconfigs(initrd, tmp_initrd, tmp_bc)
...
virt_apex_files = {
    'microdroid_kernel.img': 'etc/fs/microdroid_kernel',
    'vbmeta.img': 'etc/fs/microdroid_vbmeta.img',
    'super.img': 'etc/fs/microdroid_super.img',
    'microdroid_initrd_normal.img': 'etc/microdroid_initrd_normal.img',
    'microdroid_initrd_debuggable.img': 'etc/microdroid_initrd_debuggable.img',
}
...
def SignVirtApex(args):
    ...
    Async(UpdateVbmetaBootConfig, args, [files['microdroid_initrd_normal.img'],
            files['microdroid_initrd_debuggable.img']], files['vbmeta.img'],
            wait=[vbmeta_f])

    # Re-sign microdroid_kernel.img
    # TODO(b/260821553): Kernel's vbmeta should contain hashes of initrd
    Async(AddHashFooter, args, key, files['microdroid_kernel.img'])
...
def VerifyVirtApex(args):
    ...
    for f in files.values():
        if f in (files['microdroid_initrd_normal.img'], files['microdroid_initrd_debuggable.img']):
            continue
