...
def check_no_size_change_on_resigned_image(image_path, original_image_info, resigned_image_info):
    ...
    assert_different_value(original_image_info, resigned_image_info, "Public key (sha1)", image_path)
...
def resign_rialto(args, key, rialto_path):
    ...
    assert_same_value(original_info, updated_info, "Rollback Index", "rialto")
    original_prop = find_value_by_key(original_descriptors, "Prop")
    updated_prop = find_value_by_key(updated_descriptors, "Prop")
    assert len(updated_prop) == 1, f"Only one Prop is expected in rialto. Updated Prop: {updated_prop}"
    assert original_prop == updated_prop, "Prop should not change for rialto." \
          f"Original Prop: {original_prop}, Updated Prop: {updated_prop}"
    ...
    assert_different_value(original_descriptor, updated_descriptor, "Digest", "rialto_hash_descriptor")
...
def update_kernel_hashes_in_rialto(original_descriptors, args, files):
    ...
    all_digests = set(find_hash_descriptor_by_partition_name(d, name)["Digest"]
                      for name in partition_names
                      for d in [original_descriptors, updated_descriptors])
    assert len(all_digests) == 4, "There should be 4 different digests for the original and " \
        "updated descriptors for initrds. Some hash collision happened. Original desciptors: " \
        f"{original_descriptors}, updated descriptors: {updated_descriptors}"
    for partition_name in partition_names:
        ...
        # TODO(b/319235308): Assert that the salt is the same once the salt is preserved for initrd.
        assert_different_value(original_descriptor, update_descriptor, "Digest", partition_name)
        ...
        assert original_digest != updated_digest, \
            f"Digest of {partition_name} should be updated. Original digest: {original_digest}"
        ...
        assert new_content != content, \
            ...
            f"Original digest: {original_digest}"
        content = new_content
