...
def AddHashFooter(args, key, image_path, additional_images=()):
    ...
    hash_descriptors_orginal = extract_hash_descriptors(descriptors, drop_digest)
    for additional_image in additional_images:
        ...
        for k, v in hash_descriptors.items():
            assert v == hash_descriptors_orginal[k], \
                f"Hash descriptor of {k} in {additional_image} and {image_path} should be " \
                f"the same. {additional_image}: {v}, {image_path}: {hash_descriptors_orginal[k]}"
            del hash_descriptors_orginal[k]
    assert len(hash_descriptors_orginal) == 1, \
        f"Only one hash descriptor is expected for {image_path} after removing " \
        f"additional images. Hash descriptors: {hash_descriptors_orginal}"
    [(original_image_partition_name, original_image_descriptor)] = hash_descriptors_orginal.items()
...
def update_initrd_hashes_in_rialto(original_descriptors, args, files):
    ...
    original_hash_descriptors = extract_hash_descriptors(original_descriptors)
    updated_hash_descriptors = extract_hash_descriptors(updated_descriptors)
    assert_same_value(original_hash_descriptors, updated_hash_descriptors,
                      "boot", "microdroid_kernel_hash_descriptor")
    ...
    for partition_name in partition_names:
        original_digest = binascii.unhexlify(original_hash_descriptors[partition_name]['Digest'])
        updated_digest = binascii.unhexlify(updated_hash_descriptors[partition_name]['Digest'])
