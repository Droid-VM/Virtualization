...
def SignVirtApex(args):
    ...
    if not args.do_not_update_bootconfigs:
        vbmeta_bc_f = Async(UpdateVbmetaBootconfig, args, [files['initrd_normal.img'],
    ...
    initrd_n_f = Async(GenVbmetaImage, args, files['initrd_normal.img'], initrd_normal_hashdesc, "initrd_normal", wait=[
                       vbmeta_bc_f] if vbmeta_bc_f is not None else [])
    initrd_d_f = Async(GenVbmetaImage, args, files['initrd_debuggable.img'], initrd_debug_hashdesc, "initrd_debug", wait=[
                       vbmeta_bc_f] if vbmeta_bc_f is not None else [])
    Async(AddHashFooter, args, key, files['kernel'], partition_name="boot", additional_descriptors=[
          initrd_normal_hashdesc, initrd_debug_hashdesc], wait=[initrd_n_f, initrd_d_f])
