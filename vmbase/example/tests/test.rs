...
impl VmLogProcessor {
    fn messages() -> (VecDeque<String>, HashSet<String>) {
        ...
        for log_lvl in ["[ERROR]", "[WARN]", "[INFO]", "[DEBUG]"] {
            expected.push_back(String::from(log_lvl) + " " + "Unsuppressed message");
            unexpected.insert(String::from(log_lvl) + " " + "Suppressed message");
        }
        (expected, unexpected)
    }
    ...
    fn verify(&mut self, msg: &str) {
        if !self.expected.is_empty() && msg == self.expected.front().unwrap() {
