    ...
    for (device_function, info) in pci_root.enumerate_bus(0) {
        ...
        if let Some(virtio_type) = virtio_device_type(&info) {
            ...
            if virtio_type == DeviceType::Block {
                ...
                info!("Found {} KiB block device.", blk.capacity() / 2);
            }
        }
    }
}
...
impl PciMemory32Allocator {
    ...
    pub fn for_pci_ranges(pci_node: &FdtNode) -> Self {
        let mut ranges = pci_node
            .getprop_cells(CStr::from_bytes_with_nul(b"ranges\0").unwrap())
            .expect("PCI node missing ranges property.");
        let mut memory_32_address = 0;
        let mut memory_32_size = 0;
        while let Some(flags) = ranges.next() {
            let prefetchable = flags & 0x80000000 != 0;
            let range_type = PciRangeType::from((flags & 0x3000000) >> 24);
            let bus_address = (ranges.next().unwrap() as u64) << 32 | ranges.next().unwrap() as u64;
            let cpu_physical =
                (ranges.next().unwrap() as u64) << 32 | ranges.next().unwrap() as u64;
            let size = (ranges.next().unwrap() as u64) << 32 | ranges.next().unwrap() as u64;
            info!(
