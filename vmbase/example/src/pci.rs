...
pub fn check_pci(pci_root: &mut PciRoot) {
    ...
    for (device_function, info) in pci_root.enumerate_bus(0) {
        ...
        if let Some(virtio_type) = virtio_device_type(&info) {
            ...
            match virtio_type {
                DeviceType::Block => {
                    check_virtio_block_device(transport, block_device_index);
                    block_device_index += 1;
                    checked_virtio_device_count += 1;
                }
                DeviceType::Console => {
                    check_virtio_console(transport);
                    checked_virtio_device_count += 1;
                }
                _ => {}
            }
...
fn check_virtio_console(transport: impl Transport) {
