...
#[inline]
pub fn handle_translation_fault(far: usize) -> Result<(), HandleExceptionError> {
...
#[inline]
pub fn handle_permission_fault(far: usize) -> Result<(), HandleExceptionError> {
    let mut guard = MEMORY.try_lock().ok_or(HandleExceptionError::PageTableUnavailable)?;
    let memory = guard.as_mut().ok_or(HandleExceptionError::PageTableNotInitialized)?;
...
#[inline]
pub fn print_non_uart_exception_failure(esr: Esr, far: usize, elr: u64, e: HandleExceptionError) {
