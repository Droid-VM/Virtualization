...
fn rnd64(n_bytes: usize) -> Result<Option<Entropy>> {
    ...
    let entropy = if matches!(result, Err(hvc::trng::Error::NoEntropy)) {
        ...
        let entropy = [r[2].to_le(), r[1].to_le(), r[0].to_le()];

        assert_eq!(size_of_val(&entropy), size_of::<Entropy>());
        // SAFETY: Both the argument and result are valid.
        Some(unsafe { transmute::<_, _>(entropy) })
    };
