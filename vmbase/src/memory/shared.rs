...
impl MemoryTracker {
    ...
    pub fn map_mmio_range(&mut self, range: MemoryRange) -> Result<()> {
        ...
        if get_hypervisor().get_mmio_guard().is_some() {
    ...
    fn refill(&mut self, pool: &mut FrameAllocator<32>, hint: Layout) {
        ...
        if let Some(mem_share) = get_hypervisor().get_mem_share() {
            ...
            for vaddr in (base..end).step_by(self.granule) {
                ...
                mem_share.share(virt_to_phys(vaddr).try_into().unwrap()).unwrap();
            }
        }
    ...
    fn drop(&mut self) {
        while let Some((base, layout)) = self.frames.pop() {
            if let Some(mem_share) = get_hypervisor().get_mem_share() {
                ...
                for vaddr in (base..end).step_by(self.granule) {
                    let vaddr = NonNull::new(vaddr as *mut _).unwrap();
                    mem_share.unshare(virt_to_phys(vaddr).try_into().unwrap()).unwrap();
                }
            }
...
fn mmio_guard_unmap_page(
    ...
    if flags.contains(Attributes::VALID) {
        ...
        get_hypervisor().get_mmio_guard().unwrap().unmap(page_base).map_err(|e| {
