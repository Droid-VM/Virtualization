...
impl MemoryTracker {
    ...
    fn handle_mmio_fault(&mut self, addr: usize) -> Result<()> {
    ...
    fn handle_permission_fault(&mut self, addr: usize) -> Result<()> {
        self.page_table
...
#[inline]
pub fn handle_translation_fault(far: usize) -> result::Result<(), HandleExceptionError> {
...
#[inline]
pub fn handle_permission_fault(far: usize) -> result::Result<(), HandleExceptionError> {
    let mut guard = MEMORY.try_lock().ok_or(HandleExceptionError::PageTableUnavailable)?;
    let memory = guard.as_mut().ok_or(HandleExceptionError::PageTableNotInitialized)?;
