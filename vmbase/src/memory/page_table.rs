...
impl PageTable {
    ...
    pub fn map_data_dbm(&mut self, range: &MemoryRegion) -> Result<()> {
        ...
        // dirty once a store hits them. Also avoid the use of the contiguous hint, as that will
        // result in BBM violations when updating the read-only attribute on individual page
        // descriptors.
