...
nix::ioctl_write_ptr_bad!(_loop_set_status, LOOP_SET_STATUS64, loop_info64);
#[cfg(test)]
...
fn loop_set_status(device_file: &File, info: &loop_info64) -> Result<i32> {
    // SAFETY: this ioctl changes the state in kernel, but not the state in this process.
    Ok(unsafe { _loop_set_status(device_file.as_raw_fd(), info) }?)
}
...
fn try_attach<P: AsRef<Path>>(path: P, offset: u64, size_limit: u64) -> Result<PathBuf> {
    ...
    loop_set_status(&device_file, &info)?;
