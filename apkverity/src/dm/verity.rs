...
impl<'a> DmVerityTargetBuilder<'a> {
    /// Constructs a builder that is initialized to safe defaults.
    pub fn new() -> DmVerityTargetBuilder<'a> {
    ...
    pub fn build(&self) -> Result<DmVerityTarget> {
        ...
        let data_device_path = self
            .data_device
            .ok_or(anyhow!("data device is not set"))?
            .to_str()
            .ok_or(anyhow!("data device path is not encoded in utf8"))?;
        ...
        let hash_device_path = self
            .hash_device
            .ok_or(anyhow!("hash device is not set"))?
            .to_str()
            .ok_or(anyhow!("hash device path is not encoded in utf8"))?;
        let stat = fstat(self.data_device.unwrap())?; // safe; checked just above
        ...
        let root_digest =
            hexstring_from(self.root_digest.ok_or(anyhow!("root digest is not set"))?);
