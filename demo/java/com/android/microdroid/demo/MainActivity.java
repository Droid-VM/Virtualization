...
public class MainActivity extends AppCompatActivity {
    ...
    private static class VirtualMachineModel extends AndroidViewModel {
        private final VirtualMachine mVirtualMachine;
        private final MutableLiveData<String> mConsoleOutput = new MutableLiveData<>();

        VirtualMachineModel(Application app) {
            super(app);

            // Create a VM and run it.
            // TODO(jiyong): remove the call to setIdsigPath
            try {
                VirtualMachineConfig config =
                        new VirtualMachineConfig.Builder(getApplication(), "assets/vm_config.json")
                                .setIdsigPath("/data/local/tmp/virt/MicrodroidDemoApp.apk.idsig")
                                .build();
