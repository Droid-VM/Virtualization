...
Result<double> measure_read_rate(const std::string& filename, bool is_rand) {
    ...
    if (is_rand) {
        return ErrnoError() << "Not supported";
    }
...
int main() {
    ...
    std::string filename = "/data/test/file";
    // std::string filename = "/mnt/encryptedstore/file"; // if testing in Microdroid

    std::vector<uint64_t> read_rates, write_rates;
    for (auto i = 0; i < n; ++i) {
        auto w_rate = measure_write_rate(filename, size);
        auto r_rate = measure_read_rate(filename, false);
