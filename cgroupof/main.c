...
int main(int argc, const char* argv[]) {
    ...
    errno = 0;
    long pid = strtol(arg1, NULL, 10);
    if (errno != 0) {
        fprintf(stderr, "%s is not a number: %s.\n", arg1, strerror(errno));
        return 1;
    }

    char cgroup_path[PATH_MAX];
    if (snprintf(cgroup_path, PATH_MAX, "/proc/%ld/cgroup", pid) < 0) {
        fprintf(stderr, "%s is too long.\n", arg1);
        return 1;
    }

    FILE* f = fopen(cgroup_path, "re");
    if (f == NULL) {
    ...
    while (getline(&line, &len, f) != -1) {
        ...
        if (memcmp(line, "0::/", 4) != 0) continue;
        found = true;

        fputs(line + 4, stdout); // print the string after the marker
        break;
    }
    ...
    if (!found) {
        fprintf(stderr, "Cannot find cgroup of PID %ld,\n", pid);
        return 1;
    }
